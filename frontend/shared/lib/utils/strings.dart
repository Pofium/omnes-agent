import 'config.dart';

class Strings {
  static const String appName = "OmnesAgent";

  static const String chatWithOmAgent = "Chat With $appName";
  static const String chatWithOmAgentSubTitle =
      "Chatting with $appName can be a fun and interactive way to explore language and learn new things. ";
  static const String settings = "Settings";
  static const String token = "Token";
  static const String cancel = "Cancel";
  static const String save = "Save";
  static const String skip = "Skip";
  static const String subscriptionPlan = "Subscription Plan";
  static const String freeSubscription = "Free Subscription";
  static const String notIncluded = "Not Included";
  static const String limitedChatting = "Limited Chatting With $appName";
  static const String limitedCImage = "Limited Image Generate";
  static String basicChatting =
      "${ApiConfig.freeMessageLimit.toString()} Chatting With $appName";
  static String basicImage =
      "${ApiConfig.freeImageGenLimit.toString()} Image Generate";
  static String unlimitedChatting =
      "${ApiConfig.premiumMessageLimit.toString()} Chatting With $appName";
  static String unlimitedImage =
      "${ApiConfig.premiumImageGenLimit.toString()} Image Generate";
  static const String freeSupport = "Free Support";
  static const String premiumSubscription = "Premium Subscription";
  static const String google = "Sign In with Google";
  static const String continueAsGuest = "Continue as Guest";
  static const String searchSomething = "Search Something";

  static const String generateAnyImage = "Image Generate";
  static const String generateAnyImageSubTitle =
      "Image generation in AI refers to the process of creating new images using artificial intelligence algorithms.";

  static const String typeYour = "Type your message...";

  static const String writeSomething = "Write Something";
  static const String perMonth = "Per\nMonth";

  static const String regenerateResponse = "Regenerate Response";
  static const String regeneratingResponse = "Regenerating Response..";
  static const String clearConversation = "Clear Conversation";
  static const String hello = "Hello";
  static const String helloGuest = "Hello Guest";

  static const String english = "English";
  static const String arabic = "Arabic";
  static const String spanish = "Spanish";

  static const String download = "Download";
  static const String share = "Share";

  static const String facebook = "Sign In with Facebook";
  static const String enterYourName = "Enter Your Name";
  static const String enterYourEmail = "Enter Your Email";
  static const String describeYourIssue = "Describe Your Issue";
  static const String faceAnyProblem = "Face Any Problem?";

  static const String emptyAnnouncement = "Empty Announcement!";
  static const String youCanSend = "You can send";
  static const String messagesToTheBot = "messages to the Bot";
  static const String messagesLeft = "messages left";
  static const String outOffLimit = "Sorry, you are out of the limit.";

  static const String logOut = "Log Out";
  static const String supportAndFeedback = "Support & Feedback";
  static const String deleteAccount = "Delete Account";
  static const String deleteYourAccount = "Do you want to Delete your Account?";
  static const String alert = "Alert!";

  static const String bengali = "Bengali";
  static const String hindi = "Hindi";

  static const String purchaseDetails = "Purchase Details";
  static const String statusSettings = "Status Settings";
  static const String chatStatus = "Chat Status";
  static const String imageGenerateStatus = "Image Generate Status";
  static const String subscriptionPlanStatus = "Subscription Plan Status";
  static const String adShowStatus = "Ad Show Status";

  static const String apple = "Sign In with Apple";
  static const String updateProfile = "Update Profile";

  static const String subscriptionWith = "Subscribe with";

  static const String changeTextModel = "Change text model";
  static const String setToken = "Set Token";
  static const String changeImageSize = "Image size";
  static const String imageNote = "Smaller sizes are faster to generate.";
  static const String fullName = "Full Name";
  static const String emailAddress = "Email Address";
  static const String mobileNumber = "Mobile Number";

  static const String themeChange = "Theme Change";
  static const String purchasePlan = "Purchase Plan";
  static const String privacyPolicy = "Privacy Policy";
  static const String terms = "Terms & Condition";
  static const String refundPolicy = "Refund Policy";

  static const String privacyPolicyUrl = "https://adbot.appdevs.net/privacy";
  static const String termsUrl = "https://adbot.appdevs.net/terms";
  static const String refundPolicyUrl = "https://adbot.appdevs.net/refund";

  static const String myPlan = "My Plan";
  static const String purchaseDate = "Purchase Date";
  static const String logIn = "Log In";

  static const String description = "Description";

  static const String writeContent = "Write Content";

  static const String contentWriting = "Content Writing";
  static const String contentWritingSubTitle =
      "Generate your content with OmAgent now! with powerful AI· Content Writer for all your content writing needs·";

  static const String selectType = "Select Type";
  static const String selectYourSocialMedia = "Select Your Social Media";
  static const String topic = "Topic";
  static const String about = "About";
  static const String forText = "For";

  static const String hashTagNumber = "Hash Tag Number";
  static const String create = "Create";
  static const String idea = "Idea";
  static const String copy = "Copy";
  static const String hashTags = "Hash Tags";

  static const String keyWordCreating = "Key Word Generate";
  static const String hashTagCreatorSubTitle =
      "Generate your hashTags with OmAgent now for your Social Media·";

//from here to //end  copy the code and pest it exactly in your code in this order (version 3.7.0)

  static const String dietChartCreating = "Diet chart Generate";

  static const String dietChartCreatorSubTitle =
      "Generate your diet chart with OmAgent now for healthy life·";
  static const String currentWeight = "Current weight in (kg)";
  static const String targetWeight = "Target weight in (kg)";
  static const String height = "Height in (cm)";
  static const String dietPlan = "Diet Plan";
  static const String gender = "Gender";
  static const String dietDuration = "Diet Duration in (weeks)";
  static const String lifeStyle = "Life Style";
  static const String selectCountry = "Select country for food category ";
  static const String createDietChart = "make a diet chart for loss weight.";
  static const String nowWeight = "Now my weight is";
  static const String kg = "kg.";
  static const String cm = "centimeters.";
  static const String weeks = "weeks.";
  static const String expectedWeight = "My target weight is";
  static const String heightIs = "My height is";
  static const String myDietDuration = "Diet Duration";
  static const String iAmA = "I am a";
  static const String myLifestyle = ".My Lifestyle is";
  static const String basedOn = ".Based On";
  static const String food = "food.";
  static const String pdfHeader = "";
  //end (version 3.7.0)

  static const String pressTheButton =
      "Press the button to generate a love note";
  static const String createARandomLoveNote =
      "Create a random short love quote for my partner, which is not created before this time";

  static const String loveNotes = "Create Love Notes";
  static const String randomLoveNotes = "Random Love Notes";
  static const String randomLoveNotesSubtitle =
      "Generate your love notes with OmAgent now for a good relationship";

  static const String enterYourPassword = "Enter Your Password";
  static const String go = "Go";
  static const String forgotPassword = "Forgot Password";

  /// 4.0.0
  static const String setSchedule = "Set Scheduled";
  static const String setScheduleReminder = "Set Scheduled Reminder";
  static const String setScheduleSubTitle =
      "With our scheduling feature, you'll never miss a task or event again. Easily set reminders for meetings, appointments, and to-do items. Stay organized and in control of your busy life.";

  static const String scheduleName = "Schedule Name";
  static const String enterScheduleName = "Enter $scheduleName";
  static const String scheduleTime = "Schedule Time";
  static const String scheduleDate = "Schedule Date";
  static const String scheduleDescription = "Schedule Description";

  static const String daily = "Daily";
  static const String weekly = "Weekly";
  static const String once = "Once";

  static const String selectWeeks = "Select Weeks";

  static const String saturday = "Saturday";
  static const String sunday = "Sunday";
  static const String monday = "Monday";
  static const String tuesday = "Tuesday";
  static const String wednesday = "Wednesday";
  static const String thursday = "Thursday";
  static const String friday = "Friday";

  static const String fillUpForm = "Fill Up Name & Description First";

  static const String coverLetter = "Cover letter";
  static const String coverLetterCreatorSubTitle =
      "Generate your cover letter with OmAgent now for smart life·";
  static const String date = "Date";
  static const String personalInformation = "Personal Information";

  static const String address = "Address";
  static const String enterAddress = "Enter Address";

  static const String enterMobileNumber = "Enter Mobile Number";

  static const String linkedInUrl = "LinkedIn Url";
  static const String portfolioUrl = "Portfolio Url";
  static const String educationalBackground = "Educational Background";

  static const String newCompanyInformation = "New Company Information";

  static const String jobTitle = "Job Title";
  static const String enterJobTitle = "Enter Job Title";

  static const String companyName = "Company Name";
  static const String enterCompanyName = "Enter Company Name";

  static const String qualifications = "Qualifications";

  static const String skills = "Skills";
  static const String mentionSkills = "Mention Your Skills [Separate by (,)]";

  static const String duration = "Duration";
  static const String totalExperience = "Total Experience";
  static const String enterTotalExperience = "1 year";

  static const String latestWorkDescribe = "Latest Work Describe";
  static const String describeShortly = "Describe Shortly";

  static const String latestCompanyInformation = "Latest Company Information";

  static const String myRole = "My Role";
  static const String enterMyRole = "Junior Software Engineer";

  static const String others = "Others";
  static const String pdf = "PDF";

  static const String doYouHaveWorkExperience = "Do You Have Work Experience?";

  static const String hereYouCanModifyBeforeGettingPDFFile =
      "Here You Can Modify Before Getting PDF File";

  // OmnesAgent New Module Strings
  static const String russian = "Russian";
  static const String sessions = "Sessions";
  static const String newChat = "New Chat";
  static const String deleteSession = "Delete Session";
  static const String deleteSessionConfirm = "Are you sure you want to delete this session?";
  static const String projects = "Projects";
  static const String createProject = "Create Project";
  static const String projectName = "Project Name";
  static const String projectDesc = "Project Description";
  static const String deleteProject = "Delete Project";
  static const String files = "Files";
  static const String notes = "Notes";
  static const String chats = "Chats";
  static const String memory = "Memory";
  static const String addMemory = "Add Memory";
  static const String key = "Key";
  static const String value = "Value";
  static const String automation = "Automation";
  static const String scheduleJob = "Schedule Job";
  static const String runNow = "Run Now";
  static const String approvals = "Approvals";
  static const String approve = "Approve";
  static const String deny = "Deny";
  static const String stats = "Stats";
  static const String tokenUsage = "Token Usage";
  static const String totalCost = "Total Cost";
  static const String voiceAutoTts = "Auto-TTS";
  static const String listening = "Listening...";

  // OmnesAgent UI, Security, and LLM Provider Strings
  static const String omnesAgent = "omnes_agent_app";
  static const String omnesAgentClient = "Omnes Agent Client";
  static const String activeAgent = "Active Agent";
  static const String change = "Change";
  static const String chatWithAgent = "Chat with Agent";
  static const String chatWithAgentSubtitle = "Streaming dialogue, reasoning, and tool execution";
  static const String projectsAndFiles = "Projects & Files";
  static const String projectsSubtitle = "Working directories, project files, and NOTES.md";
  static const String sessionHistory = "Session History";
  static const String sessionsSubtitle = "Resume conversations and manage agent threads";
  static const String agentMemory = "Agent Memory";
  static const String memorySubtitle = "Long-term semantic memory and recorded facts";
  static const String cronTasks = "Automation (Cron)";
  static const String cronSubtitle = "Background periodic tasks and run histories";
  static const String costAndTokens = "Cost & Tokens";
  static const String costSubtitle = "Detailed token usage and model expense breakdown";
  static const String toolsAndSkills = "Tools & Skills";
  static const String toolsSubtitle = "Active capabilities, MCP servers, and skills";
  static const String systemStatus = "System Diagnostics";
  static const String systemStatusSubtitle = "Node status, memory, channels, and security posture";
  static const String selectAgent = "Select Agent";

  // Security & App Lock
  static const String securityAndLock = "Security & App Lock";
  static const String pinProtection = "PIN Protection";
  static const String pinProtectionSubtitle = "Require PIN code to open OmnesAgent";
  static const String setPinCode = "Set / Change PIN";
  static const String biometricAuth = "Biometric Login (Fingerprint / Face ID)";
  static const String biometricAuthSubtitle = "Unlock instantly using device biometrics";
  static const String enterPinToUnlock = "Enter PIN to Unlock";
  static const String enterPin = "Enter PIN";
  static const String incorrectPin = "Incorrect PIN code. Try again.";
  static const String pinSaved = "PIN code saved successfully";
  static const String pinDisabled = "PIN protection disabled";
  static const String enterNewPin = "Enter New PIN (4-6 digits)";
  static const String confirmPin = "Confirm PIN";
  static const String pinsDoNotMatch = "PIN codes do not match";
  static const String unlock = "Unlock";

  // LLM Providers & Models
  static const String llmProviders = "LLM Providers & Models";
  static const String llmProvidersSubtitle = "Manage LLM backends, custom OpenAI/OpenAPI endpoints, and keys";
  static const String activeModel = "Active Agent Model";
  static const String backendCatalog = "Gateway Providers Catalog";
  static const String customProvider = "Custom API Provider";
  static const String providerName = "Provider Name";
  static const String apiFormat = "API Wire Format";
  static const String baseUrl = "API Base URL";
  static const String apiKey = "API Key / Bearer Token";
  static const String modelName = "Model Identifier";
  static const String testConnection = "Test Connection";
  static const String connectionSuccess = "Connection successful!";
  static const String connectionFailed = "Connection failed";
  static const String addCustomProvider = "Add Custom Provider";
  static const String providerSaved = "Provider configuration saved";
  static const String configuredProviders = "Configured Providers";
  static const String noConfiguredProviders = "No additional providers configured yet.";
  static const String setActiveModel = "Set as Active";

  // Voice, Gateway & General Settings
  static const String gatewaySettings = "OmnesAgent Gateway";
  static const String gatewaySettingsSubtitle = "Local daemon connection (127.0.0.1:42617)";
  static const String advancedNetworkSettings = "Advanced Network Settings";
  static const String voiceAndSpeech = "Voice & Speech Synthesis";
  static const String autoTtsTitle = "Auto-speak replies (TTS)";
  static const String autoTtsSubtitle = "Read agent final responses aloud";
  static const String appLanguage = "App Language";
}
