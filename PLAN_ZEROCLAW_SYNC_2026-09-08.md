...
- **D7. ✅ ВЫПОЛНЕН (09.09.2026, коммиты `2dec397`, `f5b13ca`, `d715ec4`)** — локальные фиксы открытых багов (НЕ порты upstream; каждый верифицирован перед реализацией):
  - **#10501 ✅ `d715ec4`** — верификация: дефолтная политика `ToolResultImagePolicy::ImageUrl` + vision-провайдер + tool-result с `[IMAGE:...]` собирает `image_url` parts в `role:"tool"` → OpenAI-совместимые API отвечают 400 (image_url допустим только в `user`). Фикс: релокация image в следующее `role:"user"` сообщение (пост-процессинг `convert_messages_for_native`), tool-сообщение сохраняет текст с notice «[tool-result image attached in the following user message]». Non-vision и Omit — поведение прежнее. Тесты: convert_messages_for_native 26/26 (2 новых).
  - **#10625 ✅ `f5b13ca`** — верификация: non-vision модель получает деградированную историю с литералом `[media attachment]` и эхом возвращает его пользователю. Фикс: плейсхолдер вынесен в `multimodal::MEDIA_ATTACHMENT_PLACEHOLDER` (единый источник), outbound-санитайзер каналов (`sanitize_channel_response_for_format_with_leak_detection`) зачищает эхо-плейсхолдеры: встроенные — с коллапсом пробелов, placeholder-only строки — удаляются, обычный текст о вложениях — нетронут. Тесты: multimodal 70/70, orchestrator-санитайзер +2 теста.
  - **#10061 ✅ `2dec397`** — верификация: user-ход с маркером коммитится в историю ДО LLM-вызова (`orchestrator/mod.rs:6540-6549`) → провайдер, отклонивший payload (content policy/формат/размер), отклоняет каждый последующий ход, сессия «отравлена». Фикс по образцу context-overflow recovery: `try_recover_image_rejection` (context_recovery.rs) при ошибке, похожей на image-rejection (4xx/invalid/policy И упоминание media; 5xx не матчится), перезаписывает media-маркеры в истории плейсхолдером деградации и делает retry. Независимые (unrelated) ошибки и история без маркеров — нетронуты. Тесты: context_recovery 13/13 (4 новых), vision_route 10/10.
  - **#10667 ✅ проверен — фикс уже есть в форке**: `omnescode/src/chat.rs:7257-7287` `commit_turn` через `turn_had_streaming_text` не дублирует финальный текст, если стриминг уже его закоммитил (fallback только при отсутствии стриминга). Реализация не требуется.

### Этап E — Низкий приоритет / опционально — ✅ ЗАКРЫТ (09.09.2026): 2 порта, 1 проверен, остальное — отложено осознанно
- **#10651 ✅** warm compatible connections через /models — warmup зондирует каталог (models_url()),
  не отклоняет HTTP-ошибки, дренирует ответ. Коммит `2ab3be1` (134+/6-); warmup-тесты 14/14.
- **#10296 ✅** forward probe feature в tool-имплементации — `omnesagent-tools/probe` в фиче probe
  hardware-крейта + компонентный тест графа фич. Коммит `310c378`; тест 2/2. Попутно исправлена
  pre-existing поломка сборки таргета component (admin_auth в AppState без обновления
  config_patch_cli.rs — от параллельной работы другого агента).
- **#9779 ✅ проверен — не воспроизводится**: документированный дефолт (`unset`, runtime выключен)
  совпадает с кодом (`default_sop_sops_dir() -> None`, `runtime_enabled()` гейт + тест
  `sop_sops_dir_defaults_to_unset_and_disables_runtime`); относительный `sops_dir` резолвится к
  install root (#9152 семантика уже есть, тест `resolve_sops_dir_joins_relative_config_value_to_install_root`).
- **#10375 ⏸ отложено** — только типизация/OpenAPI; wire-контракт `/api/status` не меняется,
  наш фронт Flutter. Ценности почти нет — делать при появлении TS-клиента.
- **#10667 ⏸ в задачу D7** — дубль стрима в omnescode; в upstream баг ОТКРЫТ, портировать нечего,
  нужен собственный фикс (chat.rs:7239,7265-7268,7289-7298).
- **#10693 ⏸ ждать upstream** — Enter в Connected; код идентичен upstream, ждём их PR.
- **#10506 ⏸ мониторить** — механизм в нашем коде отсутствует (per-request dial), не воспроизводится.
