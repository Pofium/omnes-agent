...
use omnesagent_config::config::CredentialSurfaceClass;
use omnesagent_config::policy::SecurityPolicy;
use omnesagent_config::schema::{RiskProfileConfig, SandboxBackend, SandboxConfig};
...
pub fn build_report(config: &Config, agent_alias: &str) -> Result<SecurityStatusReport> {
    ...
    let sandbox_extra_roots = omnesagent_runtime::security::SandboxExtraRoots {
    ...
    let sandbox = omnesagent_runtime::security::sandbox_posture(
        &sandbox_config,
        config.runtime.kind,
...
pub fn print_report(report: &SecurityStatusReport) {
    println!(
        "{}",
        crate::t("cli-security-status-title", "OmnesAgent Security Status")
    );
    println!(
        "{}",
        crate::ta(
...
fn credential_classification_counts(
    fields: &[omnesagent_config::config::PropFieldInfo],
...
fn autonomy_level_name(level: omnesagent_config::autonomy::AutonomyLevel) -> &'static str {
    match level {
        omnesagent_config::autonomy::AutonomyLevel::ReadOnly => "read-only",
        omnesagent_config::autonomy::AutonomyLevel::Supervised => "supervised",
        omnesagent_config::autonomy::AutonomyLevel::Full => "full",
    }
}
...
mod tests {
    ...
    fn workspace_access_reports_effective_root_tiers() {
        use omnesagent_config::multi_agent::{AccessMode, AgentAlias};
    ...
    fn docker_runtime_with_explicit_docker_backend_reports_runtime_containment() {
        ...
        let mut config = config_with_agent("ops", "ops-risk", profile);
        config.runtime.kind = omnesagent_config::schema::RuntimeKind::Docker;
