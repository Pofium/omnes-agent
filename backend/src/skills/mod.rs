...
pub use omnesagent_runtime::skills::*;

use anyhow::{Context, Result};
use std::path::{Path, PathBuf};
use omnesagent_runtime::i18n::{get_required_cli_string, get_required_cli_string_with_args};
use omnesagent_runtime::skills::{ScaffoldOptions, SkillFrontmatter, SkillsService};
...
pub mod creator {
    #[allow(unused_imports)]
    pub use omnesagent_runtime::skills::creator::*;
}
pub mod audit {
    #[allow(unused_imports)]
    pub use omnesagent_runtime::skills::audit::*;
}
pub mod skill_tool {
    #[allow(unused_imports)]
    pub use omnesagent_runtime::skills::skill_tool::*;
}
pub mod skill_http {
    #[allow(unused_imports)]
    pub use omnesagent_runtime::skills::skill_http::*;
}
    ...
    match command {
        crate::SkillCommands::List { agent, bundle } => {
            ...
            if let Some(ref b) = bundle {
                ...
                let dir =
                    omnesagent_config::skill_bundles::resolve_directory(config, &install_root, b)
                ...
                for alias in config.skill_bundles.keys() {
                    if let Ok(dir) = omnesagent_config::skill_bundles::resolve_directory(
                        config,
                        &install_root,
                        alias,
                    ) {
        ...
        crate::SkillCommands::Remove {
            ...
            if let Some(ref b) = bundle {
                ...
                let target = service
                    ...
                    .map_err(anyhow::Error::msg)?;
                service
                    .remove_skill(&target, omnesagent_runtime::skills::RemoveMode::Archive)
                    .map_err(anyhow::Error::msg)?;
                println!(
                    "{}",
                    get_required_cli_string_with_args(
        ...
        crate::SkillCommands::Test { name, verbose } => {
            let results = if let Some(ref skill_name) = name {
                ...
                let mut dirs: Vec<PathBuf> = config
                    ...
                    .keys()
                    .filter_map(|a| {
                        omnesagent_config::skill_bundles::resolve_directory(config, &install_root, a)
                            .ok()
                    })
                    .collect();
...
fn resolve_install_location(
    ...
    if let Some(alias) = bundle {
        ...
        let dir = omnesagent_config::skill_bundles::resolve_directory(config, &install_root, alias)
            .map_err(anyhow::Error::msg)?;
        ...
        && let Some(agent_cfg) = config.agent(alias)
    {
        match agent_cfg.skill_bundles.as_slice() {
            [one] => {
                let dir =
                    omnesagent_config::skill_bundles::resolve_directory(config, &install_root, one)
                        .map_err(anyhow::Error::msg)?;
                return Ok(SkillLocation::Bundle {
                    alias: one.clone(),
                    dir,
...
fn collect_skill_locations(
    ...
    for alias in config.skill_bundles.keys() {
        ...
        if let Ok(dir) =
            omnesagent_config::skill_bundles::resolve_directory(config, &install_root, alias)
        {
...
fn handle_add(
    ...
    println!(
        "{}",
        omnesagent_runtime::i18n::get_required_cli_string_with_args(
    ...
    if edit {
        open_in_editor(
            &skill_dir.join(omnesagent_runtime::skills::constants::SKILL_MANIFEST_FILENAME),
        )?;
    }
    Ok(())
}
...
fn handle_edit(
    ...
    let path = match file {
        ...
        None => summary
            .directory
            .join(omnesagent_runtime::skills::constants::SKILL_MANIFEST_FILENAME),
    };
    if !path.exists() {
...
async fn handle_bundle_add(
    ...
    match omnesagent_config::skill_bundles::resolve_directory(&working, &install_root, &alias) {
        Ok(dir) => {
...
async fn handle_bundle_remove(
    ...
    let refs = omnesagent_config::alias_refs::find_bundle_refs(config, &alias);
    if !yes {
    ...
    let bundle_dir =
        omnesagent_config::skill_bundles::resolve_directory(&working, &install_root, &alias)
            .ok()
            .filter(|d| d.exists());
    ...
    let mut dirty = omnesagent_config::alias_refs::scrub_bundle_refs(&mut working, &alias);
    working
...
async fn handle_bundle_rename(
    ...
    let old_dir =
        omnesagent_config::skill_bundles::resolve_directory(&working, &install_root, &from).ok();
    ...
    let mut dirty = omnesagent_config::alias_refs::rewrite_bundle_refs(&mut working, &from, &to);
    ...
    let new_dir =
        omnesagent_config::skill_bundles::resolve_directory(&working, &install_root, &to).ok();
    for p in &dirty {
        working.mark_dirty(p);
    }
...
fn handle_bundle_list(config: &crate::config::Config) -> Result<()> {
    ...
    if bundles.is_empty() {
        println!(
            "{}",
            omnesagent_runtime::i18n::get_required_cli_string("cli-skills-bundle-list-empty")
        );
        return Ok(());
    }
    println!(
...
fn handle_bundle_show(config: &crate::config::Config, alias: String) -> Result<()> {
    ...
    if skills.is_empty() {
        println!(
            "  {}",
            omnesagent_runtime::i18n::get_required_cli_string("cli-skills-bundle-show-no-skills")
        );
    } else {
        println!(
            "  {}",
...
mod install_location_tests {
    ...
    use omnesagent_config::schema::SkillBundleConfig;
    ...
    async fn install_command_then_runtime_loads_the_skill() {
        ...
        // agent's single assigned bundle, exactly like `omnesagent skills install`.
        handle_command(
            crate::SkillCommands::Install {
