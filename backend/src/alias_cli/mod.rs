//! CLI for alias CRUD: `omnesagent {agents,providers,channels}
//! {create,list,rename,delete}`.

use anyhow::{Context, Result, bail};
use omnesagent::{AgentsCommands, ChannelsCommands, ProvidersCommands};
use omnesagent_config::alias_refs::{
...
type OwnedStateHandles = (
    ...
    Option<std::sync::Arc<dyn omnesagent_infra::session_backend::SessionBackend>>,
);

#[cfg(all(feature = "gateway", feature = "agent-runtime"))]
fn build_owned_state_handles(config: &Config) -> Result<OwnedStateHandles> {
    use std::sync::Arc;
    let mem: Arc<dyn omnesagent_memory::Memory> = if config.agents.is_empty() {
        Arc::new(omnesagent_memory::NoneMemory::new("none"))
    } else {
        Arc::from(
            omnesagent_memory::create_memory_from_config(config, None)
                .context("open memory backend for the owned-state cascade")?,
        )
    };
    let session_backend = if config.gateway.session_persistence {
        Some(
            omnesagent_infra::make_session_backend(
                &config.data_dir,
    ...
    match cmd {
        ChannelsCommands::List { channel_type } => {
            ...
            let types: Vec<String> = match channel_type {
                Some(t) => vec![t],
                None => omnesagent_config::schema::v2::V3_CHANNEL_TYPES
                    .iter()
                    .map(|s| (*s).to_string())
                    .collect(),
            };
