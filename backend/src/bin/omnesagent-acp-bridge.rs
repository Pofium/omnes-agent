...
use omnesagent_config::schema::resolve_runtime_dirs;

const CONFIG_NOT_FOUND_ERROR: &str = "ERROR: config.toml not found.  Are you sure the bridge and OmnesAgent are running on the same host?  Tool use will not work remotely!";
const PAIRING_TOKEN_NOT_FOUND_ERROR: &str = "ERROR: Gateway pairing is active but no ACP bridge token is cached. Run `omnesagent gateway get-paircode --new`, then run `omnesagent-acp-bridge --pair-code <code>`, or set OMNESAGENT_ACP_BRIDGE_TOKEN.";
const ACP_BRIDGE_TOKEN_ENV: &str = "OMNESAGENT_ACP_BRIDGE_TOKEN";
const ACP_BRIDGE_PAIRING_CODE_ENV: &str = "OMNESAGENT_ACP_PAIRING_CODE";
...
async fn run() -> Result<()> {
    ...
    let stdin_to_ws = omnesagent_spawn::spawn!(async move {
        let stdin = io::stdin();
    ...
    let ws_to_stdout = omnesagent_spawn::spawn!(async move {
...
mod tests {
    ...
    fn acp_websocket_url_includes_path_prefix() {
        assert_eq!(
            acp_websocket_url("ws", "127.0.0.1", 42617, Some("/omnesagent")),
            "ws://127.0.0.1:42617/omnesagent/acp"
        );
    }
    ...
    fn token_from_env_uses_plaintext_bridge_token() {
        let _guard = EnvGuard::set("OMNESAGENT_ACP_BRIDGE_TOKEN", "zc_plaintext");
    ...
    fn cached_token_path_lives_next_to_config_without_using_config_toml() {
        let dir = std::path::Path::new("/tmp/omnesagent-config");
    ...
    fn config_dir_from_args_supports_flag_forms() {
        assert_eq!(
            config_dir_from_args(["--config-dir".to_string(), "/tmp/omnesagent".to_string()])
                .unwrap(),
            Some("/tmp/omnesagent".to_string())
        );
        assert_eq!(
            config_dir_from_args(["--config-dir=/tmp/omnesagent".to_string()]).unwrap(),
            Some("/tmp/omnesagent".to_string())
        );
    }
    ...
    fn http_gateway_url_honors_path_prefix_and_ipv6() {
        assert_eq!(
            http_gateway_url("https", "::1", 42617, Some("/omnesagent"), "/pair"),
            "https://[::1]:42617/omnesagent/pair"
        );
    }
    ...
    fn acp_bridge_target_honors_tls_and_path_prefix() {
        let config = BridgeGatewayConfig {
            require_pairing: false,
            path_prefix: Some("/omnesagent".to_string()),
        ...
        assert_eq!(target.url, "wss://127.0.0.1:42617/omnesagent/acp");
    }
    ...
    fn missing_config_error_matches_acp_client_guidance() {
        assert_eq!(
            ...
            "ERROR: config.toml not found.  Are you sure the bridge and OmnesAgent are running on the same host?  Tool use will not work remotely!"
        );
    }
}
