//! `omnesagent update` — self-update pipeline with rollback.
...
use omnesagent_runtime::i18n::{get_required_cli_string, get_required_cli_string_with_args};
...
fn install_dir_not_writable_message(dir: &str, error: &str) -> String {
    ...
    {
        format!(
            "install directory {dir} is not writable ({error}); re-run `omnesagent update` with \
...
const GITHUB_RELEASES_LATEST_URL: &str =
    "https://api.github.com/repos/omnesagent-labs/omnesagent/releases/latest";
const GITHUB_RELEASES_TAG_URL: &str =
    "https://api.github.com/repos/omnesagent-labs/omnesagent/releases/tags";
...
pub async fn check(target_version: Option<&str>) -> Result<UpdateInfo> {
    ...
    let client = reqwest::Client::builder()
        .user_agent(format!("omnesagent/{current}"))
...
fn is_installable_release_asset(name: &str, target: &str) -> bool {
    ...
    if name == format!("omnesagent-{target}.tar.gz") || name == format!("omnesagent-{target}.tgz") {
        return true;
    }
    // On Windows the release artifacts are published as .zip
    if target.contains("windows") && name == format!("omnesagent-{target}.zip") {
        return true;
    }
    false
}
...
/// binary and written through as `staging/omnesagent`, preserving the legacy
/// single-file behavior for older release channels.
///
/// Returns the path to the freshly unpacked main `omnesagent` (or `omnesagent.exe`)
...
async fn download_release(
    ...
    let client = reqwest::Client::builder()
        .user_agent(format!("omnesagent/{}", env!("CARGO_PKG_VERSION")))
...
/// allowed to install next to the running binary, beyond the main `omnesagent`
...
/// mirrors (currently: `zerocode` next to `omnesagent`, plus the `web/dist`
...
///
/// Walks the staged tree looking for a `omnesagent` (or `omnesagent.exe`) file. The
...
/// `omnesagent-v0.9/omnesagent.exe`, which Windows zip tooling sometimes produces).
...
async fn validate_binary(path: &Path) -> Result<()> {
    ...
    if !stdout.contains("omnesagent") {
        bail!("downloaded binary does not appear to be omnesagent");
    }
...
async fn ensure_install_dir_writable(exe: &Path) -> Result<()> {
    ...
    let probe = dir.join(format!(".omnesagent-update-probe-{}", std::process::id()));
...
/// `omnesagent.exe` -> `omnesagent.exe.<pid>.old`.
#[cfg(windows)]
...
///
/// Best-effort by design — the `omnesagent` binary has already been swapped and
...
async fn install_companion_artifacts(
    ...
    if staged_web_dist.is_dir() {
        match install_web_dist(&staged_web_dist, current_exe, host_candidates).await {
            Ok(target) => ::omnesagent_log::record!(
                INFO,
                ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note)
                    .with_outcome(::omnesagent_log::EventOutcome::Success)
...
/// gateway's dashboard auto-detection ([`crates/omnesagent-gateway/src/lib.rs`])
/// and `install.sh`:
...
/// may already hold a OmnesAgent dashboard and so divert the install target.
fn host_dashboard_candidates() -> Vec<PathBuf> {
    let mut c = vec![
        PathBuf::from("/omnesagent-data/web/dist"),
        PathBuf::from("/usr/share/omnesagentlabs/web/dist"),
    ];
...
mod tests {
    ...
    fn find_asset_url_picks_correct_gnu_over_android() {
        let release = make_release(&[
            "omnesagent-aarch64-linux-android.tar.gz",
            "omnesagent-aarch64-unknown-linux-gnu.tar.gz",
            "omnesagent-x86_64-unknown-linux-gnu.tar.gz",
            "omnesagent-x86_64-apple-darwin.tar.gz",
            "omnesagent-aarch64-apple-darwin.tar.gz",
            "omnesagent-x86_64-pc-windows-msvc.tar.gz",
            "omnesagent-aarch64-pc-windows-msvc.tar.gz",
        ]);
    ...
    fn find_asset_url_ignores_non_installable_assets() {
        ...
        let release = make_release(&[
            &format!("omnesagent-{target}.tar.gz.sha256"),
            &format!("omnesagent-{target}.zip.sha256"),
            &format!("omnesagent-{target}.zip"),
            &format!("omnesagent-{target}.tar.gz"),
        ]);
    ...
    fn find_asset_url_skips_matching_asset_with_unusable_url() {
        ...
        let release = serde_json::json!({
            "assets": [
                {
                    "name": format!("omnesagent-{target}.tar.gz"),
                    "browser_download_url": ""
                },
                {
                    "name": format!("omnesagent-{target}.tgz"),
                    "browser_download_url": null
                },
                {
                    "name": format!("omnesagent-{target}.tar.gz"),
                    "browser_download_url": format!("https://example.com/omnesagent-{target}.tar.gz")
                }
            ]
        });

        let url = find_asset_url(&release).expect("should skip unusable URLs");
        assert_eq!(url, format!("https://example.com/omnesagent-{target}.tar.gz"));
    }

    #[test]
    fn find_asset_url_ignores_non_omnesagent_assets() {
        let target = current_target_triple().expect("supported test platform");
        let release = make_release(&[
        ...
        let url = find_asset_url(&release).expect("should select omnesagent asset");
        assert!(
            url.contains(&format!("omnesagent-{target}.tar.gz")),
            "should select omnesagent archive, got: {url}"
        );
    }
    ...
    fn find_sha256sums_url_skips_missing_or_unusable_url() {
        let release = serde_json::json!({
            "assets": [
                {
                    "name": "omnesagent-x86_64-unknown-linux-gnu.tar.gz",
    ...
    fn expected_sha256_for_asset_matches_text_and_binary_mode_entries() {
        ...
        let sums = format!(
            "{digest}  omnesagent-aarch64-apple-darwin.tar.gz\n\
             {digest} *omnesagent-x86_64-unknown-linux-gnu.tar.gz\n"
        );

        assert_eq!(
            expected_sha256_for_asset(&sums, "omnesagent-aarch64-apple-darwin.tar.gz").unwrap(),
            digest
        );
        assert_eq!(
            expected_sha256_for_asset(&sums, "omnesagent-x86_64-unknown-linux-gnu.tar.gz").unwrap(),
            digest
        );
    }
    ...
    fn expected_sha256_for_asset_rejects_missing_or_malformed_entry() {
        let err = expected_sha256_for_asset(
            "not-a-hex-digest  omnesagent-x86_64-unknown-linux-gnu.tar.gz\n",
            "omnesagent-x86_64-unknown-linux-gnu.tar.gz",
        )
        .unwrap_err()
        .to_string();
        ...
        let err = expected_sha256_for_asset(
            "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855  omnesagent-x86_64-unknown-linux-gnu.tar.gz extra\n",
            "omnesagent-x86_64-unknown-linux-gnu.tar.gz",
        )
        .unwrap_err()
        .to_string();
    ...
    fn verify_checksum_bytes_accepts_matching_digest_and_rejects_mismatch() {
        let asset_name = "omnesagent-x86_64-unknown-linux-gnu.tar.gz";
    ...
    fn asset_name_from_url_uses_last_path_component() {
        assert_eq!(
            asset_name_from_url(
                "https://github.com/omnesagent-labs/omnesagent/releases/download/v0.8.0/omnesagent-aarch64-apple-darwin.tar.gz"
            ),
            Some("omnesagent-aarch64-apple-darwin.tar.gz".to_string())
        );
        assert_eq!(
            asset_name_from_url(
                "https://github.com/omnesagent-labs/omnesagent/releases/download/v0.8.0/omnesagent-aarch64-apple-darwin.tar.gz?download=1#asset"
            ),
            Some("omnesagent-aarch64-apple-darwin.tar.gz".to_string())
        );
    ...
    async fn download_release_verifies_checksum_before_writing() {
        ...
        let sums = format!("{digest}  omnesagent-test.bin\n");

        Mock::given(method("GET"))
            .and(path("/omnesagent-test.bin"))
        ...
        let binary = download_release(
            &format!("{}/omnesagent-test.bin", server.uri()),
    ...
    async fn download_release_rejects_checksum_mismatch_without_writing() {
        ...
        assert!(err.contains("checksum mismatch"));
        assert!(!staging.join("omnesagent").exists());
    }
    ...
    fn unpack_tar_gz_extracts_full_tree() {
        ...
        let binary = locate_main_binary(&staging).unwrap();
        assert_eq!(std::fs::read(&binary).unwrap(), omnesagent);
        assert_eq!(
    ...
    /// Regression: verify the zip unpacker writes the omnesagent.exe
    /// binary bytes from a minimal Windows ZIP release asset.
    #[test]
    fn unpack_zip_writes_omnesagent_exe() {
        let fake_exe = b"fake omnesagent windows binary content";
        let zip_buf = make_zip(&[("omnesagent.exe", fake_exe)]);
        ...
        assert_eq!(
            std::fs::read(staging.join("omnesagent.exe")).unwrap(),
            fake_exe
        );
    }

    #[test]
    fn unpack_zip_finds_omnesagent_exe_in_subdirectory() {
        // Windows archive tools sometimes produce paths like
        // `omnesagent-v0.9/omnesagent.exe`. `locate_main_binary` walks the tree, so
        // a nested binary is still found.
        let fake_exe = b"omnesagent-exe-in-subdir";
        let zip_buf = make_zip(&[("omnesagent-v0.9/omnesagent.exe", fake_exe)]);
        ...
        // gate hides omnesagent.exe on non-Windows hosts — assert the staged
        // path directly so the test runs everywhere CI does.
        assert_eq!(
            std::fs::read(staging.join("omnesagent-v0.9/omnesagent.exe")).unwrap(),
            fake_exe
        );
    }
    ...
    fn unpack_zip_refuses_path_traversal() {
        let zip_buf = make_zip(&[("omnesagent.exe", b"fake"), ("../escape.txt", b"evil")]);
    ...
    async fn ensure_install_dir_writable_rejects_missing_dir() {
        ...
        assert!(
            err.contains("no-such-omnesagent-install-dir-9f1c"),
            "got: {err}"
        );
    }
    ...
    async fn swap_binary_replaces_target_contents() {
        let tmp = tempfile::tempdir().unwrap();
        let target = tmp.path().join("omnesagent");
        let new = tmp.path().join("omnesagent_new");
    ...
    async fn rollback_binary_restores_backup_contents() {
        let tmp = tempfile::tempdir().unwrap();
        let target = tmp.path().join("omnesagent");
        let backup = tmp.path().join("omnesagent.bak");
    ...
    fn resolve_web_dist_target_prefers_binary_adjacent_dir() {
        ...
        let exe = bin_dir.join("omnesagent");
    ...
    fn resolve_web_dist_target_falls_back_to_binary_adjacent_when_no_candidate_exists() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let exe = tmp.path().join("bin").join("omnesagent");
        std::fs::create_dir_all(tmp.path().join("bin")).unwrap();

        let target = resolve_web_dist_target(&exe, &[]);
        // Fallback either lands on the binary-adjacent dir or on the platform
        // data dir, depending on whether `/usr/share/omnesagentlabs/web/dist`
    ...
    async fn install_web_dist_swaps_directory_dropping_removed_files() {
        ...
        std::fs::create_dir_all(&bin_dir).unwrap();
        let exe = bin_dir.join("omnesagent");
        std::fs::write(&exe, b"omnesagent").unwrap();
    ...
    async fn install_companion_artifacts_swaps_top_level_siblings() {
        ...
        std::fs::write(staging.join(main_binary_name()), b"new omnesagent").unwrap();
        ...
        assert_eq!(std::fs::read(&exe).unwrap(), b"omnesagent");
    }
    ...
    async fn install_companion_artifacts_skips_unknown_top_level_files() {
        ...
        // `zerodash`, `.bashrc`, `evil.so`, etc. next to `omnesagent` just by
