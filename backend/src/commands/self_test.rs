//! `omnesagent self-test` — quick and full diagnostic checks.

use anyhow::Result;
use std::path::Path;
use omnesagent_runtime::i18n::get_required_cli_string_with_args;
...
fn check_channel_config(config: &crate::config::Config) -> CheckResult {
    let channels = omnesagent_channels::listing::compiled_channels(&config.channels);
    let configured = channels.iter().filter(|e| e.configured).count();
    let uncompiled = omnesagent_channels::listing::configured_uncompiled_channels(&config.channels);
...
fn check_web_dist_dir(config: &crate::config::Config) -> CheckResult {
    ...
    match config.gateway.web_dist_dir.as_deref() {
        None => CheckResult::pass(
            name,
            omnesagent_runtime::i18n::get_required_cli_string(
        ...
        Some(value) => match web_dist_dir_expansion_reason_key(value) {
            ...
            Some(reason_key) => {
                let reason = omnesagent_runtime::i18n::get_required_cli_string(reason_key);
                CheckResult::fail(
                    name,
                    omnesagent_runtime::i18n::get_required_cli_string_with_args(
...
fn web_dist_dir_check_name() -> &'static str {
    ...
    CACHED.get_or_init(|| {
        let resolved =
            omnesagent_runtime::i18n::get_required_cli_string("cli-self-test-web-dist-dir-name");
        ...
        return CheckResult::fail(
            ...
            format!(
                "pairing required but no bearer token available for self-test \
                 (set OMNESAGENT_GATEWAY_TOKEN or keep a plaintext zc_* entry in \
...
/// Precedence: `OMNESAGENT_GATEWAY_TOKEN`, then `OMNESAGENT_ACP_BRIDGE_TOKEN`,
...
fn resolve_gateway_bearer_token(config: &crate::config::Config) -> Option<String> {
    for key in ["OMNESAGENT_GATEWAY_TOKEN", "OMNESAGENT_ACP_BRIDGE_TOKEN"] {
...
mod tests {
    ...
    #[cfg(unix)]
    use omnesagent_config::schema::LucidStorageConfig;
...
"#,
    ...
    fn check_web_dist_dir_emits_localized_fail_for_tilde() {
        ...
        let expected_reason =
            omnesagent_runtime::i18n::get_required_cli_string("cli-web-dist-dir-reason-tilde");
        let expected_detail = omnesagent_runtime::i18n::get_required_cli_string_with_args(
            "cli-self-test-web-dist-dir-fail-expansion",
    ...
    fn check_web_dist_dir_emits_localized_pass_when_unset() {
        ...
        let expected_detail = omnesagent_runtime::i18n::get_required_cli_string(
            "cli-self-test-web-dist-dir-pass-unset",
        );
        assert_eq!(result.detail, expected_detail);
    }
