...
use std::sync::Arc;
use omnesagent::agent::agent::Agent;
use omnesagent::agent::dispatcher::{NativeToolDispatcher, XmlToolDispatcher};
use omnesagent::config::MemoryConfig;
use omnesagent::memory;
use omnesagent::memory::Memory;
use omnesagent::observability::{NoopObserver, Observer};
use omnesagent::providers::{ChatResponse, ModelProvider, ToolCall};
use omnesagent::tools::Tool;
...
pub fn build_agent(model_provider: Box<dyn ModelProvider>, tools: Vec<Box<dyn Tool>>) -> Agent {
    Agent::builder()
        ...
        .tools(omnesagent::tools::scoped::ScopedToolRegistry::from_raw_for_test(tools))
        .memory(make_memory())
...
pub fn build_recording_agent(
    ...
    memory: Option<Arc<dyn omnesagent::memory::Memory>>,
) -> Agent {
    Agent::builder()
        .model_provider(model_provider)
        .tools(omnesagent::tools::scoped::ScopedToolRegistry::from_raw_for_test(tools))
...
#[async_trait]
impl omnesagent::memory::Memory for StaticRecallMemory {
    fn name(&self) -> &str {
        "static-recall"
    }
    async fn store(
        ...
        _key: &str,
        _content: &str,
        _category: omnesagent::memory::MemoryCategory,
    ...
    async fn recall(
        ...
        _since: Option<&str>,
        _until: Option<&str>,
    ) -> anyhow::Result<Vec<omnesagent::memory::MemoryEntry>> {
        Ok(self
            .entries
            .iter()
            .map(|(k, v)| omnesagent::memory::MemoryEntry {
                id: k.clone(),
                key: k.clone(),
                content: v.clone(),
                category: omnesagent::memory::MemoryCategory::Core,
    ...
    async fn get(&self, _key: &str) -> anyhow::Result<Option<omnesagent::memory::MemoryEntry>> {
        Ok(None)
    }
    async fn list(
        &self,
        _category: Option<&omnesagent::memory::MemoryCategory>,
        _session_id: Option<&str>,
    ) -> anyhow::Result<Vec<omnesagent::memory::MemoryEntry>> {
        Ok(vec![])
    }
...
impl omnesagent_api::attribution::Attributable for StaticRecallMemory {
    fn role(&self) -> omnesagent_api::attribution::Role {
        omnesagent_api::attribution::Role::Memory(omnesagent_api::attribution::MemoryKind::InMemory)
    }
    fn alias(&self) -> &str {
        "StaticRecallMemory"
    }
