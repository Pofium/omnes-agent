#!/usr/bin/env bash
# Exercise omnesagent quickstart's provider input paths via its non-interactive flags.
set -euo pipefail

BIN="${BIN:-./target/debug/omnesagent}"
TMPROOT="$(mktemp -d)"
...
run_case() {
  ...
  mkdir -p "$cfgdir"
  echo "─── $label ───"
  env OMNESAGENT_CONFIG_DIR="$cfgdir" "$BIN" quickstart "$@"
  echo
