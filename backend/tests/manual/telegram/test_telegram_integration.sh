#!/bin/bash
# OmnesAgent Telegram Integration Test Suite
...
if [ -f "target/release/omnesagent" ]; then
    BINARY_SIZE=$(ls -lh target/release/omnesagent | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/omnesagent 2>/dev/null || stat -c%s target/release/omnesagent)
...
CONFIG_PATH="$HOME/.omnesagent/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'omnesagent config set channels.telegram.default.bot-token <token>' first"
    fi
else
    warn "No config file found - run 'omnesagent quickstart' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/omnesagent channel doctor 2>&1 || true)
END_TIME=$(date +%s)
