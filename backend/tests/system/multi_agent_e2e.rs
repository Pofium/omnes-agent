...
fn legacy_install_upgrades_cleanly_with_backup() {
    ...
    let report = omnesagent_config::schema::v2::migrate_v2_to_v3_install_filesystem(install_root)
    ...
    let report_again =
        omnesagent_config::schema::v2::migrate_v2_to_v3_install_filesystem(install_root)
...
async fn two_sqlite_agents_on_one_install_have_isolated_memory() {
    ...
    let alpha_mem = omnesagent_memory::create_memory_for_agent(&cfg, "alpha", None)
        .await
        .expect("per-agent memory for alpha");
    let beta_mem = omnesagent_memory::create_memory_for_agent(&cfg, "beta", None)
        .await
    ...
    alpha_mem
        .store(
            "alpha-key",
            "alpha owns this row",
            omnesagent_memory::MemoryCategory::Core,
            None,
        )
        .await
        .expect("alpha store");
...
async fn peer_group_routes_messages_only_within_resolved_peer_set() {
    ...
    use omnesagent_config::schema::{AliasedAgentConfig, Config, RiskProfileConfig};
    use omnesagent_runtime::peers::resolve_peer_set;
    use omnesagent_runtime::tools::SendMessageToPeerTool;
    ...
    assert_eq!(
        gamma_peers,
        omnesagent_runtime::peers::ResolvedPeers::default(),
...
async fn peer_group_dotted_channel_refs_remain_alias_scoped_for_dispatch() {
    ...
    use std::sync::Arc;
    use omnesagent_api::tool::Tool;
    use omnesagent_config::multi_agent::{AgentAlias, PeerGroupConfig};
    use omnesagent_config::providers::ChannelRef;
    use omnesagent_config::schema::{AliasedAgentConfig, Config, RiskProfileConfig};
    use omnesagent_runtime::peers::resolve_peer_set;
    use omnesagent_runtime::tools::SendMessageToPeerTool;
