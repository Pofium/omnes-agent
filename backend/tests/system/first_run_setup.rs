...
//! (`omnesagent_runtime::quickstart::apply_with_surface`) driven with a
...
//! `Config::load_or_init()` loader, reached by spawning the actual `omnesagent`
...
//! (`crates/omnesagent-gateway/src/api_quickstart.rs`). A bug that lives purely
...
//!    [`run_omnesagent`] with a read-only subcommand and check its output.
...
use omnesagent_config::schema::{Config, DEFAULT_WEBHOOK_CHANNEL_PORT};
use omnesagent_config::secrets::SecretStore;
use omnesagent_runtime::quickstart::{self, FieldSection, Surface};
...
impl FirstRun {
    ...
    async fn quickstart(submission: BuilderSubmission) -> Self {
        ...
        let reloaded = omnesagent_config::migration::migrate_to_current(&raw_text)
    ...
    fn with_corrupted_disk_view(mut self, mutate: impl FnOnce(&mut toml::Table)) -> Self {
        ...
        self.reloaded = omnesagent_config::migration::migrate_to_current(&raw_text)
    ...
    /// (4a) `omnesagent agents list` through the real loader reports exactly
    ...
    fn assert_loader_lists_agents(&self, expected: &[&str]) {
        let stdout = stdout_of(
            &run_omnesagent(self.install_root(), &["agents", "list"]),
            "agents list",
        );
    ...
    /// (4b) `omnesagent config get <agent>.channels` through the real loader
    ...
    fn assert_loader_reports_agent_channels(&self, agent: &str, expected: &[&str]) {
        ...
        let stdout = stdout_of(
            &run_omnesagent(self.install_root(), &["config", "get", &path, "--json"]),
...
/// Run the real `omnesagent` binary against a first-run install root.
///
/// Every `OMNESAGENT_*` variable inherited from the developer's shell is stripped
/// from the child before `OMNESAGENT_CONFIG_DIR` is set, so an operator env var
/// (the `OMNESAGENT_*` config-override grammar included) cannot leak into the
...
fn run_omnesagent(install_root: &Path, args: &[&str]) -> std::process::Output {
    let mut command = std::process::Command::new(env!("CARGO_BIN_EXE_omnesagent"));
    for (key, _) in std::env::vars_os() {
        if key.to_string_lossy().starts_with("OMNESAGENT_") {
            command.env_remove(&key);
        }
    }
    command
        .args(args)
        .env("OMNESAGENT_CONFIG_DIR", install_root)
        .output()
        .expect("failed to spawn the omnesagent binary")
}
...
fn stdout_of(output: &std::process::Output, what: &str) -> String {
    assert!(
        ...
        "`omnesagent {what}` exited with {:?} against a first-run config\n--- stdout ---\n{}\n--- stderr ---\n{}",
        output.status.code(),
...
/// `Config::load_or_init()`. Here the actual `omnesagent` binary is spawned
...
async fn first_run_config_loads_through_the_real_binary() {
    ...
    let listing = stdout_of(&run_omnesagent(root, &["channel", "list"]), "channel list");
    assert!(
...
/// written: `omnesagent doctor` must see the channel as configured and must not
...
async fn first_run_doctor_agrees_the_channel_is_configured() {
    ...
    let report = omnesagent_runtime::doctor::diagnose(&run.reloaded_with_paths());
...
async fn guard_real_binary_reports_a_channel_that_is_no_longer_configured() {
    ...
    let listing = stdout_of(
        &run_omnesagent(run.install_root(), &["channel", "list"]),
        "channel list",
    );
    assert!(
        listing.contains("❌ Telegram"),
