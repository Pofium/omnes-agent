...
use omnesagent::providers::{ChatResponse, ConversationMessage, ToolCall};
...
async fn e2e_multi_turn_with_memory_enrichment() {
    ...
    let mem = Arc::new(StaticRecallMemory::new(&[("project", "omnesagent")]));
    ...
    assert!(req1_user.content.contains("project: omnesagent"));
