...
use omnesagent::memory::sqlite::SqliteMemory;
use omnesagent::memory::traits::{Memory, MemoryCategory};
use omnesagent::providers::ToolCall;
...
async fn agent_survives_large_tool_output() {
    use omnesagent::tools::{Tool, ToolResult};

    /// Tool that returns a very large output.
    struct LargeOutputTool;

    impl ::omnesagent_api::attribution::Attributable for LargeOutputTool {
...
async fn consolidation_extracts_facts_to_memory() {
    ...
    let result = omnesagent::memory::consolidation::consolidate_turn(
        &model_provider,
        "test-model",
        None,
        mem.as_ref(),
...
async fn memory_survives_rapid_consolidation() {
    ...
    for i in 0..10 {
        ...
        let _ = omnesagent::memory::consolidation::consolidate_turn(
            &model_provider,
            "test-model",
            None,
            mem.as_ref(),
...
async fn session_backend_persists_messages() {
    use omnesagent::channels::session_backend::SessionBackend;
    use omnesagent::channels::session_sqlite::SqliteSessionBackend;
    use omnesagent::providers::traits::ChatMessage;
