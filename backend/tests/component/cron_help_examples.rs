...
const PROMPT_EXAMPLE_TZ: &str = "omnesagent cron add '0 9 * * 1-5' 'Good morning' --agent sentinel --prompt --tz America/New_York";
const PROMPT_EXAMPLE: &str =
    "omnesagent cron add '*/30 * * * *' 'Check system health' --agent sentinel --prompt";
const SHELL_EXAMPLE: &str = "omnesagent cron add '*/5 * * * *' 'echo ok' --agent sentinel";
...
fn run(config_dir: &Path, args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_omnesagent"))
        .env("OMNESAGENT_CONFIG_DIR", config_dir)
        .env("RUST_LOG", "off")
        .args(args)
        .output()
        .expect("failed to run omnesagent binary")
}
...
fn add_at_examples(help: &str) -> Vec<&str> {
    help.lines()
        .map(str::trim)
        .filter(|line| line.starts_with("omnesagent cron add-at "))
        .collect()
}
    ...
    for expected in ["'Send reminder'", "'Happy New Year!'"] {
        assert!(
            add_at_examples.iter().any(|example| {
                example.starts_with("omnesagent cron add-at --agent morning-shift --prompt ")
...
fn cron_list_empty_state_shows_runnable_hint() {
    ...
    assert!(
        stdout.contains("omnesagent cron add '0 9 * * *' 'echo ok' --agent sentinel"),
