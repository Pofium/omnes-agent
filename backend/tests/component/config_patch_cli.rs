//! Regression coverage for `omnesagent config patch --json` output.
//!
...
impl Attributable for MockModelProvider {
    fn role(&self) -> omnesagent_api::attribution::Role {
        omnesagent_api::attribution::Role::Provider(omnesagent_api::attribution::ProviderKind::Model(
...
fn test_state(config: Config) -> AppState {
    ...
    AppState {
        ...
        admin_auth: omnesagent_gateway::admin_auth::AdminAuthManager::mock(),
...
fn run_cli_patch_output(config_dir: &std::path::Path, patch_doc: &[u8]) -> Output {
    let bin = env!("CARGO_BIN_EXE_omnesagent");
    Command::new(bin)
        .env("OMNESAGENT_CONFIG_DIR", config_dir)
        .env("RUST_LOG", "off")
        ...
        .expect("run omnesagent config patch")
}

/// Run `omnesagent config patch - ` **without** `--json`, exercising the
...
fn run_cli_patch_output_human(config_dir: &std::path::Path, patch_doc: &[u8]) -> Output {
    ...
    Command::new(bin)
        ...
        .expect("run omnesagent config patch (human mode)")
}
...
fn config_patch_replace_on_dotted_resource_id_does_not_plant_phantom_sibling() {
    let config_dir = tempfile::tempdir().expect("temp config dir");
    let version = omnesagent_config::migration::CURRENT_SCHEMA_VERSION;
    std::fs::write(
...
fn run_cli_init(config_dir: &std::path::Path, section: &str) -> serde_json::Value {
    ...
    let output = Command::new(bin)
        ...
        .stderr(Stdio::piped())
        .output()
        .expect("run omnesagent config init");
    assert!(
...
fn run_cli_get(config_dir: &std::path::Path, path: &str) -> serde_json::Value {
    ...
    let output = Command::new(bin)
        ...
        .stderr(Stdio::piped())
        .output()
        .expect("run omnesagent config get");
    assert!(
        output.status.success(),
...
fn run_cli_set(config_dir: &std::path::Path, path: &str, value: &str) {
    ...
    let output = Command::new(bin)
        ...
        .stderr(Stdio::piped())
        .output()
        .expect("run omnesagent config set");
    assert!(
        output.status.success(),
...
fn required_field_sections_can_be_completed_across_cli_processes() {
    let cases: &[(&str, &[(&str, &str)])] = &[
        (
            "gateway.tls",
            &[
                ("gateway.tls.cert_path", "/tmp/omnesagent-test-cert.pem"),
                ("gateway.tls.key_path", "/tmp/omnesagent-test-key.pem"),
            ],
        ),
        (
        ...
        (
            "tunnel.openvpn",
            &[("tunnel.openvpn.config_file", "/tmp/omnesagent-test.ovpn")],
        ),
    ];
