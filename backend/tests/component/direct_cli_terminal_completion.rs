//! Regression coverage for direct `omnesagent agent` terminal-failure delivery.
//!
...
"#,
    ...
    let output = std::thread::spawn(move || {
        Command::new(env!("CARGO_BIN_EXE_omnesagent"))
            ...
            .output()
            .expect("run omnesagent agent")
    })
    .join()
    .expect("omnesagent agent process must not panic");

    let stderr = String::from_utf8_lossy(&output.stderr);
    let expected = omnesagent_runtime::agent::semantic_empty_terminal_completion_message(None);
    assert!(
...
"#,
    ...
    let config_dir_arg = config_dir.path().to_path_buf();
    let output = std::thread::spawn(move || {
        let mut child = Command::new(env!("CARGO_BIN_EXE_omnesagent"))
            .env("RUST_LOG", "off")
            .args([
                "--config-dir",
                config_dir_arg.to_str().expect("UTF-8 config path"),
            ...
            .stderr(Stdio::piped())
            .spawn()
            .expect("start interactive omnesagent agent");
        child
            .stdin
            .as_mut()
            .expect("piped stdin")
        ...
        child
            .wait_with_output()
            .expect("wait for interactive omnesagent agent")
    })
    .join()
    .expect("interactive omnesagent agent process must not panic");
