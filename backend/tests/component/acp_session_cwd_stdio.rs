...
//!
//! The unit tests in `crates/omnesagent-channels/.../acp_server.rs` drive
//! `handle_session_new` / `serve_reader` directly; this test crosses the real
//! `omnesagent acp` **process boundary** required by the repository's
...
/// root is the directory holding `config.toml` (i.e. `OMNESAGENT_CONFIG_DIR`).
///
...
fn acp_stdio_session_new_omitted_cwd_returns_agent_workspace() {
    ...
    // Launch the real `omnesagent acp` command against the isolated config/home.
