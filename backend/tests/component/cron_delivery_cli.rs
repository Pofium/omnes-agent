//! Regression: `omnesagent cron` delivery flags at the shipped CLI boundary.
//!
...
fn seed_declarative_job(config_dir: &Path) {
    ...
    let decl = omnesagent_config::schema::CronJobDecl {
        ...
        uses_memory: true,
        session_target: None,
        delivery: Some(omnesagent_config::schema::DeliveryConfigDecl {
...
fn cron_update_rejects_delivery_on_a_declarative_job() {
    ...
    // how `omnesagent-gateway`'s own declarative test sets this up, and it keeps the
