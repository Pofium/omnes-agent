...
fn root_schema_export_feature_graph_is_gated() {
    ...
    assert!(
        schema_export
            .iter()
            .any(|value| value.as_str() == Some("omnesagent-config/schema-export")),
        "root features.schema-export must forward omnesagent-config/schema-export; \
         got {schema_export:?}"
    );
    ...
    let omnesagent_config = workspace_dependencies
        .get("omnesagent-config")
        .and_then(toml::Value::as_table)
        .expect("workspace.dependencies must define omnesagent-config as a table");
    assert_eq!(
        omnesagent_config
            .get("default-features")
            .and_then(toml::Value::as_bool),
        Some(false),
        "workspace.dependencies.omnesagent-config must set default-features=false"
    );
}
