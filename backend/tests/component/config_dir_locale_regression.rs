...
const JA_CLI_FTL: &str = include_str!("../../crates/omnesagent-runtime/locales/ja/cli.ftl");
const ES_CLI_FTL: &str = include_str!("../../crates/omnesagent-runtime/locales/es/cli.ftl");
    ...
    S: AsRef<OsStr>,
{
    let out = Command::new(env!("CARGO_BIN_EXE_omnesagent"))
        .args(args)
        .output()
        .expect("failed to run omnesagent --help");
    let stdout = String::from_utf8_lossy(&out.stdout);
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        out.status.success(),
        "omnesagent help failed with status {:?}\nstdout:\n{stdout}\nstderr:\n{stderr}",
        out.status.code()
    );
    stdout.into_owned()
}
