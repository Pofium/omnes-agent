...
use omnesagent::config::migration;
use omnesagent::config::{ChannelsConfig, Config, GatewayConfig, RiskProfileConfig, SecurityConfig};
...
fn gateway_config_toml_roundtrip() {
    let gw = GatewayConfig {
        ...
        path_prefix: Some("/omnesagent".into()),
        ..Default::default()
    };
    ...
    assert_eq!(parsed.path_prefix.as_deref(), Some("/omnesagent"));
}
...
fn gateway_path_prefix_rejects_missing_leading_slash() {
    ...
    config.gateway.path_prefix = Some("omnesagent".into());
...
fn gateway_path_prefix_accepts_valid_prefixes() {
    for prefix in ["/omnesagent", "/apps/omnesagent", "/api/hassio_ingress/abc123"] {
        let mut config = Config::default();
...
fn gateway_path_prefix_rejects_unsafe_characters() {
    ...
    for prefix in [" /omnesagent ", " /omnesagent"] {
        let mut config = Config::default();
        config.gateway.path_prefix = Some(prefix.into());
        assert!(
...
session_path = "~/.omnesagent/state/whatsapp-web/session.db"
allowed_numbers = ["*"]
"#;
    let parsed: Config = toml::from_str(toml_str)
    ...
    assert_eq!(
        ...
        Some("~/.omnesagent/state/whatsapp-web/session.db")
    );
}
