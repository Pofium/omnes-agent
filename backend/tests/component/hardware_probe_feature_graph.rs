...
fn distribution_inputs(root: &toml::Value) -> Vec<&str> {
    let extra_features = root
        ...
        .and_then(toml::Value::as_table)
        .and_then(|metadata| metadata.get("omnesagent"))
        .and_then(toml::Value::as_table)
        .and_then(|omnesagent| omnesagent.get("dist_extra_features"))
        .and_then(toml::Value::as_array)
...
fn probe_boundary_violations<'a>(reachable: &BTreeSet<&'a str>) -> Vec<&'a str> {
    ...
    reachable
        ...
        .copied()
        .filter(|reference| {
            if matches!(*reference, "hardware" | "probe" | "dep:omnesagent-hardware") {
                return true;
            }
            ...
            dependency_feature_is_active
                && ((dependency == "omnesagent-hardware" && matches!(feature, "hardware" | "probe"))
                    || (dependency == "omnesagent-tools" && feature == "probe"))
        })
        .collect()
}
...
fn probe_boundary_applies_weak_dependency_feature_semantics() {
    let active_weak_edge = BTreeSet::from(["dep:omnesagent-tools", "omnesagent-tools?/probe"]);
    assert_eq!(
        probe_boundary_violations(&active_weak_edge),
        vec!["omnesagent-tools?/probe"]
    );

    let inactive_weak_edge = BTreeSet::from(["omnesagent-tools?/probe"]);
    assert!(probe_boundary_violations(&inactive_weak_edge).is_empty());

    let strong_edge = BTreeSet::from(["omnesagent-tools/probe"]);
    assert_eq!(
...
fn probe_feature_graph_preserves_forwarding_and_distribution_boundaries() {
    ...
    let hardware = parse_manifest(
        include_str!("../../crates/omnesagent-hardware/Cargo.toml"),
        "omnesagent-hardware Cargo.toml",
    );
    let tools = parse_manifest(
        include_str!("../../crates/omnesagent-tools/Cargo.toml"),
        "omnesagent-tools Cargo.toml",
    );
    let root_features = feature_table(&root, "root Cargo.toml");
    let hardware_features = feature_table(&hardware, "omnesagent-hardware Cargo.toml");
    let tools_features = feature_table(&tools, "omnesagent-tools Cargo.toml");

    let root_hardware = feature_values(root_features, "hardware");
    assert_feature_contains("root hardware", &root_hardware, "dep:omnesagent-hardware");
    assert_feature_contains(
        "root hardware",
        &root_hardware,
        "omnesagent-hardware/hardware",
    );

    let root_probe = feature_values(root_features, "probe");
    assert_feature_contains("root probe", &root_probe, "dep:omnesagent-hardware");
    assert_feature_contains("root probe", &root_probe, "omnesagent-hardware/probe");
    ...
    assert_feature_contains("omnesagent-hardware probe", &hardware_probe, "dep:probe-rs");
    assert_feature_contains(
        "omnesagent-hardware probe",
        &hardware_probe,
        "omnesagent-tools/probe",
    );

    let tools_probe = feature_values(tools_features, "probe");
    assert_feature_contains("omnesagent-tools probe", &tools_probe, "dep:probe-rs");
