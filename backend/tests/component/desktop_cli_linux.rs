//! Linux process-boundary proof for `omnesagent desktop`.

use std::path::{Path, PathBuf};
use std::process::{Command, Output};
use std::time::{Duration, Instant};

const DOWNLOAD_URL: &str = "https://github.com/omnesagent-labs/omnesagent/releases/latest";
...
fn desktop_command(config: &Path, home: &Path, xdg_data: &Path, path: &Path) -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_omnesagent"));
    command
        .env_clear()
        .env("HOME", home)
...
fn desktop_launch_and_install_reach_the_linux_process_boundary() {
    ...
    let output = desktop_command(&config, &home, &xdg_data, &path_dir)
        .output()
        .expect("failed to run `omnesagent desktop`");
    ...
    let output = desktop_command(&config, &home, &xdg_data, &path_dir)
        .arg("--install")
        .output()
        .expect("failed to run `omnesagent desktop --install`");
    assert_success(&output, "desktop install-page open");
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Opening the OmnesAgent companion app download page"),
    ...
    let output = desktop_command(&config, &home, &xdg_data, &path_dir)
        .arg("--help")
        .output()
        .expect("failed to run `omnesagent desktop --help`");
