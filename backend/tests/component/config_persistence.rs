...
use std::fs;
use omnesagent::config::{Config, MemoryConfig};
...
fn config_toml_roundtrip_preserves_provider() {
    use omnesagent::config::{DeepseekModelProviderConfig, ModelProviderConfig};
    ...
    let parsed = omnesagent::config::migration::migrate_to_current(&toml_str)
...
fn config_file_write_read_roundtrip() {
    use omnesagent::config::{MistralModelProviderConfig, ModelProviderConfig};
    ...
    let parsed = omnesagent::config::migration::migrate_to_current(&read_back)
        .expect("TOML should round-trip through migration");
