...
//! capability is absent from the registry. `omnesagent doctor` reads
...
use omnesagent_config::schema::LogPersistence;
...
fn doctor_stdout(dir: &Path) -> String {
    let out = Command::new(env!("CARGO_BIN_EXE_omnesagent"))
        .env("OMNESAGENT_CONFIG_DIR", dir)
        .arg("doctor")
        .output()
        .expect("run omnesagent doctor");
...
/// `hide_internal_drops_internal_category` in `omnesagent-log` already pins the
...
fn the_withheld_notice_is_recorded_once_per_config_application() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_omnesagent"))
        .env("OMNESAGENT_CONFIG_DIR", dir.path())
        .args(["peripheral", "add", "rpi-gpio", "native"])
        .output()
        .expect("run omnesagent peripheral add");
    assert!(
        out.status.success(),
