...
use omnesagent_config::providers::{ChannelRef, ModelProviderRef};
...
const MANIFEST: &str =
    "crates/omnesagent-plugins/tests/fixtures/channel-fixture/plugin-manifest.toml";
    ...
    FIXTURE
        .get_or_init(|| {
            let fixture_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("crates/omnesagent-plugins/tests/fixtures/channel-fixture");
            let target_dir =
...
async fn configured_channel_reaches_real_guest_and_shared_listener_contract() {
    ...
    let channels =
        omnesagent_runtime::plugin_runtime::configured_plugin_channels(Arc::new(config), None).await;
    ...
    let listener = omnesagent_spawn::spawn!(async move { listener_channel.listen(tx).await });
