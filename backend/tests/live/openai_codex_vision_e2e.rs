...
use anyhow::Result;
use omnesagent::providers::{ChatMessage, ChatRequest, ModelProviderRuntimeOptions};
...
async fn openai_codex_second_vision_support() -> Result<()> {
    ...
    let model_provider =
        omnesagent::providers::create_model_provider_with_options("openai-codex", None, &opts)?;
