...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.omnesagent/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let omnesagent_dir = PathBuf::from(home).join(".omnesagent");
    let auth_profiles_path = omnesagent_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: omnesagent auth login --model-provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::Error::msg(
                "No Gemini OAuth profile found. Run: omnesagent auth login --model-provider gemini",
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.omnesagent/auth-profiles.json (with expired token)
    let model_provider = omnesagent::providers::create_model_provider("gemini", None)?;
