...
const SCAN_ROOTS: &[&str] = &[
    "src",
    "crates/omnesagent-gateway/src",
    "crates/omnesagent-providers/src/auth",
    "crates/omnesagent-runtime/src/approval",
];
...
fn user_facing_strings_route_through_fluent() {
    ...
    assert!(
        ...
        "Bare user-facing string literal baseline changed. User-facing text must \
         ...
         uses a `cli-*` Fluent key via `omnesagent_runtime::i18n`; other presentation \
