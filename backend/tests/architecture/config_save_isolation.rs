...
//! path. `Config::default()` targets the real ~/.omnesagent, so an
...
const ISOLATION_MARKERS: &[&str] = &["config_path", "OMNESAGENT_CONFIG_DIR", "set_var(\"HOME\""];
...
fn tests_that_persist_config_isolate_the_path() {
    ...
    assert!(
        ...
        "Config-persisting test code without path isolation detected. \
         `Config::default()` targets the real ~/.omnesagent; a test that \
         saves it clobbers the developer's live config. Set `config_path` \
         to a TempDir (or override HOME / OMNESAGENT_CONFIG_DIR to a tempdir) \
