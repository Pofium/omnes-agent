...
fn root_compose_publishes_on_host_loopback_by_default() {
    ...
    let required_overrides =
        "- OMNESAGENT_gateway__host=0.0.0.0\n      - OMNESAGENT_gateway__allow_public_bind=true";
    ...
    assert!(
        compose.contains("${HOST_PORT:-127.0.0.1:42617}:${OMNESAGENT_GATEWAY_PORT:-42617}"),
...
fn compose_smoke_proves_override_precedence_through_the_published_port() {
    ...
    for required in [
        ...
        "port omnesagent 42617",
        "http://127.0.0.1:${published_port}/health",
        ":/omnesagent-data/.omnesagent/config.toml:ro",
    ] {
        assert!(
...
fn plugin_enabled_container_variants_stage_wit_and_have_source_build_coverage() {
    ...
    for (dockerfile, tag, cache_scope) in [
        (
            "Dockerfile.debian",
            "omnesagent-pr-source-debian-wasm:build",
            "docker-source-debian-wasm",
        ),
        (
            "Dockerfile.alpine",
            "omnesagent-pr-source-alpine-wasm:build",
    ...
    assert!(
        build.contains(
            ...
            "build-args: ${{ matrix.cargo_flags && format('OMNESAGENT_CARGO_FLAGS={0}', matrix.cargo_flags) || '' }}"
        ),
