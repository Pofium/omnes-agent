...
fn macos_desktop_release_notarizes_published_dmg() {
    ...
    for required in [
        "MACOS_DMG_PATH: desktop-assets/OmnesAgent.dmg",
...
fn package_publishers_use_canonical_sources_and_scoped_credentials() {
    ...
    assert!(
        scoop.contains(
            "bash scripts/release/scoop_metadata.sh dist/scoop/omnesagent.json \"$version\""
        ),
    ...
    for required in [
        ...
        "tarball_url=\"https://github.com/omnesagent-labs/omnesagent/archive/refs/tags/${RELEASE_TAG}.tar.gz\"",
    ...
    let clone_position = aur
        .find("git clone --quiet ssh://aur@aur.archlinux.org/omnesagentlabs.git")
...
fn aur_publisher_rejects_stale_release_downgrades() {
    ...
    let srcinfo = |epoch: Option<u32>, version: &str, release: &str| {
        ...
        format!(
            "pkgbase = omnesagentlabs\n{epoch}pkgver = {version}\npkgrel = {release}\npkgname = omnesagentlabs\n"
        )
    };
    let pkgbuild = |epoch: Option<u32>, version: &str, release: &str| {
        let epoch = epoch.map_or_else(String::new, |value| format!("epoch={value}\n"));
        format!("pkgname=omnesagentlabs\n{epoch}pkgver={version}\npkgrel={release}\n")
    };
...
fn scoop_publisher_metadata_follows_canonical_url_template() {
    ...
    let manifest_path = temp.path().join("omnesagent.json");
    fs::write(
        &manifest_path,
        r#"{
  "autoupdate": {
    "architecture": {
      "64bit": {
        "url": "https://downloads.example.test/renamed/repository/releases/v$version/omnesagent-renamed.zip"
      }
    }
  }
}"#,
    ...
    assert_eq!(
        metadata["zip_url"],
        "https://downloads.example.test/renamed/repository/releases/v1.2.3/omnesagent-renamed.zip"
    );
    assert_eq!(metadata["asset_name"], "omnesagent-renamed.zip");
    assert_eq!(
        metadata["sums_url"],
    ...
    for invalid_template in [
        ...
        "https://downloads.example.test/releases/latest/omnesagent.zip",
    ] {
