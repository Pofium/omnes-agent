...
struct Args {
    ...
    /// ~/.omnesagent). Mirrors `omnesagent --config-dir`.
    #[arg(long)]
...
async fn translate_batch(
    provider: &dyn omnesagent_api::model_provider::ModelProvider,
    model: &str,
    locale: &str,
    batch: &[&str],
) -> BatchResult {
    ...
    for source in batch {
        ...
        let content = omnesagent_providers::ProviderDispatch::from_ref(provider)
...
mod tests {
    ...
    fn rejects_translation_that_introduces_local_absolute_path() {
        assert!(matches!(
            check_for_leak(
                ...
                "Le journal est dans /private/tmp/omnesagent/fr.failures.log.",
            ),
    ...
    fn allows_translation_that_preserves_source_path() {
        assert!(matches!(
            check_for_leak(
                "Write `/home/alice/omnesagent/web/dist`.",
                "Écrivez `/home/alice/omnesagent/web/dist`.",
            ),
            LeakCheck::Clean
        ));
    }
