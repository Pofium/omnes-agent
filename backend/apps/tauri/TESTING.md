...
The desktop app is a thin shell over a running OmnesAgent **web gateway**. There
...
> daemon, or spawns `omnesagent daemon` itself (preferring a kernel bundled
> next to the app executable, then `PATH` and the common install dirs — see
> `src/daemon.rs::find_omnesagent_binary`). The self-contained installer below
> bundles the kernel as a Tauri sidecar — the "full experience" distribution
> from architecture RFC fnd-001, D5.
>
> **Run `omnesagent daemon`, not `omnesagent gateway start`.** Both serve the
...
`omnesagent`. To produce the zero-install artifact — double-click on a machine
...
`OMNESAGENT_KERNEL_PATH` can reuse a prebuilt single-target kernel, but that
...
app executable as `omnesagent`, which is the first place
`find_omnesagent_binary()` looks — so the bundled app starts its own daemon
from its own kernel.

To verify self-containment, launch on a machine (or shell) where `omnesagent`
is not on `PATH` and not in `~/.cargo/bin`, then check the daemon's process
path points inside the app bundle:

```sh
pgrep -fl 'omnesagent daemon'   # expect .../OmnesAgent.app/Contents/MacOS/omnesagent
```
...
```sh
pkill -f 'target/debug/omnesagent-desktop'
rm "$HOME/Library/Application Support/ai.omnesagentlabs.desktop/settings.json"
...
cp -R target/release/bundle/macos/OmnesAgent.app /Applications/
xattr -dr com.apple.quarantine /Applications/OmnesAgent.app
open /Applications/OmnesAgent.app
```

### What to verify
- With **no gateway running**: splash shows "Connecting to your OmnesAgent
  gateway…" and, after a few seconds, the "make sure the gateway is running"
  hint. The tray icon shows Disconnected.
- Start the daemon (`cargo run -p omnesagent -- daemon`, or `omnesagent daemon`):
