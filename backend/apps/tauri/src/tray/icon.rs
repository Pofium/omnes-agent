...
pub fn tooltip_for_state(connected: bool, status: AgentStatus) -> &'static str {
    if !connected {
        return "OmnesAgent — Disconnected";
    }
    match status {
        AgentStatus::Idle => "OmnesAgent — Idle",
        AgentStatus::Working => "OmnesAgent — Working",
        AgentStatus::Error => "OmnesAgent — Error",
    }
}
