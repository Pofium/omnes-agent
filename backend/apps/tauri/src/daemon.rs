//! Locate and launch a `omnesagent daemon` when none is already running.

use std::path::{Path, PathBuf};
use std::process::{Child, Command, Stdio};

/// Filename of the kernel binary on the current platform.
fn omnesagent_exe_name() -> &'static str {
    if cfg!(windows) {
        "omnesagent.exe"
    } else {
        "omnesagent"
...
/// Find the `omnesagent` binary. Checks, in order: the directory next to this
/// app (installed side-by-side), every `PATH` entry, then the common install
/// locations a GUI launch's minimal `PATH` usually misses.
pub fn find_omnesagent_binary() -> Option<PathBuf> {
    let exe_name = omnesagent_exe_name();
...
/// Spawn `omnesagent daemon -p <port>`, detached so it outlives the app, with
...
pub fn spawn_daemon(binary: &Path, port: u16) -> std::io::Result<Child> {
    let log_path = std::env::temp_dir().join("omnesagent-desktop-daemon.log");
