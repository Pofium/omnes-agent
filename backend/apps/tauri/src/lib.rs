//! OmnesAgent Desktop — Tauri application library.
...
/// Status the splash listens for (`omnesagent://splash-status`). Drives the
...
/// otherwise launch a fresh `omnesagent daemon`. The splash window's health
...
async fn ensure_daemon(app: tauri::AppHandle, state: state::SharedState) {
    ...
    match daemon::find_omnesagent_binary() {
        Some(bin) => {
            let _ = app.emit(
                "omnesagent://splash-status",
                SplashStatus {
                    kind: "starting",
                    message: "Starting the OmnesAgent daemon…".to_string(),
                },
            );
            if let Err(e) = daemon::spawn_daemon(&bin, GATEWAY_PORT) {
                let _ = app.emit(
                    ...
                    SplashStatus {
                        kind: "error",
                        message: format!("Couldn't start the OmnesAgent daemon: {e}"),
                    },
                );
            }
        ...
        None => {
            let _ = app.emit(
                ...
                SplashStatus {
                    kind: "missing",
                    message: "Couldn't find the `omnesagent` binary. Install OmnesAgent \
...
async fn open_dashboard(
    ...
    if let Some(token) = token {
        ...
        let script = format!(
            "try {{ localStorage.setItem('omnesagent_token', '{escaped}'); }} catch (e) {{}}"
        );
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Reuse a running gateway/daemon, or start a fresh `omnesagent daemon`
