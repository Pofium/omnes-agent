//! `zerorelay` - the OmnesAgent nominated relay (blind forwarder).
//!
...
//! `tokio::spawn` is the right primitive here; the `omnesagent_spawn::spawn!` rule
...
const VERSION: &str = env!("OMNESRELAY_VERSION");

#[derive(Parser, Debug)]
#[command(
    name = "omnesrelay",
    about = "OmnesAgent nominated relay (blind forwarder)",
    version = VERSION
)]
struct Cli {
    #[command(subcommand)]
...
fn build_outer_client_verifier(
    ...
    match mode {
        ...
        "optional" | "required" => {
            ...
            let verifier = omnesagent_tls::build_client_verifier(&omnesagent_tls::ClientAuthParams {
                ca_cert_path: ca,
...
/// run, reusing the daemon's `omnesagent-tls` machinery so no openssl is needed.
...
fn provision_tls_acceptor(
    ...
    let mats = omnesagent_tls::ensure_server_materials(&dir, &sans)
