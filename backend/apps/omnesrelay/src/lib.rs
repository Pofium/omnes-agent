//! The OmnesAgent nominated relay: a standalone **blind forwarder**.
//!
//! Each party reaches the relay over an **outer** TLS + WebSocket session
//! (`omnesagent.relay.v1`). A daemon opens one persistent WS and registers a
...
//!
//! `omnesrelay` is a standalone networking app (not daemon-path code), so bare
//! `tokio::spawn` is the right primitive here; the `omnesagent_spawn::spawn!` rule
//! is for in-daemon tasks. Mirrors the `apps/omnescode` exemption.
                ...
                let handshake = async move {
                    ...
                    let cert_node_id = if tls {
                        accepted
                            ...
                            .peer_certificates()
                            .and_then(|c| c.first())
                            .and_then(|c| omnesagent_tls::client_cert_node_id(c.as_ref()))
                    } else {
                        None
                    };
