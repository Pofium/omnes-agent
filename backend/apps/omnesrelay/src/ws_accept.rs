...
//! complete a `omnesagent.relay.v1` WebSocket upgrade or refuse the connection.
//!
...
use omnesagent_relay_proto::SUBPROTOCOL;
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    let body = "this is a OmnesAgent relay endpoint; it speaks only the \
                omnesagent.relay.v1 WebSocket protocol. Enroll with zerocode.\n";
...
/// budget in `omnesagent-relay-proto` so the transport and application bounds
/// cannot drift apart.
pub(crate) fn relay_ws_config() -> tokio_tungstenite::tungstenite::protocol::WebSocketConfig {
    ...
    cfg.max_message_size = Some(omnesagent_relay_proto::MAX_WS_MESSAGE);
    cfg.max_frame_size = Some(omnesagent_relay_proto::MAX_WS_MESSAGE);
    cfg
}
...
mod tests {
    ...
    async fn plain_http_request_is_refused_with_404() {
        ...
        assert!(text.contains("omnesagent.relay.v1"), "got: {text}");
