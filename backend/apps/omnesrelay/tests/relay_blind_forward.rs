...
use omnesagent_relay_proto::{Control, PEER_HINT_ENROLL, SUBPROTOCOL, decode_data, encode_data};
use omnesrelay::{Admission, AdmissionPolicy, PublicOpenGuard, RelayConfig, RelayServer};
...
async fn start_relay_handle(cfg: RelayConfig) -> (std::net::SocketAddr, RelayServer) {
    ...
    let certs = omnesagent_tls::load_certs(mats.server_cert_path.to_str().unwrap()).unwrap();
    let key = omnesagent_tls::load_private_key(mats.server_key_path.to_str().unwrap()).unwrap();
...
async fn oversized_and_nonprintable_node_ids_are_rejected() {
    ...
    let oversized = "n".repeat(omnesrelay::MAX_NODE_ID_LEN + 1);
    let key = gen_key();
    ...
    let at_limit = "n".repeat(omnesrelay::MAX_NODE_ID_LEN);
    let key = gen_key();
