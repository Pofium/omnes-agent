...
/// CLI flag > `$OMNESAGENT_SOCKET` > `<config_dir>/data/daemon.sock` on Unix
/// or a `\\.\pipe\omnesagent-<hash>` derived name on Windows.
pub fn resolve_socket_path(config_dir: &Path) -> Result<PathBuf> {
    if let Ok(p) = std::env::var("OMNESAGENT_SOCKET") {
        let p = p.trim();
        if !p.is_empty() {
...
/// Resolve config dir: CLI flag > `$OMNESAGENT_CONFIG_DIR` > home directory.
pub fn resolve_config_dir(cli_override: Option<&Path>) -> Result<PathBuf> {
    if let Some(dir) = cli_override {
        return Ok(dir.to_path_buf());
    }
    if let Ok(d) = std::env::var("OMNESAGENT_CONFIG_DIR") {
        let d = d.trim();
        if !d.is_empty() {
    ...
    #[cfg(unix)]
    {
        ...
        Ok(PathBuf::from(home).join(".omnesagent"))
    }
    #[cfg(windows)]
    {
        let profile = std::env::var("USERPROFILE").context("USERPROFILE not set")?;
        Ok(PathBuf::from(profile).join(".omnesagent"))
    }
}
...
impl RpcClient {
    ...
    // `omnesagent_runtime::rpc::dispatch` and call into
    // `omnesagent_runtime::quickstart::{validate_only,apply}_with_surface`.
...
pub struct ConfigSectionEntry {
    ...
    /// `omnesagent_config::sections::SectionGroup::label()`. Empty when
...
/// match `omnesagent_api::model_provider::ModelPricing`; only the rates the
...
// `omnesagent_runtime::rpc::types` (the daemon-side single source of
// truth, which itself mirrors the gateway's HTTP route shapes). The
// types live in `omnesagent-runtime`, but that crate is not on the
...
/// Mirror of `omnesagent_runtime::quickstart::Surface` (`snake_case` on the wire).
...
/// Mirror of `omnesagent_runtime::quickstart::QuickstartStep`.
#[derive(Debug, Clone, Copy, serde::Serialize, serde::Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `omnesagent_runtime::quickstart::QuickstartError`.
...
/// Mirror of `omnesagent_runtime::quickstart::AppliedAgent`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `omnesagent_runtime::quickstart::FieldSection`.
#[derive(Debug, Clone, Copy, serde::Serialize, serde::Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `omnesagent_config::traits::PropKind` (wire form).
#[derive(Debug, Clone, Copy, serde::Serialize, serde::Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `omnesagent_runtime::quickstart::FieldDescriptor`.
...
/// Mirror of `omnesagent_runtime::quickstart::QuickstartState`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `omnesagent_config::presets::RiskPreset` / `RuntimePreset`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
/// Mirror of `omnesagent_runtime::rpc::types::QuickstartTypeOption`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
...
// The shape here MUST match `omnesagent-sop-graph`'s serde projection byte for
...
/// Mirror of `omnesagent_runtime::rpc::types::LogsGetResult`. Full log
...
/// `omnesagent_runtime::rpc::session::SessionOverrides`. Sent on
...
mod dashboard_status_tests {
    ...
    fn status_result_decodes_runtime_context_fields() {
        let value = serde_json::json!({
            ...
            "protocol_version": 1,
            "active_sessions": 2,
            "config_dir": "/tmp/omnesagent-profile",
            "config_file": "/tmp/omnesagent-profile/config.toml",
            "config_kind": "temporary",
            "local_ipc_endpoint": "/tmp/omnesagent-profile/data/daemon.sock"
        });

        let status: StatusResult = serde_json::from_value(value).unwrap();

        assert_eq!(status.config_dir.as_deref(), Some("/tmp/omnesagent-profile"));
        assert_eq!(
            status.config_file.as_deref(),
            Some("/tmp/omnesagent-profile/config.toml")
        );
        assert_eq!(status.config_kind.as_deref(), Some("temporary"));
        assert_eq!(
            status.local_ipc_endpoint.as_deref(),
            Some("/tmp/omnesagent-profile/data/daemon.sock")
        );
    }
...
/// Mirror of `omnesagent_runtime::rpc::types::MemoryGetResult`. Full
...
mod sop_method_tests {
    ...
    /// omnesagent-runtime. If this shape drifts, fix both sides together.
...
/// derived from `omnesagent-relay-proto` so transport and application bounds
/// cannot drift apart.
...
/// carries attachments up to `omnesagent_runtime::rpc::attachments::MAX_REQUEST_BYTES`
