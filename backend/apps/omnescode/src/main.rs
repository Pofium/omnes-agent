// `apps/omnescode` is a standalone TUI client, not daemon-path code.
// It speaks JSON-RPC to whatever OmnesAgent daemon is at the configured
...
// `omnesagent_spawn::spawn!` rule is daemon-path only (see `clippy.toml`'s
...
#[command(
    name = "zerocode",
    about = "Interactive TUI config manager for OmnesAgent",
    version,
    long_version = concat!(
        env!("CARGO_PKG_VERSION"),
        "\n\nThis version must exactly match the running omnesagent daemon. ",
...
struct Cli {
    /// Path to the OmnesAgent config directory
    #[arg(long)]
...
fn ephemeral_daemon_command(
    ...
    let exe = std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|d| d.join("omnesagent")))
        .unwrap_or_else(|| PathBuf::from("omnesagent"));
...
mod connection_tests {
    ...
    fn ephemeral_daemon_command_sets_selected_socket() {
        let mut cmd = std::process::Command::new("omnesagent");
        configure_ephemeral_daemon_command(
            &mut cmd,
            std::path::Path::new("/tmp/omnesagent-config"),
            std::path::Path::new("/tmp/omnesagent.sock"),
        );
        ...
        assert_eq!(
            ...
            [
                ...
                "--ephemeral",
                "--config-dir",
                "/tmp/omnesagent-config",
            ]
        );
        assert_eq!(
            cmd.get_envs()
                .find(|(name, _)| *name == OsStr::new("OMNESAGENT_SOCKET"))
                .and_then(|(_, value)| value),
            Some(OsStr::new("/tmp/omnesagent.sock"))
        );
    }
