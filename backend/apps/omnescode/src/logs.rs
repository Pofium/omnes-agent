...
impl LogDetail {
    ...
    fn duration_ms(&self) -> Option<u64> {
        self.raw.get("omnesagent")?.get("duration_ms")?.as_u64()
    }

    fn omnesagent(&self) -> BTreeMap<String, String> {
        let mut out = BTreeMap::new();
        if let Some(Value::Object(map)) = self.raw.get("omnesagent") {
            for (k, val) in map {
                if k == "duration_ms" {
                    continue;
                }
...
mod tests {
    ...
    fn preview_fallback_render_shows_active_persisted_path() {
        ...
        let rendered: String = detail
            .detail_lines(Some("/var/lib/omnesagent/runtime-trace.jsonl"))
            .iter()
        ...
        assert!(rendered.contains(&crate::i18n::t_args(
            ...
            &[("path", "/var/lib/omnesagent/runtime-trace.jsonl")],
        )));
    }
