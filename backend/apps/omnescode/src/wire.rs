...
//! wire between `zerocode` and the OmnesAgent daemon.
#![allow(dead_code)]
...
/// Wire view of `omnesagent_config::presets::RiskPreset` / `RuntimePreset`.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
...
/// Schema field-kind tag mirroring `omnesagent_config::traits::PropKind`.
...
impl PropKind {
    ...
    /// `omnesagent_config::traits::PropKind::wire_name`. Used by the
...
/// `omnesagent_config::traits::AliasSource`; zerocode does not depend on
/// `omnesagent-config`.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// `omnesagent_config::traits::ConfigTab`. `Default` is `None` — the
...
/// `config/sections`. Mirrors `omnesagent_config::traits::ConfigFieldEntry`.
...
/// `omnesagent_config::sections::SectionShape`.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// `omnesagent_api::elicitation::decode_*` helpers.
...
impl ElicitationShape {
    ...
    /// `omnesagent-api`, so a return of `None` means a future schema
