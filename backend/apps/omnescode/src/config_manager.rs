...
/// Which pane of the omnesagent split holds keyboard focus. The section list
...
/// Top-level Config sub-tab: the daemon RPC editor (`omnesagent`) first,
...
const CONFIG_SECTIONS: [ConfigSection; 2] = [ConfigSection::Omnesagent, ConfigSection::Zerocode];

impl ConfigSection {
    fn label(self) -> &'static str {
        match self {
            Self::Omnesagent => "omnesagent",
...
pub(crate) struct App {
    ...
    screen: Screen,
    omnesagent_pane: OmnesagentPane,
...
impl App {
    ...
    /// section sub-tab bar (`omnesagent` / `zerocode`).
    pub(crate) fn draw_into(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let on_sections = self.omnesagent_pane == OmnesagentPane::Sections;
    ...
    fn bottom_hint(&self) -> String {
        ...
        match &self.screen {
            Screen::FieldList { .. } if self.omnesagent_pane == OmnesagentPane::Detail => {
    ...
    fn detail_highlight(&self) -> (ratatui::style::Style, &'static str) {
        let focused = matches!(
            (&self.screen, self.omnesagent_pane),
            (Screen::FieldEdit { .. }, _) | (_, OmnesagentPane::Detail)
        );
    ...
    fn draw_section_tab_bar(&self, frame: &mut Frame, area: Rect) {
        ...
        // --config-dir, $OMNESAGENT_CONFIG_DIR, or a daemon backed by a
    ...
    pub(crate) async fn handle_key(&mut self, key: KeyEvent, term: &mut Term) -> Result<bool> {
        ...
        // Tab / Shift+Tab cycle the outer Config section (omnesagent ↔
        ...
        if self.section == ConfigSection::Zerocode {
            if !self.zerocode.handle_key(key) {
                ...
                // cross back to the outer left (omnesagent) pane.
                self.cycle_section(-1);
            }
            self.sync_zerocode_locales().await;
            return Ok(false);
        ...
        if self.omnesagent_pane == OmnesagentPane::Sections {
        ...
        // Deeper levels fall through to the drill handlers' one-level pop.
        {
            ...
            {
                self.omnesagent_pane = OmnesagentPane::Sections;
                return Ok(false);
            }
        }
    ...
    async fn open_section(&mut self, idx: usize) -> Result<bool> {
        ...
        if !matches!(self.screen, Screen::SectionList) {
            self.omnesagent_pane = OmnesagentPane::Detail;
        }
        Ok(false)
    }
        ...
        if let Some(buf) = &self.filter {
            ...
            frame.render_widget(
                Paragraph::new(Span::styled(
                    format!("OmnesAgent v{}", self.rpc.server_version),
                    theme::dim_style(),
                )),
                rows[0],
            );
    ...
    pub(crate) fn claims_pane_navigation(&self, key: &KeyEvent) -> bool {
        self.section == ConfigSection::Omnesagent
            && self.omnesagent_pane == OmnesagentPane::Detail
...
impl crate::widgets::HelpContext for App {
    fn help_context(&self) -> crate::widgets::HelpNode {
        ...
        let mut node = self.omnesagent_help_context();
        node.entries.insert(0, section_nav);
        node
    }
}
...
impl App {
    fn omnesagent_help_context(&self) -> crate::widgets::HelpNode {
        use crate::keymap::ConfigTabAction as A;
...
mod tests {
    ...
    async fn pane_navigation_claims_only_scalar_field_edits() {
        ...
        manager.omnesagent_pane = OmnesagentPane::Detail;
        ...
        manager.section = ConfigSection::Omnesagent;
        manager.omnesagent_pane = OmnesagentPane::Sections;
        assert!(!manager.claims_pane_navigation(&word_left));
