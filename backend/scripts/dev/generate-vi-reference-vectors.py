...
356c29635f1c44df7de02edb58699ca9f29bece6, not by OmnesAgent. Regenerate with:

    python3 scripts/dev/generate-vi-reference-vectors.py /path/to/verifiable-intent \
        > crates/omnesagent-runtime/tests/fixtures/vi-reference-vectors.json
...
def verify_reference_source(reference_root: Path, expected_commit: str) -> None:
    ...
    is the reason the vectors are evidence at all rather than a OmnesAgent round
...
def main(reference_root: Path) -> int:
    ...
    document = {
        "_README": (
            ...
            "implementation, not by OmnesAgent. See the generator named below."
        ),
