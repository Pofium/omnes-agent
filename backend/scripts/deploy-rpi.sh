#!/usr/bin/env bash
# deploy-rpi.sh — cross-compile OmnesAgent for Raspberry Pi and deploy via SSH.
#
...
#   RPI_DIR         — remote deployment dir           (default: /home/$RPI_USER/omnesagent)
...
RPI_DIR="${RPI_DIR:-/home/${RPI_USER}/omnesagent}"
TARGET="aarch64-unknown-linux-gnu"
FEATURES="hardware,peripheral-rpi"
BINARY="target/${TARGET}/release/omnesagent"
...
echo "==> Building OmnesAgent for Raspberry Pi (${TARGET})"
...
echo ""
echo "==> Stopping omnesagent service (if running)"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo systemctl stop omnesagent 2>/dev/null || true"
...
# ── 4. Deploy binary ──────────────────────────────────────────────────────────
echo ""
echo "==> Deploying binary to ${RPI_USER}@${RPI_HOST}:${RPI_DIR}/omnesagent"
${SCP_CMD} ${SCP_OPTS} "${BINARY}" "${RPI_USER}@${RPI_HOST}:${RPI_DIR}/omnesagent"
...
# ── 5. Deploy config ─────────────────────────────────────────────────────────
CONFIG_DEST="/home/${RPI_USER}/.omnesagent/config.toml"
echo ""
echo "==> Deploying config to ${CONFIG_DEST}"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" "mkdir -p /home/${RPI_USER}/.omnesagent"
...
SERVICE_DEST="/etc/systemd/system/omnesagent.service"
echo ""
echo "==> Installing systemd service (requires sudo on the Pi)"
_RENDERED_SERVICE=$(mktemp)
sed "s|@@RPI_USER@@|${RPI_USER}|g" scripts/omnesagent.service > "${_RENDERED_SERVICE}"
${SCP_CMD} ${SCP_OPTS} "${_RENDERED_SERVICE}" "${RPI_USER}@${RPI_HOST}:/tmp/omnesagent.service"
rm -f "${_RENDERED_SERVICE}"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo mv /tmp/omnesagent.service ${SERVICE_DEST} && \
   sudo systemctl daemon-reload && \
   sudo systemctl enable omnesagent && \
   sudo systemctl restart omnesagent && \
   sudo systemctl status omnesagent --no-pager || true"
...
# ── 8. Reset ACT LED trigger so OmnesAgent can control it ──────────────────────
echo ""
...
echo ""
echo "    OmnesAgent is running at http://${RPI_HOST}:8080"
echo "    POST /api/chat  — chat with the agent"
echo "    GET  /health    — health check"
echo ""
echo "    To check logs: ssh ${RPI_USER}@${RPI_HOST} 'journalctl -u omnesagent -f'"
