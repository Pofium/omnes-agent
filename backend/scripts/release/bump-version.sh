...
bump "marketplace/dokploy/blueprints/omnesagent/docker-compose.yml" \
  'ghcr\.io/omnesagent-labs/omnesagent:[0-9]+\.[0-9]+\.[0-9]+([.-][0-9A-Za-z.-]*)?' \
  "ghcr.io/omnesagent-labs/omnesagent:${VERSION}"
...
#   - container image tags    `omnesagentlabs/omnesagent:vX.Y.Z`
...
# omnesagent-docs-translations submodule and own their own version-literal swaps,
...
for f in "${docs_files[@]}"; do
  rel="${f#"$REPO_ROOT"/}"
  bump "$rel" \
    'omnesagentlabs/omnesagent:v[0-9]+\.[0-9]+\.[0-9]+([.-][0-9A-Za-z.-]*)?' \
    "omnesagentlabs/omnesagent:v${VERSION}"
  bump "$rel" \
    '"version":( ?)"[0-9]+\.[0-9]+\.[0-9]+([.-][0-9A-Za-z.-]*)?"' \
    "\"version\":\\1\"${VERSION}\""
  bump "$rel" \
...
# feature content of setup.bat, dist/aur/PKGBUILD, dist/scoop/omnesagent.json,
