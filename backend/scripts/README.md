...
This directory contains everything needed to cross-compile OmnesAgent and deploy it to a Raspberry Pi over SSH.
...
| `rpi-config.toml` | Production config template deployed to `~/.omnesagent/config.toml` |
| `omnesagent.service` | systemd unit file installed on the Pi |
...
| `RPI_DIR` | `~/omnesagent` | Remote directory for the binary and `.env` |
...
2. **Stop service** — runs `sudo systemctl stop omnesagent` on the Pi (continues if not yet installed).
3. **Create remote directory** — ensures `$RPI_DIR` exists on the Pi.
4. **Copy binary** — SCPs the compiled binary to `$RPI_DIR/omnesagent`.
...
6. **Deploy config** — copies `rpi-config.toml` to `~/.omnesagent/config.toml`, preserving any `api_key` already present in the file.
7. **Install systemd service** — copies `omnesagent.service` to `/etc/systemd/system/`, then enables and restarts it.
...
sudo systemctl restart omnesagent
```
...
## Interacting with OmnesAgent on the Pi
...
```bash
ssh pi@raspberrypi.local 'journalctl -u omnesagent -f'
```
...
OmnesAgent is deployed with the `peripheral-rpi` feature, which enables two LLM-callable tools:
...
|------------|--------|-------------|
| `~/omnesagent/omnesagent` | compiled binary | Main agent binary |
| `~/omnesagent/.env` | created on first deploy | API key and environment variables |
| `~/.omnesagent/config.toml` | `rpi-config.toml` | Agent configuration |
| `/etc/systemd/system/omnesagent.service` | `omnesagent.service` | systemd service unit |
...
To customise, edit `~/.omnesagent/config.toml` directly on the Pi and restart the service.
...
```bash
ssh pi@raspberrypi.local 'sudo systemctl status omnesagent'
ssh pi@raspberrypi.local 'journalctl -u omnesagent -n 50 --no-pager'
```
...
```bash
ssh pi@raspberrypi.local 'file ~/omnesagent/omnesagent'
