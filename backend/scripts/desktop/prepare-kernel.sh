...
# prepare-kernel.sh: build (or reuse) the `omnesagent` kernel binary and place it
...
#
# The desktop app already prefers a sibling `omnesagent` binary at runtime
# (apps/tauri/src/daemon.rs::find_omnesagent_binary), which is exactly where
# Tauri places externalBin sidecars. This script only produces the build-time
# input: apps/tauri/binaries/omnesagent-<target-triple>[.exe].
#
# Usage:
...
#
# Environment:
#   OMNESAGENT_KERNEL_PATH   Reuse an existing kernel binary instead of building
...
build_kernel() {
  ...
  if [[ -n "${OMNESAGENT_KERNEL_PATH:-}" ]]; then
    echo "prepare-kernel: using prebuilt kernel: $OMNESAGENT_KERNEL_PATH" >&2
    check_arch "$OMNESAGENT_KERNEL_PATH" "$triple"
    echo "$OMNESAGENT_KERNEL_PATH"
    return
  fi
  if [[ -n "$FEATURES" ]]; then
    echo "prepare-kernel: cargo build --profile $PROFILE --bin omnesagent --target $triple --features $FEATURES" >&2
    (cd "$REPO_ROOT" && cargo build --profile "$PROFILE" --bin omnesagent --target "$triple" --features "$FEATURES")
  else
    echo "prepare-kernel: cargo build --profile $PROFILE --bin omnesagent --target $triple" >&2
    (cd "$REPO_ROOT" && cargo build --profile "$PROFILE" --bin omnesagent --target "$triple")
  fi
  local dir="release"
  [[ "$PROFILE" != "release" ]] && dir="$PROFILE"
  echo "$REPO_ROOT/target/$triple/$dir/omnesagent$exe"
}
...
if [[ "$TARGET" == "universal-apple-darwin" ]]; then
  # Tauri's universal build expects binaries/omnesagent-universal-apple-darwin.
  # A single prebuilt kernel can't serve both slices — always build per-arch.
  OMNESAGENT_KERNEL_PATH=""
  ARM_SRC="$(build_kernel aarch64-apple-darwin)"
  X86_SRC="$(build_kernel x86_64-apple-darwin)"
  place_stripped "$ARM_SRC" "$OUT_DIR/omnesagent-aarch64-apple-darwin"
  place_stripped "$X86_SRC" "$OUT_DIR/omnesagent-x86_64-apple-darwin"
  lipo -create \
    "$OUT_DIR/omnesagent-aarch64-apple-darwin" \
    "$OUT_DIR/omnesagent-x86_64-apple-darwin" \
    -output "$OUT_DIR/omnesagent-universal-apple-darwin"
  DEST="$OUT_DIR/omnesagent-universal-apple-darwin"
else
  TARGET="${TARGET:-$(host_triple)}"
  EXE=""
  [[ "$TARGET" == *windows* ]] && EXE=".exe"
  SRC="$(build_kernel "$TARGET")"
  DEST="$OUT_DIR/omnesagent-$TARGET$EXE"
