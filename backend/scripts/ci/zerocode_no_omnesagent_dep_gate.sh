...
echo "==> zerocode gate: no omnesagent-* crate dependency"

repo_root="$(git rev-parse --show-toplevel)"
cd "$repo_root"

manifest="apps/omnescode/Cargo.toml"
...
def flag(label):
    if label.startswith("omnesagent-") or label.startswith("omnesagent_"):
        found.add(label)
...
for table in dep_tables:
    for name, spec in table.items():
        ...
        # rename like `x = { package = "omnesagent-providers" }` cannot slip past.
...
if [ -n "$offending" ]; then
    echo "::error file=${manifest}::zerocode must not depend on any omnesagent-* crate; found:"
...
echo "zerocode gate passed: no omnesagent-* dependencies."
