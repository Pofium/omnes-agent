...
#!/usr/bin/env bash
printf 'sudo' >> "$OMNESAGENT_APT_TEST_CALLS"
printf '|%s' "$@" >> "$OMNESAGENT_APT_TEST_CALLS"
printf '\n' >> "$OMNESAGENT_APT_TEST_CALLS"
"$@"
EOF

    cat > "${fixture}/bin/timeout" <<'EOF'
#!/usr/bin/env bash
printf 'timeout' >> "$OMNESAGENT_APT_TEST_CALLS"
printf '|%s' "$@" >> "$OMNESAGENT_APT_TEST_CALLS"
printf '\n' >> "$OMNESAGENT_APT_TEST_CALLS"
...
phase="${2:-unknown}"
count_file="${OMNESAGENT_APT_TEST_STATE}/timeout-${phase}"
count=0
...
if [[ "${OMNESAGENT_APT_TEST_TIMEOUT_PHASE:-}" == "$phase" ]] && \
   [[ "$count" -le "${OMNESAGENT_APT_TEST_TIMEOUT_ATTEMPTS:-0}" ]]; then
    exit 124
fi
...
#!/usr/bin/env bash
printf 'apt-get' >> "$OMNESAGENT_APT_TEST_CALLS"
printf '|%s' "$@" >> "$OMNESAGENT_APT_TEST_CALLS"
printf '\n' >> "$OMNESAGENT_APT_TEST_CALLS"

phase="${1:-unknown}"
count_file="${OMNESAGENT_APT_TEST_STATE}/apt-get-${phase}"
count=0
if [[ -f "$count_file" ]]; then
    count="$(cat "$count_file")"
fi
...
if [[ "${OMNESAGENT_APT_TEST_FAIL_PHASE:-}" == "$phase" ]] && \
   [[ "$count" -le "${OMNESAGENT_APT_TEST_FAIL_ATTEMPTS:-0}" ]]; then
    exit 42
fi
EOF
...
#!/usr/bin/env bash
printf 'sleep' >> "$OMNESAGENT_APT_TEST_CALLS"
printf '|%s' "$@" >> "$OMNESAGENT_APT_TEST_CALLS"
printf '\n' >> "$OMNESAGENT_APT_TEST_CALLS"
EOF
...
run_helper() {
    PATH="${fixture}/bin:${PATH}" \
        OMNESAGENT_CI_APT_SOURCES_DIR="${fixture}/sources" \
        OMNESAGENT_APT_TEST_CALLS="$calls" \
        OMNESAGENT_APT_TEST_STATE="${fixture}/state" \
        OMNESAGENT_APT_TEST_FAIL_PHASE="${fail_phase:-}" \
        OMNESAGENT_APT_TEST_FAIL_ATTEMPTS="${fail_attempts:-0}" \
        OMNESAGENT_APT_TEST_TIMEOUT_PHASE="${timeout_phase:-}" \
        OMNESAGENT_APT_TEST_TIMEOUT_ATTEMPTS="${timeout_attempts:-0}" \
