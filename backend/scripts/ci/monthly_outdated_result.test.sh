...
cat > "$clean_json" <<'JSON'
{"crate_name":"omnesagent","dependencies":[]}
{"crate_name":"omnesagent-runtime","dependencies":[]}
JSON

cat > "$outdated_json" <<'JSON'
{"crate_name":"omnesagent","dependencies":[{"name":"serde","project":"1.0.0","compat":"1.0.1","latest":"1.1.0","kind":"Normal","platform":null}]}
{"crate_name":"omnesagent-runtime","dependencies":[]}
JSON

: > "$empty_json"
printf '%s\n' 'error: failed to parse manifest' > "$malformed_json"
printf '%s\n' '{"crate_name":"omnesagent","dependencies":"not-an-array"}' > "$invalid_schema_json"
...
grep -F $'omnesagent\tserde\t1.0.0\t1.0.1\t1.1.0\tNormal\t---' <<< "$rendered" >/dev/null
