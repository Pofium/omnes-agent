...
CLIPPY_WORKSPACE_ARGS=(--workspace --exclude omnesagent-desktop --all-targets)

if [ "$MODE" = "strict" ]; then
    # Local `--strict` path: same lint set and feature surface as required
    # CI (both compile with `--features ci-all`).
    echo "==> rust quality: cargo clippy --locked --workspace --exclude omnesagent-desktop --all-targets --features ci-all -- -D warnings"
    ...
    echo "==> rust quality: cargo clippy --locked --workspace --exclude omnesagent-desktop --all-targets -- -D clippy::correctness"
...
#     `impl ModelProvider for Arc<T>` in omnesagent-api — must call
...
VIOLATIONS=$(printf '%s\n' "$RG_OUTPUT" | awk -F: '
    BEGIN {
        allowed["crates/omnesagent-providers/src/dispatch.rs"] = 1
        allowed["crates/omnesagent-providers/tests/dispatch_integration.rs"] = 1
    }
...
if [ -n "$VIOLATIONS" ]; then
    ...
    echo "$VIOLATIONS"
    echo
    echo "Route the call through omnesagent_providers::ProviderDispatch:"
