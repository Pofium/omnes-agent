...
mkdir -p \
  ...
  "$context_dir/omnesagent-data/.omnesagent" \
  "$context_dir/omnesagent-data/data"

case "$mode" in
  from-artifacts)
    tar xzf "$artifact_dir/omnesagent-x86_64-unknown-linux-gnu.tar.gz" -C "$context_dir/bin/amd64"
    tar xzf "$artifact_dir/omnesagent-aarch64-unknown-linux-gnu.tar.gz" -C "$context_dir/bin/arm64"
    for arch in amd64 arm64; do
      for bin in omnesagent zerocode; do
...
EOF
      ...
      printf '<!doctype html><title>OmnesAgent smoke dashboard</title>\n' \
...
printf '%s\n' \
  ...
  > "$context_dir/omnesagent-data/.omnesagent/config.toml"
