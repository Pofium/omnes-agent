...
expect_run "runtime source" "all" \
    "crates/omnesagent-runtime/src/agent/loop_.rs"
...
expect_run "channel then runtime" "all" \
    "crates/omnesagent-channels/src/orchestrator/mod.rs" \
    "crates/omnesagent-runtime/src/lib.rs"
...
expect_skip "unrelated crate changes" "crates/omnesagent-providers/src/openai.rs"
...
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$OMNESAGENT_PARALLEL_TEST_CARGO_LOG"
EOF
...
PATH="${mock_dir}:$PATH" \
    OMNESAGENT_PARALLEL_TEST_CARGO_LOG="$cargo_log" \
    OMNESAGENT_PARALLEL_TEST_RUNS=1 \
    OMNESAGENT_PARALLEL_TEST_SCOPE=channels \
    bash "$gate" >/dev/null

expect_cargo_log "channels scope" \
    'test --locked --quiet -p omnesagent-channels --lib -- --test-threads=16'
...
expect_cargo_log "default scope" \
    'test --locked --quiet -p omnesagent-runtime --lib -- --test-threads=16' \
    'test --locked --quiet -p omnesagent-channels --lib -- --test-threads=16'
...
PATH="${mock_dir}:$PATH" \
    OMNESAGENT_PARALLEL_TEST_CARGO_LOG="$cargo_log" \
    OMNESAGENT_PARALLEL_TEST_RUNS=1 \
    OMNESAGENT_PARALLEL_TEST_SCOPE='' \
    bash "$gate" >/dev/null
...
PATH="${mock_dir}:$PATH" \
    OMNESAGENT_PARALLEL_TEST_CARGO_LOG="$cargo_log" \
    OMNESAGENT_PARALLEL_TEST_RUNS=1 \
    OMNESAGENT_PARALLEL_TEST_SCOPE=unsupported \
