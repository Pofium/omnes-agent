...
image="${OMNESAGENT_COMPOSE_SMOKE_IMAGE:?set OMNESAGENT_COMPOSE_SMOKE_IMAGE to a locally loaded image}"
requested_host_port="${OMNESAGENT_COMPOSE_SMOKE_HOST_PORT:-42618}"
smoke_root="$(mktemp -d)"
override_file="$smoke_root/compose.override.yml"
config_file="$smoke_root/config.toml"
project_name="omnesagent-compose-smoke-$$"
container_name="omnesagent-compose-smoke-$$"

compose() {
  HOST_PORT="127.0.0.1:${requested_host_port}" \
    OMNESAGENT_GATEWAY_PORT=42617 \
    OMNESAGENT_COMPOSE_SMOKE_CONFIG="$config_file" \
    OMNESAGENT_COMPOSE_SMOKE_IMAGE="$image" \
    OMNESAGENT_COMPOSE_SMOKE_CONTAINER="$container_name" \
    docker compose \
...
services:
  omnesagent:
    image: "${OMNESAGENT_COMPOSE_SMOKE_IMAGE}"
    pull_policy: never
    container_name: "${OMNESAGENT_COMPOSE_SMOKE_CONTAINER}"
    restart: "no"
    volumes:
      - "${OMNESAGENT_COMPOSE_SMOKE_CONFIG}:/omnesagent-data/.omnesagent/config.toml:ro"
COMPOSE

compose config --quiet
compose up --detach

published="$(compose port omnesagent 42617 | sed -n '1p')"
