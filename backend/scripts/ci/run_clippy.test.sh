...
#!/usr/bin/env bash
printf '%s\n' "$@" > "$OMNESAGENT_CLIPPY_TEST_ARGS"
printf '%s\n' \
    'Downloaded demo-crate v1.0.0' \
    'Compiling omnesagent-runtime v0.8.5 (/workspace/omnesagent/crates/omnesagent-runtime)' \
    'Compiling serde v1.0.0'
exit "${OMNESAGENT_CLIPPY_TEST_STATUS:-0}"
EOF
...
run_runner() {
    ...
    PATH="${mock_dir}/bin:$PATH" \
        ...
        RUNNER_OS=TestOS \
        RUST_CACHE_HIT=true \
        OMNESAGENT_CLIPPY_TEST_ARGS="$cargo_args" \
...
PATH="${mock_dir}/bin:$PATH" \
    ...
    OMNESAGENT_CLIPPY_TEST_ARGS="$cargo_args" \
    OMNESAGENT_CLIPPY_TEST_STATUS=17 \
    bash "$runner" \
        --scope workspace \
