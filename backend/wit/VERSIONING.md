...
wit/
  ...
  v0/                 ← omnesagent:plugin@0.x (experimental → stable)
    ...
    tool.wit
    types.wit
  v1/                 ← (future) breaking changes → omnesagent:plugin@1.0.0
```
...
1. Update the `package` declaration to `omnesagent:plugin@1.0.0`.
