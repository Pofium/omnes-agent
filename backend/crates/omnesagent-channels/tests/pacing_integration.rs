...
use omnesagent_channels::paced_channel::PacedChannel;
use omnesagent_config::schema::HasReplyPacing;
...
async fn whatsapp_web_shape_queue_overflow_drops_newest() {
    ...
    let h_a =
        omnesagent_spawn::spawn!(async move { paced_a.send(&SendMessage::new("a", jid)).await });
    let h_b =
        omnesagent_spawn::spawn!(async move { paced_b.send(&SendMessage::new("b", jid)).await });
