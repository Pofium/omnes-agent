...
use omnesagent_config::schema::{Config, LineDmPolicy, LineGroupPolicy};
            ...
            match state.dm_policy {
                ...
                LineDmPolicy::Pairing => {
                    if !is_line_user_allowed(&*state, user_id) {
                            ...
                            if let Some(ref guard) = state.pairing {
                                match guard.try_pair(code, user_id).await {
                                    Ok(Some(_)) => {
                                        ...
                                        {
                                            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"user_id": user_id, "e": e.to_string()})), "paired userId= but persist failed");
                                        } else {
                                            ::omnesagent_log::record!(
                                                INFO,
                                                ::omnesagent_log::Event::new(
                                    ...
                                    Err(wait_ms) => {
                                        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"user_id": user_id, "wait_ms": wait_ms})), "bind rate-limited for userId=, retry after ms");
                                        if let Some(ref token) = bind_reply_token {
                        ...
                        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"user_id": user_id, "LINE_BIND_COMMAND": LINE_BIND_COMMAND})), "ignoring message from unpaired user: . Send ` <code>` to pair.");
                        continue;
                    }
                }
            }
...
impl LineChannel {
    pub fn new(
        ...
        let pairing = if dm_policy == LineDmPolicy::Pairing && configured_peers.is_empty() {
            ...
            if let Some(code) = guard.pairing_code() {
                ...
                // structured log where `omnesagent service logs` / the gateway
        ...
        Self {
            ...
            client: omnesagent_config::schema::build_channel_proxy_client("channel.line", None),
    ...
    pub fn from_config(
        config: &omnesagent_config::schema::LineConfig,
        alias: impl Into<String>,
        peer_resolver: Arc<dyn Fn() -> Vec<String> + Send + Sync>,
        sender_name_resolver: Arc<dyn Fn() -> Option<String> + Send + Sync>,
    ) -> Self {
    ...
    pub fn with_proxy_url(mut self, proxy_url: Option<String>) -> Self {
        self.client = omnesagent_config::schema::build_channel_proxy_client(
            "channel.line",
            proxy_url.as_deref(),
        );
        self
...
impl ::omnesagent_api::attribution::Attributable for LineChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(::omnesagent_api::attribution::ChannelKind::Line)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
...
mod tests {
    ...
    async fn spawn_webhook(
        ...
        let jh = omnesagent_spawn::spawn!(async move {
    ...
    async fn with_transcription_disabled_config_is_noop() {
        ...
        .with_transcription(omnesagent_config::schema::TranscriptionConfig {
            enabled: false,
            ..Default::default()
        });
    ...
    async fn webhook_audio_transcribed_and_forwarded() {
        ...
        let transcription_config = omnesagent_config::schema::TranscriptionConfig {
            enabled: true,
            api_key: None,
            api_url: String::new(),
    ...
    async fn webhook_bind_success_sends_paired_reply() {
        ...
        assert_eq!(
            ...
            omnesagent_runtime::i18n::get_required_cli_string("channel-line-bind-success")
        );
    ...
    async fn webhook_bind_invalid_code_sends_error_reply() {
        ...
        assert_eq!(
            body["messages"][0]["text"],
            omnesagent_runtime::i18n::get_required_cli_string("channel-line-bind-invalid-code")
        );
    ...
    async fn listen_sets_sender_icon_from_bot_info() {
        ...
        let abort = omnesagent_spawn::spawn!(async move {
