    ...
    pub fn parse_webhook_payload(&self, payload: &serde_json::Value) -> Vec<ChannelMessage> {
        ...
        if !self.is_sender_allowed(&normalized_from) {
            ::omnesagent_log::record!(
                ...
                "ignoring message from unauthorized sender: . Add to channels.linq.allowed_senders in config.toml, or run `omnesagent config set channels.linq.allowed-senders='[\"<sender-id>\"]'`."
            );
            return messages;
        }
...
impl ::omnesagent_api::attribution::Attributable for LinqChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(::omnesagent_api::attribution::ChannelKind::Linq)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        if resp.status() == reqwest::StatusCode::NOT_FOUND {
            ...
            if !create_resp.status().is_success() {
                ...
                ::omnesagent_log::record!(ERROR, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Fail).with_outcome(::omnesagent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"status": status.to_string(), "error_body": error_body})), "create chat failed:");
