...
use omnesagent_api::elicitation::{
    ...
    async fn request_choice_via_elicitation(
        ...
        match parsed {
            ElicitationResponse::Accept { content } => {
                let text =
                    omnesagent_api::elicitation::decode_single_select_accept(&content, choices)?;
                Ok(Some(text))
            }
...
impl ::omnesagent_api::attribution::Attributable for AcpChannel {
...
impl Channel for AcpChannel {
    ...
    async fn request_multi_choice(
        ...
        match parsed {
            ElicitationResponse::Accept { content } => {
                let texts =
                    omnesagent_api::elicitation::decode_multi_select_accept(&content, choices)?;
                Ok(Some(texts))
            }
            ElicitationResponse::Decline | ElicitationResponse::Cancel => Ok(None),
        }
...
mod tests {
    use super::*;
    use tokio::sync::mpsc;
    use omnesagent_api::elicitation::single_select_schema_with_property_name;
    use omnesagent_api::jsonrpc::JSONRPC_VERSION;
    ...
    async fn request_approval_emits_request_permission_and_resolves_approve() {
        ...
        let task = omnesagent_spawn::spawn!(async move { ch.request_approval("", &request).await });
        ...
        let expected_title = omnesagent_runtime::i18n::get_required_cli_string_with_args(
            "channel-approval-title",
            &[("tool", "git")],
        );
