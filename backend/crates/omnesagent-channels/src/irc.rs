...
impl ::omnesagent_api::attribution::Attributable for IrcChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(::omnesagent_api::attribution::ChannelKind::Irc)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
...
impl Channel for IrcChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        Self::send_raw(
            &mut writer,
            &format!("USER {} 0 * :OmnesAgent", self.username),
        )
        .await?;
...
mod tests {
    ...
    fn new_stores_all_fields() {
        ...
        assert_eq!(ch.username, "omnesagent");
    ...
    fn irc_config_serde_roundtrip() {
        use omnesagent_config::schema::IrcConfig;
        ...
        assert_eq!(parsed.username.as_deref(), Some("omnesagent"));
