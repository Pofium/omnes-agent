...
impl MochatChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        omnesagent_config::schema::build_runtime_proxy_client("channel.mochat")
    }
...
impl ::omnesagent_api::attribution::Attributable for MochatChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(::omnesagent_api::attribution::ChannelKind::MoChat)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
        ...
        loop {
                ...
                .await
            {
                ...
                Ok(resp) => {
                    let status = resp.status();
                    let err = resp.text().await.unwrap_or_default();
                    ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", err), "status": status.to_string()})), "poll request failed");
                }
                Err(e) => {
                    ::omnesagent_log::record!(
                        WARN,
