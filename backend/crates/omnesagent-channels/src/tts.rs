...
use omnesagent_config::schema::{Config, TtsProviderConfig};
...
pub trait TtsProvider: Send + Sync + ::omnesagent_api::attribution::Attributable {
        ...
        let api_key = config
            ...
            .context(
                "Missing OpenAI TTS API key: set `[tts_providers.openai.<alias>].api_key` (or via \
                 `OMNESAGENT_providers__tts__openai__<alias>__api_key=...`).",
            )?;
        ...
        let api_key = config
            ...
            .context(
                "Missing ElevenLabs API key: set `[tts_providers.elevenlabs.<alias>].api_key` (or \
                 via `OMNESAGENT_providers__tts__elevenlabs__<alias>__api_key=...`).",
            )?;
        ...
        let api_key = config
            ...
            .context(
                "Missing Google TTS API key: set `[tts_providers.google.<alias>].api_key` (or via \
                 `OMNESAGENT_providers__tts__google__<alias>__api_key=...`).",
            )?;
...
impl TtsProvider for EdgeTtsProvider {
    ...
    async fn synthesize(&self, text: &str, voice: &str) -> Result<Vec<u8>> {
        ...
        let output_file = temp_dir.join(format!("omnesagent_tts_{}.mp3", uuid::Uuid::new_v4()));
        ...
        // reader task stays owned by the artifact so cancellation aborts it.
        {
            ...
            let pipe = artifact
                ...
                .take()
                .expect("stderr piped");
            artifact.stderr_reader = Some(omnesagent_spawn::spawn!(async move {
                let mut buf = Vec::new();
                let mut pipe = pipe;
...
impl TtsManager {
    ...
    pub async fn synthesize_with_provider(
        ...
        let span = ::omnesagent_log::attribution_span!(tts.as_ref());
        ::omnesagent_log::scope!(voice: voice, => tts.synthesize(text, voice))
            .instrument(span)
            .await
    }
...
impl ::omnesagent_api::attribution::Attributable for OpenAiTtsProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(::omnesagent_api::attribution::ProviderKind::Tts(
            ::omnesagent_api::attribution::TtsProviderKind::OpenAi,
        ))
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl ::omnesagent_api::attribution::Attributable for ElevenLabsTtsProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(::omnesagent_api::attribution::ProviderKind::Tts(
            ::omnesagent_api::attribution::TtsProviderKind::ElevenLabs,
        ))
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl ::omnesagent_api::attribution::Attributable for GoogleTtsProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(::omnesagent_api::attribution::ProviderKind::Tts(
            ::omnesagent_api::attribution::TtsProviderKind::Google,
        ))
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl ::omnesagent_api::attribution::Attributable for EdgeTtsProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(::omnesagent_api::attribution::ProviderKind::Tts(
            ::omnesagent_api::attribution::TtsProviderKind::Edge,
        ))
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl ::omnesagent_api::attribution::Attributable for PiperTtsProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(::omnesagent_api::attribution::ProviderKind::Tts(
            ::omnesagent_api::attribution::TtsProviderKind::Piper,
        ))
    }
    fn alias(&self) -> &str {
        &self.alias
...
mod tests {
    ...
    fn config_with_edge_alias() -> Config {
        ...
        cfg.providers.tts.edge.insert(
            "default".to_string(),
            omnesagent_config::schema::EdgeTtsProviderConfig {
    ...
    fn config_with_piper_alias() -> Config {
        ...
        cfg.providers.tts.piper.insert(
            "default".to_string(),
            omnesagent_config::schema::PiperTtsProviderConfig {
                base: TtsProviderConfig {
    ...
    fn tts_config_defaults() {
        let config = omnesagent_config::schema::TtsConfig::default();
    ...
    async fn edge_tts_removes_temp_output_when_read_fails() {
        ...
        let temp_dir = std::env::temp_dir();
        let script_path =
            temp_dir.join(format!("omnesagent_edgetts_test_{}.sh", uuid::Uuid::new_v4()));
        let out_path_file = temp_dir.join(format!(
            "omnesagent_edgetts_path_{}.txt",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn edge_tts_cancellation_reaps_child_and_removes_temp_output() {
        ...
        let mut handle = omnesagent_spawn::spawn!(async move {
    ...
    fn edge_tts_cancellation_cleanup_does_not_block_current_thread_runtime() {
        ...
        let script_path =
            temp_dir.join(format!("omnesagent_edgetts_test_{}.sh", uuid::Uuid::new_v4()));
        let artifact_path =
            temp_dir.join(format!("omnesagent_edgetts_out_{}.mp3", uuid::Uuid::new_v4()));
        let script = script_path.to_str().unwrap();
        ...
        rt.block_on(async {
            ...
            let probe = omnesagent_spawn::spawn!(async {
    ...
    async fn edge_tts_descendant_holding_stderr_is_bounded_and_cleaned() {
        ...
        let pid_file = temp_dir.join(format!("omnesagent_edgetts_pid_{}.txt", uuid::Uuid::new_v4()));
        let script = script_path.to_str().unwrap();
        let sidecar = out_path_file.to_str().unwrap();
