...
impl ::omnesagent_api::attribution::Attributable for DingTalkChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(
            ::omnesagent_api::attribution::ChannelKind::DingTalk,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl Channel for DingTalkChannel {
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        let title = message.subject.as_deref().unwrap_or("OmnesAgent");
        let body = serde_json::json!({
            "msgtype": "markdown",
            "markdown": {
                "title": title,
