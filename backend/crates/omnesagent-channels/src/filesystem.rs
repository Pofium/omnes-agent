...
use omnesagent_config::schema::FilesystemConfig;
use omnesagent_runtime::sop::audit::SopAuditLogger;
use omnesagent_runtime::sop::dispatch::dispatch_untrusted_fan_in;
use omnesagent_runtime::sop::engine::SopEngine;
use omnesagent_runtime::sop::types::{FilesystemEventKind, SopTriggerSource};
...
impl FilesystemChannel {
    ...
    async fn watch_and_dispatch(&self, tx: &mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        let span = omnesagent_log::attribution_span!(self);
    ...
    async fn watch_and_dispatch_inner(
        ...
        omnesagent_runtime::health::mark_component_ok("filesystem");
...
impl ::omnesagent_api::attribution::Attributable for FilesystemChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(
            ::omnesagent_api::attribution::ChannelKind::Filesystem,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
...
mod tests {
    ...
    fn test_channel(config: FilesystemConfig, alias: &str) -> FilesystemChannel {
        ...
        let memory = Arc::new(omnesagent_memory::NoneMemory::new(alias));
    ...
    async fn invalid_glob_rejection_is_span_attributed() {
        ...
        let memory = Arc::new(omnesagent_memory::NoneMemory::new("fs-proof"));
        ...
        let zc = value
            .get("omnesagent")
            .expect("omnesagent attribution block present");
        assert_eq!(
