...
impl WebhookChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        omnesagent_config::schema::build_runtime_proxy_client("channel.webhook")
    }
...
impl ::omnesagent_api::attribution::Attributable for WebhookChannel {
...
mod tests {
    ...
    fn incoming_payload_deserializes_all_fields() {
        let json = r#"{"sender": "omnesagent_user", "content": "hello", "thread_id": "t1"}"#;
        let payload: IncomingWebhook = serde_json::from_str(json).unwrap();
        assert_eq!(payload.sender, "omnesagent_user");
    ...
    fn incoming_payload_without_thread() {
        let json = r#"{"sender": "omnesagent_user", "content": "hi"}"#;
        let payload: IncomingWebhook = serde_json::from_str(json).unwrap();
        assert_eq!(payload.sender, "omnesagent_user");
    ...
    fn outgoing_payload_serializes_content() {
        let payload = OutgoingWebhook {
            ...
            recipient: Some("omnesagent_user".into()),
        };
        let json = serde_json::to_value(&payload).unwrap();
        assert_eq!(json["content"], "response");
        assert_eq!(json["thread_id"], "t1");
        assert_eq!(json["recipient"], "omnesagent_user");
    }
    ...
    fn test_message() -> SendMessage {
        SendMessage::new("hello", "omnesagent_user")
    }
