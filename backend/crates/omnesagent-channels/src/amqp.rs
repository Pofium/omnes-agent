...
use omnesagent_config::schema::SopDispatch;
use omnesagent_runtime::sop::audit::SopAuditLogger;
use omnesagent_runtime::sop::dispatch::{dispatch_untrusted_fan_in, results_need_redelivery};
use omnesagent_runtime::sop::engine::SopEngine;
use omnesagent_runtime::sop::types::SopTriggerSource;
...
const PKCS12_PASSWORD: &str = "omnesagent-amqp";
...
fn pem_to_pkcs12_der(cert_pem: &[u8], key_pem: &[u8], alias: &str) -> anyhow::Result<Vec<u8>> {
    ...
    let local_key_id = b"omnesagent-amqp-client";
...
impl ::omnesagent_api::attribution::Attributable for AmqpChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(::omnesagent_api::attribution::ChannelKind::Amqp)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
...
impl Channel for AmqpChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        omnesagent_runtime::health::mark_component_ok("amqp");
...
"#;
    ...
    fn sop_handles_at_exec_slot_full() -> (Arc<Mutex<SopEngine>>, Arc<SopAuditLogger>) {
        use omnesagent_runtime::sop::SopStepKind;
