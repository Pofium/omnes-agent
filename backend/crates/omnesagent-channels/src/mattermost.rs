...
pub(crate) use omnesagent_config::schema::MattermostListenMode;
    ...
    async fn try_transcribe_audio_attachment(&self, post: &serde_json::Value) -> Option<String> {
        ...
        if let Some(duration_ms) = audio_file.get("duration").and_then(|d| d.as_u64()) {
            ...
            if duration_secs > config.max_duration_secs {
                ::omnesagent_log::record!(DEBUG, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"duration_secs": duration_secs, "max": config.max_duration_secs})), "audio attachment exceeds max duration, skipping");
                return None;
            }
        }
...
impl ::omnesagent_api::attribution::Attributable for MattermostChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(
            ::omnesagent_api::attribution::ChannelKind::Mattermost,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
...
mod tests {
    ...
    async fn test_ws_handshake_sends_auth_before_waiting_for_hello() {
        ...
        let server_task = omnesagent_spawn::spawn!(async move {
            let first = server
                .next()
                .await
