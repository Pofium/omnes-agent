...
impl NextcloudTalkChannel {
    ...
    pub fn new_with_proxy(
        ...
        Self {
            ...
            alias: alias.into(),
            peer_resolver,
            client: omnesagent_config::schema::build_channel_proxy_client(
    ...
    /// Prevents feedback loops where OmnesAgent reacts to its own messages.
    fn is_bot_name(&self, name: &str) -> bool {
        let name = name.to_ascii_lowercase();
        // Match the configured bot name, or the known bot name "omnesagent".
        (!self.bot_name.is_empty() && name == self.bot_name) || name == "omnesagent"
    }
    ...
    fn parse_as2_payload(&self, payload: &serde_json::Value) -> Vec<ChannelMessage> {
        ...
        if !self.is_user_allowed(actor_id) {
            ::omnesagent_log::record!(
                ...
                "Talk: ignoring message from unauthorized actor: . Add to channels.nextcloud_talk.allowed_users in config.toml, or run `omnesagent config set channels.nextcloud_talk.allowed-users='[\"<user-id>\"]'`."
            );
            return messages;
        }
...
impl ::omnesagent_api::attribution::Attributable for NextcloudTalkChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(
            ::omnesagent_api::attribution::ChannelKind::NextcloudTalk,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
    ...
    async fn nextcloud_talk_send_uses_signed_bot_api_with_correct_wire_shape() {
        ...
        let result = channel
            .send(&SendMessage::new(
                "Hello from OmnesAgent!".to_string(),
                "test-room".to_string(),
            ))
            .await;
        assert!(result.is_ok());
        ...
        assert_eq!(
            body,
            serde_json::json!({ "message": "Hello from OmnesAgent!" })
        );

        let expected_payload = format!("{random}Hello from OmnesAgent!");
        let mut mac = Hmac::<Sha256>::new_from_slice(secret.as_bytes()).unwrap();
