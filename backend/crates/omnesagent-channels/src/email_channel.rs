...
use omnesagent_tools::email_imap::{ImapSession, TlsStreamTolerant};

pub use omnesagent_config::scattered_types::EmailConfig;
...
// `omnesagent_tools::email_imap`, the canonical IMAP utility shared by the
...
pub struct EmailChannel {
    ...
    auth_service: Option<Arc<omnesagent_providers::auth::AuthService>>,
}
...
impl EmailChannel {
    ...
    pub fn with_auth_service(
        mut self,
        auth_service: Arc<omnesagent_providers::auth::AuthService>,
    ) -> Self {
    ...
    fn extract_attachments(
    ...
    ) -> Vec<omnesagent_api::media::MediaAttachment> {
        ...
        for part in parsed.attachments() {
            ...
            attachments.push(omnesagent_api::media::MediaAttachment {
                file_name,
                data,
                mime_type: mime_str,
                marker: None,
    ...
    fn sanitize_subject(subject: &str) -> String {
        ...
        while let Some(rest) =
            cleaned.strip_prefix(omnesagent_api::channel::CHANNEL_SOP_SUBJECT_PREFIX)
        {
            cleaned = rest;
        }
...
struct ParsedEmail {
    ...
    content: String,
    timestamp: u64,
    attachments: Vec<omnesagent_api::media::MediaAttachment>,
}
...
impl ::omnesagent_api::attribution::Attributable for EmailChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(::omnesagent_api::attribution::ChannelKind::Email)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
...
fn build_attachment_parts(attachments: &[omnesagent_api::media::MediaAttachment]) -> Vec<SinglePart> {
    attachments
        .iter()
        .map(|attachment| {
...
mod tests {
    ...
    fn sanitize_subject_strips_reserved_sop_prefix() {
        let forged = "omnesagent:sop-event:git.main:pull_request.opened";
        let cleaned = super::EmailChannel::sanitize_subject(forged);
        assert!(
            !cleaned.starts_with(omnesagent_api::channel::CHANNEL_SOP_SUBJECT_PREFIX),
    ...
    fn rendered_attachment(attachment: omnesagent_api::media::MediaAttachment) -> Vec<u8> {
