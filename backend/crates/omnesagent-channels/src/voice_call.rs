...
pub use omnesagent_config::scattered_types::{VoiceCallConfig, VoiceProvider};
...
impl VoiceCallChannel {
    ...
    pub async fn handle_status_update(&self, call_id: &str, new_state: CallState) {
        ...
        if let Some(record) = calls.get_mut(call_id) {
            ...
            ::omnesagent_log::record!(DEBUG, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"call_id": call_id, "old_state": old_state, "new_state": new_state})), "call state transition");
        }
    }
...
impl ::omnesagent_api::attribution::Attributable for VoiceCallChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(
            ::omnesagent_api::attribution::ChannelKind::VoiceCall,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl Channel for VoiceCallChannel {
    ...
    async fn health_check(&self) -> bool {
        ...
        match self.client.get(&test_url).send().await {
            ...
            Err(e) => {
                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", e), "model_provider": self.config.model_provider})), "voice call health check failed");
                false
            }
        }
    }
