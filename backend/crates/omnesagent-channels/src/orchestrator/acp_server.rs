...
use uuid::Uuid;
use omnesagent_api::agent::ToolArtifact;
use omnesagent_api::elicitation::ElicitationCapabilities;
pub use omnesagent_api::jsonrpc::RpcOutbound;
...
use omnesagent_api::plan::PlanEntry;
use omnesagent_config::schema::Config;
use omnesagent_infra::acp_session_store::AcpSessionStore;
use omnesagent_runtime::agent::agent::{Agent, TurnEvent};
...
pub struct AcpServer {
    ...
    /// standalone `omnesagent acp` mode where no gateway is running.
...
impl AcpServer {
    ...
    pub async fn run(self: Arc<Self>) -> Result<()> {
        ...
        omnesagent_spawn::spawn!(writer_task(writer_rx));
    ...
    fn handle_initialize(&self, params: &Value) -> RpcResult {
        ...
        Ok(serde_json::json!({
            ...
            "agentInfo": {
                "name": "omnesagent-acp",
                "title": "OmnesAgent ACP",
                "version": env!("CARGO_PKG_VERSION"),
            },
            "authMethods": [],
            "_meta": {
                "omnesagent": omnesagent_meta,
            }
        }))
    }
    ...
    async fn handle_session_new(&self, params: &Value) -> RpcResult {
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                let model = config
                    ...
                    .unwrap_or_default();
                let error = omnesagent_runtime::security::scrub(
                    &omnesagent_providers::sanitize_api_error(&e.to_string()),
                );
                ::omnesagent_log::scope!(
        ...
        ::omnesagent_log::record!(
            INFO,
            ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Start)
                .with_category(::omnesagent_log::EventCategory::Channel)
    ...
    async fn handle_session_prompt(&self, params: &Value, _request_id: &Value) -> RpcResult {
        ...
        ::omnesagent_log::record!(
            INFO,
            ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Start).with_category(::omnesagent_log::EventCategory::Channel)
                .with_attrs(::serde_json::json!({
        ...
        let config = self.config_snapshot();
        let cost_tracker = omnesagent_runtime::cost::CostTracker::get_or_init_global(
            config.cost.clone(),
            &config.data_dir,
        );
        let cost_pricing = std::sync::Arc::new(
            omnesagent_runtime::agent::cost::build_model_provider_pricing(&config),
        );
        ...
        let turn_handle = omnesagent_spawn::spawn!(async move {
            ...
            let result = {
                ...
                omnesagent_runtime::agent::loop_::scope_session_key(
        ...
        while let Some(event) = event_rx.recv().await {
            ...
            match &event {
                ...
                TurnEvent::ToolResult {
                    ...
                    ::omnesagent_log::record!(
                        DEBUG,
                        ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Complete).with_category(::omnesagent_log::EventCategory::Channel)
                            .with_outcome(::omnesagent_log::EventOutcome::Success)
                            .with_attrs(::serde_json::json!({
                                "tool_call_id": id,
                                "tool": name,
    ...
    fn cancelled_prompt_result(session_id: String, accumulated_text: &str) -> Value {
        let marker = omnesagent_runtime::i18n::get_required_cli_string("turn-cancelled-client-rpc");
...
fn deliver_file_tool_result_content(
    ...
    let bytes = omnesagent_runtime::tools::read_delivered_artifact_bounded(Path::new(path)).ok()?;
    ...
    let ext = Path::new(path)
        ...
        .and_then(|e| e.to_str())
        .unwrap_or_default();
    let expected_uri = omnesagent_runtime::tools::attachment_deliver_uri(
...
fn map_tool_kind(name: &str) -> &'static str {
    match name {
        ...
        // OmnesAgent tools return user-visible text, so use `other` to keep the
...
fn notification_for_turn_event(session_id: &str, event: &TurnEvent) -> Option<JsonRpcNotification> {
    Some(match event {
        ...
        TurnEvent::HistoryTrimmed {
            ...
            method: "_omnesagent/history_trimmed",
            params: serde_json::json!({
                "sessionId": session_id,
...
fn acp_turn_failure(error: &anyhow::Error) -> (String, RpcError) {
    ...
    let user_message = omnesagent_runtime::agent::terminal_completion_error_message(error, None);
    (
        diagnostic.clone(),
...
mod tests {
    ...
    use omnesagent_api::model_provider::ModelProvider;
    ...
    impl ::omnesagent_api::attribution::Attributable for EmptyTerminalProvider {
    ...
    fn acp_test_agent(workspace_dir: std::path::PathBuf) -> Agent {
        Agent::builder()
            ...
            .tools(
                omnesagent_runtime::tools::scoped::ScopedToolRegistry::from_raw_for_test(Vec::new()),
            )
            .observer(Arc::from(omnesagent_runtime::observability::NoopObserver {}))
            .tool_dispatcher(Box::new(
                omnesagent_runtime::agent::dispatcher::NativeToolDispatcher,
            ))
            .workspace_dir(workspace_dir)
            .exclude_memory(true)
            .build()
    ...
    async fn session_prompt_projects_a_terminal_failure_through_acp_rpc() {
        ...
        assert_eq!(
            error.message,
            omnesagent_runtime::agent::semantic_empty_terminal_completion_message(None)
        );
        assert_ne!(
    ...
    fn initialize_response_uses_acp_v1_shape() {
        ...
        assert_eq!(result["agentInfo"]["name"], "omnesagent-acp");
        assert_eq!(result["agentInfo"]["title"], "OmnesAgent ACP");
    ...
    fn initialize_advertises_load_session_when_store_present() {
        let cwd = tempfile::tempdir().unwrap();
        let store =
            Arc::new(omnesagent_infra::acp_session_store::AcpSessionStore::new(cwd.path()).unwrap());
    ...
    /// (`serve_reader`, the exact framing loop `run()` uses for `omnesagent acp`)
    ...
    async fn stdio_front_door_omitted_cwd_uses_agent_workspace() {
        ...
        let reader_task =
            omnesagent_spawn::spawn!(async move { reader.serve_reader(server_stdin).await });
    ...
    async fn session_new_does_not_wait_for_configured_mcp_servers() {
        ...
        let mut config = Config {
            ...
            providers: {
                let mut p = omnesagent_config::providers::Providers::default();
                p.models.openrouter.insert(
                    "default".to_string(),
                    omnesagent_config::schema::OpenRouterModelProviderConfig {
            ...
            mcp: omnesagent_config::schema::McpConfig {
                enabled: true,
                servers: vec![omnesagent_config::schema::McpServerConfig {
                    name: "slow".to_string(),
                    transport: omnesagent_config::schema::McpTransport::Stdio,
        ...
        config.runtime_profiles.insert(
            "default".to_string(),
            omnesagent_config::schema::RuntimeProfileConfig::default(),
        );
        ...
        config.mcp_bundles.insert(
            ...
            omnesagent_config::schema::McpBundleConfig {
    ...
    async fn session_new_agent_init_failure_log_is_attributed_and_redacted() {
        ...
        let mut config = Config {
            ...
            providers: {
                ...
                providers.models.openrouter.insert(
                    ...
                    omnesagent_config::schema::OpenRouterModelProviderConfig {
                        base: omnesagent_config::schema::ModelProviderConfig {
                            api_key: Some("sk-ant-omnesagent_test_credential".to_string()),
                            model: Some("test-model".to_string()),
                            ..Default::default()
                        },
                    },
        ...
        assert_eq!(event["omnesagent"]["channel_type"], "acp");
        assert_eq!(event["omnesagent"]["agent_alias"], "test-agent");
        assert_eq!(event["omnesagent"]["model_provider"], "openrouter.default");
        assert_eq!(event["omnesagent"]["model"], "test-model");
        assert!(
            event["omnesagent"]["session_key"]
                .as_str()
    ...
    fn agent_acp_enable_mcp_defaults_off() {
        assert!(
            !omnesagent_config::schema::AliasedAgentConfig::default().acp_enable_mcp,
    ...
    fn handle_initialize_default_model_absent_when_unconfigured() {
        ...
        let result = server.handle_initialize(&serde_json::json!({})).unwrap();
        assert!(
            result["_meta"]["omnesagent"].get("defaultModel").is_none(),
            "defaultModel must be absent when no model_provider is configured, got: {}",
            result["_meta"]["omnesagent"]["defaultModel"]
        );
    }

    #[test]
    fn handle_initialize_default_model_reflects_configured_provider() {
        use omnesagent_config::schema::{ModelProviderConfig, OllamaModelProviderConfig};
        let mut config = Config::default();
        ...
        let server = AcpServer::new(config, AcpServerConfig::default());
        let result = server.handle_initialize(&serde_json::json!({})).unwrap();
        assert_eq!(result["_meta"]["omnesagent"]["defaultModel"], "llama3.2");
    }
    ...
    fn cancelled_prompt_result_preserves_content_string_shape() {
        ...
        assert_eq!(
            ...
            format!(
                "partial text\n\n{}",
                omnesagent_runtime::i18n::get_required_cli_string("turn-cancelled-client-rpc")
            )
        );
    ...
    fn restore_trim_event_maps_to_extension_notification() {
        ...
        assert_eq!(value["method"], "_omnesagent/history_trimmed");
        assert_eq!(
            value["params"],
            serde_json::json!({
    ...
    fn deliver_artifact(
        ...
        let uri = if uri.is_empty() {
            ...
            let ext = path
                ...
                .and_then(|e| e.to_str())
                .unwrap_or_default();
            omnesagent_runtime::tools::attachment_deliver_uri(&acp_embedded::content_hash_name(
                &bytes, ext,
            ))
        } else {
            uri.to_string()
    ...
    fn deliver_file_resource_uri_is_the_content_hash() {
        ...
        std::fs::write(&path, b"%PDF").unwrap();
        let expected = omnesagent_runtime::tools::attachment_deliver_uri(
    ...
    fn deliver_file_result_caps_oversized_artifact_path() {
        ...
        std::fs::write(
            &path,
            vec![0u8; (omnesagent_runtime::tools::MAX_DELIVER_FILE_BYTES as usize) + 1],
        )
        .unwrap();
        let event = TurnEvent::ToolResult {
            id: "tc1".into(),
    ...
    fn plan_event_projects_to_acp_plan_update() {
        ...
        // OmnesAgent extension carried but additive:
    ...
    fn gateway_backed_server_initialize_uses_reloaded_config() {
        ...
        assert_eq!(
            server.handle_initialize(&serde_json::json!({})).unwrap()["_meta"]["omnesagent"]["defaultModel"],
            "reloaded-model"
        );
    }
    ...
    async fn session_load_replays_only_history_retained_after_restore_trim() {
        ...
        assert_eq!(notifications[0]["method"], "_omnesagent/history_trimmed");
        assert_eq!(
