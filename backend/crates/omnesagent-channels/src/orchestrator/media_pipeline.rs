...
use std::borrow::Cow;
use omnesagent_config::schema::MediaPipelineConfig;

use super::super::transcription::TranscriptionManager;

// Re-export media types from omnesagent-types for backwards compatibility.
pub use omnesagent_api::media::{MarkerKind, MediaAttachment, MediaKind, RenderedMarker};
...
impl<'a> MediaPipeline<'a> {
    ...
    async fn process_audio(&self, attachment: &MediaAttachment) -> String {
            ...
            .await
        {
            ...
            Err(err) => {
                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"file": attachment.file_name, "error": format!("{}", err)})), "Media pipeline: audio transcription failed");
