...
impl QQChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        omnesagent_config::schema::build_channel_proxy_client("channel.qq", self.proxy_url.as_deref())
    }
    ...
    async fn fetch_access_token_with_retry(&self) -> anyhow::Result<(String, u64)> {
        ...
        for attempt in 1..=AUTH_RETRY_MAX_ATTEMPTS {
            match self.fetch_access_token().await {
                Ok(result) => {
                    if attempt > 1 {
                        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"attempt": attempt, "AUTH_RETRY_MAX_ATTEMPTS": AUTH_RETRY_MAX_ATTEMPTS})), "getAppAccessToken succeeded on attempt /");
                    }
                    return Ok(result);
                }
                Err(e) => {
                    ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"attempt": attempt, "AUTH_RETRY_MAX_ATTEMPTS": AUTH_RETRY_MAX_ATTEMPTS, "e": e.to_string()})), "getAppAccessToken failed (attempt /)");
                    last_err = Some(e);
...
impl ::omnesagent_api::attribution::Attributable for QQChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(::omnesagent_api::attribution::ChannelKind::Qq)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        for attachment in &attachments {
            if let Err(e) = self
                ...
                .await
            {
                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"target": attachment.target, "error": format!("{}", e)})), "failed to send media attachment; falling back to text");
            ...
            tokio::select! {
                _ = hb_rx.recv() => {
                    ...
                    if missed_ack_count > 0 {
                        if missed_ack_count >= MAX_MISSED_ACKS {
                            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"missed_ack_count": missed_ack_count, "hb_interval": hb_interval, "grace_ms": grace_ms})), "consecutive heartbeat ACKs missed (interval ms + ms grace); connection appears zombied");
                            exit_reason = ExitReason::HeartbeatTimeout;
                            break;
                        }
                        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"missed_ack_count": missed_ack_count, "MAX_MISSED_ACKS": MAX_MISSED_ACKS})), "heartbeat ACK missed (/); tolerating transient delay");
                    }
                ...
                msg = read.next() => {
                    ...
                    match op {
                        ...
                        // Reconnect
                        7 => {
                            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown), "received Reconnect (op 7); will resume");
                            exit_reason = ExitReason::Reconnect;
                            break;
                        }
                        // Invalid Session
                        9 => {
                            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown), "received Invalid Session (op 9); clearing session for fresh auth");
                    ...
                    if event_type == "READY" || event_type == "RESUMED" {
                        if let Some(sid) = d.get("session_id").and_then(|s| s.as_str()) {
                            *self.session_id.write().await = Some(sid.to_string());
                            ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"sid": sid, "event_type": event_type})), "session established (session_id=, event=)");
                        }
                        continue;
                    }

                    ::omnesagent_log::record!(DEBUG, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"event_type": event_type, "d": d})), "event_type= payload=");
                    ...
                    match event_type {
                        "C2C_MESSAGE_CREATE" => {
                            ...
                            if !self.is_user_allowed(user_openid) {
                                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"user_openid": user_openid})), "ignoring C2C message from unauthorized user");
                                continue;
                            }
                ...
                channel_alias: Some(self.alias.clone()),
                    ...
                    attachments: vec![],
                            ...
                            if tx.send(channel_msg).await.is_err() {
                                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown), "message channel closed");
                        ...
                        "GROUP_AT_MESSAGE_CREATE" => {
                            ...
                            if !self.is_user_allowed(author_id) {
                                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"author_id": author_id})), "ignoring group message from unauthorized user");
                                continue;
                            }
        ...
        *self.last_sequence.write().await = if sequence >= 0 { Some(sequence) } else { None };
        ...
        match exit_reason {
            ...
            ExitReason::Close(ref frame) => {
                ...
                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"code": code.to_string(), "reason": reason.to_string()})), "WebSocket closed with code=, reason=\"\"; resume will be attempted on reconnect");
                anyhow::bail!(
...
allowed_users = ["user1"]
"#;
        let cfg = omnesagent_config::migration::migrate_to_current(v2_toml)
