...
use omnesagent_api::channel::{Channel, ChannelConversationScope, ChannelMessage, SendMessage};
use omnesagent_config::schema::{StreamMode, WeComWsConfig};
use omnesagent_runtime::i18n;
use omnesagent_tool_call_parser::TERMINAL_MARKERS;
...
impl WeComWsChannel {
    ...
    pub(crate) fn new_with_alias(
        ...
        let client = omnesagent_config::schema::build_channel_proxy_client_with_timeouts(
            "channel.wecom_ws",
...
impl ::omnesagent_api::attribution::Attributable for WeComWsChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(
            ::omnesagent_api::attribution::ChannelKind::WeComWs,
        )
    }
...
impl Channel for WeComWsChannel {
    ...
    async fn listen(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> Result<()> {
        ...
        loop {
            ...
            let ws_stream = match omnesagent_config::schema::ws_connect_with_proxy(
                WECOM_WS_URL,
                "channel.wecom_ws",
...
mod tests {
    ...
    fn runtime_model_switch_command_with_mentions() {
        ...
        assert_eq!(
            extract_runtime_model_switch_command(" /MODEL@omnesagent qwen-max "),
            Some("/MODEL@omnesagent qwen-max".to_string())
        );
    }
    ...
    #[test]
    fn parse_scope_user() {
        let (chat_type, chatid) = parse_scope("user--omnesagent_user").unwrap();
        assert_eq!(chat_type, 1);
        assert_eq!(chatid, "omnesagent_user");
    }

    #[test]
    fn parse_scope_group() {
        let (chat_type, chatid) = parse_scope("group--omnesagent_group").unwrap();
        assert_eq!(chat_type, 2);
        assert_eq!(chatid, "omnesagent_group");
    }
    ...
    fn access_decision_denies_when_allowlists_missing() {
        let inbound = test_inbound("single", None, "omnesagent_user");
        assert_eq!(
    ...
    fn access_decision_allows_userid_in_single_chat() {
        let inbound = test_inbound("single", None, "omnesagent_user");
        assert_eq!(
            evaluate_access_decision(&["omnesagent_user".to_string()], &[], &inbound),
            AccessDecision::Allowed
        );
    }
    ...
    fn access_decision_allows_group_chatid() {
        let inbound = test_inbound("group", Some("omnesagent_group"), "omnesagent_user");
        assert_eq!(
            evaluate_access_decision(&[], &["omnesagent_group".to_string()], &inbound),
            AccessDecision::Allowed
        );
    }
    ...
    fn denied_group_message_mentions_chatid_and_userid() {
        ...
        assert!(text.contains("omnesagent_group"));
        assert!(text.contains("omnesagent_user"));
    ...
    async fn send_draft_returns_none_when_stream_mode_off() {
        ...
        let id = channel
            .send_draft(&SendMessage::new("draft", "user--omnesagent_user"))
            .await
            .unwrap();
    ...
    async fn send_draft_failure_does_not_record_req_id_mapping() {
        ...
        let result = channel
            .send_draft(
                &SendMessage::new("draft", "user--omnesagent_user")
    ...
    async fn finalize_draft_failure_cleans_req_id_mapping() {
        ...
        let result = channel
            .finalize_draft("user--omnesagent_user", "stream-1", "final", false)
            .await;
    ...
    async fn send_with_req_id_uses_respond_msg_when_stream_mode_off() {
        ...
        *channel.ws_tx.lock().await = Some(ws_tx);
        ...
        let responder = omnesagent_spawn::spawn!(async move {
        ...
        channel
            .send(
                &SendMessage::new("runtime ok", "user--omnesagent_user")
    ...
    async fn send_without_req_id_uses_send_msg() {
        ...
        *channel.ws_tx.lock().await = Some(ws_tx);
        ...
        channel
            .send(&SendMessage::new("hello proactive", "user--omnesagent_user"))
            .await
            .unwrap();
    ...
    async fn clear_command_forwards_runtime_new_session_without_immediate_ws_reply() {
        let mut config = test_wecom_ws_config();
        config.allowed_users = vec!["omnesagent_user".to_string()];
        let channel = WeComWsChannel::new(&config, Path::new("/tmp")).unwrap();
    ...
    async fn unauthorized_group_message_replies_with_chatid_and_does_not_forward() {
        ...
        *channel.ws_tx.lock().await = Some(ws_tx);
        ...
        assert!(denied.contains("omnesagent_group"));
        assert!(denied.contains("omnesagent_user"));
    ...
    async fn stream_reply_serializes_same_req_id_updates() {
        ...
        *channel.ws_tx.lock().await = Some(tx);
        ...
        let first = omnesagent_spawn::spawn!(async move {
            first_channel
        ...
        let second = omnesagent_spawn::spawn!(async move {
            second_channel
