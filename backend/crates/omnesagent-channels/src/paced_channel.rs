...
use omnesagent_api::attribution::{Attributable, Role};
...
use omnesagent_config::schema::{DEFAULT_REPLY_QUEUE_DEPTH, HasReplyPacing, PACING_RECIPIENT_CAP};
...
impl Channel for PacedChannel {
    ...
    fn listener_health(&self) -> Option<omnesagent_api::channel::ListenerHealth> {
...
mod tests {
    ...
    use omnesagent_api::channel::DraftProgressKind;
    ...
    impl Attributable for CountingChannel {
        fn role(&self) -> Role {
            ...
            Role::Channel(omnesagent_api::attribution::ChannelKind::Cli)
        }
        fn alias(&self) -> &str {
            "counting"
        }
    ...
    impl Attributable for BatchProgressChannel {
        fn role(&self) -> Role {
            Role::Channel(omnesagent_api::attribution::ChannelKind::Matrix)
        }
        fn alias(&self) -> &str {
            "batch-progress"
        }
    ...
    async fn slow_immediate_send_forces_concurrent_send_to_enqueue() {
        ...
        let a = omnesagent_spawn::spawn!(async move {
        ...
        let paced_b = Arc::clone(&paced);
        let b = omnesagent_spawn::spawn!(async move {
