...
async fn resolve_approval_reply_with_group_admission(
    ...
    group_policy: &omnesagent_config::schema::WhatsAppChatPolicy,
    dm_policy: &omnesagent_config::schema::WhatsAppChatPolicy,
...
pub struct WhatsAppWebChannel {
    ...
    mode: omnesagent_config::schema::WhatsAppWebMode,
    ...
    persist: Option<Arc<parking_lot::RwLock<omnesagent_config::schema::Config>>>,
...
struct WhatsAppInboundContext {
    ...
    dm_mention_patterns: Arc<Vec<regex::Regex>>,
    group_mention_patterns: Arc<Vec<regex::Regex>>,
    transcription_config: Option<omnesagent_config::schema::TranscriptionConfig>,
...
impl WhatsAppWebChannel {
    #[cfg(feature = "whatsapp-web")]
    pub fn new(
        config: &omnesagent_config::schema::WhatsAppConfig,
        ...
        if omnesagent_config::schema::whatsapp_empty_group_list_is_newly_closed(&mode, &group_policy)
            && allowed_groups_resolver().is_empty()
        {
            ::omnesagent_log::record!(
                ...
                format!(
                    ...
                    omnesagent_config::schema::whatsapp_empty_group_list_remedy(&group_policy)
                )
            );
        }
    ...
    pub fn with_persistence(
        mut self,
        config: Arc<parking_lot::RwLock<omnesagent_config::schema::Config>>,
    ) -> Self {
    ...
    async fn handle_inbound_message_event(
        ...
        // omnesagent-gateway lib.rs sits inside the Cloud WEBHOOK handler, and
        ...
        if context.mode == omnesagent_config::schema::WhatsAppWebMode::Personal {
...
fn is_group_chat_allowed(
    ...
    if allowed_groups.is_empty() {
        return matches!(
            group_policy,
            omnesagent_config::schema::WhatsAppChatPolicy::All
        );
    }
...
fn chat_type_policy_decision(
    ...
    use omnesagent_config::schema::WhatsAppChatPolicy as Policy;
...
fn self_chat_verdict(
    ...
    if *mode != omnesagent_config::schema::WhatsAppWebMode::Personal {
...
fn composed_chat_policy_decision(
    ...
    if *mode == omnesagent_config::schema::WhatsAppWebMode::Personal && operator_self_chat {
        ChatPolicyDecision::Admit
    } else {
...
#[cfg(feature = "whatsapp-web")]
impl ::omnesagent_api::attribution::Attributable for WhatsAppWebChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(
            ::omnesagent_api::attribution::ChannelKind::WhatsappWeb,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
                        ...
                        match &*event {
                            ...
                            Event::Connected(_) => {
                                ...
                                if mention_only {
                                    ...
                                    {
                                        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), &format!("resolved bot identity from device: +{}", digits));
                                    }
                                    if let Some(ref lid) = device.lid
                                        && let Some(digits) =
                                            Self::store_jid_digits(&bot_lid_inner, lid.user())
                                    {
                                        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), &format!("resolved bot LID from device: {}", digits));
                                    }
                                }
                                ...
                                if let Some(desired) = WhatsAppWebChannel::push_name_to_apply(
                                    ...
                                    match client.profile().set_push_name(desired).await {
                                        Ok(()) => {
                                            ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), &format!("set WhatsApp push name (configured len={}, previous len={})", desired.len(), device.push_name.len()));
                                        }
                                        Err(e) => {
                                            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown), &format!("failed to set WhatsApp push name (configured len={}); keeping the account's current name: {e}", desired.len()));
                                        }
                                    }
                                }
                                ...
                                if let Some(ref pn) = device.pn {
                                    ...
                                    if !digits.is_empty()
                                        && let Err(e) =
                                            ...
                                            .await
                                    {
                                        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown), &format!("failed to persist linked WhatsApp identity: {e}"));
                                    }
                                }
                            }
                            Event::LoggedOut(_) => {
                            ...
                            Event::StreamError(stream_error) => {
                                ::omnesagent_log::record!(ERROR, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Fail).with_outcome(::omnesagent_log::EventOutcome::Failure), &format!("stream error: {:?}", stream_error));
                            }
                            ...
                            Event::PairingQrCode { code, .. } => {
                                ...
                                match Self::render_pairing_qr(code) {
                                    ...
                                    Err(err) => {
                                        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown), &format!("failed to render pairing QR in terminal: {}", err));
                                        eprintln!();
            ...
            *self.bot_handle.lock() = Some(bot_handle);
            ...
            let should_reconnect = select! {
                ...
                _ = tokio::signal::ctrl_c() => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), "channel received Ctrl+C");
                    false
                }
            };
...
impl WhatsAppWebChannel {
    pub fn new(
        ...
        _mention_only: bool,
        _mode: omnesagent_config::schema::WhatsAppWebMode,
        _dm_policy: omnesagent_config::schema::WhatsAppChatPolicy,
        _group_policy: omnesagent_config::schema::WhatsAppChatPolicy,
        _self_chat_mode: bool,
    ) -> Self {
        Self { _private: () }
    }

    pub fn with_transcription(self, _config: omnesagent_config::schema::TranscriptionConfig) -> Self {
        self
    }

    pub fn with_tts(self, _config: omnesagent_config::schema::TtsConfig) -> Self {
        self
    }
}
...
mod tests {
    ...
    fn allowed_groups_empty_admits_only_under_policy_all() {
        ...
        assert!(!super::is_group_chat_allowed(
            jid,
            &[],
            &omnesagent_config::schema::WhatsAppChatPolicy::Allowlist
        ));
        assert!(!super::is_group_chat_allowed(
            jid,
            &[],
            &omnesagent_config::schema::WhatsAppChatPolicy::Ignore
        ));
    }
    ...
    fn composed_policy_decision_enforces_both_modes() {
        use omnesagent_config::schema::{WhatsAppChatPolicy as Policy, WhatsAppWebMode as Mode};
    ...
    async fn whatsapp_client_persistent_lid_mapping_drives_allowlist() {
        ...
        let context = WhatsAppInboundContext {
            ...
            mode: omnesagent_config::schema::WhatsAppWebMode::Personal,
            dm_policy: omnesagent_config::schema::WhatsAppChatPolicy::Allowlist,
            group_policy: omnesagent_config::schema::WhatsAppChatPolicy::Allowlist,
    ...
    fn push_name_to_apply_sends_only_on_a_real_difference() {
        ...
        let cases: [(Option<&str>, &str, Option<&str>); 11] = [
            ...
            (Some(""), "Old", None),
            (Some("   \t "), "Old", None),
            (Some("OmnesAgentAgent"), "Galaxy S21", Some("OmnesAgentAgent")),
            (Some("OmnesAgentAgent"), "", Some("OmnesAgentAgent")),
            (Some("OmnesAgentAgent"), "OmnesAgentAgent", None),
            (Some("  OmnesAgentAgent  "), "OmnesAgentAgent", None),
    ...
    #[cfg(feature = "whatsapp-web")]
    fn approval_cfg(approval_timeout_secs: u64) -> omnesagent_config::schema::WhatsAppConfig {
    ...
    async fn request_approval_surfaces_a_send_failure_to_the_caller() {
        ...
        let cfg = omnesagent_config::schema::WhatsAppConfig::default();
        let channel =
    ...
    async fn approval_prompt_prose_comes_from_the_catalogue_in_both_chat_shapes() {
        async fn capture_prompt(recipient: &str) -> String {
            ...
            let channel = WhatsAppWebChannel::new(
                &omnesagent_config::schema::WhatsAppConfig::default(),
                "alias-prompt",
                Arc::new(Vec::new),
                Arc::new(Vec::new),
            )
    ...
    async fn cancelled_request_leaves_no_live_token() {
        ...
        let request = omnesagent_spawn::spawn!(async move {
    ...
    /// operator told OmnesAgent to ignore.
    #[tokio::test]
    #[cfg(feature = "whatsapp-web")]
    ...
    fn self_chat_verdict_matrix() {
        use omnesagent_config::schema::WhatsAppWebMode as Mode;
    ...
    ///
    /// The split is fixed in `omnesagent-config`, so this asserts the channel end
    /// of it: whichever way an operator's alias came into being, the channel
    /// waits the documented timeout. The `omnesagent-config` side additionally
