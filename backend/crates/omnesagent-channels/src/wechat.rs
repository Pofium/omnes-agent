...
use omnesagent_config::paths::{normalize_lexical, resolve_under};
...
impl WeChatChannel {
    pub fn new(
        ...
        let pairing = if has_peers {
            ...
            if let Some(code) = guard.pairing_code() {
                ...
                // log where `omnesagent service logs` / the gateway can find it.
    ...
    /// is unset: `~/.omnesagent/wechat`.
    fn default_state_dir() -> PathBuf {
        directories::UserDirs::new()
            .map(|u| u.home_dir().join(".omnesagent").join("wechat"))
            .unwrap_or_else(|| PathBuf::from(".omnesagent/wechat"))
    }
    ...
    async fn send_message_items(
        ...
        let client_id = format!("omnesagent-{}", uuid::Uuid::new_v4());
        let body = serde_json::json!({
            "msg": {
                "from_user_id": "",
                "to_user_id": to,
                ...
                match pairing.try_pair(code, from_user_id).await {
                    Ok(Some(_token)) => {
                        if let Err(e) = self.persist_allowed_identity(from_user_id).await {
                            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"from_user_id": from_user_id, "e": e.to_string()})), "failed to persist bound identity");
                        }
...
impl ::omnesagent_api::attribution::Attributable for WeChatChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(::omnesagent_api::attribution::ChannelKind::Wechat)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
        ...
        loop {
            ...
            let resp = match result {
                ...
                Ok(Err(e)) => {
                    ...
                    ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"consecutive_failures": consecutive_failures, "MAX_CONSECUTIVE_FAILURES": MAX_CONSECUTIVE_FAILURES, "e": e.to_string()})), "getUpdates error (/)");
