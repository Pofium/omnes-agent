...
impl TwitterChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        omnesagent_config::schema::build_runtime_proxy_client("channel.twitter")
    }
...
impl ::omnesagent_api::attribution::Attributable for TwitterChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(
            ::omnesagent_api::attribution::ChannelKind::Twitter,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
        ...
        loop {
                ...
                .await
            {
                ...
                Ok(resp) => {
                    ...
                    let err = resp.text().await.unwrap_or_default();
                    ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", err), "status": status.to_string()})), "mentions request failed");
                }
                Err(e) => {
                    ::omnesagent_log::record!(
                        WARN,
...
bearer_token = "AAAA"
"#;
        let config: omnesagent_config::schema::TwitterConfig = toml::from_str(toml_str).unwrap();
