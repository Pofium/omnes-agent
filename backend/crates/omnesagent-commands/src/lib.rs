...
mod tests {
    ...
    fn command_lookup_accepts_slash_alias_and_bot_suffix() {
        assert_eq!(
            command_by_name("/new@omnesagent_bot").map(|spec| spec.id),
    ...
    fn command_lookup_normalizes_case_whitespace_and_bot_suffix() {
        assert_eq!(
            normalize_command_name("  /MODEL@OmnesAgent_Bot  "),
            Some("model".to_string())
        );
        assert_eq!(
            command_by_name("  /THINK@OmnesAgent_Bot  ").map(|spec| spec.id),
            Some(BuiltinCommandId::Thinking)
        );
        assert_eq!(
            parse_command_token("  /NEW-SESSION@OmnesAgent_Bot  ", CommandSurface::Channel)
