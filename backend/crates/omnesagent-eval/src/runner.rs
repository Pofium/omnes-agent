...
use omnesagent_memory::{Memory, create_memory};
use omnesagent_runtime::agent::agent::Agent;
use omnesagent_runtime::agent::dispatcher::NativeToolDispatcher;
...
pub async fn run_case(trace: &LlmTrace) -> anyhow::Result<RunRecord> {
    ...
    let eval_config = omnesagent_config::schema::Config::default();
    let eval_security = Arc::new(omnesagent_config::policy::SecurityPolicy::default());
    let eval_registry = omnesagent_runtime::tools::scoped::ScopedToolRegistry::assemble(
        omnesagent_runtime::tools::scoped::ScopedAssembly {
            config: &eval_config,
            agent_alias: "eval",
            security: &eval_security,
            built: omnesagent_runtime::tools::AllToolsResult::from_prebuilt_tools(default_tools()),
            skills: &[],
            runtime: Arc::new(omnesagent_runtime::platform::NativeRuntime::new()),
            caller_allowed: None,
            connect_mcp: false,
