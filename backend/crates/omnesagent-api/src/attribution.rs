...
impl Role {
    ...
    /// Closest `omnesagent_log::EventCategory` for this role, used by
    /// the layer to default `event.category` when the call site doesn't
    /// override. Returned as a `&'static str` to keep `omnesagent-api`
    /// free of a back-dep on `omnesagent-log`.
    #[must_use]
