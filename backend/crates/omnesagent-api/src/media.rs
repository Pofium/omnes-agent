...
mod tests {
    ...
    fn from_file_reads_data_and_maps_extension_to_mime() {
        let path = std::env::temp_dir().join("omnesagent_media_kind_test_sample.png");
        std::fs::write(&path, b"\x89PNG fake-bytes").unwrap();
        let att = MediaAttachment::from_file(path.to_str().unwrap()).unwrap();
        assert_eq!(att.file_name, "omnesagent_media_kind_test_sample.png");
    ...
    fn from_file_propagates_read_error_for_missing_path() {
        let missing = std::env::temp_dir().join("omnesagent_media_kind_missing_xyz.bin");
