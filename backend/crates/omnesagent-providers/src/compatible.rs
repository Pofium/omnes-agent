...
use omnesagent_config::schema::ToolResultImagePolicy;
...
pub struct OpenAiCompatibleModelProvider {
    ...
    schema_cache: std::sync::Arc<omnesagent_api::schema::SchemaCleanCache>,
}
...
/// cannot get around the bound). Mirrors the bounded reader in `omnesagent-channels`
...
struct ModelEntry {
    ...
    #[serde(default)]
    pricing: Option<omnesagent_api::model_provider::ModelPricing>,
}
...
fn normalize_models_with_pricing(
    body: ModelsResponse,
) -> Vec<omnesagent_api::model_provider::ModelInfo> {
    use omnesagent_api::model_provider::ModelInfo;
...
impl OpenAiCompatibleBuilder {
    ...
    pub fn build(self) -> OpenAiCompatibleModelProvider {
        ...
        OpenAiCompatibleModelProvider {
            ...
            schema_cache: std::sync::Arc::new(omnesagent_api::schema::SchemaCleanCache::new()),
        }
    }
}
...
impl OpenAiCompatibleModelProvider {
    ...
    fn streaming_http_client(&self) -> Client {
        ...
        let builder = Client::builder()
            ...
            .read_timeout(STREAM_IDLE_TIMEOUT);
        let builder =
            omnesagent_config::schema::apply_runtime_proxy_to_builder(builder, "provider.compatible");
...
impl UsageInfo {
    ...
    fn into_provider_usage(self) -> omnesagent_api::model_provider::TokenUsage {
        let cached_input_tokens = self.cached_input_tokens();
        omnesagent_api::model_provider::TokenUsage {
...
struct ToolCall {
    ...
    /// See [`omnesagent_api::ToolCall::extra_content`].
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl OpenAiCompatibleModelProvider {
    ...
    fn convert_tool_specs(
        &self,
        tools: Option<&[omnesagent_api::tool::ToolSpec]>,
                ...
                .map(|tool| NativeToolSpec {
                    ...
                    function: NativeToolFunctionSpec {
                        ...
                        parameters: self.schema_cache.clean_shared(
                            &tool.parameters,
                            omnesagent_api::schema::CleaningStrategy::OpenAI,
                        ),
                    },
                })
                .collect()
    ...
    fn convert_tool_specs_for_model(
        ...
        for tool in &mut converted {
            ...
            let cleaned = omnesagent_api::schema::SchemaCleanr::clean(
                raw_parameters,
                omnesagent_api::schema::CleaningStrategy::Conservative,
            );
    ...
    async fn normalize_messages_for_upstream(
        ...
        let config = omnesagent_config::schema::MultimodalConfig::default();
        let sanitized;
    ...
    fn with_prompt_guided_tool_instructions(
        ...
        let instructions = omnesagent_api::model_provider::build_tool_instructions_text(tools);
...
impl ModelProvider for OpenAiCompatibleModelProvider {
    ...
    fn capabilities(&self) -> omnesagent_api::model_provider::ProviderCapabilities {
        omnesagent_api::model_provider::ProviderCapabilities {
    ...
    async fn list_models_with_pricing(
        &self,
    ) -> anyhow::Result<Vec<omnesagent_api::model_provider::ModelInfo>> {
    ...
    fn stream_chat(
        ...
        let tools_owned: Option<Vec<omnesagent_api::tool::ToolSpec>> =
            request.tools.map(<[omnesagent_api::tool::ToolSpec]>::to_vec);
...
impl ::omnesagent_api::attribution::Attributable for OpenAiCompatibleModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::Plugin,
            ),
        )
    }
    fn alias(&self) -> &str {
...
mod tests {
    ...
    fn convert_tool_specs_serializes_openai_wire_shape() {
        ...
        let tools = vec![
            omnesagent_api::tool::ToolSpec::new(
                "get_weather",
                "Fetch the weather",
                serde_json::json!({
                    "type": "object",
    ...
    fn streaming_native_tool_request_serializes_tools_and_guards_tool_choice() {
        ...
        let tools = vec![omnesagent_api::tool::ToolSpec::new(
            "get_weather",
            "Fetch the weather",
    ...
    fn with_tls_ca_cert_path_invalid_pem_stores_bytes_and_http_client_still_builds() {
        ...
        let path = format!("/tmp/omnesagent-test-invalid-pem-{}.pem", std::process::id());
    ...
    fn with_tls_ca_cert_path_invalid_pem_streaming_http_client_still_builds() {
        ...
        let path = format!(
            "/tmp/omnesagent-test-invalid-pem-streaming-{}.pem",
            std::process::id()
        );
        std::fs::write(&path, b"not-a-valid-pem").unwrap();
        let p = OpenAiCompatibleModelProvider::builder("test")
    ...
    fn request_serializes_correctly() {
        let req = ApiChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: MessageContent::Text("You are OmnesAgent".to_string()),
                },
                Message {
                    role: "user".to_string(),
    ...
    fn user_agent_constructor_keeps_native_tool_calling_enabled() {
        let p = OpenAiCompatibleModelProvider::builder("test")
            ...
            .credential(Some("k"))
            .auth_style(AuthStyle::Bearer)
            .user_agent("omnesagent-test/1.0")
            .build();
        let caps = <OpenAiCompatibleModelProvider as ModelProvider>::capabilities(&p);
        assert!(caps.native_tool_calling);
        assert!(!caps.vision);
        assert_eq!(p.user_agent.as_deref(), Some("omnesagent-test/1.0"));
    }
    ...
    fn user_agent_and_vision_constructor_preserves_capability_flags() {
        let p = OpenAiCompatibleModelProvider::builder("test")
            ...
            .credential(Some("k"))
            .auth_style(AuthStyle::Bearer)
            .user_agent("omnesagent-test/vision")
            .vision(true)
            .build();
        let caps = <OpenAiCompatibleModelProvider as ModelProvider>::capabilities(&p);
        assert!(caps.native_tool_calling);
        assert!(caps.vision);
        assert_eq!(p.user_agent.as_deref(), Some("omnesagent-test/vision"));
    }
    ...
    fn extra_headers_sets_headers() {
        ...
        headers.insert("X-Title".to_string(), "omnesagent".to_string());
        headers.insert(
        ...
        assert_eq!(p.extra_headers.get("X-Title").unwrap(), "omnesagent");
        assert_eq!(
