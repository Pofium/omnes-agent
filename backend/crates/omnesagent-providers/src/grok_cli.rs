...
//! explicit per-alias opt-in: export `XAI_API_KEY` into the OmnesAgent process and
...
//!
//! The child is a tool-capable agent outside OmnesAgent's native tool approval
...
//! - Native OmnesAgent tool calls are not emitted; `ChatRequest.tools` is ignored.
...
pub struct GrokCliModelProvider {
    ...
    /// advertise (still false through 0.2.118); only OmnesAgent's send path.
...
/// before spawn, so this does not signal OmnesAgent itself.
#[cfg(unix)]
...
impl GrokCliModelProvider {
            ...
            if RESERVED_EXTRA_ARG_FLAGS.contains(&flag) {
                anyhow::bail!(
                    "grok_cli extra_args must not include reserved flag `{flag}`. \
                     ACP transport, prompt, model, session, cwd, and update policy are owned by OmnesAgent."
                );
            }
    ...
    fn acp_prompt_content(&self, message: &str) -> Vec<acp::AcpPromptContent> {
        ...
        // `promptCapabilities.image = false`. With `vision = true`, OmnesAgent
...
impl ::omnesagent_api::attribution::Attributable for GrokCliModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::GrokCli,
            ),
        )
    }
...
mod tests {
    ...
    fn env_allowlist_blocks_unrelated_secrets() {
        ...
        assert!(!env_var_allowed("OMNESAGENT_SLACK_BOT_TOKEN", none));
    ...
    fn apply_env_allowlist_copies_only_builtin_and_explicit_entries() {
        ...
        let xai_api_key_available = GrokCliModelProvider::apply_env_allowlist_from(
            ...
            [
                ...
                (
                    "OMNESAGENT_SLACK_BOT_TOKEN".to_string(),
        ...
        assert!(
            !copied
                .iter()
                .any(|(name, _)| name == "OMNESAGENT_SLACK_BOT_TOKEN")
        );
