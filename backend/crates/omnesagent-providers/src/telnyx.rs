...
impl ModelProvider for TelnyxModelProvider {
    ...
    async fn chat_with_system(
        ...
        let api_key = self.api_key.as_ref().ok_or_else(|| {
            ...
            anyhow::Error::msg(
                "Telnyx API key not set. Set TELNYX_API_KEY environment variable or run `omnesagent quickstart --model-provider telnyx --api-key <key>`.",
            )
        })?;
...
impl ::omnesagent_api::attribution::Attributable for TelnyxModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::Telnyx,
            ),
        )
    }
    fn alias(&self) -> &str {
