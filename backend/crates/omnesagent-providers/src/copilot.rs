...
impl CopilotModelProvider {
    ...
    fn new_impl(alias: String, github_token: Option<String>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "omnesagent")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("omnesagent-copilot-{user}"))
            });
...
async fn write_file_secure(path: &Path, content: &str) {
    ...
    match result {
        Ok(Ok(())) => {}
        Ok(Err(err)) => ::omnesagent_log::record!(
            WARN,
            ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note)
                .with_outcome(::omnesagent_log::EventOutcome::Unknown)
...
impl ::omnesagent_api::attribution::Attributable for CopilotModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::Copilot,
            ),
        )
    }
    fn alias(&self) -> &str {
