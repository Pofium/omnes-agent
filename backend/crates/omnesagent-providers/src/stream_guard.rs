...
pub(crate) async fn finish_sse_stream(
    ...
    if saw_completion {
        ...
        let _ = tx
            .send(Ok(::omnesagent_api::model_provider::StreamEvent::Final))
            .await;
        return;
    }
    ::omnesagent_log::record!(
    ...
    let _ = tx
        .send(Err(::omnesagent_api::model_provider::StreamError::Http(
...
#[cfg(test)]
mod tests {
    use ::omnesagent_api::model_provider::{StreamError, StreamEvent, StreamResult};
