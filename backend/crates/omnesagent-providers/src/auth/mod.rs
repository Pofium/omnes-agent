...
async fn refresh_openai_access_token_with_retries(
    ...
    refresh_oauth_access_token_with_retries(
        ...
        |failure| {
            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"attempt": failure.attempt, "max_attempts": failure.max_attempts, "retry": failure.should_retry, "error": format!("{}", failure.error)})), "OpenAI token refresh failed");
        },
    )
    .await
}
...
async fn refresh_gemini_access_token_with_retries(
    ...
    refresh_oauth_access_token_with_retries(
        ...
        |failure| {
            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"attempt": failure.attempt, "max_attempts": failure.max_attempts, "retry": failure.should_retry, "error": format!("{}", failure.error)})), "Gemini token refresh failed");
        },
    )
    .await
}
...
fn pending_oauth_secret_store(config: &Config) -> omnesagent_config::secrets::SecretStore {
    omnesagent_config::secrets::SecretStore::new(
...
impl AuthProviderFlow for OpenaiCodexFlow {
    async fn login(
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                println!(
                    "Run `omnesagent auth paste-redirect --model-provider openai-codex --profile {profile}`"
                );
                return Ok(());
            }
        };
    ...
    async fn paste_redirect(
        ...
        let pending = load_pending_oauth_login(ctx.config, "openai")?.ok_or_else(|| {
            ...
            anyhow::Error::msg(
                "No pending OpenAI login found. Run `omnesagent auth login --model-provider openai-codex` first.",
            )
        })?;
...
impl AuthProviderFlow for GeminiFlow {
    async fn login(
        ...
        .await
        {
            ...
            Err(e) => {
                println!("Callback capture failed: {e}");
                println!(
                    "Run `omnesagent auth paste-redirect --model-provider gemini --profile {profile}`",
                );
                return Ok(());
            }
        };
    ...
    async fn paste_redirect(
        ...
        let pending = load_pending_oauth_login(ctx.config, "gemini")?.ok_or_else(|| {
            ...
            anyhow::Error::msg(
                "No pending Gemini login found. Run `omnesagent auth login --model-provider gemini` first.",
            )
        })?;
        if pending.profile != profile {
            anyhow::bail!(
    ...
    async fn paste_redirect(
        ...
        let pending = load_pending_oauth_login(ctx.config, "xai")?.ok_or_else(|| {
            anyhow::Error::msg(ctx.cli_text(
                ...
                &[],
                "No pending xAI login found. Run `omnesagent auth login --model-provider xai` first.",
            ))
        })?;
        if pending.profile != profile {
            anyhow::bail!(
