...
use omnesagent_config::schema::{MultimodalConfig, build_runtime_proxy_client_with_timeouts};
...
/// `crates/omnesagent-channels/src/util.rs`) recognizes these same kinds plus
...
mod tests {
    ...
    fn strip_media_markers_replaces_image_local_path() {
        let input = "Look at [IMAGE:/omnesagent-data/workspace/telegram_files/photo_1.jpg]";
    ...
    fn strip_media_markers_replaces_all_supported_kinds() {
        ...
        // `crates/omnesagent-channels/src/util.rs`, which is the source of
    ...
    fn parse_image_markers_collapses_line_wrapped_path() {
        ...
        let input = "from the logs whether the agent emits\n  [IMAGE:/home/omnesagent_user/.omnesagent/workspace/signal_i\n  nbound/attachment.jpg] (which the\n  channel resolves)";
        let (_, refs) = parse_image_markers(input);
        assert_eq!(refs.len(), 1);
        assert_eq!(
            refs[0],
            "/home/omnesagent_user/.omnesagent/workspace/signal_inbound/attachment.jpg"
        );
    }
