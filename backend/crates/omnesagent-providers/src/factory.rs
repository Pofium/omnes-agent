...
//! `omnesagent-config::providers::ModelProviders` declares its own construction
...
//!
//! [`for_each_model_provider_slot!`]: omnesagent_config::for_each_model_provider_slot
...
pub(crate) trait CompatFamilySpec {
    ...
    fn wire_api(&self) -> Option<omnesagent_config::schema::WireApi> {
        None
    }
...
fn build_responses_provider_if_requested(
    wire_api: Option<omnesagent_config::schema::WireApi>,
    alias: &str,
    base_url: Option<&str>,
    key: Option<&str>,
    opts: &ModelProviderRuntimeOptions,
) -> Option<Box<dyn ModelProvider>> {
    if wire_api != Some(omnesagent_config::schema::WireApi::Responses) {
        return None;
    }
...
pub fn dispatch_family_factory(
        ...
        ($(($field:ident, $type_str:literal, $cfg_ty:ty)),+ $(,)?) => {
            match family {
                "openai-compatible" | "openai_compatible" => {
                    let default_cfg = omnesagent_config::schema::ModelProviderConfig::default();
                    let cfg = config
                ...
                _ => {
                    ...
                    Err(anyhow::Error::msg(format!(
                        "Unknown model_provider family: {family}. After the V2 to typed-family migration, \
                         only canonical family names are valid. Run `omnesagent quickstart` to reconfigure, \
    ...
    omnesagent_config::for_each_model_provider_slot!(emit_dispatch)
}
...
pub(crate) fn fallback_auth_ready_for_alias(
    ...
    omnesagent_config::for_each_model_provider_slot!(emit_auth_ready)
}
...
pub fn endpoint_for_family(provider_type: &str) -> Option<ProviderEndpoint> {
    ...
    omnesagent_config::for_each_model_provider_slot!(emit_endpoint)
}
...
impl CompatFamilySpec for ZerorouterModelProviderConfig {
    ...
    const DEFAULT_URL: &'static str = omnesagent_config::schema::ZEROROUTER_DEFAULT_URL;
    const AUTH: AuthStyle = AuthStyle::Bearer;
    const PUBLIC_MODEL_LISTING: bool = true;
}
...
impl FamilyProviderFactory for XaiModelProviderConfig {
    ...
    fn create_provider(
        ...
        if !has_api_key(key) {
            let state_dir = opts.omnesagent_dir.clone().unwrap_or_else(|| {
                directories::UserDirs::new().map_or_else(
                    || std::path::PathBuf::from(".omnesagent"),
...
fn openai_missing_entry_fallback_config() -> OpenAIModelProviderConfig {
    OpenAIModelProviderConfig {
        base: omnesagent_config::schema::ModelProviderConfig {
            wire_api: None,
            ..omnesagent_config::schema::ModelProviderConfig::default()
        },
    }
}
    ...
    fn create_provider(
        ...
        Ok(Box::new(
            crate::grok_cli::GrokCliModelProvider::builder(alias)
                ...
                // OmnesAgent emit ACP image blocks. Grok still advertises
...
impl CompatFamilySpec for KiloModelProviderConfig {
    ...
    // `KiloEndpoint::Gateway` in omnesagent-config — see the
...
impl FamilyProviderFactory for omnesagent_config::schema::ModelProviderConfig {
    const ENDPOINT: ProviderEndpoint = ProviderEndpoint::OperatorRequired;
...
mod tests {
    use super::*;
    use omnesagent_config::schema::{ModelProviderConfig, WireApi};
    ...
    fn endpoint_registry_classifies_every_canonical_family() {
        ...
        let families = omnesagent_config::for_each_model_provider_slot!(collect_names);
    ...
    fn every_fixed_endpoint_constructs_without_an_override() {
        ...
        omnesagent_config::for_each_model_provider_slot!(assert_fixed_construction);
    }
    ...
    fn kilo_gateway_default_url_matches_schema_endpoint() {
        use omnesagent_config::schema::{KiloEndpoint, ModelEndpoint};
        assert_eq!(
    ...
    fn atlascloud_default_url_matches_schema_endpoint() {
        use omnesagent_config::schema::{AtlasCloudEndpoint, ModelEndpoint};
        assert_eq!(
    ...
    fn zerorouter_default_url_matches_schema_endpoint() {
        use omnesagent_config::schema::{ModelEndpoint, ZerorouterEndpoint};
        assert_eq!(
    ...
    fn llamacpp_factory_routes_to_responses_provider_when_wire_api_responses() {
        use omnesagent_config::schema::{LlamacppModelProviderConfig, ModelProviderConfig, WireApi};
    ...
    fn llamacpp_factory_defaults_to_chat_completions_without_wire_api() {
        use omnesagent_config::schema::LlamacppModelProviderConfig;
    ...
    fn custom_factory_routes_to_responses_provider_when_wire_api_responses() {
        use omnesagent_config::schema::{CustomModelProviderConfig, ModelProviderConfig, WireApi};
    ...
    fn custom_factory_defaults_to_chat_completions_without_wire_api() {
        use omnesagent_config::schema::{CustomModelProviderConfig, ModelProviderConfig};
        let cfg = CustomModelProviderConfig {
            base: ModelProviderConfig {
                uri: Some("http://10.0.0.15:8000/v1".to_string()),
                ..Default::default()
