...
fn require_terminal_text(content: String) -> anyhow::Result<String> {
    if omnesagent_api::model_provider::strip_think_tags(&content).is_empty() {
...
impl ::omnesagent_api::attribution::Attributable for OpenAiModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::OpenAi,
            ),
        )
    }
    fn alias(&self) -> &str {
...
impl ModelProvider for OpenAiResponsesModelProvider {
    ...
    fn stream_chat(
        ...
        let alias = ::omnesagent_log::debug_enabled().then(|| self.alias.clone());
...
impl ::omnesagent_api::attribution::Attributable for OpenAiResponsesModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::OpenAi,
...
mod tests {
    ...
    async fn chat_with_system_fails_without_key() {
        let p = OpenAiModelProvider::builder("test")
            ...
            .build();
        let result = p
            .chat_with_system(Some("You are OmnesAgent"), "test", "gpt-4o", Some(0.5))
            .await;
        assert!(result.is_err());
    }
    ...
    fn request_serializes_with_system_message() {
        let req = ChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: "You are OmnesAgent".to_string(),
                },
                Message {
                    role: "user".to_string(),
