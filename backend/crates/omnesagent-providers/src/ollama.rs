            ...
            Err(first_err) if self.reasoning_enabled == Some(true) => {
                ...
                self.send_request_inner(&messages, model, temperature, should_auth, tools, None)
                    ...
                    .map_err(|retry_err| {
                        ...
                        ::omnesagent_log::record!(ERROR, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Fail).with_outcome(::omnesagent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"model": model, "original_error": first_err.to_string(), "retry_error": retry_err.to_string()})), "Ollama request also failed without think; returning original error");
                        first_err
                    })
            }
            Err(e) => Err(e),
...
impl ::omnesagent_api::attribution::Attributable for OllamaModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::Ollama,
            ),
        )
    }
    fn alias(&self) -> &str {
...
mod tests {
    ...
    async fn chat_with_tool_specs_omits_native_tools_payload() {
        ...
        use omnesagent_api::model_provider::ChatRequest;
