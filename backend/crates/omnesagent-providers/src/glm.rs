...
impl GlmModelProvider {
        ...
        if self.api_key_id.is_empty() || self.api_key_secret.is_empty() {
            anyhow::bail!(
                "GLM API key not set or invalid format. Expected 'id.secret'. \
                 Set GLM_API_KEY env var or run `omnesagent quickstart --model-provider glm --api-key <id.secret>`."
            );
        }
    ...
    fn http_client(&self) -> Client {
        omnesagent_config::schema::build_runtime_proxy_client_with_timeouts("model_provider.glm", 120, 10)
    }
}
