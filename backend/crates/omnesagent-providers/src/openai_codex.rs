...
const DEFAULT_CODEX_INSTRUCTIONS: &str =
    "You are OmnesAgent, a concise and helpful coding assistant.";
...
impl OpenAiCodexModelProvider {
    pub fn new(
        ...
        let state_dir = options
            .omnesagent_dir
            .clone()
            .unwrap_or_else(default_omnesagent_dir);
    ...
    fn http_client(&self) -> Client {
        omnesagent_config::schema::build_runtime_proxy_client_with_read_timeout(
...
fn default_omnesagent_dir() -> PathBuf {
    directories::UserDirs::new().map_or_else(
        || PathBuf::from(".omnesagent"),
        |dirs| dirs.home_dir().join(".omnesagent"),
    )
}
    ...
    async fn resolve_credentials(&self) -> anyhow::Result<ResolvedCodexCredentials> {
        ...
        let access_token = if use_gateway_api_key_auth {
            ...
            Some(oauth_access_token.ok_or_else(|| {
                if had_profile {
                    ...
                    anyhow::Error::msg(
                        "OpenAI Codex credentials are present but expired or could not be refreshed. Re-run `omnesagent auth login --model-provider openai-codex` to sign in again.",
                    )
                } else {
                    ::omnesagent_log::record!(
                        ERROR,
                    ...
                    anyhow::Error::msg(
                        "No OpenAI Codex credentials found. Run `omnesagent auth login --model-provider openai-codex` to sign in.",
                    )
                }
            })?)
        };
        ...
        let account_id = if use_gateway_api_key_auth {
            ...
            Some(account_id.ok_or_else(|| {
                ...
                anyhow::Error::msg(
                    "OpenAI Codex account id not found in auth profile/token. Run `omnesagent auth login --model-provider openai-codex` again.",
                )
            })?)
        };
...
impl ::omnesagent_api::attribution::Attributable for OpenAiCodexModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::OpenAiCodex,
            ),
        )
    }
    fn alias(&self) -> &str {
...
mod tests {
    ...
    async fn mock_codex_provider(
        ...
        let options = ModelProviderRuntimeOptions {
            ...
            omnesagent_dir: Some(temp_dir.path().to_path_buf()),
    ...
    async fn codex_responses_provider_honors_runtime_proxy_after_construction() {
        ...
        use tokio::net::TcpListener;
        use omnesagent_config::schema::{ProxyConfig, ProxyScope, set_runtime_proxy_config};
        ...
        let proxy_server = omnesagent_spawn::spawn!(async move {
        ...
        let direct_server = omnesagent_spawn::spawn!(async move {
    ...
    async fn chat_with_empty_tools_list_omits_tool_choice_and_parallel_tool_calls() {
        ...
        let messages = vec![ChatMessage::user("hello")];
        let empty_tools: Vec<omnesagent_api::tool::ToolSpec> = vec![];
        let response = provider
            .chat(
                ProviderChatRequest {
                    messages: &messages,
    ...
    fn default_state_dir_is_non_empty() {
        let path = default_omnesagent_dir();
