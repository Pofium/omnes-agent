...
pub(crate) fn sane_mtok(rate: f64) -> Option<f64> {
    omnesagent_config::cost::is_sane_usd_rate(rate).then_some(rate)
}
...
mod tests {
    ...
    fn sane_mtok_rejects_nonfinite_negative_and_absurd() {
        ...
        assert_eq!(
            sane_mtok(omnesagent_config::cost::MAX_SANE_USD_RATE),
            Some(omnesagent_config::cost::MAX_SANE_USD_RATE)
        );
        ...
        assert!(sane_mtok(omnesagent_config::cost::MAX_SANE_USD_RATE * 2.0).is_none());
    }
