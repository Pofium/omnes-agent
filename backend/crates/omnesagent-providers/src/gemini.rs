...
//! - OmnesAgent auth-profiles OAuth tokens
        ...
        let auth = self.auth.as_ref().ok_or_else(|| {
            ...
            anyhow::Error::msg(
                "Gemini API key not found. Options:\n\
                 ...
                 3. Run `omnesagent auth login --model-provider gemini`\n\
                 4. Get an API key from https://aistudio.google.com/app/apikey\n\
                 5. Run `omnesagent quickstart --model-provider gemini --api-key <key>` to configure",
            )
        })?;
            ...
            GeminiAuth::ManagedOAuth => {
                ...
                let token = auth_service
                    ...
                    .ok_or_else(|| {
                        ...
                        anyhow::Error::msg(
                            "Gemini auth profile not found. Run `omnesagent auth login --model-provider gemini`.",
                        )
                    })?;
                let proj = self.resolve_oauth_project(&token).await?;
                (Some(token), Some(proj))
                ...
                GeminiAuth::ManagedOAuth => {
                    ...
                    let _token = auth_service
                        ...
                        .ok_or_else(|| {
                            ...
                            anyhow::Error::msg(
                                "Gemini auth profile not found or expired. Run: omnesagent auth login --model-provider gemini",
                            )
                        })?;
...
impl ::omnesagent_api::attribution::Attributable for GeminiModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::Gemini,
            ),
        )
    }
    fn alias(&self) -> &str {
