...
pub struct AnthropicModelProvider {
    ...
    schema_cache: omnesagent_api::schema::SchemaCleanCache,
}
...
impl AnthropicBuilder {
    ...
    /// [`omnesagent_api::model_provider::BASELINE_MAX_TOKENS`] when unset.
    ...
    /// [`omnesagent_api::model_provider::BASELINE_TIMEOUT_SECS`] when unset.
    ...
    pub fn build(self) -> AnthropicModelProvider {
        AnthropicModelProvider {
            ...
            max_tokens: self
                .max_tokens
                .unwrap_or(omnesagent_api::model_provider::BASELINE_MAX_TOKENS),
            timeout_secs: self
                .timeout_secs
                .unwrap_or(omnesagent_api::model_provider::BASELINE_TIMEOUT_SECS),
            schema_cache: omnesagent_api::schema::SchemaCleanCache::new(),
        }
    }
}
...
impl AnthropicModelProvider {
    ...
    fn apply_auth(
        ...
        ::omnesagent_log::record!(DEBUG, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"header": if is_setup { "Authorization" } else { "x-api-key" }, "credential_len": len})), "Anthropic auth header applied");
        ...
        let mut native_tools: Vec<NativeToolSpec> = items
            ...
            .map(|tool| NativeToolSpec {
                ...
                input_schema: self.schema_cache.clean_shared(
                    &tool.parameters,
                    omnesagent_api::schema::CleaningStrategy::Anthropic,
                ),
                cache_control: None,
            })
            .collect();
    ...
    fn resolve_thinking(
        &self,
        thinking: Option<omnesagent_api::model_provider::NativeThinkingParams>,
        temperature: Option<f64>,
        model: &str,
    ...
    fn http_client(&self) -> Client {
        omnesagent_config::schema::build_runtime_proxy_client_with_timeouts(
    ...
    fn streaming_http_client(&self) -> Result<Client, reqwest::Error> {
        ...
        let builder = omnesagent_config::schema::apply_runtime_proxy_to_builder(
            builder,
            "model_provider.anthropic",
        );
        builder.build()
        ...
        R: tokio::io::AsyncBufRead + Unpin,
    {
        ...
        let collect_debug_metadata = ::omnesagent_log::debug_enabled();
        ...
        loop {
            ...
            match event_type {
                "message_start" => {
                    ...
                    ::omnesagent_log::record!(DEBUG, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"model": model, "input_tokens": observed_input, "cached_input_tokens": observed_cached, "cache_creation_input_tokens": observed_cache_create})), "stream: message_start");
                }
                "content_block_start" => {
                ...
                "message_delta" => {
                    ...
                    if stop_reason == "max_tokens" {
                        ...
                        ::omnesagent_log::record!(DEBUG, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"stop_reason": stop_reason, "output_tokens": observed_output})), "stream: message_delta");
                    }
                }
                "message_stop" => {
...
impl ModelProvider for AnthropicModelProvider {
    ...
    async fn chat(
        ...
        let tool_choice_override = omnesagent_api::TOOL_CHOICE_OVERRIDE
    ...
    fn stream_chat(
        ...
        ::omnesagent_log::record!(
            DEBUG,
            ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Spawn)
                .with_category(::omnesagent_log::EventCategory::Provider)
                .with_attrs(::serde_json::json!({
        ...
        let parser_handle = ::omnesagent_spawn::spawn!(async move {
            let mut req = client
                .post(&url)
                .header("anthropic-version", "2023-06-01")
                .header("content-type", "application/json")
...
impl ::omnesagent_api::attribution::Attributable for AnthropicModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::Anthropic,
            ),
        )
    }
    fn alias(&self) -> &str {
...
data: {\"type\":\"message_stop\"}\n\n";
    ...
    async fn stream_chat_without_credential_returns_error() {
        ...
        assert_eq!(
            ...
            "ModelProvider error: Anthropic credentials not set. Run `omnesagent quickstart` or `omnesagent config set` to configure."
        );
    }
                ...
                while let Ok(event) = rx.try_recv() {
                    ...
                    let matches_source = event
                        ...
                        .is_some_and(|file| {
                            ...
                            file.replace('\\', "/")
                                .ends_with("omnesagent-providers/src/anthropic.rs")
                        });
    ...
    fn chat_request_serializes_with_system() {
        let req = ChatRequest {
            model: "claude-3-opus".to_string(),
            max_tokens: 4096,
            system: Some("You are OmnesAgent".to_string()),
            messages: vec![Message {
                role: "user".to_string(),
                content: "hello".to_string(),
            }],
        ...
        let json = serde_json::to_string(&req).unwrap();
        assert!(json.contains("\"system\":\"You are OmnesAgent\""));
    }
    ...
    fn resolve_thinking_uses_adaptive_for_opus_4_7() {
        ...
        let provider = AnthropicModelProvider::builder("test")
            .credential(Some("test-key"))
            .build();
        let params = omnesagent_api::model_provider::NativeThinkingParams {
    ...
    fn string_projection_preserves_semantic_empty_terminal_cause() {
        ...
        assert!(error.chain().any(|cause| {
            cause.is::<omnesagent_api::model_provider::SemanticEmptyTerminalCompletion>()
        }));
    }
    ...
    async fn anthropic_factory_forwards_timeout_to_native_provider() {
        ...
        use omnesagent_config::schema::AnthropicModelProviderConfig;
...
data: {\"type\":\"message_stop\"}\n\n";
    ...
    async fn display_enabled_thinking_uses_streaming_production_path() {
        use axum::{Router, response::IntoResponse, routing::post};
        use futures_util::StreamExt as _;
        use omnesagent_api::model_provider::NativeThinkingParams;
