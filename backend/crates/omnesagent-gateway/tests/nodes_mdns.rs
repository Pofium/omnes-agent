...
use omnesagent_gateway::nodes::mdns::{
...
fn advertised_gateway_uses_runtime_port_and_path_prefix() {
    let gateway = MdnsAdvertisedGateway::new(42617, Some("/omnesagent"));
    ...
    assert_eq!(packet.path_prefix.as_deref(), Some("/omnesagent"));
    ...
    assert_eq!(peer.base_url(), "http://192.168.1.42:42617/omnesagent");
}
