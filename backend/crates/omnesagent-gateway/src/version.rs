...
//! `omnesagent update --check --json` — keeping a single source of truth for
...
use omnesagent_runtime::i18n::get_required_cli_string;
...
/// Upper bound on the `omnesagent update --check` subprocess.
...
/// Parsed output of `omnesagent update --check --json`. Field names must match
...
async fn run_cli_check(version: Option<&str>) -> anyhow::Result<CliCheck> {
    ...
    let output = tokio::time::timeout(CHECK_TIMEOUT, cmd.output())
        ...
        .context("failed to spawn `omnesagent update --check`")?;
...
/// POST /api/version/upgrade — apply an upgrade via `omnesagent update`.
///
...
pub async fn handle_version_upgrade(
    ...
    *slot = Some(progress.clone());
    ...
    ::omnesagent_spawn::spawn!(run_upgrade(progress, req.version, action));
...
enum RestartAction {
    ...
    /// a daemon wrapper (`omnesagent gateway start`). In that mode no one is
...
/// Drive `omnesagent update`, then either mark done or (Phase 3) restart.
async fn run_upgrade(
    ...
    let mut child = match cmd.spawn() {
        Ok(c) => c,
        Err(e) => {
            fail(&progress, format!("failed to start `omnesagent update`: {e}"));
            return;
        }
    };
    ...
    if let Some(out) = child.stdout.take() {
        ...
        pumps.push(::omnesagent_spawn::spawn!(pump_lines(out, p)));
    }
    if let Some(err) = child.stderr.take() {
        let p = progress.clone();
        pumps.push(::omnesagent_spawn::spawn!(pump_lines(err, p)));
    }
    ...
    match action {
        ...
        RestartAction::SelfRespawn {
            ...
            omnesagent_runtime::restart::request_respawn();
            set_state(&progress, UpgradeState::Restarting);
            tokio::time::sleep(RESTART_GRACE).await;
            match standalone_shutdown_tx {
                // Standalone gateway (`omnesagent gateway start`): no daemon
...
fn trigger_graceful_shutdown() {
    ...
    #[cfg(not(unix))]
    {
        omnesagent_runtime::restart::request_shutdown();
    }
}
...
mod tests {
    ...
    /// `omnesagent gateway start` runs without a daemon wrapper, so nothing is
