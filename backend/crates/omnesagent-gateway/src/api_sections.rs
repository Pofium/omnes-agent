...
use omnesagent_runtime::rpc::types::{
...
pub async fn handle_catalog(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let model_providers: Vec<CatalogModelProvider> = omnesagent_providers::list_model_providers()
        .into_iter()
...
pub async fn handle_catalog_models(
    ...
    let local = omnesagent_runtime::quickstart::model_provider_is_local(&q.model_provider);
    ...
    let (models, pricing, live) =
        omnesagent_runtime::quickstart::model_catalog_with_config(Some(&cfg), &q.model_provider)
            .await;
...
#[must_use]
pub fn derive_section_status(cfg: &omnesagent_config::schema::Config) -> ConfigStatusResult {
...
fn quickstart_missing_requirements(cfg: &omnesagent_config::schema::Config) -> Vec<String> {
...
fn quickstart_agent_missing_requirements(
    cfg: &omnesagent_config::schema::Config,
    alias: &str,
    agent: &omnesagent_config::schema::AliasedAgentConfig,
) -> Vec<String> {
    let mut missing = Vec::new();
    if !agent.enabled {
    ...
    if model_ref.is_empty() {
        ...
        if !has_model {
            ...
            provider,
            omnesagent_runtime::quickstart::model_provider_is_local(family),
        ) {
            missing.push(format!(
...
pub fn build_agent_options(cfg: &omnesagent_config::schema::Config) -> AgentOptionsResponse {
    use omnesagent_config::traits::AliasSource;
...
pub async fn handle_sections(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let all_map_paths: Vec<&'static str> = omnesagent_config::schema::Config::map_key_sections()
        .iter()
        .map(|s| s.path)
        .collect();
    ...
    for s in omnesagent_config::sections::QUICKSTART_SECTIONS {
    ...
    ordered.sort_by(|a, b| {
        match (
            omnesagent_config::sections::section_index_for_key(a),
            omnesagent_config::sections::section_index_for_key(b),
        ) {
    ...
    let sections: Vec<ConfigSectionEntry> = ordered
        .into_iter()
        .map(|key| {
            let wizard = omnesagent_config::sections::Section::from_key(&key);
            let has_picker = match wizard {
                Some(w) => !matches!(
                    w,
                    omnesagent_config::sections::Section::Hardware
                        | omnesagent_config::sections::Section::Mcp
                        | omnesagent_config::sections::Section::Skills
                ),
            ...
            ConfigSectionEntry {
                ...
                label: omnesagent_config::sections::humanize_section_key(&key),
                help: section_help(&key).to_string(),
                has_picker,
                group: section_group(&key).to_string(),
                is_quickstart: wizard.is_some(),
                shape: wizard.map(omnesagent_config::sections::Section::shape),
                cost_category: omnesagent_config::schema::cost_category_for_provider_section(&key)
                    .unwrap_or_default()
                    .to_string(),
                key,
            }
...
fn section_ready(cfg: &omnesagent_config::schema::Config, key: &str, completed_marker: bool) -> bool {
    use omnesagent_config::sections::Section;
...
fn section_group(key: &str) -> &'static str {
    omnesagent_config::sections::section_group_for_key(key).label()
}

/// Help text for a section. Delegates to `omnesagent_config::sections::section_help`
...
fn section_help(key: &str) -> &'static str {
    omnesagent_config::sections::section_help(key)
}
...
fn picker_items_for(
    section: omnesagent_config::sections::Section,
    cfg: &omnesagent_config::schema::Config,
) -> PickerDispatch {
    use omnesagent_config::sections::Section;
    match section {
...
fn providers_picker(cfg: &omnesagent_config::schema::Config) -> Vec<PickerItem> {
...
fn any_usable_model_provider(cfg: &omnesagent_config::schema::Config) -> bool {
    cfg.providers
        .models
        .iter_entries()
        .any(|(family, _, base)| {
...
fn storage_picker(cfg: &omnesagent_config::schema::Config) -> Vec<PickerItem> {
...
fn memory_picker(cfg: &omnesagent_config::schema::Config) -> Vec<PickerItem> {
    ...
    omnesagent_memory::selectable_memory_backends()
        .iter()
        .map(|b| PickerItem {
            key: b.key.to_string(),
...
fn schema_walk_picker(cfg: &omnesagent_config::schema::Config, section: &str) -> Vec<PickerItem> {
    ...
    let all: std::collections::BTreeSet<String> =
        omnesagent_config::schema::Config::map_key_sections()
            .into_iter()
            .filter_map(|s| {
                s.path
...
fn agents_picker(cfg: &omnesagent_config::schema::Config) -> Vec<PickerItem> {
...
fn apply_first_run_agent_defaults(cfg: &mut omnesagent_config::schema::Config, alias: &str) {
...
fn mark_section_completed(cfg: &mut omnesagent_config::schema::Config, section: &str) {
    if !cfg
        .onboard_state
        .completed_sections
        .iter()
...
fn tunnel_provider_picker(cfg: &omnesagent_config::schema::Config) -> Vec<PickerItem> {
...
pub async fn handle_section_select(
    ...
    let (fields_prefix, created) = match section_enum {
        ...
        | Section::EmbeddingRoutes => {
            ...
            let created = match omnesagent_config::alias_refs::create_map_key_checked(
                &mut working,
                section_key,
                &key,
            ) {
            ...
            if created && matches!(section_enum, Section::Agents) {
                ...
                if let Err(err) = omnesagent_runtime::agent::personality::seed_default_personality(
                    &working,
                    &key,
                    &workspace_dir,
                )
                .await
                {
                    ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": key, "workspace": workspace_dir.display().to_string(), "err": err.to_string()})), "agent workspace scaffolded but personality seed failed (continuing)");
                }
            }
...
mod tests {
    ...
    fn parse_dev_template(raw: &str, name: &str) -> omnesagent_config::schema::Config {
        toml::from_str(raw).unwrap_or_else(|err| panic!("{name} must parse as schema V3: {err}"))
    }

    fn assert_common_dev_template_contract(cfg: &omnesagent_config::schema::Config) {
        ...
        assert_eq!(
            ...
            std::path::PathBuf::from("/omnesagent-data/workspace")
        );
        ...
        assert_eq!(
            risk.level,
            omnesagent_config::autonomy::AutonomyLevel::Supervised
        );
    ...
    fn dev_config_template_preserves_dispatch_contract() {
        ...
        assert_eq!(
            ...
            Some("/usr/share/omnesagentlabs/web/dist")
        );
    ...
    fn empty_cfg() -> omnesagent_config::schema::Config {
        omnesagent_config::schema::Config::default()
    }

    fn section_test_state(config: omnesagent_config::schema::Config) -> AppState {
        ...
        AppState {
            ...
            auto_save: false,
            pairing: std::sync::Arc::new(omnesagent_runtime::security::pairing::PairingGuard::new(
                false,
                &[],
            )),
            ...
            gmail_push: None,
            observer: std::sync::Arc::new(omnesagent_runtime::observability::NoopObserver),
    ...
    fn handle_sections_derives_every_top_level_field_from_schema() {
        ...
        // populated entries).
        for s in omnesagent_config::schema::Config::map_key_sections() {
    ...
    fn providers_picker_sources_from_list_providers() {
        // Single source of truth: omnesagent_providers::list_model_providers().
    ...
    fn memory_picker_sources_from_selectable_backends() {
        ...
        // Mirrors omnesagent_memory::selectable_memory_backends() exactly.
    ...
    fn tunnel_picker_badges_present_option_fields_as_configured() {
        ...
        cfg.tunnel.cloudflare = Some(omnesagent_config::schema::CloudflareTunnelConfig::default());
        let items = tunnel_provider_picker(&cfg);
