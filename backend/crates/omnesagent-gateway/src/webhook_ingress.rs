...
use omnesagent_memory::MemoryCategory;
...
async fn process_verified_message(
        ...
        .expect("authenticated dispatch adapters define a session-key policy")
    {
        ...
        SessionKeyPolicy::AliasSenderSanitized => {
            ...
            omnesagent_api::session_keys::sanitize_session_key(&sender_session_id(&channel_ref, msg))
        }
    };

    if state.auto_save && !omnesagent_memory::should_skip_autosave_content(&msg.content) {
