...
/// (stamped [`omnesagent_log::EPHEMERAL_BROADCAST_MARKER`] by the log layer when
...
fn sse_frame_for_stream(
    ...
    if omnesagent_log::frame_carries_ephemeral_credentials(&value) && !auth_enforced {
        return None;
    }
    ...
    omnesagent_log::strip_ephemeral_broadcast_marker(&mut value);
    Some(value)
}
...
fn history_events_payload(buffer: &EventBuffer) -> serde_json::Value {
    let events: Vec<_> = buffer
        ...
        .filter(|event| !omnesagent_log::frame_carries_ephemeral_credentials(event))
        .collect();
...
impl omnesagent_runtime::observability::Observer for BroadcastObserver {
    fn record_event(&self, event: &omnesagent_runtime::observability::ObserverEvent) {
        ...
        let json = match event {
            ...
            omnesagent_runtime::observability::ObserverEvent::ToolCall {
                tool,
                duration,
                success,
                channel,
            ...
            omnesagent_runtime::observability::ObserverEvent::ToolCallStart {
                tool,
                channel,
                agent_alias,
                parent_agent_alias,
            ...
            omnesagent_runtime::observability::ObserverEvent::Error { component, message } => {
                serde_json::json!({
                    "type": "error",
                    "source": "observability",
                    "component": component,
            ...
            omnesagent_runtime::observability::ObserverEvent::HistoryTrimmed {
                dropped_messages,
                kept_turns,
                reason,
                channel,
...
mod tests {
    use super::*;
    use omnesagent_runtime::observability::{Observer, ObserverEvent};
    ...
    /// Build a broadcast frame stamped the way `omnesagent_log::record_event`
    ...
    fn ephemeral_credential_frame_reaches_authenticated_stream_without_marker() {
        ...
        assert!(
            delivered
                .get(omnesagent_log::EPHEMERAL_BROADCAST_MARKER)
                .is_none(),
    ...
    fn factory_observer_events_reach_broadcast_hook() {
        ...
        omnesagent_runtime::observability::clear_broadcast_hook();

        let (tx, mut rx) = tokio::sync::broadcast::channel(16);
        let buffer = Arc::new(EventBuffer::new(16));
        let bo: Arc<dyn Observer> = Arc::new(BroadcastObserver::new(tx, buffer.clone()));
        omnesagent_runtime::observability::set_broadcast_hook(bo);

        // Same factory call site as `process_message` in the agent loop.
        let cfg = omnesagent_config::schema::ObservabilityConfig {
            backend: omnesagent_config::schema::ObservabilityBackend::None,
            ..Default::default()
        };
        let observer = omnesagent_runtime::observability::create_observer(&cfg);
    ...
    fn history_payload_never_recovers_pairing_credentials() {
        ...
        assert!(!dump.contains(omnesagent_log::EPHEMERAL_BROADCAST_MARKER));
    }
    ...
    fn pairing_guard(
        require: bool,
        tokens: &[&str],
    ) -> Arc<omnesagent_runtime::security::pairing::PairingGuard> {
        let owned: Vec<String> = tokens.iter().map(|t| (*t).to_string()).collect();
        Arc::new(omnesagent_runtime::security::pairing::PairingGuard::new(
            require, &owned,
        ))
    }
    ...
    async fn route_delivers_credential_without_marker_to_authenticated_client() {
        ...
        assert!(
            !body.contains(omnesagent_log::EPHEMERAL_BROADCAST_MARKER),
