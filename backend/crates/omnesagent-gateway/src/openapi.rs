...
pub fn build_spec() -> serde_json::Value {
    ...
    use omnesagent_config::api_error::ConfigApiError;
    ...
    let components = serde_json::json!({
        "schemas": {
            ...
            "Config":           schema_value::<omnesagent_config::schema::Config>(),
            ...
            "Sop":              schema_value::<omnesagent_runtime::sop::Sop>(),
            "SopGraph":         schema_value::<omnesagent_runtime::sop::SopGraph>(),
            "GraphLegend":      schema_value::<omnesagent_runtime::sop::GraphLegend>(),
            "RunOverlay":       schema_value::<omnesagent_runtime::sop::RunOverlay>(),
            "ApprovalDecision": schema_value::<omnesagent_runtime::sop::ApprovalDecision>(),
            "TriggerSourceRegistry": schema_value::<omnesagent_runtime::sop::TriggerSourceRegistry>(),
    ...
    let paths = serde_json::json!({
        ...
        "/api/config/migrate": {
            "post": {
                ...
                "description": "Mirrors `omnesagent config migrate`. Backs up the previous file as `config.toml.bak` before writing.",
                "responses": {
                    "200": {
        ...
        "/api/version/check": {
            "get": {
                ...
                "description": "Runs `omnesagent update --check --json` server-side (1h cache, force-refreshable). Never fails the dashboard: on any error it still returns 200 with `is_newer: false` and an `error` string so the version badge degrades gracefully.",
        ...
        "/api/version/upgrade": {
            "post": {
                "tags": ["version"],
                "summary": "Apply an upgrade via `omnesagent update`",
    ...
    let mut spec = serde_json::json!({
        "openapi": "3.1.0",
        "info": {
            "title": "OmnesAgent Gateway — Config CRUD",
            "version": env!("CARGO_PKG_VERSION"),
            "description": "Per-property CRUD endpoints over the same `Config` mutation core that `omnesagent config get/set/list/init/migrate` uses on the CLI. See https://github.com/omnesagent-labs/omnesagent/issues/6175 for the full surface and acceptance checklist.",
        },
        "security": [{"bearerAuth": []}],
        "paths": paths,
