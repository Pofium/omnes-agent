...
use omnesagent_config::schema::{ChannelAliasInfo, Config};
use omnesagent_memory::MemoryEntry;

const MEMORY_API_CONTENT_MAX_CHARS: usize = 4096;

fn integration_entry_json(
    entry: &omnesagent_runtime::integrations::IntegrationEntry,
) -> serde_json::Value {
    serde_json::json!({
        "name": &entry.name,
...
pub struct CronAddBody {
    ...
    pub delivery: Option<omnesagent_runtime::cron::DeliveryConfig>,
    ...
    #[serde(default)]
    pub shell_output_format: Option<omnesagent_config::schema::CronShellOutputFormat>,
}
...
fn cron_schedule_from_api(
    expr: String,
    tz: Option<String>,
) -> Result<omnesagent_runtime::cron::Schedule, (StatusCode, Json<serde_json::Value>)> {
    let schedule = omnesagent_runtime::cron::Schedule::Cron { expr, tz };
    omnesagent_runtime::cron::validate_schedule(&schedule, chrono::Utc::now())
...
pub async fn handle_api_status(
    ...
    let health = omnesagent_runtime::health::snapshot();
    ...
    let locale = config
        ...
        .filter(|s| !s.is_empty())
        .map(String::from)
        .unwrap_or_else(omnesagent_runtime::i18n::detect_locale);
    ...
    let process = omnesagent_runtime::process_stats::sample();
    ...
    let body = serde_json::json!({
        ...
        "daemon_started_at": omnesagent_runtime::health::daemon_started_at(),
...
pub async fn handle_api_cron_list(
    ...
    let config = state.config.read().clone();
    match omnesagent_runtime::cron::list_jobs(&config) {
...
pub async fn handle_api_cron_add(
    ...
    if let Err(e) = omnesagent_runtime::cron::validate_delivery_config(delivery.as_ref()) {
        return (
            StatusCode::BAD_REQUEST,
    ...
    let result = if is_agent {
        ...
        let session_target = match session_target {
            Some(raw) => match omnesagent_runtime::cron::SessionTarget::try_parse(&raw) {
                Ok(target) => target,
                Err(e) => return bad_request(e).into_response(),
            },
            None => omnesagent_runtime::cron::SessionTarget::Isolated,
        };

        let default_delete = matches!(schedule, omnesagent_runtime::cron::Schedule::At { .. });
        let delete_after_run = delete_after_run.unwrap_or(default_delete);

        omnesagent_runtime::cron::add_agent_job(
            &config,
            &agent_alias,
            name,
            schedule,
        ...
        omnesagent_runtime::cron::add_shell_job_with_approval_and_format(
            &config,
            &agent_alias,
            name,
            schedule,
...
pub async fn handle_api_cron_runs(
    ...
    if let Err(e) = omnesagent_runtime::cron::get_job(&config, &id) {
        return (
    ...
    match omnesagent_runtime::cron::list_runs(&config, &id, limit) {
        Ok(runs) => {
...
pub async fn handle_api_cron_run(
    ...
    let job = match omnesagent_runtime::cron::get_job(&config, &id) {
        Ok(job) => job,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
    ...
    let result = omnesagent_runtime::cron::scheduler::run_manual_job(
        &config,
        &job,
        omnesagent_runtime::cron::scheduler::CronDeliveryContext::GatewayManual,
        &event_tx,
    )
    .await;
...
pub async fn handle_api_cron_patch(
    ...
    let existing = match omnesagent_runtime::cron::get_job(&config, &id) {
        Ok(j) => j,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
    ...
    let is_agent = matches!(existing.job_type, omnesagent_runtime::cron::JobType::Agent);
    if shell_output_format.is_some() {
        if is_agent {
            return bad_request(
                "shell_output_format is not applicable to agent jobs; agent execution ignores it",
        ...
        let (expr, existing_tz) = match (&existing.schedule, new_expr) {
            (_, Some(expr)) => {
                let existing_tz = match &existing.schedule {
                    omnesagent_runtime::cron::Schedule::Cron { tz, .. } => tz.clone(),
                    _ => None,
                };
                (expr, existing_tz)
            }
            (omnesagent_runtime::cron::Schedule::Cron { expr, tz }, None) => {
                (expr.clone(), tz.clone())
            }
            (_, None) => {
    ...
    let patch = omnesagent_runtime::cron::CronJobPatch {
        name,
        schedule,
        ...
        uses_memory,
        shell_output_format,
        ..omnesagent_runtime::cron::CronJobPatch::default()
    };

    match omnesagent_runtime::cron::update_shell_job_with_approval(
        &config,
        &agent_alias,
        &id,
        patch,
...
pub async fn handle_api_cron_delete(
    ...
    let config = state.config.read().clone();
    match omnesagent_runtime::cron::remove_job(&config, &id) {
...
pub async fn handle_api_integrations(
    ...
    let config = state.config.read().clone();
    let entries = omnesagent_runtime::integrations::registry::all_integrations(&config);
...
pub async fn handle_api_integrations_settings(
    ...
    for entry in &entries {
        let enabled = matches!(
            entry.status,
            omnesagent_runtime::integrations::IntegrationStatus::Active
        );
        settings.insert(
            entry.name.clone(),
            serde_json::json!({
...
pub async fn handle_api_doctor(
    ...
    let config = state.config.read().clone();
    let results = omnesagent_runtime::doctor::diagnose(&config);

    let ok_count = results
        .iter()
        .filter(|r| r.severity == omnesagent_runtime::doctor::Severity::Ok)
        .count();
    let warn_count = results
        .iter()
        .filter(|r| r.severity == omnesagent_runtime::doctor::Severity::Warn)
        .count();
    let error_count = results
        .iter()
        .filter(|r| r.severity == omnesagent_runtime::doctor::Severity::Error)
        .count();
...
async fn resolve_memory_handle(
...
) -> Result<std::sync::Arc<dyn omnesagent_memory::Memory>, (StatusCode, Json<serde_json::Value>)> {
    ...
    omnesagent_memory::create_memory_for_agent(&config, alias, api_key.as_deref())
        .await
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn handle_api_memory_list(
    ...
    if params.query.is_some() || params.since.is_some() || params.until.is_some() {
        ...
        let category = params.category.as_deref().map(|cat| match cat {
            "core" => omnesagent_memory::MemoryCategory::Core,
            "daily" => omnesagent_memory::MemoryCategory::Daily,
            "conversation" => omnesagent_memory::MemoryCategory::Conversation,
            other => omnesagent_memory::MemoryCategory::Custom(other.to_string()),
        });
...
pub async fn handle_api_memory_store(
    ...
    let category = body
        ...
        .unwrap_or(omnesagent_memory::MemoryCategory::Core);
...
pub async fn handle_api_cli_tools(
    ...
    let tools = match tokio::task::spawn_blocking(|| {
        omnesagent_tools::cli_discovery::discover_cli_tools(&[], &[])
    })
    .await
    {
        Ok(tools) => tools,
...
pub async fn handle_api_channels(
    ...
    let channels: Vec<serde_json::Value> = config
        ...
        .map(|info| {
            ...
            let compiled = omnesagent_channels::listing::is_channel_type_compiled(compiled_key);
...
/// ([`omnesagent_channels::login_relink::relink`]); the gateway performs no
...
pub async fn handle_api_channel_relink(
    ...
    let compiled_key = compiled_readiness_key_for_alias(&config, &info);
    let Some(qr_channel) = omnesagent_channels::listing::qr_pairing_channel(compiled_key) else {
        return (
            StatusCode::CONFLICT,
            Json(serde_json::json!({
                "channel": channel,
    ...
    match omnesagent_channels::login_relink::relink(qr_channel, &config, &info.alias) {
        Ok(omnesagent_channels::login_relink::RelinkOutcome::Cleared { removed }) => {
            ::omnesagent_log::record!(
                INFO,
                ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note)
        ...
        Ok(omnesagent_channels::login_relink::RelinkOutcome::NothingToClear) => {
            Json(serde_json::json!({
                "channel": channel,
...
fn channel_readiness(
    ...
    info: &omnesagent_config::schema::ChannelAliasInfo,
    health: &omnesagent_runtime::health::HealthSnapshot,
    state: &AppState,
) -> ChannelReadiness {
...
/// probe (`omnesagent_channels::login_probe`). The probe resolves the same
...
fn apply_persisted_login_readiness(
    ...
    use omnesagent_channels::login_probe::PersistedLogin;

    // Resolve the string key to the typed QR-pairing channel once; all
    // downstream dispatch is on the enum.
    let compiled_key = compiled_readiness_key_for_alias(config, info);
    let Some(channel) = omnesagent_channels::listing::qr_pairing_channel(compiled_key) else {
    ...
    match omnesagent_channels::login_probe::persisted_login(channel, config, &info.alias) {
...
fn component_health_ok_and_fresh(component: &omnesagent_runtime::health::ComponentHealth) -> bool {
...
pub async fn handle_api_health(
    ...
    let snapshot = omnesagent_runtime::health::snapshot();
...
pub async fn handle_api_session_message_post(
    ...
    let message = omnesagent_providers::ChatMessage::assistant(&body.content);
...
pub async fn handle_claude_code_hook(
    State(state): State<AppState>,
    Json(payload): Json<omnesagent_tools::claude_code_runner::ClaudeCodeHookEvent>,
) -> impl IntoResponse {
    ...
    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"session_id": payload.session_id, "event_type": payload.event_type, "tool_name": payload.tool_name, "summary": payload.summary})), "Claude Code hook event received");
...
pub(crate) mod tests {
    ...
    use omnesagent_infra::session_sqlite::SqliteSessionBackend;
    use omnesagent_infra::session_store::SessionStore;
    ...
    fn with_test_agent(
        mut config: omnesagent_config::schema::Config,
    ) -> omnesagent_config::schema::Config {
        config.providers.models.openrouter.insert(
            "default".to_string(),
            omnesagent_config::schema::OpenRouterModelProviderConfig::default(),
        );
    ...
    async fn api_status_includes_connected_nodes_and_mdns_peers() {
        let state = test_state(omnesagent_config::schema::Config::default());
    ...
    fn integration_entry_json_derives_category_label_from_category() {
        let entry = omnesagent_runtime::integrations::IntegrationEntry {
            name: "Browser".into(),
            description: "Run browser automation".into(),
            category: omnesagent_runtime::integrations::IntegrationCategory::ToolsAutomation,
            status: omnesagent_runtime::integrations::IntegrationStatus::Active,
        };
    ...
    fn api_channels_readiness_key_tracks_whatsapp_backend_type() {
        ...
        config.channels.whatsapp.insert(
            "web".to_string(),
            omnesagent_config::schema::WhatsAppConfig {
                enabled: true,
                session_path: Some("~/.omnesagent/state/whatsapp-web/session.db".into()),
                ..Default::default()
            },
        );
        config.channels.whatsapp.insert(
        ...
        let web = omnesagent_config::schema::ChannelAliasInfo {
            channel_type: "whatsapp".to_string(),
            alias: "web".to_string(),
            owning_agent: None,
            enabled: true,
        };
        let cloud = omnesagent_config::schema::ChannelAliasInfo {
            channel_type: "whatsapp".to_string(),
            alias: "cloud".to_string(),
            owning_agent: None,
            enabled: true,
        };
        let ambiguous = omnesagent_config::schema::ChannelAliasInfo {
            channel_type: "whatsapp".to_string(),
            alias: "ambiguous".to_string(),
            owning_agent: None,
            enabled: true,
        };
        let discord = omnesagent_config::schema::ChannelAliasInfo {
    ...
    async fn api_channels_marks_configured_uncompiled_channel_unavailable() {
        ...
        config.channels.nextcloud_talk.insert(
            "default".to_string(),
            omnesagent_config::schema::NextcloudTalkConfig {
                enabled: true,
    ...
    fn bind_channel_to_agent(config: &mut omnesagent_config::schema::Config, channel_ref: &str) {
        config.agents.insert(
            "rowan".to_string(),
            omnesagent_config::schema::AliasedAgentConfig {
                channels: vec![omnesagent_config::providers::ChannelRef::new(
    ...
    async fn api_channels_wechat_authenticated_tracks_persisted_login() {
        ...
        config.channels.wechat.insert(
            "admin".to_string(),
            omnesagent_config::schema::WeChatConfig {
                enabled: true,
    ...
    async fn api_channel_relink_unknown_channel_is_not_found() {
        let response = handle_api_channel_relink(
            State(test_state(omnesagent_config::schema::Config::default())),
    ...
    fn config_with_webhook(
        ...
        if bound {
            config.agents.insert(
                "rowan".to_string(),
                omnesagent_config::schema::AliasedAgentConfig {
                    channels: vec![omnesagent_config::providers::ChannelRef::new(format!(
                        "webhook.{alias}"
                    ))],
                    ..Default::default()
                },
    ...
    fn config_with_telegram(alias: &str) -> omnesagent_config::schema::Config {
        let mut config = omnesagent_config::schema::Config::default();
    ...
    fn first_channel_info(
        config: &omnesagent_config::schema::Config,
    ) -> omnesagent_config::schema::ChannelAliasInfo {
        config
            .channels_by_alias()
            .into_iter()
            .next()
    ...
    fn channel_readiness_webhook_uses_supervised_listener_health_for_custom_path() {
        ...
        omnesagent_runtime::health::mark_component_ok("channel:webhook.supervised");
        let state = test_state(config.clone());
        let health = omnesagent_runtime::health::snapshot();
        let info = first_channel_info(&config);
        let readiness = channel_readiness(&config, &info, &health, &state);
    ...
    fn channel_readiness_webhook_rejects_stale_listener_health() {
        ...
        .to_rfc3339();
        let health = omnesagent_runtime::health::HealthSnapshot {
            pid: std::process::id(),
            updated_at: chrono::Utc::now().to_rfc3339(),
            uptime_seconds: 1,
            components: std::collections::BTreeMap::from([(
                component,
                omnesagent_runtime::health::ComponentHealth {
                    status: "ok".to_string(),
                    updated_at: old,
                    last_ok: None,
                    last_error: None,
    ...
    fn channel_readiness_webhook_uses_live_pairing_guard_for_auth() {
        ...
        omnesagent_runtime::health::mark_component_ok("channel:webhook.paired");
    ...
    async fn api_channels_serializes_readiness_without_duplicate_summary_fields() {
        ...
        let webhook_compiled = omnesagent_channels::listing::is_channel_type_compiled("webhook");
    ...
    async fn session_message_post_persists_and_broadcasts_to_session() {
        ...
        backend
            .append(
                "gw_operator-1",
                &omnesagent_providers::ChatMessage::assistant("existing"),
            )
            .unwrap();
    ...
    async fn cron_api_accepts_agent_jobs() {
        ...
        let config = state.config.read().clone();
        let jobs = omnesagent_runtime::cron::list_jobs(&config).unwrap();
        assert_eq!(jobs.len(), 1);
        assert_eq!(jobs[0].job_type, omnesagent_runtime::cron::JobType::Agent);
        assert_eq!(jobs[0].prompt.as_deref(), Some("summarize the latest logs"));
        assert_eq!(
            jobs[0].session_target,
            omnesagent_runtime::cron::SessionTarget::Isolated
        );
    }
    ...
    async fn cron_api_agent_job_persists_session_target_main() {
        ...
        assert_eq!(
            jobs[0].session_target,
            omnesagent_runtime::cron::SessionTarget::Main
        );
    }
    ...
    async fn cron_api_agent_job_accepts_session_target_case_and_whitespace() {
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let jobs = omnesagent_runtime::cron::list_jobs(&state.config.read().clone()).unwrap();
        assert_eq!(
            jobs[0].session_target,
            omnesagent_runtime::cron::SessionTarget::Main
        );
    ...
    async fn cron_api_agent_job_rejects_invalid_session_target_as_bad_request() {
        ...
        assert!(
            omnesagent_runtime::cron::list_jobs(&state.config.read().clone())
                .unwrap()
                .is_empty(),
    ...
    async fn cron_api_timezone_add_persists_explicit_timezone() {
        ...
        assert_eq!(
            jobs[0].schedule,
            omnesagent_runtime::cron::Schedule::Cron {
                expr: "0 9 * * *".to_string(),
    ...
    async fn cron_api_timezone_patch_schedule_preserves_existing_timezone() {
        ...
        std::fs::create_dir_all(&config.data_dir).unwrap();
        let state = test_state(with_test_agent(config));
        let job = omnesagent_runtime::cron::add_shell_job_with_approval(
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let updated = omnesagent_runtime::cron::get_job(&state.config.read().clone(), &job.id)
            .expect("updated job");
        assert_eq!(
            updated.schedule,
            omnesagent_runtime::cron::Schedule::Cron {
    ...
    async fn cron_api_patch_agent_prompt_without_agent() {
        ...
        let id = omnesagent_runtime::cron::list_jobs(&state.config.read().clone()).unwrap()[0]
            .id
            .clone();
        ...
        let updated = omnesagent_runtime::cron::get_job(&state.config.read().clone(), &id)
            .expect("updated job");
    ...
    async fn cron_api_patch_updates_imperative_shell_output_format() {
        ...
        assert_eq!(
            ...
            omnesagent_config::schema::CronShellOutputFormat::Wrapped,
        ...
        assert_eq!(
            ...
            omnesagent_config::schema::CronShellOutputFormat::Raw,
    ...
    async fn cron_api_patch_rejects_shell_output_format_for_declarative_job() {
        ...
        let decl = omnesagent_config::schema::CronJobDecl {
            name: Some("decl-job".to_string()),
            job_type: "shell".to_string(),
            schedule: omnesagent_config::schema::CronScheduleDecl::Cron {
                expr: "0 2 * * *".to_string(),
                tz: None,
            },
            ...
            session_target: None,
            delivery: None,
            shell_output_format: omnesagent_config::schema::CronShellOutputFormat::Wrapped,
        };
        let mut decls = std::collections::HashMap::new();
        decls.insert("decl-job".to_string(), decl.clone());
        config.cron.insert("decl-job".to_string(), decl);
        omnesagent_runtime::cron::sync_declarative_jobs(&config, &decls).unwrap();
        let state = test_state(config);
        ...
        let unchanged =
            omnesagent_runtime::cron::get_job(&state.config.read().clone(), "decl-job").unwrap();
        assert_eq!(
    ...
    async fn cron_api_rejects_announce_delivery_without_target() {
        ...
        let config = state.config.read().clone();
        assert!(
            omnesagent_runtime::cron::list_jobs(&config)
                .unwrap()
                .is_empty()
        );
    }
    ...
    async fn cron_api_run_executes_shell_job_and_records_run() {
        ...
        let runs = omnesagent_runtime::cron::list_runs(&state.config.read().clone(), &job.id, 10)
            .expect("runs listed");
    ...
    async fn cron_api_run_records_best_effort_delivery_failure_as_degraded() {
        ...
        let job = omnesagent_runtime::cron::add_shell_job_with_approval(
            ...
            "echo hello-from-manual-trigger",
            Some(omnesagent_runtime::cron::DeliveryConfig {
                mode: "announce".into(),
        ...
        let config = state.config.read().clone();
        let updated = omnesagent_runtime::cron::get_job(&config, &job.id).expect("updated job");
        ...
        let runs = omnesagent_runtime::cron::list_runs(&config, &job.id, 10).expect("runs listed");
        assert_eq!(runs.len(), 1);
    ...
    mod a2a_auth {
        ...
        fn paired_state() -> AppState {
            ...
            let agent = omnesagent_config::schema::AliasedAgentConfig {
                a2a: omnesagent_config::multi_agent::AgentA2aConfig {
                    published: true,
