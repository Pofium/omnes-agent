//! HTTP adapter over `omnesagent_runtime::browse::list_directory`.
...
use omnesagent_runtime::browse::{
    ...
    read_agent_workspace_file, remove_directory, write_agent_workspace_file,
};
...
/// `GET /api/agents/{alias}/workspace/raw?path=<rel>` — download or stream a raw file
/// (binary or text) with proper Content-Type and optional Content-Disposition.
...
/// `POST /api/agents/{alias}/workspace/write` body `{ path: "<rel>", content: "<text>" }`.
pub async fn handle_agent_workspace_write(
    State(state): State<AppState>,
    headers: HeaderMap,
    AxumPath(alias): AxumPath<String>,
    ...
    let config = state.config.read().clone();
    match write_agent_workspace_file(&config, &alias, &body.path, body.content.as_bytes()) {
        Ok(()) => Json(serde_json::json!({ "written": body.path })).into_response(),
        Err(err) => browse_error_response(err),
    }
}
