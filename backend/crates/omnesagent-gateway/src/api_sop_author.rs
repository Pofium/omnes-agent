...
//! `omnesagent_runtime::sop` authoring core (load/save/delete, graph
...
use omnesagent_runtime::sop::SopGraphExt;
...
fn sops_dir_and_mode(
    state: &AppState,
) -> (std::path::PathBuf, omnesagent_runtime::sop::SopExecutionMode) {
    let config = state.config.read();
    let install_root = config.install_root_dir();
    let dir =
        omnesagent_runtime::sop::resolve_sops_dir(&install_root, config.sop.sops_dir.as_deref());
    let mode = omnesagent_runtime::sop::parse_execution_mode(&config.sop.default_execution_mode);
    (dir, mode)
}

fn sop_tool_specs(state: &AppState) -> omnesagent_runtime::sop::ToolSpecs {
    let config = state.config.read();
    let agent = config.agents.keys().min().cloned().unwrap_or_default();
    omnesagent_runtime::sop::tool_specs_from_config(&config, &agent)
}
...
pub async fn handle_sops_list(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let sops = omnesagent_runtime::sop::load_sops_from_directory(&dir, mode);
...
pub async fn handle_sop_trigger_sources(
    ...
    let registry = {
        let config = state.config.read();
        omnesagent_runtime::sop::registry_from_config(&config)
    };
...
pub struct ParamOptionsBody {
    pub domain: omnesagent_api::tool::OptionDomain,
    #[serde(default)]
...
pub async fn handle_sop_graph(
    ...
    let (dir, mode) = sops_dir_and_mode(&state);
    match omnesagent_runtime::sop::load_sop_by_name(&dir, &name, mode) {
        Ok(sop) => {
            let graph =
                omnesagent_runtime::sop::SopGraph::from_sop_with_specs(&sop, &sop_tool_specs(&state));
...
pub async fn handle_sop_run(
    ...
    let event = omnesagent_runtime::sop::SopEvent {
        source: omnesagent_runtime::sop::SopTriggerSource::Manual,
        topic: None,
        payload,
        timestamp: omnesagent_runtime::sop::engine::now_iso8601(),
    };

    let results =
        omnesagent_runtime::sop::dispatch::dispatch_sop_event_to(engine, audit, event, &name).await;
    omnesagent_runtime::sop::dispatch::process_headless_results(&results);

    for result in &results {
        match result {
            omnesagent_runtime::sop::dispatch::DispatchResult::Started { run_id, action, .. } => {
                let needs_driver = matches!(
                    action.as_ref(),
                    omnesagent_runtime::sop::SopRunAction::ExecuteStep { .. }
                        | omnesagent_runtime::sop::SopRunAction::DeterministicStep { .. }
                );
                if needs_driver {
                    let config = state.config.read().clone();
                    omnesagent_runtime::sop::spawn_headless_run_driver(
                        config,
            ...
            omnesagent_runtime::sop::dispatch::DispatchResult::Skipped { reason, .. } => {
                return (
                    StatusCode::CONFLICT,
            ...
            omnesagent_runtime::sop::dispatch::DispatchResult::BlockedUnsafe { reason, .. } => {
                return (
            ...
            omnesagent_runtime::sop::dispatch::DispatchResult::Deferred { reason, .. } => {
                return (
                    StatusCode::SERVICE_UNAVAILABLE,
                    Json(serde_json::json!({ "error": reason })),
                )
...
pub async fn handle_sop_runs(
    ...
    match omnesagent_runtime::sop::run_summaries_for(engine, query.sop.as_deref()) {
...
pub async fn handle_sop_run_overlay(
    ...
    let (dir, mode) = sops_dir_and_mode(&state);
    let sop = match omnesagent_runtime::sop::load_sop_by_name(&dir, &name, mode) {
        Ok(sop) => sop,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
    ...
    match omnesagent_runtime::sop::run_overlay_for(&sop, engine, &run_id) {
...
pub async fn handle_sop_decide(
    ...
    let principal = omnesagent_runtime::sop::approval::ApprovalPrincipal::http(subject);
    let decision: omnesagent_runtime::sop::approval::ApprovalDecision =
    ...
    let mut pending_quorum = false;
    {
        ...
        match status {
            Some(
                omnesagent_runtime::sop::types::SopRunStatus::WaitingApproval
                | omnesagent_runtime::sop::types::SopRunStatus::PausedCheckpoint,
            ) => {
                use omnesagent_runtime::sop::approval::{BrokerOutcome, ResolveOutcome};
    ...
    if pending_quorum {
        return match omnesagent_runtime::sop::run_overlay_for(&sop, engine, &run_id) {
...
struct SopCancelResponse {
    run_id: String,
    sop_name: String,
    status: omnesagent_runtime::sop::types::SopRunStatus,
    already_terminal: bool,
    run: omnesagent_runtime::sop::types::SopRunSummary,
}
...
fn authorize_sop_cancel(
    ...
    peer: &SocketAddr,
    headers: &HeaderMap,
) -> Result<omnesagent_runtime::sop::approval::ApprovalPrincipal, Box<Response>> {
    if state.pairing.require_pairing() {
        ...
        return Ok(omnesagent_runtime::sop::approval::ApprovalPrincipal::http(
            subject,
        ));
    }
    ...
    if effective_client_ip.is_some_and(|ip| ip.is_loopback()) {
        return Ok(omnesagent_runtime::sop::approval::ApprovalPrincipal::cli(
            None,
        ));
    }
    Err(Box::new(
...
pub async fn handle_sop_cancel(
    ...
    use omnesagent_runtime::sop::{CancelOutcome, err_is_cancellation_persistence_retained};
    ...
    match outcome {
        Ok(Some(outcome)) => {
            ...
            let summary = omnesagent_runtime::sop::types::SopRunSummary::from_run(
                run,
...
pub async fn handle_sop_create(
    State(state): State<AppState>,
    headers: HeaderMap,
    Json(sop): Json<omnesagent_runtime::sop::Sop>,
) -> Response {
    if let Err(e) = require_auth(&state, &headers) {
        return e.into_response();
    }
    let (dir, _mode) = sops_dir_and_mode(&state);
    match omnesagent_runtime::sop::create_sop_typed(&dir, &sop) {
        Ok(()) => Json(serde_json::json!({ "created": sop.name })).into_response(),
        Err(e) => {
            let code = match e {
                omnesagent_runtime::sop::SopAuthorError::AlreadyExists(_) => StatusCode::CONFLICT,
...
pub async fn handle_sop_save(
    ...
    headers: HeaderMap,
    Path(name): Path<String>,
    Json(mut sop): Json<omnesagent_runtime::sop::Sop>,
) -> Response {
    if let Err(e) = require_auth(&state, &headers) {
        return e.into_response();
    }
    ...
    sop.name = name;
    let (dir, _mode) = sops_dir_and_mode(&state);
    match omnesagent_runtime::sop::save_sop(&dir, &sop) {
...
pub async fn handle_sop_delete(
    ...
    let (dir, _mode) = sops_dir_and_mode(&state);
    match omnesagent_runtime::sop::delete_sop_typed(&dir, &name) {
        Ok(()) => Json(serde_json::json!({ "deleted": name })).into_response(),
        Err(e) => {
            let code = match e {
                omnesagent_runtime::sop::SopAuthorError::NotFound(_) => StatusCode::NOT_FOUND,
...
pub struct WireDraftRequest {
    pub sop: omnesagent_runtime::sop::Sop,
    pub edit: omnesagent_runtime::sop::WireEdit,
}
...
pub async fn handle_sop_wire_draft(
    ...
    let mut sop = req.sop;
    if let Err(e) = omnesagent_runtime::sop::apply_wire(&mut sop, &req.edit) {
        return (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({ "error": e.to_string() })),
        )
    ...
    let graph = omnesagent_runtime::sop::SopGraph::from_sop_with_specs(&sop, &sop_tool_specs(&state));
...
pub async fn handle_sop_graph_draft(
    ...
    let graph =
        omnesagent_runtime::sop::SopGraph::from_sop_with_specs(&req.sop, &sop_tool_specs(&state));
    Json(graph).into_response()
}
...
pub async fn handle_sop_graph_legend(
    ...
    Json(omnesagent_runtime::sop::GraphLegend::canonical()).into_response()
}
...
mod tests {
    ...
    use omnesagent_runtime::sop::approval::ApprovalBroker;
    use omnesagent_runtime::sop::engine::{SopEngine, now_iso8601};
    use omnesagent_runtime::sop::types::{
    ...
    fn authoring_state_with_policied_gate(
        ...
        omnesagent_runtime::sop::save_sop(&sops_dir, &sop).unwrap();
    ...
    async fn authoring_decide_rejects_run_id_from_different_sop_before_broker_resolution() {
        ...
        omnesagent_runtime::sop::save_sop(&sops_dir, &sop_a).unwrap();
        omnesagent_runtime::sop::save_sop(&sops_dir, &sop_b).unwrap();
    ...
    fn authoring_state_with_running_run(token: &str) -> (AppState, String) {
        ...
        let mut state = crate::api::test_state(omnesagent_config::schema::Config::default());
        state.sop_engine = Some(Arc::new(Mutex::new(engine)));
        state.pairing = Arc::new(PairingGuard::new(true, &[token.to_string()]));
        (state, run_id)
    }
