...
pub async fn cascade_owned_state(
    ...
    let cron_jobs = match omnesagent_runtime::cron::list_jobs_by_agent(config, alias) {
        Ok(jobs) => jobs,
        Err(e) => {
    ...
    let cron_removed = match omnesagent_runtime::cron::remove_jobs_by_agent(config, alias) {
        Ok(n) => n,
        Err(e) => {
...
pub async fn cascade_rename_agent(
    ...
    let cron_jobs = match omnesagent_runtime::cron::rename_jobs_by_agent(config, from, to) {
        Ok(n) => n,
        Err(e) => {
