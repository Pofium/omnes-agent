...
use omnesagent_runtime::skills::SkillsService;
...
/// the per-alias spec-conforming cards and the OmnesAgent discovery catalog
...
/// Build the OmnesAgent discovery catalog card served at the origin root. Lists
...
pub(crate) fn build_catalog_card_with_endpoint(
    ...
    AgentCard {
        name: "OmnesAgent agents".to_string(),
        description: "Discovery catalog enumerating published A2A agents on \
                      this OmnesAgent install. Not a runnable agent; each entry \
...
fn identity_description(config: &Config, alias: &str) -> Option<String> {
    ...
    let aieos = omnesagent_runtime::identity::load_aieos_identity(&agent.identity, &workspace_dir)
        .ok()
        .flatten()?;
...
fn identity_name_line(names: omnesagent_runtime::identity::Names) -> Option<String> {
...
mod tests {
    use super::*;
    use omnesagent_config::multi_agent::AgentA2aConfig;
    ...
    fn exposed_skills_drop_when_bundle_filter_excludes_them() {
        ...
        let mut config = config_with_published_alias("maker", true);
        config.config_path = tmp.path().join("config.toml");
        let bundle = omnesagent_config::schema::SkillBundleConfig {
    ...
    fn card_description_falls_back_to_neutral_default_without_identity() {
        let config = config_with_published_alias("researcher", true);
        let card = build_agent_card(&config, "researcher").expect("card");
        assert_eq!(card.description, "OmnesAgent agent 'researcher'.");
    }
