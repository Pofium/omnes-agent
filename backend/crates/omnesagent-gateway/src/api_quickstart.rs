...
use omnesagent_config::presets::BuilderSubmission;
...
pub async fn handle_state(State(state): State<AppState>, headers: HeaderMap) -> impl IntoResponse {
    ...
    let body = omnesagent_runtime::quickstart::snapshot_state(&cfg);
    (StatusCode::OK, Json(body)).into_response()
}

#[derive(Debug, Deserialize)]
pub struct FieldsRequest {
    pub section: omnesagent_runtime::quickstart::FieldSection,
    pub type_key: String,
}

#[derive(Debug, Serialize)]
pub struct FieldsResult {
    pub fields: Vec<omnesagent_runtime::quickstart::FieldDescriptor>,
}
...
pub async fn handle_fields(
    ...
    let body = FieldsResult {
        fields: omnesagent_runtime::quickstart::field_shape(req.section, &req.type_key),
    };
    (StatusCode::OK, Json(body)).into_response()
}
...
// `omnesagent_runtime::quickstart::snapshot_state` so both transports
