...
use omnesagent_runtime::sop::approval::{
...
const WS_PROTOCOL: &str = "omnesagent.v1";
...
async fn resolve_ws_memory_handle(
    config: &omnesagent_config::schema::Config,
    agent_alias: &str,
) -> anyhow::Result<Option<Arc<dyn omnesagent_memory::Memory>>> {
    if config.agent(agent_alias).is_some_and(|agent| {
        matches!(
            agent.memory.backend,
            omnesagent_config::multi_agent::MemoryBackendKind::None
        )
    }) {
        return Ok(None);
    }
    ...
    omnesagent_memory::create_memory_for_agent(config, agent_alias, api_key.as_deref())
        .await
        .map(Some)
}
    ...
    S: SinkExt<Message> + Unpin,
{
    ...
    let Some(decision) = decision.filter(|_| !run_id.is_empty()) else {
        let err = serde_json::json!({
            "type": "error",
            "message": omnesagent_runtime::i18n::get_required_cli_string(
    ...
    let frame = if let Some(engine) = state.sop_engine.as_ref() {
        ...
        match resolved {
            Some(Ok(outcome)) => {
                ...
                omnesagent_runtime::sop::drive_resumed_broker_action(
                    &config,
            ...
            Some(Err(e)) => serde_json::json!({
                "type": "error",
                "message": omnesagent_runtime::i18n::get_required_cli_string_with_args(
...
async fn handle_socket(
    ...
    let mut memory_session_id = omnesagent_api::session_keys::sanitize_session_key(&session_id);
            ...
            Some(Ok(Message::Text(text))) => {
                if let Ok(cp) = serde_json::from_str::<ConnectParams>(&text) {
                    if cp.msg_type == "connect" {
                        ::omnesagent_log::record!(DEBUG, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"session_id": cp.session_id, "device_name": cp.device_name, "capabilities": cp.capabilities, "cwd": cp.cwd})), "WebSocket connect params received");
                        if let Some(sid) = &cp.session_id {
                            memory_session_id =
                                omnesagent_api::session_keys::sanitize_session_key(sid);
                            ::omnesagent_log::record!(
                                DEBUG,
    ...
    let mut agent =
        match omnesagent_runtime::agent::Agent::from_live_config_with_session_cwd_and_mcp_backchannel(
    ...
    let (approval_event_tx, mut approval_event_rx) =
        tokio::sync::mpsc::channel::<omnesagent_api::agent::TurnEvent>(8);
    ...
    if let Some(omnesagent_api::agent::TurnEvent::HistoryTrimmed {
        dropped_messages,
        kept_turns,
        reason,
    }) = restore_trim_event
        ...
        tokio::select! {
            ...
            client_msg = receiver.next() => {
                ...
                if msg_type == "approval_response" {
                    ...
                    if let Some(tx) = pending_approvals.lock().remove(request_id) {
                        ...
                        ::omnesagent_log::record!(DEBUG, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"request_id": request_id})), "approval_response with no matching pending request");
                    }
                    continue;
                }
            ...
            approval_event = approval_event_rx.recv() => {
                ...
                let frame = match event {
                    omnesagent_api::agent::TurnEvent::ApprovalRequest {
                        request_id,
                        tool_name,
                        arguments_summary,
                        timeout_secs,
                    ...
                    other => {
                        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"kind": format!("{:?}", other)})), "non-ApprovalRequest event leaked into approval channel");
                        continue;
                    }
                };
                let _ = sender.send(Message::Text(frame.to_string().into())).await;
...
fn persist_conversation_messages(
    backend: &dyn omnesagent_infra::session_backend::SessionBackend,
    session_key: &str,
    messages: &[omnesagent_providers::ConversationMessage],
) {
    ...
    for message in messages {
        let omnesagent_providers::ConversationMessage::Chat(message) = message else {
            continue;
        };
...
fn has_assistant_chat_message(messages: &[omnesagent_providers::ConversationMessage]) -> bool {
    messages.iter().any(|message| {
        matches!(
            message,
            omnesagent_providers::ConversationMessage::Chat(message)
...
async fn process_chat_message(
    state: &AppState,
    agent: &mut omnesagent_runtime::agent::Agent,
    sender: &mut futures_util::stream::SplitSink<WebSocket, Message>,
    receiver: &mut futures_util::stream::SplitStream<WebSocket>,
    approval_event_rx: &mut tokio::sync::mpsc::Receiver<omnesagent_api::agent::TurnEvent>,
    ...
    let turn_fut = async {
        ...
        omnesagent_runtime::agent::loop_::scope_session_key(
            ...
            omnesagent_runtime::agent::cost::TOOL_LOOP_TURN_USAGE.scope(
                turn_usage.clone(),
                    ...
                    match parsed["type"].as_str() {
                        Some("approval_response") => {
                            ...
                            if let Some(tx) = pending_approvals.lock().remove(request_id) {
                                ...
                                ::omnesagent_log::record!(DEBUG, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"request_id": request_id})), "approval_response with no matching pending request (mid-turn)");
                            }
                        }
                        Some("message") => {
                            let content = parsed["content"].as_str().unwrap_or("").to_string();
                ...
                match &result {
                    Err(error) if !error.new_messages.is_empty() => {
                        ...
                        if !has_assistant_chat_message(&error.new_messages) {
                            let marker = omnesagent_runtime::i18n::get_required_cli_string(
                            ...
                            let assistant_msg =
                                omnesagent_providers::ChatMessage::assistant(&truncated);
                    ...
                    _ => {
                        ...
                        let assistant_msg = omnesagent_providers::ChatMessage::assistant(&truncated);
                        if backend.session_exists(session_key) {
                            let _ = backend.append(session_key, &assistant_msg);
                        }
                    }
    ...
    match result {
        Ok(outcome) => {
            ...
            if state.auto_save {
                if let Some(mem) = ws_memory.clone() {
                    ...
                    omnesagent_spawn::spawn!(async move {
                        if let Err(e) = omnesagent_memory::consolidation::consolidate_turn(
            ...
            // Append a runtime-trace.jsonl record so a `omnesagent doctor`
        ...
        Err(e) => {
            ...
            let user_message =
                omnesagent_runtime::agent::terminal_completion_error_message(&e.error, None);
            ...
            ::omnesagent_log::record!(
                ...
                ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Fail)
                    ...
                    .with_attrs(::serde_json::json!({
                        ...
                        "model": turn_model,
                        "session_key": session_key,
                        "error": omnesagent_providers::sanitize_api_error(&e.error.to_string()),
                        "error_code": err["code"],
                        "trace_id": turn_id,
                    })),
                "gateway_ws_turn"
...
fn ws_turn_failure_frame(
    ...
    let sanitized = omnesagent_providers::sanitize_api_error(diagnostic);
...
data: {\"type\":\"message_stop\"}\n\n",
        ...
        let mock_server = omnesagent_spawn::spawn!(async move {
        ...
        let gateway_server = omnesagent_spawn::spawn!(async move {
    ...
    async fn ws_back_channel_registers_under_the_ingress_identity() {
        ...
        let handle: omnesagent_runtime::tools::PerToolChannelHandle =
            Arc::new(parking_lot::RwLock::new(std::collections::HashMap::new()));
        handle.write().insert(
            WS_CHANNEL_KEY.to_string(),
            approval_channel as Arc<dyn omnesagent_api::channel::Channel>,
        );
    ...
    fn sop_ws_error_frames_resolve_via_fluent() {
        ...
        for key in [
            ...
            let s = omnesagent_runtime::i18n::get_required_cli_string(key);
            assert!(
                !s.starts_with('{') || !s.ends_with('}'),
                "fluent missing-key fallback leaked for {key}: {s:?}"
            );
        }
        let resolved = omnesagent_runtime::i18n::get_required_cli_string_with_args(
            "cli-sop-ws-resolve-failed",
            &[("error", "boom")],
        );
        assert!(
    ...
    fn extract_ws_token_from_subprotocol() {
        ...
        headers.insert(
            ...
            "omnesagent.v1, bearer.zc_sub456".parse().unwrap(),
        );
    ...
    fn extract_ws_token_subprotocol_with_multiple_entries() {
        ...
        headers.insert(
            "sec-websocket-protocol",
            "omnesagent.v1, bearer.zc_tok, other".parse().unwrap(),
        );
    ...
    fn needs_onboarding_ws_error_uses_current_configured_model() {
        ...
        config.providers.models.openai.insert(
            "default".to_string(),
            omnesagent_config::schema::OpenAIModelProviderConfig {
                base: omnesagent_config::schema::ModelProviderConfig {
    ...
    impl omnesagent_infra::session_backend::SessionBackend for DeletedSessionBackend {
        fn load(&self, _session_key: &str) -> Vec<omnesagent_providers::ChatMessage> {
            Vec::new()
        }
        fn append(
            &self,
            session_key: &str,
            message: &omnesagent_providers::ChatMessage,
        ) -> std::io::Result<()> {
    ...
    fn persist_conversation_messages_skips_deleted_session() {
        use omnesagent_providers::{ChatMessage, ConversationMessage};
