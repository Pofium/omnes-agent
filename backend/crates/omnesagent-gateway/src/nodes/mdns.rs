//! LAN-local multicast peer discovery for OmnesAgent gateways.
//!
...
use uuid::Uuid;
use omnesagent_config::schema::MdnsConfig;

// Proprietary OmnesAgent discovery uses an administratively scoped multicast group
...
/// A OmnesAgent LAN discovery packet.
...
fn resolve_node_name(config: &MdnsConfig) -> String {
    config
        ...
        .filter(|name| !name.trim().is_empty())
        .unwrap_or_else(|| "omnesagent-node".to_string())
}
