...
use std::sync::Arc;
use tokio::sync::mpsc;
use omnesagent_channels::orchestrator::acp_server::{AcpServer, AcpServerConfig};
use omnesagent_infra::acp_session_store::AcpSessionStore;

const ACP_WS_PROTOCOL: &str = "omnesagent.acp.v1";
...
async fn handle_socket(socket: WebSocket, state: AppState, default_agent: Option<String>) {
    ...
    let server_task = omnesagent_spawn::spawn!(Arc::clone(&server).run_messages(input_rx));

    let output_task = omnesagent_spawn::spawn!(async move {
...
mod tests {
    ...
    //!
    //! The `session/new` unit tests in `omnesagent-channels` call
    ...
    fn front_door_config(install_root: &std::path::Path) -> omnesagent_config::schema::Config {
    ...
    async fn acp_ws_front_door_omitted_cwd_uses_agent_workspace() {
        ...
        let server = omnesagent_spawn::spawn!(crate::run_gateway(
            "127.0.0.1",
            port,
            cfg,
            None,
