...
//!
//! Thin adapter over the shared [`omnesagent_tls`] crate: it maps the gateway's
//! [`GatewayTlsConfig`] onto the transport-neutral
//! [`omnesagent_tls::ServerConfigParams`] and delegates construction. The reusable
//! verifier / certificate-pinning / PEM-loading logic lives in `omnesagent-tls` so
//! it can also be consumed by `omnesagent-runtime` without an upward dependency on
//! the gateway.

use anyhow::Result;
use tokio_rustls::TlsAcceptor;
use omnesagent_config::schema::GatewayTlsConfig;
use omnesagent_tls::{ClientAuthParams, ServerConfigParams};
...
pub fn build_tls_acceptor(config: &GatewayTlsConfig) -> Result<TlsAcceptor> {
    omnesagent_tls::build_tls_acceptor(&to_params(config))
}
...
pub fn build_server_config(config: &GatewayTlsConfig) -> Result<rustls::ServerConfig> {
    omnesagent_tls::build_server_config(&to_params(config))
}
...
mod tests {
    use super::*;
    use omnesagent_config::schema::{GatewayClientAuthConfig, GatewayTlsConfig};
