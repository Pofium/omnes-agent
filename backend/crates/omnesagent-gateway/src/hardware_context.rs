...
/// Return `~/.omnesagent/hardware/` or an error string.
fn hardware_dir() -> Result<PathBuf, String> {
    directories::BaseDirs::new()
        .map(|b| b.home_dir().join(".omnesagent").join("hardware"))
...
pub async fn handle_hardware_pin(
    ...
    match append_to_file(&device_path, &line).await {
        Ok(()) => {
            ...
            ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"device": req.device, "pin": req.pin, "component": component})), &format!("{}", message));
            (
                StatusCode::OK,
...
pub async fn handle_hardware_reload(
    ...
    let context = omnesagent_hardware::load_hardware_context_prompt(&[]);
