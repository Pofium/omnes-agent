...
use omnesagent_config::sections::section_for_path;
...
/// OmnesAgent's `comment` extension. Every operation requires `path`; `add`,
...
pub struct PatchResponse {
    ...
    pub warnings: Vec<omnesagent_config::validation_warnings::ValidationWarning>,
}
...
pub struct ListEntry {
    ...
    pub populated: bool,
    pub is_secret: bool,
    /// Whether this field was populated by a `OMNESAGENT_*` env-var override
    /// at load time. The dashboard renders the 💉 badge and a persistent
    /// warning *"Edits here won't take effect — overridden by OMNESAGENT_..."*
    /// when this is `true`.
...
fn prop_kind_wire(kind: omnesagent_config::traits::PropKind) -> &'static str {
...
// Typed-value coercion lives in `omnesagent_config::typed_value` — both the
...
use omnesagent_config::typed_value::coerce_for_set_prop as json_to_setprop_string;
...
fn lookup_prop_field(
    ...
    path: &str,
) -> Option<omnesagent_config::traits::PropFieldInfo> {
    config
        .prop_fields()
        .into_iter()
        .find(|info| info.name == path)
        .or_else(|| {
            omnesagent_config::schema::Config::prop_is_secret(path).then(|| {
                omnesagent_config::traits::PropFieldInfo {
                    name: path.to_string(),
                    category: "Secrets",
                    display_value: omnesagent_config::traits::UNSET_DISPLAY.to_string(),
                    type_hint: "String",
                    kind: omnesagent_config::traits::PropKind::String,
                    is_secret: true,
                    enum_variants: None,
                    description: "",
                    derived_from_secret: false,
                    credential_class: Some(
                        omnesagent_config::traits::CredentialSurfaceClass::EncryptedSecret,
                    ),
                    tab: omnesagent_config::traits::ConfigTab::None,
                    alias_source: None,
                    multiline: false,
                }
            })
...
fn scoped_validate(
    working: &omnesagent_config::schema::Config,
) -> Result<Vec<omnesagent_config::validation_warnings::ValidationWarning>, ConfigApiError> {
    if let Err(e) = working.validate() {
        ...
        return Ok(vec![
            omnesagent_config::validation_warnings::ValidationWarning::new(
...
async fn persist_and_swap(
    state: &AppState,
    mut new_config: omnesagent_config::schema::Config,
...
/// `omnesagent channel bind-<type> <identity> --alias <alias>`: authorize an
...
pub async fn handle_api_channel_bind(
    ...
    if omnesagent_channels::orchestrator::channel_identity_normalizer(channel_type).is_none() {
    ...
    // runtime never reads.
    if !omnesagent_channels::orchestrator::channel_alias_configured(&working, channel_type, alias) {
        return error_response(ConfigApiError::new(
    ...
    let newly = match omnesagent_channels::orchestrator::bind_channel_identity_into(
        &mut working,
        channel_type,
        alias,
        &body.identity,
...
pub async fn compute_drift(in_memory: &omnesagent_config::schema::Config) -> Vec<DriftEntry> {
    ...
    let on_disk: omnesagent_config::schema::Config =
        match toml::from_str::<omnesagent_config::schema::Config>(&raw) {
            Ok(mut cfg) => {
    ...
    let in_memory_props: std::collections::HashMap<String, omnesagent_config::traits::PropFieldInfo> =
        in_memory
            .prop_fields()
            .into_iter()
            .map(|p| (p.name.clone(), p))
            .collect();
    let on_disk_props: std::collections::HashMap<String, omnesagent_config::traits::PropFieldInfo> =
        on_disk
            .prop_fields()
            .into_iter()
            .map(|p| (p.name.clone(), p))
    ...
    for name in all_names {
        ...
        // Env overrides (`OMNESAGENT_<path>`) apply in memory but never persist to
        ...
        let mem_display = mem
            ...
            .unwrap_or(omnesagent_config::traits::UNSET_DISPLAY);
...
pub async fn handle_prop_get(
    ...
    if info.is_secret || info.derived_from_secret {
        let populated = info.display_value != omnesagent_config::traits::UNSET_DISPLAY;
...
pub async fn handle_prop_put(
    ...
    if let Some(comment) = body.comment.as_ref() {
        ...
        if let Err(e) =
            omnesagent_config::comment_writer::apply_comments(&config_path, &annotations).await
        {
            ::omnesagent_log::record!(
                WARN,
                ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note)
...
pub async fn handle_templates(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let templates: Vec<TemplateEntry> = omnesagent_config::schema::Config::map_key_sections()
        .into_iter()
        .map(|s| TemplateEntry {
            path: s.path,
            kind: match s.kind {
                omnesagent_config::traits::MapKeyKind::Map => "map",
                omnesagent_config::traits::MapKeyKind::List => "list",
            },
...
pub struct AliasSourceQuery {
    pub source: omnesagent_config::traits::AliasSource,
}
...
pub async fn handle_delete_map_key(
    ...
    match omnesagent_config::alias_refs::alias_kind_for_map_path(&q.path) {
        Some(omnesagent_config::alias_refs::AliasKind::Agent) => {
...
async fn delete_agent_cascade(
    state: &AppState,
    mut working: omnesagent_config::schema::Config,
    alias: &str,
    guard: ConfigWriteGuard,
) -> Response {
    use omnesagent_config::alias_refs::{self, AliasKind, CascadePolicy};
    ...
    if let Err(err) = tokio::fs::create_dir_all(&archive_dir).await {
        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": alias, "archive": archive_dir.display().to_string(), "err": err.to_string()})), "agent delete: archive dir creation failed");
    ...
    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"agent": alias, "memory": owned.memory_purged, "cron": owned.cron_removed, "acp": owned.acp_removed, "sessions_cleared": owned.sessions_cleared, "archive": archive_dir.display().to_string(), "warnings": warnings.len()})), "agent deleted with owned-state cascade");
...
async fn delete_config_cascade(
    ...
    let report = match omnesagent_config::alias_refs::delete_with_cascade(
        &mut working,
        kind,
        key,
        omnesagent_config::alias_refs::CascadePolicy::RefuseOnHard,
    ) {
        Ok(r) => r,
...
pub async fn handle_map_key(
    ...
    let created =
        match omnesagent_config::alias_refs::create_map_key_checked(&mut working, &path, &key) {
            Ok(b) => b,
            Err(omnesagent_config::alias_refs::CreateError::Reserved(a)) => {
                return error_response(
                    ConfigApiError::new(
                        ConfigApiCode::ValidationFailed,
            ...
            Err(omnesagent_config::alias_refs::CreateError::Invalid(msg)) => {
                return error_response(
    ...
    if created {
        ...
        if path == "skill_bundles" {
            ...
            if let Ok(dir) =
                omnesagent_config::skill_bundles::resolve_directory(&working, &install_root, &key)
...
pub async fn handle_delete_plan(
    ...
    let config = state.config.read().clone();
    let to_dto = |s: &omnesagent_config::alias_refs::RefSite| RefSiteDto {
        path: s.path.clone(),
        raw_value: s.raw_value.clone(),
    };
    let Some(kind) = omnesagent_config::alias_refs::alias_kind_for_map_path(&q.path) else {
    ...
    let plan = omnesagent_config::alias_refs::plan_delete(&config, &kind, &q.key);
...
fn unsupported_delete_cascade_message(
    ...
    use omnesagent_config::alias_refs::{AliasKind, ProviderCategory};
    match kind {
        AliasKind::Provider {
...
/// Map a [`RenameError`](omnesagent_config::alias_refs::RenameError) to the HTTP
...
/// Map a [`CascadeError`](omnesagent_config::alias_refs::CascadeError) to the
...
fn delete_error_response(
    path: &str,
    key: &str,
    err: omnesagent_config::alias_refs::CascadeError,
) -> Response {
    use omnesagent_config::alias_refs::CascadeError;
    let (code, msg) = match err {
...
pub async fn handle_rename_map_key(
    ...
    match omnesagent_config::alias_refs::alias_kind_for_map_path(&body.path) {
        Some(omnesagent_config::alias_refs::AliasKind::Agent) => {
...
async fn rename_config_cascade(
    ...
    let report = match omnesagent_config::alias_refs::rename_with_cascade(
        &mut working,
        kind,
        &body.from,
        &body.to,
    ...
    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"path": body.path, "from": body.from, "to": body.to, "dirty_paths": report.dirty_paths.len()})), "alias renamed with config-ref cascade");
    axum::Json(RenameMapKeyResponse {
...
async fn rename_residue_exists(
    ...
    if omnesagent_runtime::cron::list_jobs_by_agent(&cfg, from)
    ...
    if let Ok(store) = omnesagent_infra::acp_session_store::AcpSessionStore::new(&cfg.data_dir)
        && store
...
async fn rename_agent_cascade(
    ...
    use omnesagent_config::alias_refs::{self, AliasKind};
    ...
    if warnings.is_empty() {
        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"from": from, "to": to, "memory": owned.memory_rows, "cron": owned.cron_jobs, "acp": owned.acp_sessions, "sessions": owned.sessions_repointed, "workspace_moved": workspace_moved, "dirty_paths": dirty_count})), "agent renamed with owned-state cascade");
    } else {
        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"from": from, "to": to, "memory": owned.memory_rows, "cron": owned.cron_jobs, "acp": owned.acp_sessions, "sessions": owned.sessions_repointed, "workspace_moved": workspace_moved, "dirty_paths": dirty_count, "warnings": warnings})), "agent rename persisted but a post-persist side-effect did not follow; re-issue the rename to converge");
    }
...
pub async fn handle_refresh_context_window(
    ...
    let provider_config = {
        ...
        omnesagent_config::schema::ModelProviderConfig {
            model: Some(model),
            uri,
            api_key,
            ..Default::default()
    ...
    let context_window = match omnesagent_providers::fetch_context_window(
        &provider_type,
        &provider_config,
    )
    .await
...
pub async fn handle_patch(
    ...
    let override_drift = headers
        .get("x-omnesagent-override-drift")
                ...
                return error_response(ConfigApiError::new(
                    ...
                    format!(
                        "on-disk config has drifted from in-memory state on \
                         {} path(s) being patched: {}. Send `X-OmnesAgent-Override-Drift: true` \
    ...
    // `omnesagent_log::record!` from `validate()`, surfaced structured so dashboard
    // callers see it.
...
pub async fn handle_migrate(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let migrated = match omnesagent_config::migration::migrate_file(&raw) {
        Ok(out) => out,
        Err(e) => {
            return error_response(ConfigApiError::new(
                ConfigApiCode::ValidationFailed,
    ...
    match migrated {
        Some(new_content) => {
            ...
            let new_cfg: omnesagent_config::schema::Config = match toml::from_str(&new_content) {
                Ok(c) => c,
                Err(e) => {
                    return error_response(ConfigApiError::new(
                        ConfigApiCode::ReloadFailed,
            ...
            *state.config.write() = new_cfg;
            ...
            axum::Json(MigrateResponse {
                ...
                schema_version: omnesagent_config::migration::CURRENT_SCHEMA_VERSION,
            })
            .into_response()
        }
...
pub async fn handle_options_config(headers: HeaderMap) -> Response {
    ...
    schema_response("omnesagent_config_schema_full")
}
...
fn schema_body_value() -> serde_json::Value {
    let schema = schemars::schema_for!(omnesagent_config::schema::Config);
...
mod tests {
    ...
    use omnesagent_runtime::security::pairing::PairingGuard;
    ...
    // omnesagent_config::alias_refs — single source of truth (the gateway and CLI
    ...
    fn temp_config(tmp: &tempfile::TempDir) -> omnesagent_config::schema::Config {
    ...
    fn test_state(config: omnesagent_config::schema::Config) -> AppState {
        ...
        AppState {
            ...
            memory_strategy: Arc::new(
                omnesagent_runtime::agent::memory_strategy::DefaultMemoryStrategy::with_config(
                    memory,
            ...
            canvas_store: omnesagent_runtime::tools::CanvasStore::new(),
    ...
    fn delete_cascade_resolves_custom_workspace_before_removing_entry() {
        ...
        let mut cfg = omnesagent_config::schema::Config::default();
        cfg.agents.insert(
            "victim".to_string(),
        ...
        omnesagent_config::alias_refs::delete_with_cascade(
            &mut cfg,
    ...
    async fn agent_rename_leaves_owned_state_put_when_persist_fails() {
        ...
        let from_agent = omnesagent_config::schema::AliasedAgentConfig {
        ...
        omnesagent_runtime::cron::add_job(&config, "from", "* * * * *", "echo hi")
            .expect("seed cron job");
        assert_eq!(
            omnesagent_runtime::cron::list_jobs_by_agent(&config, "from")
                .unwrap()
                .len(),
            1
        );
        ...
        assert!(
            omnesagent_runtime::cron::list_jobs_by_agent(&config, "to")
                .unwrap()
                .is_empty(),
    ...
    async fn compute_drift_returns_empty_when_in_memory_matches_disk() {
        ...
        let cfg = omnesagent_config::schema::Config {
    ...
    fn scrub_credentials_catches_credential_shaped_strings() {
        use omnesagent_runtime::agent::loop_::scrub_credentials;
    ...
    fn lookup_prop_field_synthesizes_dynamic_http_request_secret_metadata() {
        let cfg = omnesagent_config::schema::Config::default();
        ...
        assert_eq!(
            ...
            Some(omnesagent_config::traits::CredentialSurfaceClass::EncryptedSecret)
        );
    }
    ...
    async fn agent_delete_leaves_owned_state_intact_when_persist_fails() {
        ...
        let agent = omnesagent_config::schema::AliasedAgentConfig {
            risk_profile: "default".into(),
            ..Default::default()
        };
        ...
        omnesagent_runtime::cron::add_job(&config, "victim", "* * * * *", "echo hi")
            .expect("seed cron job");
        assert_eq!(
            omnesagent_runtime::cron::list_jobs_by_agent(&config, "victim")
                .unwrap()
                .len(),
            1
        );
    ...
    fn config_with_telegram_alias(
        ...
        config.channels.telegram.insert(
            alias.to_string(),
            omnesagent_config::schema::TelegramConfig {
                enabled: true,
                bot_token: "test-token".to_string(),
                api_base_url: omnesagent_config::schema::TELEGRAM_OFFICIAL_API_BASE_URL.to_string(),
                ..Default::default()
            },
        );
        config
    ...
    async fn refresh_context_window_forwards_api_key() {
        ...
        cfg.providers.models.groq.insert(
            "test".to_string(),
            omnesagent_config::schema::GroqModelProviderConfig {
