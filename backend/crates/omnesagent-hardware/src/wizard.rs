...
use omnesagent_config::schema::{HardwareConfig, HardwareTransport};
...
pub fn run_setup() -> Result<HardwareConfig> {
    println!(
        "  {} {}",
        style("ℹ").dim(),
        style("OmnesAgent can talk to physical hardware (LEDs, sensors, motors).").dim()
    );
    println!(
        "  {} {}",
        style("ℹ").dim(),
    ...
    let choice = Select::new()
        .with_prompt("  How should OmnesAgent interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
