//! `pico_flash` tool — flash OmnesAgent firmware to a Pico in BOOTSEL mode.
...
impl Tool for PicoFlashTool {
    ...
    fn description(&self) -> &str {
        "Flash OmnesAgent firmware to a Raspberry Pi Pico in BOOTSEL mode. \
                ...
                return Ok(ToolResult {
                    ...
                    error: Some(format!(
                        "UF2 copied to {} but serial port did not appear within {PORT_WAIT_SECS}s. \
                         Unplug and replug the Pico, then restart OmnesAgent.",
                        mount.display()
                    )),
                });
            }
        ...
        match final_port {
            Some(p) => {
                ...
                let suffix = if reconnected {
                    ...
                    "Restart OmnesAgent to reconnect as pico0."
                };
                Ok(ToolResult {
                    success: true,
                    output: format!(
            ...
            None => Ok(ToolResult {
                ...
                output: format!(
                    "Pico flashed successfully. \
                         ...
                         unplug and replug the Pico, then restart OmnesAgent to connect as pico0."
                )
                .into(),
                error: None,
            }),
...
mod tests {
    ...
    async fn execute_with_confirm_true_does_not_panic_when_explicitly_opted_in() {
        const HARDWARE_TEST_OPT_IN: &str = "OMNESAGENT_RUN_PICO_HARDWARE_TESTS";
