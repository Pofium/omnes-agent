//! Deploy OmnesAgent Bridge app to Arduino Uno Q.
...
pub fn setup_uno_q_bridge(host: Option<&str>) -> Result<()> {
    ...
    if let Some(h) = host {
        if bridge_dir.exists() {
            ...
            anyhow::bail!(
                "Bridge app not found at {}. Run from omnesagent repo root.",
                bridge_dir.display()
            );
        }
    } else {
...
fn deploy_remote(host: &str, bridge_dir: &std::path::Path) -> Result<()> {
    ...
    println!("OmnesAgent Bridge app started. Add to config.toml:");
...
fn deploy_local(bridge_dir: Option<&std::path::Path>) -> Result<()> {
    ...
    println!("OmnesAgent Bridge app started.");
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn write_embedded_bridge_main_py_contains_omnesagent() {
        let tmp = tempfile::tempdir().expect("create temp dir");
        let dest = tmp.path();
        ...
        let main_py = std::fs::read_to_string(dest.join("python").join("main.py")).unwrap();
        assert!(
            main_py.contains("OmnesAgent") || main_py.contains("omnesagent"),
            "main.py should contain OmnesAgent marker"
        );
    }
