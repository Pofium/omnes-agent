...
use omnesagent_config::schema::{PeripheralBoardConfig, PeripheralsConfig};
#[cfg(feature = "hardware")]
use omnesagent_tools::hardware_memory_map::HardwareMemoryMapTool;
...
pub async fn create_peripheral_tools(config: &PeripheralsConfig) -> Result<Vec<Box<dyn Tool>>> {
    ...
    if !tools.is_empty() {
        ...
        tools.push(Box::new(
            omnesagent_tools::hardware_board_info::HardwareBoardInfoTool::new(board_names.clone()),
        ));
        tools.push(Box::new(
            omnesagent_tools::hardware_memory_read::HardwareMemoryReadTool::new(board_names),
        ));
    }
...
pub fn create_board_info_tools(config: &PeripheralsConfig) -> Vec<Box<dyn Tool>> {
    ...
    vec![
        Box::new(
            omnesagent_tools::hardware_memory_map::HardwareMemoryMapTool::new(board_names.clone()),
        ),
        Box::new(
            omnesagent_tools::hardware_board_info::HardwareBoardInfoTool::new(board_names.clone()),
        ),
        Box::new(omnesagent_tools::hardware_memory_read::HardwareMemoryReadTool::new(board_names)),
    ]
}
