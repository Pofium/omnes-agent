//! Flash OmnesAgent Nucleo-F401RE firmware via probe-rs.
...
/// Flash OmnesAgent Nucleo firmware. Builds from firmware/nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (omnesagent's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from omnesagent repo root.",
            firmware_dir.display()
        );
    }

    println!("Building OmnesAgent Nucleo firmware...");
    ...
    println!("OmnesAgent Nucleo firmware flashed successfully.");
