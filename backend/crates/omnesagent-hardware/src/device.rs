...
pub enum DeviceKind {
    ...
    Nucleo,
    /// Unknown VID that passed the OmnesAgent firmware ping handshake.
    Generic,
}
...
impl DeviceRegistry {
    ...
    pub async fn discover() -> Self {
        ...
        for info in scan_serial_devices() {
            ...
            let probe_transport = if !is_known_vid {
                ...
                if !probe.ping_handshake().await {
                    ::omnesagent_log::record!(
                        ...
                        "skipping unknown device: no OmnesAgent firmware response"
                    );
                    continue;
                }
                Some(probe)
            ...
            ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"alias": alias, "port": info.port_path, "vid": info.vid})), "device registered");
        }
