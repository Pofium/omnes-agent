...
use omnesagent_api::attribution::ToolKind;
...
impl Tool for DeviceReadCodeTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"alias": alias, "port": port, "runtime": format!("{:?}", runtime)})), "reading main.py from device");
...
impl Tool for DeviceWriteCodeTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"alias": alias, "port": port, "runtime": format!("{:?}", runtime), "code_len": code.len()})), "writing main.py to device");

        // Write code to an atomic, owner-only temp file via tempfile crate.
        let named_tmp = match tokio::task::spawn_blocking(|| {
            tempfile::Builder::new()
                .prefix("omnesagent_main_")
                .suffix(".py")
                .tempfile()
        })
        .await
...
impl Tool for DeviceExecTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"alias": alias, "port": port, "runtime": format!("{:?}", runtime), "code_len": code.len()})), "executing snippet on device");

        // Write snippet to an atomic, owner-only temp file via tempfile crate.
        let named_tmp = match tokio::task::spawn_blocking(|| {
            tempfile::Builder::new()
                .prefix("omnesagent_exec_")
                .suffix(".py")
                .tempfile()
        })
        .await
