...
/// Manages device datasheet files in `~/.omnesagent/hardware/datasheets/`.
...
impl DatasheetManager {
    /// Create a manager rooted at the default OmnesAgent datasheets directory.
    pub fn new() -> Option<Self> {
        let home = directories::BaseDirs::new()?.home_dir().to_path_buf();
        Some(Self {
            datasheet_dir: home.join(".omnesagent").join("hardware").join("datasheets"),
        })
    }
    ...
    /// The file is saved as `~/.omnesagent/hardware/datasheets/<device_name>.pdf`.
    ...
    pub async fn download_datasheet(
        ...
        let client = reqwest::Client::builder()
            .user_agent("OmnesAgent/0.1 (datasheet downloader)")
...
impl Default for DatasheetManager {
    fn default() -> Self {
        Self::new().unwrap_or_else(|| Self {
            datasheet_dir: PathBuf::from(".omnesagent/hardware/datasheets"),
        })
    }
}
        ...
        match action.as_str() {
                    ...
                    Ok(path) => Ok(ToolResult {
                        ...
                        output: format!(
                            "Datasheet for '{device}' downloaded successfully.\n\
                             ...
                             ~/.omnesagent/hardware/devices/<device>.md with the key \
            ...
            "list" => {
                ...
                let output = if datasheets.is_empty() {
                    ...
                    format!(
                        "{} cached datasheet(s) in ~/.omnesagent/hardware/datasheets/:\n{}",
                        datasheets.len(),
                        datasheets
                            .iter()
                            .map(|n| format!("  - {n}"))
