...
impl RpiSystemContext {
    ...
    /// Write an `rpi0.md` hardware context file to `~/.omnesagent/hardware/devices/`.
    ...
    pub fn write_hardware_context_file(&self) {
        ...
        let devices_dir = home.join(".omnesagent").join("hardware").join("devices");
    ...
    fn device_profile_markdown(&self) -> String {
        ...
        let _ = writeln!(
            s,
            "- Runtime: OmnesAgent native (rppal — no serial, no mpremote)"
        );
        if let Some(ref iface) = self.wifi_interface {
            let _ = writeln!(s, "- WiFi interface: {}", iface);
        }
