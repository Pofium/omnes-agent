//! Plugin manifest loader — scans `~/.omnesagent/tools/` at startup.
...
/// Return the path `~/.omnesagent/tools/` using the `directories` crate.
pub fn plugin_tools_dir() -> Result<PathBuf> {
    ...
    Ok(base.home_dir().join(".omnesagent").join("tools"))
}
...
"#;
    ...
    fn scan_plugin_dir_skips_broken_manifests_without_panicking() {
        ...
        // always uses ~/.omnesagent/tools), but we can verify load_one_plugin
    ...
    #[test]
    fn plugin_tools_dir_returns_path_ending_in_omnesagent_tools() {
        let path = plugin_tools_dir().expect("should resolve");
        let display = path.to_string_lossy();
        let expected = std::path::Path::new(".omnesagent").join("tools");
