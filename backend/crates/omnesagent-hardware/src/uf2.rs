...
const PICO_UF2: &[u8] = include_bytes!(concat!(
    ...
    "/../../firmware/pico/omnesagent-pico.uf2"
));
...
pub fn ensure_firmware_dir() -> Result<PathBuf> {
    ...
    let uf2_path = firmware_dir.join("omnesagent-pico.uf2");
    if !uf2_path.exists() {
        if PICO_UF2.len() < 8 || PICO_UF2[..4] != UF2_MAGIC1 {
            bail!(
                "Bundled UF2 is a placeholder — download the real MicroPython UF2 from \
                 ...
                 src/firmware/pico/omnesagent-pico.uf2, then rebuild OmnesAgent."
            );
        }
...
pub async fn flash_uf2(mount_point: &Path, firmware_dir: &Path) -> Result<()> {
    let uf2_src = firmware_dir.join("omnesagent-pico.uf2");
    ...
    if data.len() < 8 || data[..4] != UF2_MAGIC1 {
        bail!(
            "UF2 at {} does not look like a valid UF2 file (magic mismatch). \
             ...
             the existing file so OmnesAgent can re-extract it.",
            uf2_src.display()
        );
    }
    ...
    // ── Attempt 1: std::fs::copy (works on Linux, sometimes blocked on macOS) ─
    {
        ...
        match result {
            ...
            Ok(Err(e)) => ::omnesagent_log::record!(
                WARN,
                ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note)
    ...
    bail!(
        "All copy methods failed. Run this command manually, then restart OmnesAgent:\n\
...
mod tests {
    ...
    fn ensure_firmware_dir_creates_directory() {
        // This test verifies ensure_firmware_dir creates the ~/.omnesagent/firmware/pico/ path.
    ...
    async fn flash_uf2_rejects_invalid_magic() {
        ...
        std::fs::write(firmware_dir.join("omnesagent-pico.uf2"), b"NOT_A_UF2_FILE").unwrap();
    ...
    async fn flash_uf2_rejects_too_small_file() {
        ...
        std::fs::write(firmware_dir.join("omnesagent-pico.uf2"), b"tiny").unwrap();
