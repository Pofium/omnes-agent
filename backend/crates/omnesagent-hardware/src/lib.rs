...
pub use omnesagent_config::schema::{HardwareConfig, HardwareTransport};
...
pub fn merge_hardware_tools(
    tools: &mut Vec<Box<dyn omnesagent_api::tool::Tool>>,
    ...
    if !hw_boot.tools.is_empty() {
        ...
        let new_hw_tools: Vec<Box<dyn omnesagent_api::tool::Tool>> = hw_boot
            .tools
            .into_iter()
...
pub struct HardwareBootResult {
    ...
    pub tools: Vec<Box<dyn omnesagent_api::tool::Tool>>,
    /// Human-readable device summary for the LLM system prompt.
    pub device_summary: String,
    /// Content of `~/.omnesagent/hardware/` context files (HARDWARE.md, device
...
pub fn load_hardware_context_prompt(aliases: &[&str]) -> String {
    ...
    load_hardware_context_from_dir(&home.join(".omnesagent").join("hardware"), aliases)
}
...
pub async fn boot(
    peripherals: &omnesagent_config::schema::PeripheralsConfig,
    ...
    if uf2::find_rpi_rp2_mount().is_some() {
        ...
        ::omnesagent_log::record!(
            INFO,
            ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note),
            "Say \"flash my pico\" to install OmnesAgent firmware automatically"
        );
    }
...
#[allow(unused_mut)] // tools and context_files_prompt are mutated on Linux+peripheral-rpi
pub async fn boot(
    _peripherals: &omnesagent_config::schema::PeripheralsConfig,
) -> anyhow::Result<HardwareBootResult> {
...
pub fn run_info(chip: &str) -> Result<()> {
    ...
    #[cfg(not(feature = "probe"))]
    {
        ...
        println!();
        println!("Then run: omnesagent hardware info --chip {}", chip);
        println!();
