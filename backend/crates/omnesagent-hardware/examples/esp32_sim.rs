...
//! `firmware/esp32/src/main.rs`, so a host OmnesAgent daemon can drive virtual
...
use omnesagent_hardware::util::serial_open_baud;
#[cfg(unix)]
use omnesagent_hardware::util::should_open_serial_nonexclusive;
...
async fn main() -> Result<()> {
    ...
    let http_handle = omnesagent_spawn::spawn!(async move {
        if let Err(e) = run_http_server(http_state, bind_for_http).await {
            eprintln!("http server crashed: {}", e);
        }
    });

    let pty_handle = omnesagent_spawn::spawn!(async move {
