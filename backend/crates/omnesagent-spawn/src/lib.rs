//! `omnesagent-spawn` — the sanctioned interface against `tokio::spawn` for
//! the OmnesAgent workspace.
...
/// callers needing `tokio`, `tracing`, or `omnesagent_log` as direct
/// dependencies.
#[doc(hidden)]
pub mod __private {
    pub use ::serde_json;
...
macro_rules! spawn {
    ($body:expr) => {{
        ...
        // entered.
        $crate::__private::omnesagent_log::record!(
            INFO,
            $crate::__private::omnesagent_log::Event::new(
                $crate::TASK_EVENT_NAME,
                $crate::__private::omnesagent_log::Action::Spawn,
            )
        ...
        let __zc_task_future = async move {
            ...
            $crate::__private::omnesagent_log::record!(
                ...
                $crate::__private::omnesagent_log::Event::new(
                    $crate::TASK_EVENT_NAME,
                    $crate::__private::omnesagent_log::Action::Complete,
                )
                .with_outcome($crate::__private::omnesagent_log::EventOutcome::Success)
...
mod tests {
    ...
    /// without depending on `omnesagent-log` formatting.
