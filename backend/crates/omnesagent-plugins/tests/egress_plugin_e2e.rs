...
//! `wasi:http` call** into OmnesAgent's hooks. Nothing is stubbed: the deny path
        ...
        .get_or_init(|| {
            ...
            let status = Command::new(env!("CARGO"))
                ...
                .args([
                    ...
                    "--quiet",
                    "--package",
                    "omnesagent-egress-plugin-fixture",
                    "--target",
                    "wasm32-wasip2",
                    "--target-dir",
                ])
            ...
            let wasm = target_dir.join("wasm32-wasip2/debug/omnesagent_egress_plugin_fixture.wasm");
...
async fn probe(url: &str, follow: bool, egress: Option<EgressHostService>) -> String {
    ...
    let mut plugin = omnesagent_plugins::runtime::create_plugin_with_egress(
        &fixture(),
        &scope,
        &services,
        limits(),
    ...
    let result = omnesagent_plugins::runtime::call_execute(&mut plugin, args.as_bytes())
        .await
...
/// `InternalError(Some("omnesagent plugin egress policy: ..."))`.
