...
//! `OMNESAGENT_CONFIG_DIR` with the README's install layout, loads the real
        ...
        .get_or_init(|| {
            ...
            let status = Command::new(env!("CARGO"))
                ...
                .args([
                    ...
                    "--quiet",
                    "--package",
                    "omnesagent-tool-plugin-fixture",
                    "--target",
                    "wasm32-wasip2",
                    "--target-dir",
                ])
            ...
            let wasm = target_dir.join("wasm32-wasip2/debug/omnesagent_tool_plugin_fixture.wasm");
...
/// Lay out a disposable config dir the way `omnesagent plugin install` does.
///
...
async fn reference_plugin_end_to_end_from_throwaway_config() {
    ...
    let prev = std::env::var("OMNESAGENT_CONFIG_DIR").ok();
    unsafe { std::env::set_var("OMNESAGENT_CONFIG_DIR", tmp.path()) };
    ...
    match prev {
        Some(v) => unsafe { std::env::set_var("OMNESAGENT_CONFIG_DIR", v) },
        None => unsafe { std::env::remove_var("OMNESAGENT_CONFIG_DIR") },
    }
