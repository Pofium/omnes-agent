...
use omnesagent_plugins::component::PluginLimits;
use omnesagent_plugins::config::{PluginConfigResolver, resolve_plugin_config};
use omnesagent_plugins::egress::{EgressHostService, EgressPolicy, EgressPolicyResolver};
use omnesagent_plugins::host::PluginHost;
use omnesagent_plugins::instance::PluginInstanceScope;
use omnesagent_plugins::runtime;
use omnesagent_plugins::services::PluginHostServices;
use omnesagent_plugins::{PluginCapability, PluginPermission};
        ...
        .get_or_init(|| {
            ...
            let status = Command::new(env!("CARGO"))
                ...
                .args([
                    ...
                    "--quiet",
                    "--package",
                    "omnesagent-tool-timeout-fixture",
                    "--target",
                    "wasm32-wasip2",
                    "--target-dir",
                ])
            ...
            let wasm = target_dir.join("wasm32-wasip2/debug/omnesagent_tool_timeout_fixture.wasm");
...
) -> anyhow::Result<omnesagent_api::tool::ToolResult> {
...
async fn blocked_host_log_writer_cannot_stall_export_or_exhaust_host_memory() {
    ...
    struct E2eAgent;
    impl omnesagent_api::attribution::Attributable for E2eAgent {
        fn role(&self) -> omnesagent_api::attribution::Role {
            omnesagent_api::attribution::Role::Agent
        }
        fn alias(&self) -> &str {
            ATTRIB_ALIAS
        }
    ...
    let installed = omnesagent_log::try_install_line_sink_for_tests(move |line| {
    ...
    let mut structured_rx = omnesagent_log::subscribe_or_install();
    let span = omnesagent_log::attribution_span!(&E2eAgent);
    let result = omnesagent_log::Instrument::instrument(
        execute(
            &mut flood_plugin,
