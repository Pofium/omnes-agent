...
use omnesagent_plugins::component::{HostInboundMessage, PluginLimits};
use omnesagent_plugins::config::{PluginConfigResolver, resolve_plugin_config};
use omnesagent_plugins::endpoint::PluginChannelEndpoint;
use omnesagent_plugins::instance::PluginInstanceScope;
use omnesagent_plugins::services::PluginHostServices;
use omnesagent_plugins::wasm_channel::WasmChannel;
use omnesagent_plugins::{PluginCapability, PluginManifest, PluginPermission};
        ...
        .get_or_init(|| {
            ...
            let status = Command::new(env!("CARGO"))
                ...
                .args([
                    ...
                    "--quiet",
                    "--package",
                    "omnesagent-channel-plugin-fixture",
                    "--target",
                    "wasm32-wasip2",
                    "--target-dir",
                ])
            ...
            let wasm = target_dir.join("wasm32-wasip2/debug/omnesagent_channel_plugin_fixture.wasm");
...
async fn externally_cancelled_channel_call_discards_store_and_recreates() {
    ...
    let call = ::omnesagent_spawn::spawn!(async move {
        cancelled_channel
...
async fn reconstruction_replays_the_constructor_generation_config_snapshot() {
    ...
    let channel = channel_with(
        "config-generation",
        vec![
            omnesagent_plugins::PluginPermission::HttpClient,
...
async fn interrupted_poll_preserves_backlog_but_not_the_dequeued_message() {
    ...
    let channel = channel_with(
        "at-most-once",
        vec![omnesagent_plugins::PluginPermission::HttpClient],
        &HashMap::new(),
