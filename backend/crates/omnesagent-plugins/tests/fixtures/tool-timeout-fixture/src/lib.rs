...
mod component {
    ...
    use exports::omnesagent::plugin::plugin_info::Guest as PluginInfo;
    use exports::omnesagent::plugin::tool::{Guest as Tool, ToolResult};
    use waki::bindings::wasi::http::{
        outgoing_handler,
        types::{ErrorCode, Fields, OutgoingBody, OutgoingRequest, RequestOptions, Scheme},
    };
    use omnesagent::plugin::logging::{LogLevel, PluginAction, PluginEvent, PluginOutcome};
    ...
    fn log_record_note(message: &str) {
        omnesagent::plugin::logging::log_record(
            LogLevel::Info,
            &PluginEvent {
                function_name: "omnesagent_tool_timeout_fixture::execute".to_string(),
