...
use crate::component::bindings::tool::exports::omnesagent::plugin::tool::ToolResult as WitToolResult;
use crate::component::{
...
pub async fn call_tool_metadata(plugin: &mut Plugin) -> Result<ToolMetadata> {
    let name = call_plugin!(
        ...
        async move |store: &mut Store<PluginState>, bindings: &mut ToolPlugin| {
            wt(
                bindings.omnesagent_plugin_tool().call_name(store).await,
                "tool.name failed",
            )
        }
    )?;
