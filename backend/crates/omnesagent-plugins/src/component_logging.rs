...
use omnesagent_log::{Action, Event, EventOutcome, record};
...
macro_rules! impl_host {
    ($world:ident) => {
        impl bindings::$world::omnesagent::plugin::types::Host for PluginState {}

        impl bindings::$world::omnesagent::plugin::logging::Host for PluginState {
            async fn log_record(
                &mut self,
                level: bindings::$world::omnesagent::plugin::logging::LogLevel,
                event: bindings::$world::omnesagent::plugin::logging::PluginEvent,
            ) {
                if !self.charge_host_call() {
                    return;
                }
                use bindings::$world::omnesagent::plugin::logging::{
                ...
                enqueue_plugin_log(QueuedPluginLog {
                    span: omnesagent_log::Span::current(),
...
impl bindings::channel::omnesagent::plugin::inbound::Host for PluginState {
    async fn inbound_poll(
        &mut self,
    ) -> Option<bindings::channel::omnesagent::plugin::inbound::HostInboundMessage> {
        if !self.charge_host_call() {
            return None;
        }
        self.inbound().poll().map(|m| {
            bindings::channel::omnesagent::plugin::inbound::HostInboundMessage {
                id: m.id,
                sender: m.sender,
