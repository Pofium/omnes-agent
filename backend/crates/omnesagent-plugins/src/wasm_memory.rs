//! Memory adapter: `WasmMemory` implements `omnesagent_api::memory_traits::Memory`
...
use crate::component::bindings::memory::exports::omnesagent::plugin::memory::{
...
use omnesagent_api::attribution::{Attributable, MemoryKind, Role};
use omnesagent_api::memory_traits::{
...
impl Memory for WasmMemory {
    ...
    async fn count(&self) -> Result<usize> {
        call_plugin!(
            ...
            async move |store: &mut Store<PluginState>, bindings: &mut MemoryPlugin| {
                let out = wt(
                    bindings.omnesagent_plugin_memory().call_count(store).await,
    ...
    async fn reindex(&self) -> Result<usize> {
        ...
        call_plugin!(
            ...
            async move |store: &mut Store<PluginState>, bindings: &mut MemoryPlugin| {
                let out = wt(
                    bindings.omnesagent_plugin_memory().call_reindex(store).await,
