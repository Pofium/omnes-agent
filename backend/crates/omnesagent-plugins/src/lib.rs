//! WASM plugin system for OmnesAgent.
//! Plugins are WebAssembly components loaded via wasmtime that can extend
//! OmnesAgent with custom tools and channels. Enable with a `plugins-wasm*` feature.
...
pub struct PluginEgressDeclaration {
    ...
    /// (`omnesagent_infra::net_guard::normalize_egress_pattern`). Invalid grammar
