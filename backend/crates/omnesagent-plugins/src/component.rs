...
/// Hard safety ceiling for OmnesAgent-owned WIT imports in one service frame.
///
...
/// OmnesAgent's policy hooks. They are one field because `WasiHttpCtxView` needs
...
impl PluginState {
    ...
    /// Charge one OmnesAgent-owned host import against the active call budget.
...
impl WasiHttpView for PluginState {
    /// Hand `wasi:http` OmnesAgent's policy hooks instead of
...
mod tests {
    ...
    async fn logging_and_inbound_share_the_frame_budget() {
        use bindings::tool::omnesagent::plugin::logging::{
        ...
        assert_eq!(
            <PluginState as bindings::channel::omnesagent::plugin::inbound::Host>::inbound_pending(
                &mut state
            )
            .await,
            0
        ...
        state.start_call(PluginCallPhase::Standard);
        let message =
            <PluginState as bindings::channel::omnesagent::plugin::inbound::Host>::inbound_poll(
                &mut state,
            )
            .await
    ...
    fn wt_instantiate_preserves_the_chain_and_appends_the_drift_hint() {
        ...
        let err =
            wasmtime::Error::msg("type mismatch for `plugin-action`: expected 5 cases, found 4")
                .context("while linking omnesagent:plugin/logging");
        ...
        assert!(
            rendered.contains("while linking omnesagent:plugin/logging"),
