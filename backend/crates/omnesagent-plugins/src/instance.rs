...
///
/// The grammar itself lives in `omnesagent_api::plugin`, which configuration
...
pub(crate) fn validate_package_name(name: &str) -> Result<(), String> {
    omnesagent_api::plugin::validate_plugin_package_name(name).map_err(|error| error.to_string())
}
