        ...
        (Some(sig), Some(pub_key)) => {
            ...
            match &result {
                VerificationResult::Valid { publisher_key } => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"plugin": plugin_name, "publisher_key": publisher_key.as_str()})), "plugin signature verified");
                    Ok(result)
                }
                VerificationResult::Untrusted => match mode {
                    ...
                    SignatureMode::Permissive => {
                        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"plugin": plugin_name, "publisher_key": pub_key})), "plugin publisher key not trusted; loading in permissive mode");
                        Ok(result)
                    }
                ...
                VerificationResult::Invalid { reason } => match mode {
                    ...
                    SignatureMode::Permissive => {
                        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"plugin": plugin_name, "reason": reason.as_str()})), "plugin signature invalid; loading in permissive mode");
                        Ok(result)
                    }
                    SignatureMode::Disabled => Ok(result),
                },
