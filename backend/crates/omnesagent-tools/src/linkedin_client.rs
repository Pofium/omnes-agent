...
use omnesagent_config::schema::LinkedInImageConfig;
    ...
    pub async fn generate(&self, prompt: &str) -> anyhow::Result<PathBuf> {
        ...
        for provider_name in &self.config.providers {
            ...
            match result {
                ...
                Err(e) => {
                    ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", e), "provider_name": provider_name})), "Image model_provider '' failed");
                }
            }
        }
...
mod tests {
    ...
    fn fallback_card_contains_svg_structure() {
        ...
        assert!(svg.contains("OmnesAgent"));
    }
