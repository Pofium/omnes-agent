...
use std::sync::Arc;
use omnesagent_api::channel::ForgeApiRequest;
...
impl GitForgeTool {
    ...
    fn resolve_channel(
        &self,
        name: &str,
    ) -> Result<Arc<dyn omnesagent_api::channel::Channel>, String> {
...
mod tests {
    ...
    impl ::omnesagent_api::attribution::Attributable for ForgeMock {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Channel(
                ::omnesagent_api::attribution::ChannelKind::Git,
            )
        }
        fn alias(&self) -> &str {
            "test"
