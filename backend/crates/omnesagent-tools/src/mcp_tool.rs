//! Wraps a discovered MCP tool as a omnesagent [`Tool`] so it is dispatched
...
use omnesagent_api::tool::{Tool, ToolOutput, ToolResult, ToolSpec};
use omnesagent_config::policy::SecurityPolicy;

/// A omnesagent [`Tool`] backed by an MCP server tool.
...
mod tests {
    ...
    // OmnesAgent's security model injects `approved: bool` into built-in tool args.
