...
impl Tool for SessionsSendTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let chat_msg = omnesagent_api::model_provider::ChatMessage::user(message);
...
impl Tool for SessionsCurrentTool {
    ...
    async fn execute(&self, _args: serde_json::Value) -> anyhow::Result<ToolResult> {
        let session_key = omnesagent_api::TOOL_LOOP_SESSION_KEY
...
mod tests {
    ...
    use omnesagent_infra::session_backend::SessionMetadata;
    ...
    async fn sessions_current_returns_key_when_scoped() {
        ...
        let result = omnesagent_api::TOOL_LOOP_SESSION_KEY
    ...
    async fn sessions_current_includes_name() {
        let tmp = TempDir::new().unwrap();
        let sqlite = omnesagent_infra::session_sqlite::SqliteSessionBackend::new(tmp.path()).unwrap();
