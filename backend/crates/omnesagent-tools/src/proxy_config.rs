...
impl ProxyConfigTool {
    ...
    fn parse_scope(raw: &str) -> Option<ProxyScope> {
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            "omnesagent" | "internal" | "core" => Some(ProxyScope::Omnesagent),
    ...
    fn dns_pinned_tool_warnings(
        config: &Config,
    ) -> Vec<omnesagent_config::validation_warnings::ValidationWarning> {
        config
            .collect_warnings()
            .into_iter()
    ...
    async fn handle_set(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        if let Some(scope_raw) = args.get("scope") {
            ...
            proxy.scope = Self::parse_scope(scope).ok_or_else(|| {
                ...
                anyhow::Error::msg(format!(
                    "Invalid scope '{scope}'. Use environment|omnesagent|services"
                ))
            })?;
        }
...
impl Tool for ProxyConfigTool {
    ...
    fn description(&self) -> &str {
        "Manage OmnesAgent proxy settings (scope: environment | omnesagent | services), including runtime and process env application"
    }
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "scope": {
                    "type": "string",
                    "description": "Proxy scope: environment | omnesagent | services"
                },
                "http_proxy": {
                    "type": ["string", "null"],
