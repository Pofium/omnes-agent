...
mod tests {
    ...
    async fn file_write_creates_file() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write");
    ...
    async fn file_write_creates_parent_dirs() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_nested");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_normalizes_workspace_prefixed_relative_path() {
        let root = std::env::temp_dir().join("omnesagent_test_file_write_workspace_prefixed");
    ...
    async fn file_write_overwrites_existing() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_overwrite");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_blocks_path_traversal() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_traversal");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_base64_writes_decoded_bytes() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_base64");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_base64_invalid_content_errors() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_base64_invalid");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_unsupported_encoding_errors() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_bad_encoding");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_rejected_encoding_does_not_create_parent_dirs() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_no_dir_on_reject");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_base64_still_blocks_path_traversal() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_base64_traversal");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_empty_content() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_empty");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("omnesagent_test_file_write_symlink_escape");
        let workspace = root.join("workspace");
    ...
    async fn file_write_blocks_ephemeral_runtime() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_ephemeral");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_blocks_readonly_mode() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_readonly");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_blocks_symlink_target_file() {
        ...
        let root = std::env::temp_dir().join("omnesagent_test_file_write_symlink_target");
        let workspace = root.join("workspace");
        let outside = root.join("outside");
    ...
    async fn file_write_absolute_path_in_workspace() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_abs_path");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_blocks_null_byte_in_path() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_write_null");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_write_blocks_path_outside_workspace() {
        let root = std::env::temp_dir().join("omnesagent_test_file_write_outside_workspace");
        let workspace = root.join("workspace");
