...
//!
//! The primitives themselves live in [`omnesagent_infra::net_guard`] so the tool
//! implementations and `omnesagent-channels` read one implementation. This
//! module is the tool-layer facade: it keeps the
//! `crate::helpers::domain_guard::*` paths the tools and `omnesagent-channels`
...
//! beside them in `omnesagent-infra`.

// ── allowlist normalization and matching ──────────────────────────
pub use omnesagent_infra::net_guard::{
...
pub use omnesagent_infra::net_guard::is_private_or_local_host;
...
pub use omnesagent_infra::net_guard::{Nat64Prefix, parse_nat64_prefixes};
...
// moved to omnesagent-infra, so the tool crate's public surface is unchanged.
pub(crate) use omnesagent_infra::net_guard::{
