...
impl Microsoft365Tool {
    pub fn new(
        config: types::Microsoft365ResolvedConfig,
        security: Arc<SecurityPolicy>,
        omnesagent_dir: &std::path::Path,
    ) -> anyhow::Result<Self> {
        let http_client = omnesagent_config::schema::build_runtime_proxy_client_with_timeouts(
            "tool.microsoft365",
            60,
            10,
        );
        let token_cache = Arc::new(auth::TokenCache::new(config.clone(), omnesagent_dir)?);
        Ok(Self {
            config,
            security,
            token_cache,
