...
impl StdioTransport {
    ...
    fn spawn(
        ...
        let server_name = config.name.clone();
        let reader = omnesagent_spawn::spawn!(stdio_read_loop(
            server_name,
            generation,
            stdout,
            pending,
        ...
        let exit_watcher =
            omnesagent_spawn::spawn!(stdio_child_exit_watcher(watcher_child, watcher_flag,));
...
impl SseTransport {
    ...
    async fn ensure_connected(&self) -> Result<SseStreamState> {
        ...
        conn.reader_task = Some(omnesagent_spawn::spawn!(async move {
            let stream = resp
                .bytes_stream()
                .map(|item| item.map_err(std::io::Error::other));
            let reader = tokio_util::io::StreamReader::new(stream);
...
mod tests {
    ...
    fn test_ca_file() -> tempfile::NamedTempFile {
        ...
        let mut params = CertificateParams::new(vec!["OmnesAgent MCP test CA".into()]).unwrap();
    ...
    async fn spawn_test_tls_server_with_behavior(
        ...
        let mut ca_params = CertificateParams::new(vec!["OmnesAgent MCP test CA".into()]).unwrap();
        ...
        let task = ::omnesagent_spawn::spawn!(async move {
            ...
            for _ in 0..connection_count {
                ...
                handlers.push(::omnesagent_spawn::spawn!(serve_test_tls_connection(
                    stream,
                    connection_acceptor,
                    addr,
                    connection_behavior,
    ...
    async fn stdio_close_delivers_eof_before_killing_the_server() {
        let marker =
            std::env::temp_dir().join(format!("omnesagent_stdio_eof_marker_{}", std::process::id()));
    ...
    async fn sse_pre_write_cancellation_does_not_leak_pending_waiter() {
        ...
        let reader = omnesagent_spawn::spawn!(std::future::pending::<()>());
        {
    ...
    fn both_remote_transports_fail_closed_for_missing_custom_ca() {
        for transport in [McpTransport::Http, McpTransport::Sse] {
            let config = McpServerConfig {
                ...
                transport,
                url: Some("https://localhost/mcp".into()),
                tls_ca_cert_path: Some("/nonexistent/omnesagent-internal-ca.pem".into()),
                ..Default::default()
            };
            let error = create_transport(&config)
                .err()
            ...
            let message = error.to_string();
            assert!(message.contains("internal"));
            assert!(message.contains("/nonexistent/omnesagent-internal-ca.pem"));
    ...
    fn create_transport_stdio_fails_without_valid_command() {
        ...
        let config = McpServerConfig {
            ...
            transport: McpTransport::Stdio,
            command: "/usr/bin/omnesagent_nonexistent_binary_abc123".into(),
            ..Default::default()
        };
