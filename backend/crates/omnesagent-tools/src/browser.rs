    ...
    async fn execute_computer_use_action(
        ...
        let payload = json!({
            ...
            "metadata": {
                ...
                "source": "omnesagent.browser",
                "version": env!("CARGO_PKG_VERSION"),
            }
        });

        let client = omnesagent_config::schema::build_runtime_proxy_client("tool.browser");
...
mod tests {
    ...
    use omnesagent_config::policy::AutonomyLevel;
    ...
    async fn computer_use_dispatch_writes_validated_png_locally_without_forwarding_path() {
        ...
        // sidecar. The sidecar returns PNG bytes and OmnesAgent performs the
        ...
        // OmnesAgent performed the validated local write: the pre-existing file
