...
use std::sync::Arc;
use omnesagent_config::schema::McpServerConfig;
...
pub fn wrap_resource_contents(
    ...
    for c in &contents.contents {
        if let Some(text) = &c.text {
            body.push_str(&omnesagent_providers::sanitize_api_error(text));
            body.push('\n');
...
pub fn render_prompt_messages(
    ...
    for m in &result.messages {
        ...
        let scrubbed = omnesagent_providers::sanitize_api_error(&text);
