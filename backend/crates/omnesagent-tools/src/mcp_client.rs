...
fn check_result_is_error(result: &serde_json::Value, op: &str, server_name: &str) -> Result<()> {
    ...
    let detail = omnesagent_providers::sanitize_api_error(&detail);
    ...
    fn start_recovery(
        ...
        omnesagent_spawn::spawn!(async move {
            ...
            if let Err(error) = &result {
                ::omnesagent_log::record!(
                    ERROR,
                    ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Reconnect)
                        .with_outcome(::omnesagent_log::EventOutcome::Failure)
                        .with_attrs(::serde_json::json!({
                            "operation": operation,
                            "error": format!("{error:#}"),
        ...
        loop {
            ...
            match send_result {
                Err(_) => {
                    ...
                    ::omnesagent_log::record!(
                        WARN,
                        ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Timeout)
                            .with_outcome(::omnesagent_log::EventOutcome::Failure)
                            .with_attrs(::serde_json::json!({
                                "mcp_server": &server_name,
...
impl McpRegistry {
    ...
    /// `omnesagent-runtime::daemon`) enable the feature via their
...
mod tests {
    ...
    use omnesagent_config::schema::McpTransport;
    ...
    async fn connect_nonexistent_command_fails_cleanly() {
        ...
        let config = McpServerConfig {
            ...
            command: "/usr/bin/this_binary_does_not_exist_omnesagent_test".to_string(),
            args: vec![],
    ...
    async fn cancellation_before_write_does_not_reset_or_replay() {
        ...
        let call =
            omnesagent_spawn::spawn!(async move { call_server.call_tool("test", json!({})).await });
    ...
    async fn mcp_tool_wrapper_execute_materializes_blob_and_preserves_full_result() {
        ...
        use omnesagent_api::tool::Tool as _;
        ...
        let security = Arc::new(omnesagent_config::policy::SecurityPolicy {
...
"#,
        ...
        let first_server = server.clone();
        let first =
            omnesagent_spawn::spawn!(async move { first_server.call_tool("A", json!({})).await });
        assert_eq!(read_fifo(&first_received).await.trim(), "ready");
        let second_server = server.clone();
        let second =
            omnesagent_spawn::spawn!(async move { second_server.call_tool("B", json!({})).await });
