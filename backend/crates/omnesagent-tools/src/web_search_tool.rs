...
impl WebSearchTool {
    ...
    fn resolve_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        if let Some(ref key) = self.boot_brave_api_key
            && !key.is_empty()
            && !omnesagent_config::secrets::SecretStore::is_encrypted(key)
        {
            return Ok(key.clone());
        }
    ...
    fn reload_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        if omnesagent_config::secrets::SecretStore::is_encrypted(&raw_key) {
            let omnesagent_dir = self.config_path.parent().unwrap_or_else(|| Path::new("."));
            let store =
    ...
    async fn search_duckduckgo_at(
        ...
        let builder =
            omnesagent_config::schema::apply_runtime_proxy_to_builder(builder, "tool.web_search");
    ...
    async fn search_jina(&self, query: &str) -> anyhow::Result<String> {
        ...
        let builder = reqwest::Client::builder()
            .timeout(Duration::from_secs(self.timeout_secs))
            .user_agent("OmnesAgent/1.0 (https://omnesagent.ai)");
        let builder =
            omnesagent_config::schema::apply_runtime_proxy_to_builder(builder, "tool.web_search");
        let client = builder.build()?;
...
/// UA is a trivially stable fingerprint across every OmnesAgent install.
...
/// pool member, so there is no "unlucky value" that yields a OmnesAgent-branded
/// or empty header.
...
/// var, so the env route is the generic `OMNESAGENT_<path>` override grammar
/// (`.` -> `__`) implemented in `omnesagent-config::env_overrides`.
...
mod tests {
    ...
    /// the generic `OMNESAGENT_<path>` override grammar.
    #[test]
    fn searxng_missing_url_message_names_only_real_surfaces() {
        ...
        assert!(message.contains("OMNESAGENT_web_search__searxng_instance_url"));
        assert!(
    ...
    #[test]
    fn duckduckgo_user_agents_look_like_browsers_not_omnesagent() {
        assert!(
    ...
    async fn throttle_serializes_concurrent_scrapes() {
        ...
        for _ in 0..3 {
            ...
            handles.push(omnesagent_spawn::spawn!(async move {
                throttle.acquire(gap).await;
                started.elapsed()
            }));
        }
