...
use omnesagent_memory::knowledge_graph::{KnowledgeGraph, NodeType, Relation};
...
impl KnowledgeTool {
    ...
    fn client_node(
        ...
        action: &'static str,
    ) -> anyhow::Result<omnesagent_memory::knowledge_graph::KnowledgeNode> {
        let client_id = args
            .get("client_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
    ...
    fn client_interactions(
        ...
        client_id: &str,
        limit: usize,
    ) -> anyhow::Result<Vec<omnesagent_memory::knowledge_graph::KnowledgeNode>> {
...
fn neighbor_json(
    node: &omnesagent_memory::knowledge_graph::KnowledgeNode,
    relation: Relation,
) -> serde_json::Value {
    json!({
        "id": &node.id,
...
mod tests {
    ...
    use omnesagent_memory::knowledge_graph::KnowledgeGraph;
