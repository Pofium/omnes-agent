...
/// `omnesagent-runtime` so the dependency direction (runtime -> tools) stays
...
fn detect_locale() -> String {
    ...
    if let Some(base) = directories::BaseDirs::new() {
        let candidates = [
            base.home_dir().join(".omnesagent/config.toml"),
            base.config_dir().join("omnesagent/config.toml"),
        ];
...
/// `omnesagent-runtime`'s `i18n::detect_locale`: `sys-locale` is
