...
use omnesagent_config::schema::{ProxyConfig, ProxyScope};
...
impl HttpRequestTool {
    ...
    fn reload_auth_secret(&self, secret_name: &str) -> anyhow::Result<String> {
        ...
        let config: omnesagent_config::schema::Config = toml::from_str(&contents).map_err(|e| {
            anyhow::Error::msg(format!(
        ...
        let secret = if omnesagent_config::secrets::SecretStore::is_encrypted(raw_secret) {
            let omnesagent_dir = config_path.parent().unwrap_or_else(|| Path::new("."));
            let store =
                omnesagent_config::secrets::SecretStore::new(omnesagent_dir, self.secrets_encrypt);
...
impl Tool for HttpRequestTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // registration time (see omnesagent-runtime::tools::mod).

        let proxy_config = omnesagent_config::schema::runtime_proxy_config();
        ...
        let ignored_environment_proxy = omnesagent_config::schema::environment_proxy_for_url(url);
...
"#,
    ...
    fn auth_secret_resolves_env_backed_config_value() {
        ...
        std::fs::write(
            ...
            r#"
[http_request.secrets]
api_token = "${OMNESAGENT_TEST_HTTP_REQUEST_SECRET}"
"#,
        )
        .unwrap();
        unsafe {
            std::env::set_var("OMNESAGENT_TEST_HTTP_REQUEST_SECRET", "Bearer from-env");
        }
        scopeguard::defer! {
            unsafe {
                std::env::remove_var("OMNESAGENT_TEST_HTTP_REQUEST_SECRET");
            }
        }
    ...
    fn auth_secret_reports_missing_env_backed_config_value() {
        ...
        std::fs::write(
            ...
            r#"
[http_request.secrets]
api_token = "${OMNESAGENT_TEST_HTTP_REQUEST_MISSING_SECRET}"
"#,
        )
        .unwrap();
        unsafe {
            std::env::remove_var("OMNESAGENT_TEST_HTTP_REQUEST_MISSING_SECRET");
        }
        ...
        assert!(
            err.contains("OMNESAGENT_TEST_HTTP_REQUEST_MISSING_SECRET"),
    ...
    fn auth_secret_decrypts_reloaded_config_value() {
        let tmp = TempDir::new().unwrap();
        let config_path = tmp.path().join("config.toml");
        let store = omnesagent_config::secrets::SecretStore::new(tmp.path(), true);
...
"#
    ...
    fn auth_secret_resolves_encrypted_env_backed_config_value() {
        ...
        let store = omnesagent_config::secrets::SecretStore::new(tmp.path(), true);
        let encrypted = store
            .encrypt("${OMNESAGENT_TEST_HTTP_REQUEST_ENCRYPTED_SECRET}")
            .unwrap();
        std::fs::write(
            &config_path,
            format!(
...
"#
        ...
        unsafe {
            std::env::set_var(
                "OMNESAGENT_TEST_HTTP_REQUEST_ENCRYPTED_SECRET",
                "Bearer encrypted-env",
            );
        }
        scopeguard::defer! {
            unsafe {
                std::env::remove_var("OMNESAGENT_TEST_HTTP_REQUEST_ENCRYPTED_SECRET");
            }
        }
...
"#,
    ...
    fn runtime_proxy_conflicts_with_dns_pinning_only_when_it_applies() {
        let global_proxy = ProxyConfig {
            ...
            scope: omnesagent_config::schema::ProxyScope::Omnesagent,
            ..ProxyConfig::default()
        };
        ...
        let environment_proxy = ProxyConfig {
            enabled: true,
            http_proxy: Some("http://proxy.example:8080".into()),
            scope: omnesagent_config::schema::ProxyScope::Environment,
            ..ProxyConfig::default()
        };
        ...
        let exact_service_proxy = ProxyConfig {
            enabled: true,
            http_proxy: Some("http://proxy.example:8080".into()),
            scope: omnesagent_config::schema::ProxyScope::Services,
