...
use omnesagent_config::schema::GoogleWorkspaceAllowedOperation;
...
use omnesagent_config::schema::DEFAULT_GWS_SERVICES;
...
impl Tool for GoogleWorkspaceTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if self.audit_log {
            ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"tool": "google_workspace", "service": service, "resource": resource, "sub_resource": sub_resource.unwrap_or(""), "method": method})), "gws audit: executing API call");
        }
