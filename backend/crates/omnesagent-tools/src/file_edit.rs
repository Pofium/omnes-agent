...
impl FileEditTool {
    ...
    async fn edit_file(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // (see omnesagent-runtime::tools::mod).
...
mod tests {
    ...
    /// stack, mirroring the registration in `omnesagent-runtime::tools::mod`. Use this
    ...
    async fn file_edit_warns_on_ephemeral_workspace() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_ephemeral");
    ...
    async fn file_edit_failure_not_warned_on_ephemeral_workspace() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_ephemeral_fail");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("doc.txt"), "hello world")
            .await
    ...
    async fn file_edit_no_warning_when_persistent() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_persistent");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("doc.txt"), "hello world")
            .await
    ...
    async fn file_edit_replaces_single_match() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_single");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_not_found() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_notfound");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "hello world")
            .await
    ...
    async fn file_edit_multiple_matches() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_multi");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_delete_via_empty_new_string() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_delete");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_rejects_empty_old_string() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_empty_old_string");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_blocks_path_traversal() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_traversal");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_normalizes_workspace_prefixed_relative_path() {
        let root = std::env::temp_dir().join("omnesagent_test_file_edit_workspace_prefixed");
        ...
        let content = tokio::fs::read_to_string(workspace.join("nested/target.txt"))
            .await
            .unwrap();
        assert_eq!(content, "hello omnesagent");
    ...
    async fn file_edit_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("omnesagent_test_file_edit_symlink_escape");
        let workspace = root.join("workspace");
    ...
    async fn file_edit_blocks_symlink_target_file() {
        ...
        let root = std::env::temp_dir().join("omnesagent_test_file_edit_symlink_target");
        let workspace = root.join("workspace");
        let outside = root.join("outside");
    ...
    async fn file_edit_blocks_readonly_mode() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_readonly");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "hello")
            .await
    ...
    async fn file_edit_nonexistent_file() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_nofile");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_absolute_path_in_workspace() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_abs_path");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_blocks_null_byte_in_path() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_edit_null_byte");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_blocks_path_outside_workspace() {
        let root = std::env::temp_dir().join("omnesagent_test_file_edit_outside_workspace");
        let workspace = root.join("workspace");
