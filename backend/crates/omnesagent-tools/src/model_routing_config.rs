...
use omnesagent_config::schema::{ClassificationRule, Config, DelegateTargetConfig, ModelRouteConfig};
...
impl ModelRoutingConfigTool {
    ...
    fn load_config_without_env(&self) -> anyhow::Result<Config> {
        ...
        let mut parsed =
            omnesagent_config::migration::migrate_to_current(&contents).map_err(|error| {
                ::omnesagent_log::record!(
                    ERROR,
                    ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Fail)
                        .with_outcome(::omnesagent_log::EventOutcome::Failure)
    ...
    async fn handle_set_default(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        {
            if omnesagent_providers::reliable::is_non_retryable(&probe_err) {
            ...
            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model": model_name, "probe_err": probe_err.to_string()})), "Model probe returned retryable error (keeping new config)");
        }
    ...
    async fn probe_model(&self, provider_name: &str, model: &str) -> anyhow::Result<()> {
        ...
        let model_provider = match omnesagent_providers::create_model_provider_with_url(
            provider_name,
            api_key,
...
mod tests {
    ...
    fn read_saved_provider_entry(
        ...
        family: &str,
        alias: &str,
    ) -> Option<omnesagent_config::schema::ModelProviderConfig> {
        let contents = std::fs::read_to_string(cfg_path).ok()?;
        let cfg = omnesagent_config::migration::migrate_to_current(&contents).ok()?;
