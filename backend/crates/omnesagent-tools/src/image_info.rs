...
mod tests {
    ...
    /// `omnesagent-runtime::tools::mod`.  Use this in tests that exercise
    ...
    async fn wrapped_blocks_external_absolute_path() {
        ...
        let workspace = std::env::temp_dir().join("omnesagent_image_info_wrap");
    ...
    async fn wrapped_blocks_path_traversal() {
        ...
        let workspace = std::env::temp_dir().join("omnesagent_image_info_trav");
        let _ = std::fs::create_dir_all(&workspace);
        let tool = wrapped_tool(workspace);
    ...
    async fn wrapped_normalizes_workspace_prefixed_relative_path() {
        ...
        let workspace = root.path().join("omnesagent-data").join("workspace");
