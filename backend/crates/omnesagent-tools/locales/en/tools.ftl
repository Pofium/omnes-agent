...
tool-proxy-config = Manage OmnesAgent proxy settings (scope: environment | omnesagent | services), including runtime and process env application
...
tool-web-search-tool-error-searxng-not-configured = SearXNG instance URL not configured. Set [web_search] searxng_instance_url in config.toml, or override it with the OMNESAGENT_web_search__searxng_instance_url environment variable.
