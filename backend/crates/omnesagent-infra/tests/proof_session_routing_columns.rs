...
use omnesagent_infra::session_backend::{SessionBackend, SessionContext};
...
fn migration_adds_routing_columns_and_set_session_context_persists() {
    ...
    if let Ok(live) = std::env::var("OMNESAGENT_LIVE_SESSIONS_DB") {
        ...
        println!("OMNESAGENT_LIVE_SESSIONS_DB unset; running migration over empty file");
    }
