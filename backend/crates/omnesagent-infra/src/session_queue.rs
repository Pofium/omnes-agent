...
mod tests {
    ...
    async fn queue_depth_limit() {
        ...
        let handle = omnesagent_spawn::spawn!(async move { queue_clone.acquire("s1").await });
    ...
    async fn registration_and_eviction_are_atomic() {
        ...
        let acquire = omnesagent_spawn::spawn!(async move { acquire_queue.acquire("s1").await });
        tokio::task::spawn_blocking(move || selected.wait())
            .await
            .unwrap();

        let evict_queue = queue.clone();
        let mut eviction = omnesagent_spawn::spawn!(async move { evict_queue.evict_idle().await });
        assert!(
    ...
    async fn cancelled_waiter_releases_registration() {
        ...
        let waiter = omnesagent_spawn::spawn!(async move { waiter_queue.acquire("s1").await });
