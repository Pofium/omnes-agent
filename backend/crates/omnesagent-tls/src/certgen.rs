//! Certificate generation for OmnesAgent's mutual-TLS transport.
//!
...
pub fn ensure_server_materials_protected(
    ...
    let (ca_cert, ca_key) = if materials.ca_cert_path.exists() && materials.ca_key_path.exists() {
        ...
        let ca_cert = ca_params("OmnesAgent WSS CA")?
            .self_signed(&ca_key)
...
impl CaKeyProtection {
    ...
    /// passphrase; threat A4). `OMNESAGENT_CA_PASSPHRASE` takes precedence; otherwise
    /// `OMNESAGENT_CA_PASSPHRASE_FILE` is read. Unset yields [`CaKeyProtection::None`]
    /// (the 0600 floor). Every CA generation + read path uses this so the on-disk
    /// form always matches.
    pub fn from_env() -> Self {
        if let Ok(p) = std::env::var("OMNESAGENT_CA_PASSPHRASE")
            && !p.trim().is_empty()
        {
            return Self::passphrase(p.trim());
        }
        if let Ok(path) = std::env::var("OMNESAGENT_CA_PASSPHRASE_FILE")
