//! Shared TLS and mutual TLS (mTLS) construction for OmnesAgent.
//!
//! This crate sits below both `omnesagent-runtime` and `omnesagent-gateway` so the
...
//! dependencies on `omnesagent-config` and friends.
...
pub fn enrollment_sas(pairing_code: &str, ca_fingerprint_hex: &str) -> String {
    let mut hasher = Sha256::new();
    hasher.update(b"omnesagent-enroll-sas-v1\0");
