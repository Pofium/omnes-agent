    ...
    async fn recall(
        ...
        match self.recall_from_lucid(recall_query).await {
            ...
            Err(error) => {
                ...
                ::omnesagent_log::record!(DEBUG, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"command": self.lucid_cmd, "error": format!("{}", error)})), "Lucid context unavailable; using local sqlite results");
                Ok(local_results)
            }
        }
    }
...
impl ::omnesagent_api::attribution::Attributable for LucidMemory {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Memory(::omnesagent_api::attribution::MemoryKind::Lucid)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
