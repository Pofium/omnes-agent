...
use uuid::Uuid;
use omnesagent_api::session_keys::sanitize_session_key;
use omnesagent_config::schema::SearchMode;
...
impl SqliteMemory {
    ...
    pub(crate) fn open_and_initialize(
        ...
        omnesagent_config::schema::v2::migrate_sqlite_memory_to_v3(db_path, &conn)?;
        Self::init_schema(&conn)?;
        omnesagent_kag::schema::migrate(&conn)?;
        Ok(conn)
    }
    ...
    fn init_schema(conn: &Connection) -> anyhow::Result<()> {
        ...
        execute_batch_retry(
            ...
            -- migration in `omnesagent_config::schema::v2::migrate_sqlite_memory_to_v3`
            ...
            Err(e) => {
                ::omnesagent_log::record!(
                    ...
                    "memory store: embedding failed; persisting row without a vector \
                     (run `omnesagent memory reindex` to backfill once the embedder recovers)"
                );
                None
            }
        };
    ...
    /// Provide a ProjectService bound to this memory database.
    pub fn project_service(&self) -> omnesagent_kag::project::ProjectService {
        omnesagent_kag::project::ProjectService::new(self.conn.clone())
    }

    /// Provide a KAG GraphService bound to this memory database.
    pub fn kag_graph_service(&self) -> omnesagent_kag::graph::GraphService {
        let embedder = omnesagent_kag::embedding::default_embedding_provider();
        omnesagent_kag::graph::GraphService::new(self.conn.clone(), embedder)
    }
...
impl Memory for SqliteMemory {
    ...
    async fn ensure_agent_uuid(&self, alias: &str) -> anyhow::Result<String> {
        ...
        tokio::task::spawn_blocking(move || -> anyhow::Result<String> {
            let conn = conn.lock();
            omnesagent_config::schema::v2::sqlite_ensure_agent_uuid(&conn, &alias)
        })
        .await?
    }
}

impl ::omnesagent_api::attribution::Attributable for SqliteMemory {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Memory(::omnesagent_api::attribution::MemoryKind::Sqlite)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
