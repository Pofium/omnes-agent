    ...
    fn initialize_client(
        ...
        let init_handle = std::thread::Builder::new()
            ...
            .spawn(move || -> Result<(Client, bool)> {
                ...
                omnesagent_config::schema::v2::migrate_postgres_memory_to_v3(
                    &mut client,
                    &schema_ident,
                    &qualified_table,
                )?;
...
impl ::omnesagent_api::attribution::Attributable for PostgresMemory {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Memory(::omnesagent_api::attribution::MemoryKind::Postgres)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
...
mod tests {
    ...
    async fn new_does_not_panic_inside_tokio_runtime() {
        let outcome = std::panic::catch_unwind(|| {
            PostgresMemory::new(
                "test",
                "postgres://omnesagent:password@127.0.0.1:1/omnesagent",
                "public",
                "memories",
                Some(1),
                None,
    ...
    /// fallback is reached.
    #[test]
    #[ignore = "requires OMNESAGENT_TEST_POSTGRES_URL"]
    fn postgres_new_with_pgvector_enabled_from_tokio_runtime_does_not_panic() {
        let database_url = std::env::var("OMNESAGENT_TEST_POSTGRES_URL")
            .expect("set OMNESAGENT_TEST_POSTGRES_URL to run ignored PostgreSQL tests");
