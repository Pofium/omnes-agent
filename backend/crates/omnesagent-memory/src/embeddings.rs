...
impl OpenAiEmbedding {
    ...
    fn http_client(&self) -> reqwest::Client {
        omnesagent_config::schema::build_runtime_proxy_client("memory.embeddings")
    }
...
// ── Candle local embedding provider (in-process 384d MiniLM) ──────────

pub struct CandleEmbedding {
    inner: std::sync::Arc<dyn omnesagent_kag::embedding::EmbeddingProvider>,
}

impl Default for CandleEmbedding {
    fn default() -> Self {
        Self::new()
    }
}
...
impl CandleEmbedding {
    pub fn new() -> Self {
        Self {
            inner: omnesagent_kag::embedding::default_embedding_provider(),
        }
    }
}
...
impl EmbeddingProvider for CandleEmbedding {
    ...
    async fn embed(&self, texts: &[&str]) -> anyhow::Result<Vec<Vec<f32>>> {
        ...
        self.inner.embed(&strings).await
    }
}
...
pub fn create_embedding_provider(
    ...
    match model_provider {
        "candle" | "local" | "kag" => Box::new(CandleEmbedding::new()),
        "openai" => {
...
mod tests {
    ...
    #[test]
    fn factory_candle() {
        let p = create_embedding_provider("candle", None, "all-MiniLM-L6-v2", 384);
