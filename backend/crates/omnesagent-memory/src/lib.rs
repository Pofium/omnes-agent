...
use std::path::Path;
use std::sync::Arc;
use omnesagent_config::providers::ModelProviders;
    ...
    F: FnMut() -> anyhow::Result<SqliteMemory>,
{
    match classify_memory_backend(backend_name) {
        ...
        MemoryBackendKind::Unknown => {
            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"backend_name": backend_name, "unknown_context": unknown_context})), "Unknown memory backend '', falling back to markdown");
            wrap_scanned_and_audit(
                MarkdownMemory::new("markdown", workspace_dir),
                policy,
                workspace_dir,
...
pub fn should_autosave_origin(origin: omnesagent_api::ingress::TurnOrigin) -> bool {
            ...
            Ok(invalidated) => {
                ::omnesagent_log::record!(
                    ...
                    "memory: embedding identity changed; stored vectors invalidated and \
                     ...
                     keyword-only until re-embedded — run `omnesagent memory reindex`"
                );
...
/// operator is pointed at `omnesagent memory reindex` instead.
    ...
    let Ok(handle) = tokio::runtime::Handle::try_current() else {
        ::omnesagent_log::record!(
            ...
            "memory: auto_reindex_on_identity_change is set but no async runtime is \
             available here; run `omnesagent memory reindex` to re-embed"
        );
        return;
    };
    let mem = mem.clone();
            ...
            Err(e) => {
                ::omnesagent_log::record!(
                    ...
                    "memory: background re-embed after embedding identity change failed; \
                     run `omnesagent memory reindex` to retry"
                );
            }
        }
    });
...
pub async fn create_memory_for_agent(
    ...
    use omnesagent_config::multi_agent::MemoryBackendKind as ConfigBackend;
...
mod tests {
    ...
    use omnesagent_config::schema::EmbeddingRouteConfig;
    ...
    async fn per_agent_markdown_factory_applies_memory_policy() {
        use omnesagent_config::multi_agent::{AgentAlias, AgentMemoryConfig, MemoryBackendKind};
    ...
    fn static_sqlite(dir: &Path, dims: usize) -> SqliteMemory {
        SqliteMemory::with_embedder(
            ...
            1000,
            None,
            omnesagent_config::schema::SearchMode::default(),
        )
        .unwrap()
    }
    ...
    async fn create_memory_for_agent_markdown_wraps_audit_when_enabled() {
        use omnesagent_config::multi_agent::{AgentMemoryConfig, MemoryBackendKind as ConfigBackend};
        use omnesagent_config::schema::{AliasedAgentConfig, Config};
...
"#
    ...
    fn factory_postgres_without_feature_gives_clear_error() {
        use omnesagent_config::schema::PostgresStorageConfig;
        let tmp = TempDir::new().unwrap();
        let cfg = MemoryConfig {
    ...
    fn storage_aware_factory_still_builds_qdrant_with_storage_config() {
        use omnesagent_config::schema::QdrantStorageConfig;
        let tmp = TempDir::new().unwrap();
        let cfg = MemoryConfig {
            backend: "qdrant.default".into(),
            ..MemoryConfig::default()
    ...
    fn agent_config(tmp: &TempDir) -> omnesagent_config::schema::Config {
