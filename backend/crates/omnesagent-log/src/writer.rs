...
//! `omnesagent-log-writer` so `record_event` does not block on file I/O
...
fn spawn_worker(rx: Receiver<WriterJob>, state: Arc<WorkerState>) {
    ...
    let builder = thread::Builder::new().name("omnesagent-log-writer".into());
    if let Err(err) = builder.spawn(move || worker_main(rx, state)) {
        tracing::warn!(
            target: "omnesagent_log",
            error = %err,
            "log: failed to spawn omnesagent-log-writer thread; persistence disabled"
        );
            ...
            match job {
                WriterJob::Write(value) => {
                    if let Err(err) = write_one(&state, &value) {
                        tracing::warn!(
                            target: "omnesagent_log_internal",
                            error = ?err,
