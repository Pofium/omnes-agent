...
//! [`omnesagent_api::observability_traits::ObserverEvent`] variants when a
...
use omnesagent_api::observability_traits::{Observer, ObserverEvent, TurnTokenUsage};
...
fn project(event: &LogEvent) -> Option<ObserverEvent> {
    ...
    let attribution = &event.omnesagent;
...
mod tests {
    ...
    fn projects_llm_request() {
        ...
        event.omnesagent.set("model", "claude-sonnet-4-6");
    ...
    fn projects_tool_call_success() {
        ...
        event.omnesagent.set("tool", "shell");
        event.omnesagent.duration_ms = Some(120);
    ...
    fn projects_memory_audit() {
        ...
        event.omnesagent.duration_ms = Some(42);
        event.set_outcome(EventOutcome::Success);
    ...
    fn projects_llm_request_with_turn_metadata() {
        ...
        event.omnesagent.set("model", "claude-sonnet-4-6");
        event.omnesagent.set("agent_alias", "default");
        event.omnesagent.set_composite("channel", "wss.default");
        event.attributes = serde_json::json!({
            "messages_count": 2,
            "turn_id": "turn-1"
        });
