//! Unified log emission surface for the OmnesAgent workspace.
...
/// reach `tracing` types via `omnesagent_log::*`. Do NOT use directly
...
pub use event::{
    ...
    Severity, OmnesagentAttribution, is_attribution_field, severity_text_from_number,
...
/// the type) but the public path is `omnesagent_log::Span` — no `tracing`
...
/// Returns whether OmnesAgent DEBUG log events are enabled for the current
/// subscriber. Use this before building expensive structured DEBUG attrs.
pub fn debug_enabled() -> bool {
    ::tracing::enabled!(
        target: "omnesagent_log_event",
        ::tracing::Level::DEBUG
    )
}
