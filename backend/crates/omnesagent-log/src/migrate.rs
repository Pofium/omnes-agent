...
fn convert_legacy_to_current(legacy: Value) -> Value {
    ...
    let mut omnesagent = serde_json::Map::new();
    if let Some(agent) = get_str("agent_alias") {
        omnesagent.insert("agent_alias".into(), Value::String(agent));
    }
    ...
    if let Some(channel) = get_str("channel") {
        ...
        if let Some((ty, alias)) = channel.split_once('.') {
            omnesagent.insert("channel".into(), Value::String(channel.clone()));
            omnesagent.insert(type_field("channel"), Value::String(ty.to_string()));
            omnesagent.insert(alias_field("channel"), Value::String(alias.to_string()));
        } else {
            omnesagent.insert("channel".into(), Value::String(channel.clone()));
            omnesagent.insert(type_field("channel"), Value::String(channel));
        }
    }
    if let Some(mp) = get_str("model_provider") {
        if let Some((ty, alias)) = mp.split_once('.') {
            omnesagent.insert("model_provider".into(), Value::String(mp.clone()));
            omnesagent.insert(type_field("model_provider"), Value::String(ty.to_string()));
            omnesagent.insert(
            ...
            omnesagent.insert("model_provider".into(), Value::String(mp.clone()));
            omnesagent.insert(type_field("model_provider"), Value::String(mp));
        }
    }
    if let Some(model) = get_str("model") {
        omnesagent.insert("model".into(), Value::String(model));
    }
    ...
    serde_json::json!({
        ...
        "service": { "name": "omnesagent", "version": env!("CARGO_PKG_VERSION") },
        "trace_id": trace_id,
        "omnesagent": Value::Object(omnesagent),
        "message": message,
...
mod tests {
    ...
    fn migrates_legacy_to_current_shape() {
        ...
        assert_eq!(v["omnesagent"]["agent_alias"], "clamps");
        assert_eq!(v["omnesagent"]["channel"], "discord.clamps");
        assert_eq!(v["omnesagent"]["channel_type"], "discord");
        assert_eq!(v["omnesagent"]["channel_alias"], "clamps");
        assert_eq!(v["omnesagent"]["model_provider"], "anthropic.clamps");
    ...
    fn already_current_is_noop() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("trace.jsonl");
        let line = r#"{"id":"id","@timestamp":"2026-05-15T19:00:00Z","severity_number":9,"severity_text":"INFO","event":{"category":"agent","action":"x","outcome":"success"},"service":{"name":"omnesagent","version":"0.7.5"},"omnesagent":{},"schema_version":2}"#;
    ...
    fn mixed_schema_file_migrates_legacy_rows_after_current_rows() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("trace.jsonl");
        let current = r#"{"id":"current","@timestamp":"2026-05-15T19:00:00Z","severity_number":9,"severity_text":"INFO","event":{"category":"agent","action":"x","outcome":"success"},"service":{"name":"omnesagent","version":"0.8.3"},"omnesagent":{},"schema_version":2}"#;
