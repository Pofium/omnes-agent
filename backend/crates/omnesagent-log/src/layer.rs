...
use crate::event::{
    ...
    OmnesagentAttribution,
};
use crate::writer::record_event;

const TARGET_EVENT: &str = "omnesagent_log_event";
const TARGET_ATTRIBUTION_SPAN: &str = "omnesagent_log_internal_attribution";
const TARGET_SCOPE_SPAN: &str = "omnesagent_log_internal_scope";
const TARGET_SUPPRESS_PREFIX: &str = "omnesagent_log_internal";
    ...
    S: Subscriber + for<'lookup> LookupSpan<'lookup>,
{
    fn on_new_span(&self, attrs: &Attributes<'_>, id: &Id, ctx: Context<'_, S>) {
        ...
        if target == TARGET_ATTRIBUTION_SPAN {
            ...
            attrs.record(&mut v);
            let mut attribution = OmnesagentAttribution::default();
    ...
    fn on_record(&self, id: &Id, values: &Record<'_>, ctx: Context<'_, S>) {
        ...
        if target == TARGET_ATTRIBUTION_SPAN {
            ...
            v.apply_into(&mut attribution);
            let mut exts = span.extensions_mut();
            if let Some(existing) = exts.get_mut::<OmnesagentAttribution>() {
    ...
    fn on_event(&self, event: &Event<'_>, ctx: Context<'_, S>) {
        ...
        if visitor.has_duration.unwrap_or(false) {
            log_event.omnesagent.duration_ms = visitor.duration_ms;
        }
        ...
        if let Some(span_ref) = ctx.lookup_current() {
            ...
            while let Some(span) = current {
                ...
                if let Some(parent) = exts.get::<OmnesagentAttribution>() {
                    log_event.omnesagent.merge_from(parent);
                }
...
impl AttributionSpanCollector {
    fn apply_into(self, attr: &mut OmnesagentAttribution) {
...
struct ScopeSpanCollector {
    category: Option<String>,
    attribution: OmnesagentAttribution,
    extra: JsonMap<String, Value>,
}
...
#[cfg(test)]
mod e2e_tests {
    use crate as omnesagent_log;
    use crate::{
        Action, Event, EventOutcome, subscribe_or_install, try_install_capture_subscriber,
    };
    use ::omnesagent_api::attribution::{Attributable, ChannelKind, Role};
    ...
    async fn attribution_span_populates_alias_bound_fields() {
...

        {
            ...
            .instrument(omnesagent_log::attribution_span!(&thing))
            .await;
        }
            ...
            match tokio::time::timeout(step, rx.recv()).await {
                Ok(Ok(value)) => {
                    if value
                        ...
                        .unwrap_or(false)
                    {
                        let zc = value.get("omnesagent").expect("omnesagent block present");
                        assert_eq!(
