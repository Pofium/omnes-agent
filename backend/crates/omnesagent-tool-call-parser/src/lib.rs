...
pub use omnesagent_api::model_provider::strip_think_tags;
...
/// (`omnesagent-runtime`'s `StreamTerminalMarkerStripper`) so a vocabulary or
...
///
/// ```
/// use omnesagent_tool_call_parser::strip_trailing_terminal_markers;
///
...
/// Map tool name aliases from various LLM model_providers to OmnesAgent tool names.
...
fn malformed_tool_block_event(payload_len: usize) -> ::omnesagent_log::Event {
        ...
        let event = 'search: loop {
            while let Ok(event) = rx.try_recv() {
                ...
                let matches_source = event
                    ...
                    .is_some_and(|file| {
                        file.replace('\\', "/")
                            .ends_with("omnesagent-tool-call-parser/src/lib.rs")
                    });
...
Let me check the result."#;
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Additional parser internals tests (moved from omnesagent-runtime to keep
