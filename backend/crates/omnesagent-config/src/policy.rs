...
use omnesagent_api::runtime_traits::ShellDialect;

// Re-export from omnesagent-config.
...
impl PerSenderTracker {
    ...
    fn current_key() -> String {
        omnesagent_api::TOOL_LOOP_THREAD_ID
            .try_with(|v| v.clone())
            .ok()
            .flatten()
...
impl SecurityPolicy {
    ...
    pub fn runtime_config_violation_message(&self, resolved: &Path) -> String {
        format!(
            "Refusing to modify OmnesAgent runtime config/state file: {}. Use dedicated config tools or edit it manually outside the agent loop.",
            resolved.display()
        )
    }
...
mod tests {
    ...
    #[cfg(not(target_os = "windows"))]
    fn tp_ws() -> PathBuf {
        PathBuf::from("/home/user/.omnesagent/workspace")
    }
    #[cfg(target_os = "windows")]
    fn tp_ws() -> PathBuf {
        PathBuf::from("C:\\Users\\user\\.omnesagent\\workspace")
    }

    #[cfg(not(target_os = "windows"))]
    fn tp_ws_shared() -> PathBuf {
        PathBuf::from("/home/user/.omnesagent/shared")
    }
    #[cfg(target_os = "windows")]
    fn tp_ws_shared() -> PathBuf {
        PathBuf::from("C:\\Users\\user\\.omnesagent\\shared")
    }
    ...
    fn workspace_only_false_allows_resolved_outside_workspace() {
        let workspace = std::env::temp_dir().join("omnesagent_test_ws_only_false");
        ...
        assert!(
            !p.is_resolved_path_allowed(Path::new("/var/omnesagent-forbidden-test/nonexistent.sock")),
    ...
    fn workspace_only_true_blocks_resolved_outside_workspace() {
        let workspace = std::env::temp_dir().join("omnesagent_test_ws_only_true");
        let _ = std::fs::create_dir_all(&workspace);
        let canonical_workspace = workspace
            .canonicalize()
            .unwrap_or_else(|_| workspace.clone());
        ...
        let outside = std::env::temp_dir()
            ...
            .join("omnesagent_outside_ws_true");
        assert!(
            !p.is_resolved_path_allowed(&outside),
    ...
    fn readable_includes_posix_device_files() {
        ...
        let p = SecurityPolicy {
            workspace_dir: PathBuf::from("/tmp/omnesagent-test-ws"),
            workspace_only: true,
            ..SecurityPolicy::default()
        };
    ...
    fn for_agent_routes_workspace_access_into_correct_allowlist_tier() {
        ...
        let mut cfg = Config {
            data_dir: PathBuf::from("/tmp/omnesagent-for-agent-test"),
            config_path: PathBuf::from("/tmp/omnesagent-for-agent-test/config.toml"),
            ..Config::default()
        };
    ...
    fn write_only_root_blocks_reads_and_admits_writes() {
        ...
        let write_only_root =
            std::env::temp_dir().join(format!("omnesagent_wo_root_{}", uuid::Uuid::new_v4()));
    ...
    fn for_agent_creates_the_per_agent_workspace_dir() {
        ...
        let root =
            std::env::temp_dir().join(format!("omnesagent-for-agent-mkdir-{}", std::process::id()));
    ...
    fn for_agent_unrestricted_filesystem_disables_workspace_only() {
        ...
        let mut cfg = Config {
            data_dir: PathBuf::from("/tmp/omnesagent-for-agent-unrestricted"),
            config_path: PathBuf::from("/tmp/omnesagent-for-agent-unrestricted/config.toml"),
            ..Config::default()
        };
        cfg.risk_profiles.insert(
            "default".into(),
    ...
    fn resolved_path_blocks_outside_workspace() {
        let workspace = std::env::temp_dir().join("omnesagent_test_resolved_path");
        let _ = std::fs::create_dir_all(&workspace);
        ...
        let canonical_temp = std::env::temp_dir()
            .canonicalize()
            .unwrap_or_else(|_| std::env::temp_dir());
        let outside = canonical_temp.join("outside_workspace_omnesagent");
        assert!(
    ...
    fn resolved_path_blocks_root_escape() {
        let policy = SecurityPolicy {
            workspace_dir: PathBuf::from("/home/omnesagent_user/project"),
            ..SecurityPolicy::default()
        };
    ...
    fn resolved_path_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("omnesagent_test_symlink_escape");
    ...
    fn forbidden_path_argument_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join(format!(
            "omnesagent_test_shell_symlink_escape_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");
        let outside = root.join("outside_target");
    ...
    fn forbidden_path_argument_allows_in_workspace_paths() {
        ...
        let root = std::env::temp_dir().join(format!(
            "omnesagent_test_shell_in_workspace_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn forbidden_path_argument_allows_read_only_root() {
        let root = std::env::temp_dir().join(format!(
            "omnesagent_test_shell_read_only_root_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");
    ...
    fn allowed_roots_permits_paths_outside_workspace() {
        ...
        let root = std::env::temp_dir().join("omnesagent_test_allowed_roots");
        let workspace = root.join("workspace");
    ...
    fn resolve_tool_path_normalizes_workspace_prefixed_relative_paths() {
        let p = SecurityPolicy {
            workspace_dir: PathBuf::from("/omnesagent-data/workspace"),
            ..SecurityPolicy::default()
        };
        let resolved = p.resolve_tool_path("omnesagent-data/workspace/scripts/daily.py");
        assert_eq!(
            resolved,
            PathBuf::from("/omnesagent-data/workspace/scripts/daily.py")
        );
    }

    #[test]
    fn resolve_tool_path_normalizes_windows_workspace_prefixed_relative_paths() {
        let workspace = PathBuf::from(r"C:\Users\me\.omnesagent\agents\default\workspace");
        let p = SecurityPolicy {
            workspace_dir: workspace.clone(),
            ..SecurityPolicy::default()
        };
        let resolved =
            p.resolve_tool_path(r"Users\me\.omnesagent\agents\default\workspace\nested\out.txt");
    ...
    fn resolve_tool_path_does_not_normalize_mismatched_drive_prefixed_relative_paths() {
        ...
        let resolved =
            p.resolve_tool_path(r"D:Users\me\.omnesagent\agents\default\workspace\nested\out.txt");
    ...
    fn runtime_config_paths_are_protected() {
        let workspace = PathBuf::from("/tmp/omnesagent-profile/workspace");
        let policy = SecurityPolicy {
            workspace_dir: workspace.clone(),
            ..SecurityPolicy::default()
        };
    ...
    fn runtime_state_files_in_data_dir_are_protected() {
        let workspace = PathBuf::from("/tmp/omnesagent-profile/workspace");
        let data_dir = PathBuf::from("/tmp/omnesagent-profile/data");
        let policy = SecurityPolicy {
            workspace_dir: workspace.clone(),
        ...
        assert!(!legacy_policy.is_runtime_config_path(&PathBuf::from(
            "/tmp/omnesagent-profile/data/webauthn_credentials.json"
        )));
    }
    ...
    fn is_runtime_config_path_protects_install_root_for_nested_agent_layout() {
        let install_root = PathBuf::from("/tmp/omnesagent-install-nested");
