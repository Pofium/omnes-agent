...
impl ModelProviders {
    ...
    /// in the existing `iter_entries` order (upstream #10638). An entry without
    /// a `model` cannot serve as the install-wide default — there is no model
    /// string to pair with the provider — so entries without one are skipped
    /// instead of seeding a provider/model mismatch at the consumer.
    #[must_use]
