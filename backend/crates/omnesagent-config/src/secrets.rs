...
// in `~/.omnesagent/.secret_key` with restrictive file permissions (0600). The
...
/// key at the given path (default: `~/.omnesagent/.secret_key`, 0600).
#[derive(Debug, Clone)]
...
impl SecretStore {
    ...
    pub fn new(omnesagent_dir: &Path, enabled: bool) -> Self {
        Self {
            key_source: Arc::new(FileKeySource::new(omnesagent_dir.join(".secret_key"))),
            enabled,
        }
    }
            ...
            let plaintext_bytes = cipher.decrypt(nonce, ciphertext).map_err(|e| {
                ...
                ::omnesagent_log::record!(
                    ...
                    "enc2: decryption failed — key mismatch or missing material. \
                         ...
                         the affected secrets via `omnesagent quickstart`."
                );
