...
use std::fmt;
use omnesagent_macros::Configurable;
...
impl ThinkingDisplayMode {
    ...
    pub fn to_display(self) -> Option<omnesagent_api::model_provider::ThinkingDisplay> {
        match self {
            Self::Off => None,
            Self::Omitted => Some(omnesagent_api::model_provider::ThinkingDisplay::Omitted),
            Self::Updates => Some(omnesagent_api::model_provider::ThinkingDisplay::Updates),
            Self::Summarized => Some(omnesagent_api::model_provider::ThinkingDisplay::Summarized),
        }
    }
}
...
/// `omnesagent eval` command. Distinct from `[agent.eval]`, which is the in-loop
...
#[derive(Debug, Clone, Serialize, Deserialize, omnesagent_macros::Configurable)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
...
#[derive(Debug, Clone, Default, Serialize, Deserialize, omnesagent_macros::Configurable)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
