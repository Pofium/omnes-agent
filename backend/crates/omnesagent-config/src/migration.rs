...
impl omnesagent_api::attribution::Attributable for ConfigLoadAttribution {
...
/// Embedded V1 fixture used by [`generate`] / the `omnesagent config generate`
    ...
    if opts.encrypt_secrets {
        let store_dir = opts.secret_store_dir.context(
            "--encrypt requires a secret-store directory \
             (typically the resolved OMNESAGENT_CONFIG_DIR)",
        )?;
...
/// Used by repair tooling (`omnesagent config migrate`, `model_routing_config`)
/// that needs the precise failure. Daemon load uses the resilient path.
pub fn migrate_to_current(input: &str) -> Result<Config> {
    let _attribution = ::omnesagent_log::attribution_span!(&ConfigLoadAttribution).entered();
        ...
        Err(err) => {
            ::omnesagent_log::record!(
                ...
                "config could not be parsed or migrated; starting on defaults so it \
                 can be repaired (gateway /api/config, `omnesagent config migrate`)"
            );
        ...
        ::omnesagent_log::record!(
            ...
            &format!(
                "SECURITY-CRITICAL config section `{path}` is invalid and was reset to \
                 ...
                 Run `omnesagent config migrate` to see the precise parse error, or fix it \
...
pub fn ensure_disk_at_current_version(path: &Path) -> Result<()> {
    ...
    anyhow::bail!(
        "config at {} is schema_version {from}; run `omnesagent config migrate` to update before modifying",
        path.display().to_string(),
    );
}
...
"#;
    ...
    fn corrupt_telegram_alias_is_still_dropped_and_recorded() {
        ...
        // exact path recorded so `doctor` can name it (see omnesagent-runtime's
