...
///
/// This is an exact allowlist: an unknown `OMNESAGENT_todotracker__*` field is
...
/// Apply every `OMNESAGENT_<lowercase>` env var to `config`. Returns the set of
...
mod tests {
    ...
    async fn walker_resolves_typed_family_alias_default() {
        ...
        let _v = EnvVarGuard::set(
            "OMNESAGENT_providers__models__anthropic__default__api_key",
            "sk-ant-fixture",
        );
    ...
    async fn walker_resolves_grok_cli_stdout_limit() {
        let _guard = super::env_test_lock().await;
        let _v = EnvVarGuard::set(
            "OMNESAGENT_providers__models__grok_cli__default__max_acp_stdout_bytes",
            "8388608",
        );
    ...
    async fn walker_accepts_alias_with_underscore() {
        ...
        let _v1 = EnvVarGuard::set(
            "OMNESAGENT_providers__models__openrouter__prod_v2__api_key",
            "sk-or-fixture",
        );
        let _v2 = EnvVarGuard::set(
            "OMNESAGENT_providers__models__openrouter__prod_v2__model",
    ...
    async fn walker_resolves_non_map_gateway_path() {
        let _guard = super::env_test_lock().await;
        let _v = EnvVarGuard::set("OMNESAGENT_gateway__request_timeout_secs", "120");
    ...
    async fn walker_rejects_unknown_path() {
        let _guard = super::env_test_lock().await;
        let _v = EnvVarGuard::set("OMNESAGENT_no__such__field", "x");

        let mut config = Config::default();
        let err = apply_env_overrides(&mut config).expect_err("must hard-error");
        let msg = format!("{err:#}");
        assert!(
            msg.contains("OMNESAGENT_no__such__field") && msg.contains("did not resolve"),
    ...
    async fn walker_propagates_alias_validator_error() {
        ...
        let _v = EnvVarGuard::set(
            "OMNESAGENT_providers__models__anthropic___invalid__api_key",
            "x",
        );
    ...
    async fn mask_restores_pre_override_snapshot_for_non_secret() {
        let _guard = super::env_test_lock().await;
        let _v = EnvVarGuard::set("OMNESAGENT_gateway__request_timeout_secs", "999");
    ...
    async fn schema_version_override_rejected() {
        let _guard = super::env_test_lock().await;
        let _v = EnvVarGuard::set("OMNESAGENT_schema_version", "99");
    ...
    async fn recognized_legacy_todotracker_env_does_not_block_startup() {
        let _guard = super::env_test_lock().await;
        let _a = EnvVarGuard::set("OMNESAGENT_todotracker__enabled", "false");
        let _b = EnvVarGuard::set("OMNESAGENT_todotracker__enabled_at_start", "true");
        let _c = EnvVarGuard::set("OMNESAGENT_todotracker__location", "left");
        let _d = EnvVarGuard::set("OMNESAGENT_todotracker__width", "40");
        let _e = EnvVarGuard::set("OMNESAGENT_todotracker__max_height", "8");
    ...
    async fn unknown_legacy_todotracker_field_still_errors() {
        ...
        let _v = EnvVarGuard::set("OMNESAGENT_todotracker__wdith", "40");

        let mut config = Config::default();
        let err = apply_env_overrides(&mut config).expect_err("unknown field must hard-error");
        let msg = format!("{err:#}");
        assert!(
            msg.contains("OMNESAGENT_todotracker__wdith") && msg.contains("did not resolve"),
            "error must name the env var and the failure: {msg}",
        );
    }
    ...
    async fn legacy_shim_does_not_shadow_other_sections() {
        ...
        let _v = EnvVarGuard::set("OMNESAGENT_todotracker_extra__enabled", "false");
