...
use omnesagent_api::runtime_traits::{RuntimeAdapter, ShellDialect, ShellProfile};
...
mod tests {
    ...
    #[test]
    fn native_storage_path_contains_omnesagent() {
        let path = NativeRuntime::new().storage_path();
        assert!(path.to_string_lossy().contains("omnesagent"));
    }
