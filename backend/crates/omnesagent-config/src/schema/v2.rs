...
fn synthesize_agent_brains(
    ...
    for (alias, agent_value) in agents {
        ...
        if let Some(toml::Value::String(raw_provider)) = provider {
            ...
            ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"alias": alias, "provider_type": provider_type, "provider_alias": provider_alias})), "agents.: inline brain → providers.models..");
        } else {
            ...
            && !skills_dir.is_empty()
        {
            ...
            if stays_inside_shared {
                ...
                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"alias": alias, "skills_dir": skills_dir, "bundle_alias": bundle_alias})), "agents..skills_directory = \"\" lies outside \
...
fn rewrite_dangling_peer_group_agents(passthrough: &mut toml::Table) {
    ...
    let mut rewritten_channel_types: Vec<String> = Vec::new();
    {
        ...
        for (group_name, group_value) in peer_groups.iter_mut() {
            ...
            ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"group_name": group_name, "replacement_alias": format!("{:?}", replacement_alias)})), "peer_groups..agents rewritten from [\"default\"] to [] (no agents.default exists)");
        }
    }
    ...
    if !added.is_empty() {
        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"replacement_alias": replacement_alias, "added": format!("{:?}", added)})), "agents..channels extended with  so the rewritten peer_groups resolve");
    }
}
