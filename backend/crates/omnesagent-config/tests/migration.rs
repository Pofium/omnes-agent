...
use omnesagent_config::migration::{
...
use omnesagent_config::schema::v2::V2Config;
...
[extra_headers]
"User-Agent" = "OmnesAgent-V1-Test/1.0"
"#;
...
"#;
    ...
    let raw_value: toml::Value = toml::from_str(
        &omnesagent_config::migration::migrate_file(raw)
            .expect("migrate_file succeeds")
...
nested_value = 42
"#;
    let migrated = omnesagent_config::migration::migrate_file(raw)
        .expect("migrate_file succeeds")
        .expect("migration ran");
...
fn ensure_disk_at_current_version_blocks_stale() {
    ...
    let err = ensure_disk_at_current_version(&path)
        ...
        .to_string();
    assert!(
        err.contains("omnesagent config migrate"),
...
fn v3_channel_types_covers_every_typed_channel_slot() {
    ...
    use omnesagent_config::schema::Config;
    use omnesagent_config::schema::v2::V3_CHANNEL_TYPES;
    ...
    assert!(
        ...
        "ChannelsConfig has typed channel slots that V3_CHANNEL_TYPES does \
         ...
         the const in `crates/omnesagent-config/src/schema/v2.rs`, or add a \
...
model = "gpt-5"
api_key = "op://omnesagent/provider/openai-api-key"
"#;
    ...
    assert_eq!(api_key, Some("op://omnesagent/provider/openai-api-key"));
}
...
fn map_key_sections_exposes_typed_family_slots() {
    ...
    let paths: std::collections::HashSet<&'static str> = Config::map_key_sections()
        .iter()
        .filter(|s| matches!(s.kind, omnesagent_config::traits::MapKeyKind::Map))
        .map(|s| s.path)
        .collect();
    for required in [
        "providers.models.anthropic",
