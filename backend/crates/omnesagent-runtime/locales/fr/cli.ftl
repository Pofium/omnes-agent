...
cli-try-quickstart = Essayez `omnesagent quickstart` pour créer votre premier agent.
...
cli-config-about = Gérer la configuration de OmnesAgent
cli-update-about = Vérifier et appliquer les mises à jour de OmnesAgent
...
cli-wechat-bound-success = ✅ Compte WeChat lié avec succès. Vous pouvez maintenant parler à OmnesAgent.
...
cli-skills-install-tier-official = Installation de {$name} v{$version} — Officiel (maintenu par omnesagent-labs)
cli-skills-install-tier-community =
    Installation de {$name} v{$version} — Soumission communautaire
    Ce skill n'est pas audité par OmnesAgent. Examinez le contenu du skill
    et exécutez `omnesagent skills audit {$name}` avant d'accorder des
...
cli-skills-bundle-add-prompt =
    ...
    omnesagent config map-key skill-bundles {$alias}
    omnesagent config set skill-bundles.{$alias}.directory {$dir}

    (La création directe de bundle via `omnesagent skills bundle add` dupliquerait la surface de mutation de configuration.)
cli-skills-bundle-remove-prompt =
    Pour supprimer le skill-bundle '{$alias}', exécutez :
    omnesagent config map-key-delete skill-bundles {$alias}
...
cli-skills-bundle-list-empty =
    ...
    Créez-en un : omnesagent config set skill-bundles.default.directory shared/skills/default
...
cli-peripheral-flash-about = Flasher le firmware de OmnesAgent sur une carte Arduino
...
cli-migrate-openclaw-about = Importer la mémoire d'un espace de travail OpenClaw vers cet espace de travail OmnesAgent
cli-agent-long-about =
    ...
    Exemples :
    omnesagent agent                              # session interactive
    omnesagent agent -m "Résumez les logs d'aujourd'hui"  # message unique
    omnesagent agent -p anthropic --model claude-sonnet-4-20250514
    omnesagent agent --peripheral nucleo-f401re:/dev/ttyACM0
cli-gateway-long-about =
    ...
    Exemples :
    omnesagent gateway start              # démarrer la gateway
    omnesagent gateway restart            # redémarrer la gateway
    omnesagent gateway get-paircode       # afficher le code d'appairage
cli-acp-long-about =
    ...
    Exemples :
    omnesagent acp                        # démarrer le serveur ACP
    omnesagent acp --max-sessions 5       # limiter les sessions concurrently
cli-daemon-long-about =
    Démarrer le daemon autonome longue durée.

    Lance l'exécution Runtime complète de OmnesAgent : serveur gateway, tous les canaux configurés (Telegram, Discord, Slack, etc., moniteur de cœur et planificateur cron. C'est la méthode recommandée pour exécuter OmnesAgent en production ou comme assistant toujours actif.

    Utilisez 'omnesagent service install' pour enregistrer le daemon en tant que service OS (systemd/launchd) pour un démarrage automatique au démarrage.

    Exemples :
    omnesagent daemon                   # utiliser les défauts de configuration
    omnesagent daemon -p 9090           # gateway sur le port 9090
    omnesagent daemon --host 127.0.0.1  # uniquement localhost
cli-cron-long-about =
    ...
    omnesagent cron add '0 9 * * 1-5' 'Bonjour' --agent sentinel --prompt --tz America/New_York
    omnesagent cron add '*/30 * * * *' 'Vérifier la santé du système' --agent sentinel --prompt
    omnesagent cron add '*/5 * * * *' 'echo ok' --agent sentinel
    omnesagent cron add-at 2099-01-15T14:00:00Z 'Envoyer un rappel' --agent sentinel --prompt
    omnesagent cron add-every 60000 'Ping de santé' --agent sentinel --prompt
    omnesagent cron once 30m 'Lancer une sauvegarde dans 30 minutes' --agent sentinel --prompt
    omnesagent cron pause IDENTIFIANT_TACHE
    omnesagent cron update IDENTIFIANT_TACHE --expression '0 8 * * *' --tz Europe/London
cli-channel-long-about =
    Gérer les canaux de communication.

    Ajouter, supprimer, lister, envoyer et vérifier la santé des canaux qui connectent OmnesAgent aux plateformes de messagerie. Types de canaux pris en charge : telegram, discord, slack, whatsapp, matrix, imessage, email.
    ...
    omnesagent channel send 'Alerte !' --channel-id telegram --recipient 123456789
...
cli-peripheral-long-about =
    ...
    Exemples :
    omnesagent peripheral connect nucleo-f401re:/dev/ttyACM0
    omnesagent peripheral disconnect nucleo-f401re
    omnesagent peripheral detect nucleo-f401re
    omnesagent peripheral probe nucleo-f401re
    omnesagent peripheral trace nucleo-f401re
    omnesagent peripheral debug nucleo-f401re
    omnesagent peripheral connect esp32-usb-serial:/dev/ttyUSB0
    omnesagent peripheral disconnect esp32-usb-serial
...
cli-config-long-about =
    ...
    Exemples :
    omnesagent config list                                  # lister toutes les propriétés
    omnesagent config list --secrets                        # lister uniquement les secrets
    omnesagent config list --filter channels.matrix         # filtrer par préfixe
    omnesagent config get channels.matrix.mention-only      # obtenir une valeur
    omnesagent config set channels.matrix.mention-only true # définir une valeur
    omnesagent config set channels.matrix.access-token      # secret : entrée masquée
    omnesagent config set channels.matrix.stream-mode       # enum : sélection interactive
    omnesagent config init channels.matrix                  # initier la section par défaut
    omnesagent config schema                                # imprimer le schéma JSON vers stdout
    omnesagent config schema > schema.json

    La complétion par tabulation du chemin de propriété est incluse automatiquement dans `omnesagent completions <shell>`.
cli-update-long-about =
    Vérifie et applique les mises à jour de OmnesAgent.
    ...
    Exemples :
    omnesagent update                      # télécharger et installer la dernière version
    omnesagent update --check              # vérifier uniquement, ne pas installer
    omnesagent update --force              # installer sans confirmation
    omnesagent update --version 0.6.0      # installer une version spécifique
cli-self-test-long-about =
    Exécute les tests d'auto-diagnostic pour vérifier l'installation de OmnesAgent.
    ...
    Exemples :
    omnesagent self-test             # ensemble complet de tests
    omnesagent self-test --quick     # tests rapides uniquement (pas de réseau)
...
cli-completions-long-about =
    Génère les scripts de complétion de shell pour `omnesagent`.
...
cli-desktop-long-about =
    Lance l'application de bureau compagnon OmnesAgent.
    ...
    Exemples :
    omnesagent desktop              # lancer l'application compagnon
    omnesagent desktop --install    # ouvrir la page de téléchargement
...
onboard-openai-codex-followup =
    ...
    Exécutez `omnesagent auth login --provider openai-codex` pour vous authentifier avant de démarrer votre agent.
...
cli-doctor-codex-auth-profile-no-slot = Des identifiants OpenAI Codex sont connectés, mais aucun slot de fournisseur de modèle ne les utilise. Définissez `requires_openai_auth = true` sur un slot de fournisseur OpenAI et pointez le `model_provider` d'un agent vers celui-ci, ou exécutez `omnesagent quickstart`.
cli-doctor-codex-auth-slot-no-profile = Les slots OpenAI {$slots} définissent `requires_openai_auth = true`, mais aucun identifiant OpenAI Codex n'est connecté. Exécutez `omnesagent auth login --provider openai-codex`.
...
cli-service-systemd-linger-disabled-warning = la persistance utilisateur systemd est désactivée. Le service utilisateur OmnesAgent peut s'arrêter après la déconnexion. Activez-la avec : loginctl enable-linger {$user}
cli-peripherals-none = Aucun périphérique configuré.
cli-peripherals-add-hint = Ajoutez-en un avec : omnesagent peripheral add <board> <path>
cli-peripherals-add-example = {"  "}Exemple : omnesagent peripheral add nucleo-f401re <serial-path>
...
cli-skills-create-hint = {"  "}Créez-en une : mkdir -p ~/.omnesagent/workspace/skills/my-skill
cli-skills-install-hint = {"  "}Ou installez : omnesagent skills install <source>
...
cli-skills-skipped-scripts-hint = {"    "}Définissez `skills.allow_scripts = true` dans votre configuration omnesagent pour l'activer.
...
cli-quickstart-needs-tty = Quickstart est interactif et nécessite un terminal sur stdin et stderr. Lancez-le depuis un shell interactif, ou utilisez `omnesagent config set <path> <value>` pour une configuration headless.
...
cli-quickstart-summary-channels-none = aucun (discussion uniquement via `omnesagent agent`)
...
cli-quickstart-openai-auth-mode-help = Choisissez `codex` pour utiliser un profil d'authentification par abonnement ChatGPT/Codex. Si vous êtes déjà connecté avec le CLI Codex, exécutez `omnesagent auth login --model-provider openai-codex --import ~/.codex/auth.json` ; sinon exécutez `omnesagent auth login --model-provider openai-codex`.
...
cli-quickstart-auth-codex-skip-hint = {"  "}Terminez plus tard avec : omnesagent auth login --model-provider openai-codex
...
cli-quickstart-auth-anthropic-skip-hint = {"  "}Terminez plus tard avec : claude setup-token, puis omnesagent config set providers.models.anthropic.{$alias}.api_key <token>
...
cli-quickstart-all-channels-bound = {"  "}Tous les canaux configurés sont déjà liés à un agent. Libérez-en un avec `omnesagent config set agents.<alias>.channels ...` avant de le réutiliser ici.
...
cli-quickstart-next-agent-command = {"  "}omnesagent agent -a {$alias}  # discuter avec cet agent dans le terminal
...
cli-onboard-deprecated = `omnesagent onboard` est obsolète — utilisez `omnesagent quickstart`.
cli-otp-initialized = Secret OTP initialisé pour OmnesAgent.
...
cli-pairing-show-only = `omnesagent gateway get-paircode` affiche uniquement un code actif existant ; il n'en génère pas de nouveau.
...
cli-status-title = 🦀 État de OmnesAgent
cli-security-status-title = État de sécurité OmnesAgent
...
cli-desktop-download = Ouverture de la page de téléchargement de l'application compagnon OmnesAgent :
...
cli-desktop-launching = Lancement de l'application compagnon OmnesAgent...
...
cli-desktop-not-installed = L'application compagnon OmnesAgent n'est pas installée.
...
cli-plugin-config-entry-seeded = [[plugins.entries]] initialisé pour '{$name}'. Définissez les valeurs de configuration du plugin avec `omnesagent config set plugins.entries.{$name}.config.<key>`.
cli-plugin-config-entry-key = Clé de configuration ({$capability}) : {$key}
cli-plugin-config-entry-seed-skipped = avertissement : initialisation de l'entrée de configuration ignorée pour '{$name}' : la section [plugins] sur disque est mal formée. Réparez-la, ajoutez un bloc [[plugins.entries]] avec `name = "{$name}"`, puis définissez les valeurs avec `omnesagent config set plugins.entries.{$name}.config.<key>`.
cli-config-section-degraded = avertissement : la section de configuration `{$section}` dans {$path} est mal formée et a été réinitialisée aux valeurs par défaut pour cette exécution. Les valeurs de cette section NE sont PAS appliquées. Exécutez `omnesagent config migrate` pour voir l'erreur d'analyse, puis réparez le fichier.
cli-plugin-removed = Plugin « {$name} » supprimé.
cli-plugin-not-found = Plugin « {$name} » introuvable.
cli-plugin-legacy-detected = Remarque : les plugins situés à un emplacement hérité ({$path}) ne sont pas chargés par l'agent. Exécutez `omnesagent plugin migrate` pour les déplacer vers {$target}.
...
cli-config-review-hint = Exécutez `omnesagent config list` pour vérifier, puis définissez les champs requis.
...
cli-auth-run-paste-redirect = Exécutez `omnesagent auth paste-redirect --model-provider { $provider } --profile { $profile }`
cli-auth-xai-no-pending-login = Aucune connexion xAI en attente trouvée. Exécutez d'abord `omnesagent auth login --model-provider xai`.
...
cli-update-not-writable = le répertoire d'installation { $dir } n'est pas accessible en écriture ({ $error }) ; relancez `omnesagent update` avec des privilèges élevés (sudo sur macOS/Linux, une console Administrateur sous Windows)
...
cli-channels-start-hint = Pour démarrer les canaux : omnesagent channel start
cli-channels-doctor-hint = Pour vérifier l'état :    omnesagent channel doctor
cli-channels-configure-hint = Pour configurer :      omnesagent config set channels.<name>.<field>=<value>
...
channel-runtime-no-cached-models = Aucune liste de modèles en cache trouvée pour `{ $provider }`. Demandez à l'opérateur d'exécuter `omnesagent models refresh --model-provider { $provider }`.
...
cli-gateway-restart-hint-kubernetes = kubectl rollout restart deployment/omnesagent
cli-gateway-restart-hint-container = docker compose restart
cli-gateway-restart-hint-systemd = systemctl restart omnesagent
cli-gateway-restart-hint-launchd = launchctl kickstart -k <your-omnesagent-label>
cli-gateway-restart-hint-process = redémarrez le processus `omnesagent daemon`

cli-daemon-gateway-already-running = Une passerelle OmnesAgent est déjà en cours d'exécution sur {$host}:{$port}. Le démon supervise sa propre passerelle et ne démarrera pas une seconde passerelle sur la même adresse. Arrêtez cette passerelle (ou pointez le démon vers un port libre avec `omnesagent config set gateway.port <port>`), puis relancez le démon.
cli-daemon-gateway-port-occupied = L'adresse de passerelle {$host}:{$port} est déjà utilisée par un autre processus. Libérez le port ou pointez le démon vers un port libre (`omnesagent config set gateway.port <port>`), puis relancez le démon.
cli-daemon-starting-title = 🧠 Le démon OmnesAgent démarre…
cli-daemon-starting-detail = Préparation des endpoints configurés du démon
cli-daemon-started-title = 🧠 Le démon OmnesAgent est prêt
...
cli-doctor-degraded-security = La section de configuration CRITIQUE POUR LA SÉCURITÉ `{$path}` est invalide et a été réinitialisée à sa valeur par défaut pour permettre au daemon de démarrer ; la posture en cours d'exécution peut être PLUS FAIBLE que prévu. Exécutez `omnesagent config migrate` pour voir l'erreur d'analyse, puis réparez le fichier.
cli-doctor-degraded-section = La section de configuration `{$path}` est malformée et a été réinitialisée aux valeurs par défaut ; les valeurs de cette section ne sont PAS en vigueur. Exécutez `omnesagent config migrate` pour voir l'erreur d'analyse, puis réparez le fichier.
