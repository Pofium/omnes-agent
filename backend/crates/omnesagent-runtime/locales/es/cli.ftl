...
cli-try-quickstart = Prueba `omnesagent quickstart` para crear tu primer agente.
...
cli-config-about = Gestiona la configuración de OmnesAgent
cli-update-about = Comprueba y aplica las actualizaciones de OmnesAgent
...
cli-wechat-bound-success = ✅ Cuenta de WeChat vinculada correctamente. Ya puedes hablar con OmnesAgent.
...
cli-skills-install-tier-official = Instalando {$name} v{$version} — Oficial (mantenida por omnesagent-labs)
cli-skills-install-tier-community =
    Instalando {$name} v{$version} — Envío de la comunidad
    Esta skill no está auditada por OmnesAgent. Revisa el contenido de la skill
    y ejecuta `omnesagent skills audit {$name}` antes de otorgar cualquier
...
cli-skills-bundle-add-prompt =
    ...
    (La creación directa de paquetes mediante `omnesagent skills bundle add` duplicaría la superficie de mutación de configuración.)
...
cli-skills-bundle-list-empty =
    ...
    Crea uno: omnesagent config set skill-bundles.default.directory shared/skills/default
...
cli-peripheral-flash-about = Flashea el firmware de OmnesAgent a una placa Arduino
...
cli-migrate-openclaw-about = Importa memoria de un espacio de trabajo OpenClaw a este espacio de trabajo OmnesAgent
cli-agent-long-about =
    ...
    Ejemplos:
    omnesagent agent                              # sesión interactiva
    omnesagent agent -m "Summarize today's logs"  # mensaje único
...
cli-gateway-long-about =
    ...
    Ejemplos:
    omnesagent gateway start              # iniciar gateway
    omnesagent gateway restart            # reiniciar gateway
    omnesagent gateway get-paircode       # mostrar código de emparejamiento
cli-acp-long-about =
    ...
    Ejemplos:
    omnesagent acp                        # iniciar servidor ACP
    omnesagent acp --max-sessions 5       # limitar sesiones concurrentes
cli-daemon-long-about =
    Inicia el daemon autónomo de larga duración.

    Lanza el entorno de ejecución completo de OmnesAgent: servidor de gateway, todos los canales configurados (Telegram, Discord, Slack, etc.), monitor de heartbeat y el programador cron. Esta es la forma recomendada de ejecutar OmnesAgent en producción o como un asistente siempre activo.

    Usa 'omnesagent service install' para registrar el daemon como un servicio del SO (systemd/launchd) para que se inicie automáticamente al arrancar.

    Ejemplos:
    omnesagent daemon                   # usar valores predeterminados de config
    omnesagent daemon -p 9090           # gateway en el puerto 9090
    omnesagent daemon --host 127.0.0.1  # solo localhost
cli-cron-long-about =
...
cli-channel-long-about =
    ...
    Agrega, elimina, lista, envía y verifica el estado de los canales que conectan OmnesAgent con plataformas de mensajería. Tipos de canal admitidos: telegram, discord, slack, whatsapp, matrix, imessage, email.
...
cli-config-long-about =
    Gestiona la configuración de OmnesAgent.
    ...
    Ejemplos:
    omnesagent config list                                  # listar todas las propiedades
    omnesagent config list --secrets                        # listar solo secretos
    omnesagent config list --filter channels.matrix         # filtrar por prefijo
    omnesagent config get channels.matrix.mention-only      # obtener un valor
    omnesagent config set channels.matrix.mention-only true # establecer un valor
    omnesagent config set channels.matrix.access-token      # secreto: entrada enmascarada
    omnesagent config set channels.matrix.stream-mode       # enum: selección interactiva
    omnesagent config init channels.matrix                  # iniciar sección con valores predeterminados
    omnesagent config schema                                # imprimir esquema JSON en stdout
    omnesagent config schema > schema.json

    El autocompletado de la ruta de propiedades se incluye automáticamente en `omnesagent completions <shell>`.
cli-update-long-about =
    Comprueba y aplica actualizaciones de OmnesAgent.
    ...
    Ejemplos:
    omnesagent update                      # descargar e instalar la última
    omnesagent update --check              # solo comprobar, no instalar
    omnesagent update --force              # instalar sin confirmación
    omnesagent update --version 0.6.0      # instalar versión específica
cli-self-test-long-about =
    Ejecuta autodiagnósticos para verificar la instalación de OmnesAgent.
    ...
    Ejemplos:
    omnesagent self-test             # suite completa
    omnesagent self-test --quick     # solo comprobaciones rápidas (sin red)
...
cli-completions-long-about =
    Genera scripts de autocompletado de shell para `omnesagent`.
...
cli-desktop-long-about =
    Lanza la aplicación de escritorio complementaria de OmnesAgent.
    ...
    Ejemplos:
    omnesagent desktop              # lanzar la aplicación complementaria
    omnesagent desktop --install    # abrir la página de descarga
...
onboard-openai-codex-followup =
    ...
    Ejecuta `omnesagent auth login --provider openai-codex` para autenticarte antes de iniciar tu agente.
...
cli-doctor-codex-auth-profile-no-slot = Las credenciales de OpenAI Codex tienen sesión iniciada, pero ningún slot de proveedor de modelo las usa. Establece `requires_openai_auth = true` en un slot de proveedor OpenAI y apunta el `model_provider` de un agente a él, o ejecuta `omnesagent quickstart`.
cli-doctor-codex-auth-slot-no-profile = Los slots OpenAI {$slots} tienen `requires_openai_auth = true`, pero no hay credenciales de OpenAI Codex con sesión iniciada. Ejecuta `omnesagent auth login --provider openai-codex`.
...
cli-service-systemd-linger-disabled-warning = la permanencia de usuario de systemd está deshabilitada. El servicio de usuario de OmnesAgent puede detenerse tras cerrar sesión. Habilítala con: loginctl enable-linger {$user}
cli-peripherals-none = No hay periféricos configurados.
cli-peripherals-add-hint = Agregue uno con: omnesagent peripheral add <board> <path>
cli-peripherals-add-example = {"  "}Ejemplo: omnesagent peripheral add nucleo-f401re <serial-path>
...
cli-skills-create-hint = {"  "}Cree uno: mkdir -p ~/.omnesagent/workspace/skills/my-skill
cli-skills-install-hint = {"  "}O instale: omnesagent skills install <source>
...
cli-skills-skipped-scripts-hint = {"    "}Establece `skills.allow_scripts = true` en tu configuración de omnesagent para habilitarla.
...
cli-quickstart-needs-tty = Quickstart es interactivo y necesita una terminal en stdin y stderr. Ejecútalo desde una shell interactiva, o usa `omnesagent config set <path> <value>` para configuración sin interfaz.
...
cli-quickstart-summary-channels-none = ninguno (chatea solo con `omnesagent agent`)
...
cli-quickstart-auth-codex-skip-hint = {"  "}Termina más tarde con: omnesagent auth login --model-provider openai-codex
...
cli-quickstart-auth-anthropic-skip-hint = {"  "}Termina más tarde con: claude setup-token, luego omnesagent config set providers.models.anthropic.{$alias}.api_key <token>
...
cli-quickstart-all-channels-bound = {"  "}Todos los canales configurados ya están vinculados a un agente. Libera uno con `omnesagent config set agents.<alias>.channels ...` antes de reutilizarlo aquí.
...
cli-quickstart-next-agent-command = {"  "}omnesagent agent -a {$alias}  # chatea con este agente en tu terminal
...
cli-onboard-deprecated = `omnesagent onboard` está obsoleto — usa `omnesagent quickstart`.
cli-otp-initialized = Secreto OTP inicializado para OmnesAgent.
...
cli-pairing-show-only = `omnesagent gateway get-paircode` solo muestra un código activo existente; no genera uno nuevo.
...
cli-status-title = 🦀 Estado de OmnesAgent
cli-security-status-title = Estado de seguridad de OmnesAgent
...
cli-desktop-download = Abriendo la página de descargas de la aplicación complementaria OmnesAgent:
...
cli-desktop-launching = Iniciando la aplicación complementaria de OmnesAgent...
...
cli-desktop-not-installed = La aplicación complementaria de OmnesAgent no está instalada.
...
cli-plugin-config-entry-seeded = Se creó [[plugins.entries]] para '{$name}'. Establece los valores de configuración del plugin con `omnesagent config set plugins.entries.{$name}.config.<key>`.
cli-plugin-config-entry-key = Clave de configuración ({$capability}): {$key}
cli-plugin-config-entry-seed-skipped = advertencia: se omitió crear la entrada de configuración para '{$name}': la sección [plugins] en disco está mal formada. Repárala, agrega un bloque [[plugins.entries]] con `name = "{$name}"`, y luego establece valores con `omnesagent config set plugins.entries.{$name}.config.<key>`.
cli-config-section-degraded = advertencia: la sección de configuración `{$section}` en {$path} está mal formada y se restableció a los valores predeterminados para esta ejecución. Los valores de esa sección NO están en efecto. Ejecuta `omnesagent config migrate` para ver el error de análisis y luego repara el archivo.
cli-plugin-removed = Complemento '{$name}' eliminado.
cli-plugin-not-found = No se encontró el complemento '{$name}'.
cli-plugin-legacy-detected = Nota: los complementos en una ubicación heredada ({$path}) no se cargan en el agente. Ejecuta `omnesagent plugin migrate` para moverlos a {$target}.
...
cli-config-review-hint = Ejecuta `omnesagent config list` para revisar y luego establece los campos requeridos.
...
cli-auth-run-paste-redirect = Ejecuta `omnesagent auth paste-redirect --model-provider {$provider} --profile {$profile}`
cli-auth-xai-no-pending-login = No se encontró un inicio de sesión pendiente de xAI. Ejecuta `omnesagent auth login --model-provider xai` primero.
...
cli-update-not-writable = el directorio de instalación {$dir} no admite escritura ({$error}); vuelve a ejecutar `omnesagent update` con privilegios elevados (sudo en macOS/Linux, una consola de administrador en Windows)
...
cli-channels-start-hint = Para iniciar canales: omnesagent channel start
cli-channels-doctor-hint = Para comprobar el estado:    omnesagent channel doctor
cli-channels-configure-hint = Para configurar:      omnesagent config set channels.<name>.<field>=<value>
...
channel-runtime-no-cached-models = No se encontró una lista de modelos en caché para `{ $provider }`. Pide al operador que ejecute `omnesagent models refresh --model-provider { $provider }`.
...
cli-gateway-restart-hint-process = reinicie el proceso `omnesagent daemon`

cli-daemon-gateway-already-running = Ya hay un gateway de OmnesAgent ejecutándose en {$host}:{$port}. El daemon supervisa su propio gateway y no iniciará un segundo en la misma dirección. Detén ese gateway (o apunta el daemon a un puerto libre con `omnesagent config set gateway.port <port>`) y luego vuelve a ejecutar el daemon.
cli-daemon-gateway-port-occupied = La dirección del gateway {$host}:{$port} ya está en uso por otro proceso. Libera el puerto o apunta el daemon a un puerto libre (`omnesagent config set gateway.port <port>`) y luego vuelve a ejecutar el daemon.
cli-daemon-starting-title = 🧠 El daemon de OmnesAgent se está iniciando…
cli-daemon-starting-detail = Preparando los endpoints configurados del daemon
cli-daemon-started-title = 🧠 El daemon de OmnesAgent está listo
...
cli-doctor-degraded-security = Sección de configuración CRÍTICA PARA LA SEGURIDAD `{$path}` no es válida y se restableció a sus valores predeterminados para que el daemon pueda arrancar; la postura en ejecución puede ser MÁS DÉBIL de lo previsto. Ejecute `omnesagent config migrate` para ver el error de análisis y luego repare el archivo.
cli-doctor-degraded-section = La sección de configuración `{$path}` está mal formada y se restableció a sus valores predeterminados; los valores de esa sección NO están en efecto. Ejecute `omnesagent config migrate` para ver el error de análisis y luego repare el archivo.
