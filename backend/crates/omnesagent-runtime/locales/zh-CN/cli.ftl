...
cli-try-quickstart = 尝试运行 `omnesagent quickstart` 来创建你的第一个智能体。
...
cli-config-about = 管理 OmnesAgent 配置
cli-update-about = 检查并应用 OmnesAgent 更新
...
cli-wechat-bound-success = ✅ WeChat 账号绑定成功。现在可以和 OmnesAgent 对话了。
...
cli-skills-install-tier-official = 正在安装 {$name} v{$version} — 官方（omnesagent-labs 维护）
cli-skills-install-tier-community =
    正在安装 {$name} v{$version} — 社区提交
    此技能未经 OmnesAgent 审计。请检查技能内容，
    并在授予任何权限或用于生产前运行 `omnesagent skills audit {$name}`。
...
cli-skills-bundle-add-prompt =
    ...
    （通过 `omnesagent skills bundle add` 直接创建包会重复配置变更接口。）
...
cli-skills-bundle-list-empty =
    未配置技能包。
    创建一个：omnesagent config set skill-bundles.default.directory shared/skills/default
...
cli-peripheral-flash-about = 将 OmnesAgent 固件刷写到 Arduino 开发板
cli-sop-list-about = 列出已加载的 SOP
cli-sop-validate-about = 验证 SOP 定义
cli-sop-show-about = 显示 SOP 的详细信息
cli-migrate-openclaw-about = 将 OpenClaw 工作区中的记忆导入到此 OmnesAgent 工作区
cli-agent-long-about =
    ...
    示例：
    omnesagent agent                              # 交互式会话
    omnesagent agent -m "Summarize today's logs"  # 单条消息
...
cli-gateway-long-about =
    ...
    示例：
    omnesagent gateway start              # 启动网关
    omnesagent gateway restart            # 重启网关
    omnesagent gateway get-paircode       # 显示配对码
cli-acp-long-about =
    ...
    示例：
    omnesagent acp                        # 启动 ACP 服务器
    omnesagent acp --max-sessions 5       # 限制并发会话数
cli-daemon-long-about =
    启动长期运行的自主守护进程。

    启动完整的 OmnesAgent 运行时：网关服务器、所有已配置的通道（Telegram、Discord、Slack 等）、心跳监视器以及 cron 调度器。这是在生产环境中或作为始终在线助手运行 OmnesAgent 的推荐方式。

    使用 'omnesagent service install' 将守护进程注册为操作系统服务（systemd/launchd），以便开机自动启动。

    示例：
    omnesagent daemon                   # 使用配置默认值
    omnesagent daemon -p 9090           # 网关在端口 9090
    omnesagent daemon --host 127.0.0.1  # 仅 localhost
cli-cron-long-about =
    配置和管理计划任务。
...
cli-channel-long-about =
    ...
    添加、删除、列出、发送以及对将 OmnesAgent 连接到消息平台的通道进行健康检查。支持的通道类型：telegram、discord、slack、whatsapp、matrix、imessage、email。
...
cli-config-long-about =
    ...
    示例：
    omnesagent config list                                  # 列出所有属性
    omnesagent config list --secrets                        # 仅列出密钥
    omnesagent config list --filter channels.matrix         # 按前缀过滤
    omnesagent config get channels.matrix.mention-only      # 获取值
    omnesagent config set channels.matrix.mention-only true # 设置值
    omnesagent config set channels.matrix.access-token      # 密钥：掩码输入
    omnesagent config set channels.matrix.stream-mode       # 枚举：交互式选择
    omnesagent config init channels.matrix                  # 使用默认值初始化部分
    omnesagent config schema                                # 将 JSON Schema 打印到 stdout
    omnesagent config schema > schema.json

    属性路径 Tab 补全会自动包含在 `omnesagent completions <shell>` 中。
cli-update-long-about =
    ...
    示例：
    omnesagent update                      # 下载并安装最新版本
    omnesagent update --check              # 仅检查，不安装
    omnesagent update --force              # 不确认直接安装
    omnesagent update --version 0.6.0      # 安装特定版本
cli-self-test-long-about =
    运行诊断自检以验证 OmnesAgent 安装。

    默认情况下，运行完整的测试套件，包括网络检查（网关健康状况、记忆往返）。使用 --quick 跳过网络检查以进行更快的离线验证。

    示例：
    omnesagent self-test             # 完整套件
    omnesagent self-test --quick     # 仅快速检查（无网络）
...
cli-desktop-long-about =
    ...
    示例：
    omnesagent desktop              # 启动配套应用
    omnesagent desktop --install    # 打开下载页面
...
onboard-openai-codex-followup =
    ...
    在启动代理之前，运行 `omnesagent auth login --provider openai-codex` 进行身份验证。
...
cli-doctor-codex-auth-profile-no-slot = OpenAI Codex 凭据已登录，但没有模型提供方槽位使用它们。请在 OpenAI 提供方槽位上设置 `requires_openai_auth = true`，并将 agent 的 `model_provider` 指向它，或运行 `omnesagent quickstart`。
cli-doctor-codex-auth-slot-no-profile = OpenAI 槽位 {$slots} 已设置 `requires_openai_auth = true`，但没有 OpenAI Codex 凭据登录。请运行 `omnesagent auth login --provider openai-codex`。
...
cli-service-systemd-linger-disabled-warning = systemd 用户 linger 已禁用。OmnesAgent 的用户服务可能会在注销后停止。启用命令：loginctl enable-linger {$user}
cli-peripherals-none = 未配置外设。
cli-peripherals-add-hint = 使用以下命令添加: omnesagent peripheral add <board> <path>
cli-peripherals-add-example = {"  "}示例: omnesagent peripheral add nucleo-f401re <serial-path>
...
cli-skills-create-hint = {"  "}创建一个: mkdir -p ~/.omnesagent/workspace/skills/my-skill
cli-skills-install-hint = {"  "}或安装: omnesagent skills install <source>
...
cli-skills-skipped-scripts-hint = {"    "}在 omnesagent 配置中设置 `skills.allow_scripts = true` 以启用它。
...
cli-quickstart-needs-tty = Quickstart 是交互式流程，需要 stdin 和 stderr 连接到终端。请从交互式 shell 中运行，或使用 `omnesagent config set <path> <value>` 进行无头配置。
...
cli-quickstart-summary-channels-none = 无（仅通过 `omnesagent agent` 聊天）
...
cli-quickstart-openai-auth-mode-help = 选择 `codex` 以使用 ChatGPT/Codex 订阅身份验证配置文件。如果你已通过 Codex CLI 登录，请运行 `omnesagent auth login --model-provider openai-codex --import ~/.codex/auth.json`；否则请运行 `omnesagent auth login --model-provider openai-codex`。
...
cli-quickstart-auth-codex-skip-hint = {"  "}稍后完成：omnesagent auth login --model-provider openai-codex
...
cli-quickstart-auth-anthropic-skip-hint = {"  "}稍后完成：claude setup-token，然后 omnesagent config set providers.models.anthropic.{$alias}.api_key <token>
...
cli-quickstart-all-channels-bound = {"  "}所有已配置的通道都已经绑定到 agent。若要在这里复用，请先用 `omnesagent config set agents.<alias>.channels ...` 释放一个通道。
...
cli-quickstart-next-agent-command = {"  "}omnesagent agent -a {$alias}  # 在终端中与此 agent 聊天
...
cli-onboard-deprecated = `omnesagent onboard` 已弃用 — 请使用 `omnesagent quickstart`。
cli-otp-initialized = 已为 OmnesAgent 初始化 OTP 密钥。
...
cli-pairing-show-only = `omnesagent gateway get-paircode` 只显示现有的活动配对码，不会生成新配对码。
...
cli-status-title = 🦀 OmnesAgent 状态
cli-security-status-title = OmnesAgent 安全状态
...
cli-desktop-not-installed = 未安装 OmnesAgent 配套应用。
...
cli-plugin-config-entry-seeded = 已为 '{$name}' 创建 [[plugins.entries]]。使用 `omnesagent config set plugins.entries.{$name}.config.<key>` 设置插件配置值。
cli-plugin-config-entry-key = 配置条目键（{$capability}）：{$key}
cli-plugin-config-entry-seed-skipped = 警告：已跳过为 '{$name}' 创建配置条目：磁盘上的 [plugins] 部分格式不正确。请修复它，添加带有 `name = "{$name}"` 的 [[plugins.entries]] 块，然后使用 `omnesagent config set plugins.entries.{$name}.config.<key>` 设置值。
cli-config-section-degraded = 警告：{$path} 中的配置部分 `{$section}` 格式不正确，本次运行已重置为默认值。该部分中的值不会生效。请运行 `omnesagent config migrate` 查看解析错误，然后修复文件。
cli-plugin-removed = 已移除插件“{$name}”。
cli-plugin-not-found = 未找到插件“{$name}”。
cli-plugin-legacy-detected = 注意：位于旧位置（{$path}）的插件未被代理加载。请运行 `omnesagent plugin migrate` 将其移动到 {$target}。
...
cli-config-review-hint = 运行 `omnesagent config list` 进行查看，然后设置必填字段。
...
cli-auth-run-paste-redirect = 运行 `omnesagent auth paste-redirect --model-provider {$provider} --profile {$profile}`
cli-auth-xai-no-pending-login = 未找到待处理的 xAI 登录。请先运行 `omnesagent auth login --model-provider xai`。
...
cli-update-not-writable = 安装目录 {$dir} 不可写（{$error}）；请使用更高权限重新运行 `omnesagent update`（在 macOS/Linux 上使用 sudo，在 Windows 上使用管理员控制台）
...
cli-channels-start-hint = 启动渠道：omnesagent channel start
cli-channels-doctor-hint = 检查健康状况：    omnesagent channel doctor
cli-channels-configure-hint = 配置方法：      omnesagent config set channels.<name>.<field>=<value>
...
channel-runtime-no-cached-models = 未找到 `{ $provider }` 的缓存模型列表。请让操作者运行 `omnesagent models refresh --model-provider { $provider }`。
...
cli-gateway-restart-hint-process = 重启 `omnesagent daemon` 进程

cli-daemon-gateway-already-running = OmnesAgent gateway 已在 {$host}:{$port} 运行。daemon 会管理自己的 gateway，不会在同一地址启动第二个 gateway。请停止该 gateway（或使用 `omnesagent config set gateway.port <port>` 将 daemon 指向空闲端口），然后重新运行 daemon。
cli-daemon-gateway-port-occupied = Gateway 地址 {$host}:{$port} 已被另一个进程占用。请释放该端口或将 daemon 指向空闲端口（`omnesagent config set gateway.port <port>`），然后重新运行 daemon。
cli-daemon-starting-title = 🧠 OmnesAgent daemon 正在启动…
cli-daemon-starting-detail = 正在准备已配置的 daemon endpoint
cli-daemon-started-title = 🧠 OmnesAgent daemon 已就绪
...
cli-doctor-degraded-security = 安全关键配置节 `{$path}` 无效，已重置为默认值以便守护进程启动；当前运行的安全态势可能弱于预期。运行 `omnesagent config migrate` 查看解析错误，然后修复该文件。
cli-doctor-degraded-section = 配置节 `{$path}` 格式错误，已重置为默认值；该节中的值当前不生效。运行 `omnesagent config migrate` 查看解析错误，然后修复该文件。
