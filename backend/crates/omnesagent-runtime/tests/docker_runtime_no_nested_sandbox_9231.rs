...
use omnesagent_config::platform::DockerRuntime;
use omnesagent_config::schema::{Config, RuntimeKind};
use omnesagent_runtime::security::{AutonomyLevel, SecurityPolicy, sandbox_posture};
use omnesagent_runtime::tools::shell_tool_for_runtime;
...
"#,
    ...
    let _config_dir = EnvGuard::set("OMNESAGENT_CONFIG_DIR", install.path());
    let _data_dir = EnvGuard::remove("OMNESAGENT_DATA_DIR");
    let _legacy_workspace = EnvGuard::remove("OMNESAGENT_WORKSPACE");
    ...
    let posture = sandbox_posture(
        ...
        config.runtime.kind,
        Some(install.path()),
        &omnesagent_runtime::security::SandboxExtraRoots::default(),
    );
    assert_eq!(
