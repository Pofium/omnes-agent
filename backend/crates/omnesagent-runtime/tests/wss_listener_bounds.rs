...
//! `omnesagent_spawn::spawn!` rule is for production daemon tasks).
...
use omnesagent_runtime::rpc::wss::WssLimits;
...
fn materials(
    ...
    omnesagent_tls::ServerMaterials,
    tempfile::NamedTempFile,
    tempfile::NamedTempFile,
) {
    let mats = omnesagent_tls::ensure_server_materials(dir, &[]).unwrap();
    let ca_pem = std::fs::read_to_string(&mats.ca_cert_path).unwrap();
    let ca_key_pem = std::fs::read_to_string(&mats.ca_key_path).unwrap();
    let client = omnesagent_tls::issue_client_cert(&ca_pem, &ca_key_pem, "bounds-test").unwrap();
    (
        mats,
...
fn client_config(client: Option<(&Path, &Path)>) -> rustls::ClientConfig {
    ...
    match client {
        Some((cert, key)) => {
            let chain = omnesagent_tls::load_certs(cert.to_str().unwrap()).unwrap();
            let key = omnesagent_tls::load_private_key(key.to_str().unwrap()).unwrap();
...
async fn start_listener(
    ...
    let acceptor = omnesagent_runtime::rpc::wss::build_tls_acceptor(
    ...
    let sessions = Arc::new(omnesagent_runtime::rpc::session::SessionStore::new(16, queue));
    let ctx = omnesagent_runtime::rpc::context::RpcContext::for_live_test(config, sessions);
    ...
    tokio::spawn(async move {
        let _ = omnesagent_runtime::rpc::wss::run_wss_listener(
            ctx,
            cancel_for_task,
