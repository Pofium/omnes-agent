...
impl ClientCred {
    fn issue(ca_pem: &str, ca_key_pem: &str, name: &str) -> Self {
        let issued = omnesagent_tls::issue_client_cert(ca_pem, ca_key_pem, name).unwrap();
        Self {
    ...
    fn rustls_config(&self) -> rustls::ClientConfig {
        ...
        let chain = omnesagent_tls::load_certs(self.cert.path().to_str().unwrap()).unwrap();
        let key = omnesagent_tls::load_private_key(self.key.path().to_str().unwrap()).unwrap();
