...
//! auto-generated server materials (`omnesagent_tls::ensure_server_materials`) and
//! a client certificate issued from that CA (`omnesagent_tls::issue_client_cert`).
...
fn materials(
    ...
    let client = omnesagent_tls::issue_client_cert(&ca_pem, &ca_key_pem, "test-device").unwrap();
    (
        mats,
...
async fn ca_issued_cert_absent_from_the_ledger_still_authenticates() {
    ...
    let ledger = omnesagent_runtime::security::cert_ledger::CertLedger::open_at(
        dir.path(),
        None,
        dir.path().join("tls").join("revoked"),
    )
    .unwrap();
    let der = omnesagent_tls::load_certs(client_cert.path().to_str().unwrap()).unwrap();
    let fp = omnesagent_tls::cert_sha256_fingerprint(der[0].as_ref());
    assert!(
