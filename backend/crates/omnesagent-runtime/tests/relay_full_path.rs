...
//!
//! Drives the REAL [`omnesagent_runtime::relay::run_relay_bridge`] (signed Ed25519
//! registration, keepalive, loopback bridging) against the REAL
//! [`omnesrelay::RelayServer`] (outer TLS, signed admission, multiplexer) and a
...
use omnesagent_relay_proto::{Control, SUBPROTOCOL, decode_data, encode_data};
...
async fn zerocode_to_relay_to_daemon_full_path() {
    ...
    let mats = omnesagent_tls::ensure_server_materials(dir.path(), &[]).unwrap();
    let acceptor: TlsAcceptor = omnesagent_runtime::rpc::wss::build_tls_acceptor(
    ...
    let issued = omnesagent_tls::issue_client_cert(&ca_pem, &ca_key_pem, "relay-device").unwrap();
    let cert_f = write_temp(&issued.cert_pem);
    let key_f = write_temp(&issued.key_pem);
    let client_chain = omnesagent_tls::load_certs(cert_f.path().to_str().unwrap()).unwrap();
    let client_key = omnesagent_tls::load_private_key(key_f.path().to_str().unwrap()).unwrap();
    ...
    tokio::spawn(
        omnesrelay::RelayServer::new(omnesrelay::RelayConfig::default())
            .serve(relay_listener, relay_acceptor),
    );

    // The REAL daemon-side bridge: signed Ed25519 registration over outer TLS.
    let signing_key = omnesagent_runtime::relay::ensure_signing_key(dir.path()).unwrap();
    let cancel = CancellationToken::new();
    tokio::spawn(omnesagent_runtime::relay::run_relay_bridge(
        omnesagent_runtime::relay::RelayBridgeConfig {
...
async fn enrollment_post_round_trips_through_relay_route() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let mats = omnesagent_tls::ensure_server_materials(dir.path(), &[]).unwrap();
    let enroll_acceptor = omnesagent_tls::build_tls_acceptor(&omnesagent_tls::ServerConfigParams {
        cert_path: mats.server_cert_path.to_string_lossy().into_owned(),
        key_path: mats.server_key_path.to_string_lossy().into_owned(),
        client_auth: None,
    })
    .unwrap();
    let pairing = omnesagent_config::pairing::PairingGuard::new(true, &[]);
    ...
    let enroll_server = Arc::new(omnesagent_runtime::enroll::EnrollServer {
        ...
        ledger: Arc::new(
            omnesagent_runtime::security::cert_ledger::CertLedger::open_in_memory(None).unwrap(),
        ),
        pairing: Arc::new(pairing),
        static_client_pins_configured: false,
        allow_unpaired_until: None,
        relay_profile: omnesagent_runtime::enroll::RelayProfile::default(),
        bridge_ports: None,
        relay_attempt_bucket: omnesagent_runtime::enroll::RelayAttemptBucket::default(),
        paircode_admin_data_dir: None,
    });
    let enroll_ledger = enroll_server.ledger.clone();
    let enroll_task = tokio::spawn(omnesagent_runtime::enroll::serve_on(
        enroll_listener,
        enroll_server,
        enroll_cancel.clone(),
    ));
    ...
    let (csr, _key) = omnesagent_tls::testing::gen_client_csr("ignored-by-daemon");
