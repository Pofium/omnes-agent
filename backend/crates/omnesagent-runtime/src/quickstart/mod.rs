...
use omnesagent_config::helpers::kebab_to_snake;
...
use omnesagent_config::schema::{Config, WireApi};
...
pub fn validate_only_with_surface(
    ...
    if ok {
        ::omnesagent_log::record!(
            INFO,
            ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Validate)
...
pub async fn apply_with_surface(
    ...
    ::omnesagent_log::record!(
        DEBUG,
        ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Write).with_attrs(
            merge_attrs(
                ctx.base_attrs(),
...
pub struct QuickstartState {
    ...
    /// Risk presets from `omnesagent_config::presets::RISK_PRESETS`.
    pub risk_presets: &'static [omnesagent_config::presets::RiskPreset],
    /// Runtime presets from `omnesagent_config::presets::RUNTIME_PRESETS`.
    pub runtime_presets: &'static [omnesagent_config::presets::RuntimePreset],
...
pub fn resolve_model_provider_type(type_key: &str) -> Option<(&'static str, bool)> {
    ...
    if let Some(info) = omnesagent_providers::list_model_providers()
        .into_iter()
    ...
    let provider = trimmed
        .parse::<omnesagent_providers::auth::AuthProvider>()
        .ok()?;
    Some(match provider {
        omnesagent_providers::auth::AuthProvider::OpenaiCodex => ("openai", true),
        omnesagent_providers::auth::AuthProvider::Anthropic => ("anthropic", false),
        omnesagent_providers::auth::AuthProvider::Gemini => ("gemini", false),
        omnesagent_providers::auth::AuthProvider::Xai => ("xai", false),
    })
}
...
/// (`omnesagent_providers::list_model_providers()` for providers,
...
pub fn snapshot_state(cfg: &Config) -> QuickstartState {
    let model_provider_types = omnesagent_providers::list_model_providers()
        .into_iter()
        .map(|info| QuickstartTypeOption {
            kind: info.name.to_string(),
            display_name: info.display_name.to_string(),
            local: info.local,
            default_runtime_profile: omnesagent_providers::recommended_runtime_profile(info.name)
    ...
    QuickstartState {
        ...
        channel_types,
        risk_presets: omnesagent_config::presets::RISK_PRESETS,
        runtime_presets: omnesagent_config::presets::RUNTIME_PRESETS,
...
fn memory_kind_keys() -> Vec<String> {
    use omnesagent_config::multi_agent::MemoryBackendKind as M;
    [
        M::Sqlite,
        M::Markdown,
        M::Postgres,
...
fn build_channel_type_options(
    channels_cfg: &omnesagent_config::schema::ChannelsConfig,
...
pub struct FieldDescriptor {
    ...
    /// `PropKind::wire_name`.
    pub kind: omnesagent_config::traits::PropKind,
...
pub fn field_shape(section: FieldSection, type_key: &str) -> Vec<FieldDescriptor> {
    ...
    for info in probe.prop_fields() {
        ...
        let default = if section_path == "channels.webhook" && field_path == "port" {
            Some(omnesagent_config::schema::DEFAULT_WEBHOOK_CHANNEL_PORT.to_string())
        } else if info.is_secret {
            None
        } else {
            let raw = info.display_value.trim();
            if raw.is_empty() || raw == omnesagent_config::traits::UNSET_DISPLAY {
                None
            } else {
                Some(raw.to_string())
            }
        ...
        SelectorChoice::Fresh(choice) => {
            ...
            for (key, value) in entries {
                ...
                if let Err(err) = config.set_prop_persistent(&format!("{prefix}.{key}"), value) {
                    errors.push(QuickstartError::new(
                        QuickstartStep::ModelProvider,
                        omnesagent_config::helpers::kebab_to_snake(key),
                        err.to_string(),
                    ));
                    return None;
                }
            ...
            let provider_config = omnesagent_config::schema::ModelProviderConfig {
            ...
            if tokio::runtime::Handle::try_current()
                ...
                && let Some(ctx) = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current().block_on(
                        omnesagent_providers::fetch_context_window(provider_type, &provider_config),
                    )
                })
            {
                let _ = config
...
fn usable_quickstart_value(value: &str) -> Option<&str> {
    ...
    (!omnesagent_config::traits::is_unset_display_value(value)).then_some(value)
}
            ...
            SelectorChoice::Fresh(entry) => {
                ...
                if config_channel_type == "webhook"
                    ...
                    && let Err(err) = staged.set_prop_persistent(
                        &format!("{prefix}.port"),
                        &omnesagent_config::schema::DEFAULT_WEBHOOK_CHANNEL_PORT.to_string(),
                    )
                {
                    errors.push(QuickstartError::new(
                        QuickstartStep::Channels,
                ...
                {
                    let structured =
                        omnesagent_config::api_error::ConfigApiError::from_validation(err);
...
fn apply_peer_groups(
    config: &mut Config,
    peer_groups: &[omnesagent_config::presets::QuickstartPeerGroup],
...
fn apply_personality_files(
    config: &Config,
    agent_alias: &str,
    files: &[omnesagent_config::presets::QuickstartPersonalityFile],
...
fn apply_agent(
    ...
    // before.
    if let Err(err) =
        omnesagent_config::alias_refs::create_map_key_checked(config, "agents", &identity.name)
    {
        errors.push(QuickstartError::new(
            QuickstartStep::Agent,
            "name",
...
pub async fn model_catalog(
    ...
    Vec<String>,
    Option<std::collections::HashMap<String, omnesagent_api::model_provider::ModelPricing>>,
    bool,
) {
...
pub async fn model_catalog_with_config(
    ...
    let handle = omnesagent_providers::create_model_provider_with_url(
        family,
        api_key.as_deref(),
        api_url.as_deref(),
    );
    if let Ok(handle) = handle
        && let Ok(models) = omnesagent_providers::ProviderDispatch::from_ref(&*handle)
        ...
        && !models.is_empty()
    {
        let raw_pricing: std::collections::HashMap<
            String,
            omnesagent_api::model_provider::ModelPricing,
        > = models
            .iter()
            .filter_map(|m| m.pricing.as_ref().map(|p| (m.id.clone(), p.clone())))
            .collect();
        let ids = models.into_iter().map(|m| m.id).collect();
        let Some(ids) = omnesagent_providers::catalog::sort_model_catalog_for_chat(family, ids)
        else {
            return (Vec::new(), None, false);
        };
        let pricing: std::collections::HashMap<String, omnesagent_api::model_provider::ModelPricing> =
            ids.iter()
    ...
    match omnesagent_providers::catalog::list_models_for_family(family).await {
        Ok(models) if !models.is_empty() => (
            omnesagent_providers::catalog::sort_model_catalog_for_chat(family, models)
                .unwrap_or_default(),
            None,
            true,
        ),
...
mod tests {
    ...
    fn existing_postgres_memory_storage_ref_is_accepted() {
        ...
        cfg.storage.postgres.insert(
            "default".into(),
            omnesagent_config::schema::PostgresStorageConfig::default(),
        );
    ...
    fn validate_only_accepts_every_builtin_risk_preset() {
        let cfg = Config::default();
        for p in omnesagent_config::presets::RISK_PRESETS {
            let mut submission = fresh_submission("bot");
    ...
    fn field_shape_openai_includes_codex_auth_mode() {
        ...
        for row in &rows {
            assert_ne!(
                ...
                Some(omnesagent_config::traits::UNSET_DISPLAY),
    ...
    async fn multiple_channels_all_bind_to_agent() {
        ...
        let store = omnesagent_config::secrets::SecretStore::new(dir.path(), true);
        assert_eq!(
            store
    ...
    async fn webhook_channel_quickstart_fields_persist() {
        ...
        assert!(omnesagent_config::secrets::SecretStore::is_encrypted(
            stored_secret
        ));
        let store = omnesagent_config::secrets::SecretStore::new(dir.path(), true);
    ...
    fn webhook_channel_defaults_port_and_rejects_unusable_secret() {
        ...
        assert_eq!(
            ...
            omnesagent_config::schema::DEFAULT_WEBHOOK_CHANNEL_PORT
        );
    }
    ...
    fn webhook_channel_rejects_duplicate_enabled_port_against_existing_alias() {
        ...
        cfg.channels.webhook.insert(
            ...
            omnesagent_config::schema::WebhookConfig {
                enabled: true,
                port: omnesagent_config::schema::DEFAULT_WEBHOOK_CHANNEL_PORT,
                ..Default::default()
            },
        );
        let channels = vec![SelectorChoice::Fresh(fresh_channel(
    ...
    fn webhook_channel_rejects_existing_selection_with_duplicate_enabled_port() {
        ...
        assert!(cfg.channels.webhook.values().all(|webhook| {
            webhook.enabled && webhook.port == omnesagent_config::schema::DEFAULT_WEBHOOK_CHANNEL_PORT
        }));
    }
    ...
    async fn fresh_whatsapp_web_channel_persists_under_whatsapp_config_family() {
        for submitted_type in ["whatsapp-web", "whatsapp_web"] {
            ...
            submission.peer_groups = vec![omnesagent_config::presets::QuickstartPeerGroup {
                name: "self_chat".into(),
    ...
    async fn model_catalog_with_config_uses_native_endpoint_when_credentialed() {
        ...
        config.providers.models.xai.insert(
            "default".to_string(),
            omnesagent_config::schema::XaiModelProviderConfig {
