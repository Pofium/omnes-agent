...
use omnesagent_plugins::PluginCapability;
#[cfg(feature = "plugins-wasm")]
use omnesagent_plugins::error::PluginError;
#[cfg(feature = "plugins-wasm")]
...
#[cfg(feature = "plugins-wasm")]
pub(crate) fn plugin_limits(config: &Config) -> omnesagent_plugins::component::PluginLimits {
...
pub async fn configured_plugin_channels(
    ...
    #[cfg(feature = "plugins-wasm")]
    {
        ...
        for scope in scopes {
            ...
            let endpoint =
                match omnesagent_plugins::endpoint::PluginChannelEndpoint::new(scope, "plugin") {
            ...
            match omnesagent_plugins::wasm_channel::WasmChannel::from_wasm(
                endpoint,
                wasm_path,
                &host_services,
                limits,
        ...
        ::omnesagent_log::record!(
            INFO,
            ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Load).with_attrs(
                ::serde_json::json!({
...
mod tests {
    ...
    use omnesagent_config::schema::{AliasedAgentConfig, PluginChannelConfig};
    ...
    fn explicit_channel_keeps_the_exact_host_alias_and_plugin_family() {
        ...
        let scope = plan
            ...
            .unwrap();
        let endpoint =
            omnesagent_plugins::endpoint::PluginChannelEndpoint::new(scope, "plugin").unwrap();
