...
use omnesagent_config::cost::tracker::CostTracker;
...
pub struct RpcContext {
    ...
    /// Memory subsystem (`dyn Memory` from `omnesagent-api`).
    pub memory: Option<Arc<dyn omnesagent_api::memory_traits::Memory>>,
    ...
    /// `sign_events = true` with no usable `OMNESAGENT_AUDIT_SIGNING_KEY`).
...
impl RpcContext {
    ...
    pub fn minimal_with_memory(
        config: Config,
        sessions: Arc<SessionStore>,
        memory: Arc<dyn omnesagent_api::memory_traits::Memory>,
    ) -> Arc<Self> {
        Arc::new(Self {
            config: Arc::new(RwLock::new(config)),
            config_write_lock: Arc::new(tokio::sync::Mutex::new(())),
