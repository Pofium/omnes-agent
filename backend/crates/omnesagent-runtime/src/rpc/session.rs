...
impl SessionStore {
    ...
    pub async fn seed_history(&self, id: &str, msgs: &[omnesagent_api::model_provider::ChatMessage]) {
    ...
    pub async fn seed_history_with_event(
        &self,
        id: &str,
        msgs: &[omnesagent_api::model_provider::ChatMessage],
    ) -> Option<TurnEvent> {
    ...
    pub async fn seed_conversation_history(
        &self,
        id: &str,
        msgs: Vec<omnesagent_api::model_provider::ConversationMessage>,
    ) {
    ...
    pub async fn history_slice_from(
        ...
        id: &str,
        from: usize,
    ) -> Option<Vec<omnesagent_api::model_provider::ConversationMessage>> {
        let sessions = self.sessions.lock().await;
        let s = sessions.get(id)?;
...
mod tests {
    ...
    fn make_agent() -> Agent {
        ...
        let mem_cfg = omnesagent_config::schema::MemoryConfig {
            backend: "none".into(),
            ..omnesagent_config::schema::MemoryConfig::default()
        };
        let mem = Arc::from(
            omnesagent_memory::create_memory(&mem_cfg, &std::env::temp_dir(), None).unwrap(),
        );
    ...
    impl omnesagent_providers::ModelProvider for StubProvider {
        ...
        async fn chat(
            &self,
            _: omnesagent_providers::ChatRequest<'_>,
            _: &str,
            _: Option<f64>,
    ...
    impl omnesagent_api::attribution::Attributable for StubProvider {
