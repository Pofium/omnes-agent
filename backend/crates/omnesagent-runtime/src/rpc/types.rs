//! Shared request/response types for the OmnesAgent RPC + gateway API surface.

use serde::{Deserialize, Serialize};
use serde_json::Value;

// ── Re-exports: types that already derive Serialize + Deserialize ────
// Consumers can `use omnesagent_runtime::rpc::types::*` and get everything.
...
pub use omnesagent_api::memory_traits::{MemoryCategory, MemoryEntry};
pub use omnesagent_api::runtime_status::RuntimeConfigKind;
pub use omnesagent_config::cost::types::CostSummary;
pub use omnesagent_config::traits::{ConfigFieldEntry, PropKind};
...
rpc_type! {
    /// Owned wire representation of a [`omnesagent_config::traits::MapKeySection`].
    ...
    pub struct ConfigTemplateEntry {
        pub path: String,
        pub kind: omnesagent_config::traits::MapKeyKind,
        pub value_type: String,
        pub description: String,
    }
}

impl From<omnesagent_config::traits::MapKeySection> for ConfigTemplateEntry {
    fn from(s: omnesagent_config::traits::MapKeySection) -> Self {
        Self {
...
rpc_type! {
    ...
    pub struct CatalogModelsResult {
        ...
        #[serde(default, skip_serializing_if = "Option::is_none")]
        pub pricing: Option<std::collections::HashMap<String, omnesagent_api::model_provider::ModelPricing>>,
        pub local: bool,
        pub live: bool,
    }
}
...
rpc_type! {
    ...
    pub struct ConfigSectionEntry {
        ...
        /// backend picker).
        #[serde(default, skip_serializing_if = "Option::is_none")]
        pub shape: Option<omnesagent_config::sections::SectionShape>,
        #[serde(default)]
...
pub enum SessionUpdateEvent {
    ...
    Plan {
        session_id: String,
        entries: Vec<omnesagent_api::plan::PlanEntry>,
    },
...
pub use omnesagent_config::presets::BuilderSubmission;

rpc_type! {
    /// Mirrors `omnesagent_gateway::api_quickstart::QuickstartState`.
    pub struct QuickstartStateResult {
        ...
        /// the canonical `omnesagent_providers::list_model_providers()`
...
mod tests {
    ...
    fn logs_query_result_exposes_active_log_path_when_present() {
        let result = LogsQueryResult {
            events: Vec::new(),
            log_path: Some("/var/lib/omnesagent/runtime-trace.jsonl".into()),
            next_cursor: None,
        ...
        assert_eq!(
            value["log_path"],
            json!("/var/lib/omnesagent/runtime-trace.jsonl")
        );
    }
}
