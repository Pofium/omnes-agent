...
mod platform {
    ...
    ///
    /// Because `OMNESAGENT_SOCKET` may place the sibling in a shared directory,
        ...
        if mode & 0o022 != 0 && mode & 0o1000 == 0 {
            anyhow::bail!(
                "local IPC endpoint lock directory {} is writable by other \
                 ...
                 Restrict it (chmod go-w or +t) or point OMNESAGENT_SOCKET at a \
                 private directory",
                parent.display()
            );
        }
    ...
    /// (e.g. `/tmp` for an external `OMNESAGENT_SOCKET`) rejects it and is
...
mod platform {
    ...
    pub fn default_endpoint(data_dir: &Path) -> PathBuf {
        ...
        PathBuf::from(format!(r"\\.\pipe\omnesagent-{:x}", hasher.finish()))
    }
...
mod tests {
    ...
    use omnesagent_api::jsonrpc::{JSONRPC_VERSION, JsonRpcRequest};
    ...
    async fn socket_rejects_before_initialize() {
        ...
        assert_eq!(
            resp["error"]["code"],
            omnesagent_api::jsonrpc::error_codes::AUTH_REQUIRED
        );
    ...
    #[cfg(unix)]
    const ENDPOINT_LOCK_TEST_CHILD_ENV: &str = "OMNESAGENT_ENDPOINT_LOCK_TEST_CHILD";
    ...
    async fn endpoint_lock_supports_sticky_shared_directory() {
        ...
        // The /tmp shape a documented external OMNESAGENT_SOCKET override can
