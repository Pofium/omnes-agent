...
use omnesagent_api::jsonrpc::JsonRpcError;
...
pub async fn process_file_entry(
    ...
    let ext = std::path::Path::new(&sanitized)
        ...
        .unwrap_or_default();
    let dest = omnesagent_tools::embedded_resource::persist_content_addressed(
...
mod tests {
    ...
    fn make_session_store(max: usize) -> SessionStore {
        SessionStore::new(
            max,
            std::sync::Arc::new(omnesagent_infra::session_queue::SessionActorQueue::new(
                4, 10, 60,
            )),
        )
    }
    ...
    // regressions in `omnesagent-tools`.
    #[tokio::test]
    ...
    async fn path_mode_image_marker_survives_original_removal() {
        ...
        let prepared = omnesagent_providers::multimodal::prepare_messages_for_provider(
            &[omnesagent_api::model_provider::ChatMessage::user(
                result.marker,
            )],
