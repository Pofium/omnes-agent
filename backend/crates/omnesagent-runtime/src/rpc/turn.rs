    ...
    Fut: std::future::Future<Output = ()> + Send,
{
    ...
    let mut turn_handle = omnesagent_spawn::spawn!(async move {
...
mod tests {
    ...
    async fn drain_must_not_idle_cancel_a_live_turn_across_a_long_tool_gap() {
        ...
        let sender = omnesagent_spawn::spawn!(async move {
            let _ = tx
    ...
    fn cancel_outcome_carries_committed_messages_not_just_partial_text() {
        let msgs = vec![ConversationMessage::Chat(
            omnesagent_providers::ChatMessage::assistant("[interrupted by user]"),
        )];
