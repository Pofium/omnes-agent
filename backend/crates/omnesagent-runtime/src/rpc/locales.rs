...
fn locale_span(tui_id: Option<&str>) -> ::omnesagent_log::Span {
    ::omnesagent_log::info_span!(
...
pub async fn handle_locales_fetch(
    ...
    async move {
        ...
        let selected: Vec<&(&str, &str, &str)> = if req.catalog.is_empty() {
            omnesagent_config::schema::FTL_CATALOGS.iter().collect()
        } else {
            let mut out = Vec::new();
...
mod tests {
    ...
    use super::*;
    use serde_json::json;
    use omnesagent_api::jsonrpc::LocalesFetchRequest;
