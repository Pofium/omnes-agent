...
const SERVICE_LABEL: &str = "com.omnesagent.daemon";
const WINDOWS_TASK_NAME: &str = "OmnesAgent Daemon";
#[cfg(any(target_os = "linux", target_os = "macos", test))]
const SERVICE_LOG_MAX_BYTES: u64 = 8 * 1024 * 1024;
#[cfg(any(target_os = "linux", target_os = "macos", test))]
const SERVICE_LOG_COMPACT_BYTES: u64 = 4 * 1024 * 1024;
#[cfg(any(target_os = "linux", test))]
const OPENRC_STDOUT_LOG: &str = "/var/log/omnesagent/access.log";
#[cfg(any(target_os = "linux", test))]
const OPENRC_STDERR_LOG: &str = "/var/log/omnesagent/error.log";
#[cfg(any(target_os = "macos", test))]
...
async fn supervise_launchd_child(
    ...
    let stdout_task = omnesagent_spawn::spawn!(drain_launchd_pipe(stdout, stdout_sink));
    let stderr_task = omnesagent_spawn::spawn!(drain_launchd_pipe(stderr, stderr_sink));
...
fn linux_service_base(config: &Config) -> String {
    let Some(dir_name) = config
        ...
        return "omnesagent".to_string();
    };
    let base = dir_name.strip_prefix('.').unwrap_or(dir_name);
    if base == "omnesagent" {
        return base.to_string();
    }
    if let Some(suffix) = base.strip_prefix("omnesagent-")
        && !suffix.is_empty()
    {
        return base.to_string();
    }
    "omnesagent".to_string()
}
...
fn ensure_linux_default_install_scope(config: &Config, action: &str) -> Result<()> {
    ...
    bail!(
        "Linux service {action} only manages the default omnesagent service. \
...
/// Returns whether the OmnesAgent daemon service is currently running.
...
pub fn uninstall(config: &Config, init_system: InitSystem) -> Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        let base_dir = config
            ...
            .map_or_else(|| PathBuf::from("."), PathBuf::from);
        for wrapper in [
            base_dir.join("omnesagent-daemon.cmd"),
            base_dir.join("logs").join("omnesagent-daemon.cmd"),
        ] {
            if wrapper.exists() {
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/omnesagent");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "omnesagent", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove omnesagent from OpenRC default runlevel: {err}"
                    );
                }
            ...
            println!("✅ Service uninstalled (/etc/init.d/omnesagent)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
pub fn homebrew_var_dir_from_exe(exe: &Path) -> Option<PathBuf> {
        ...
        .find(|path| path.file_name().is_some_and(|name| name == "Cellar"))
    {
        return cellar
            .parent()
            .map(|prefix| prefix.join("var").join("omnesagent"));
    }
    ...
    prefix
        .join("Cellar")
        .is_dir()
        .then(|| prefix.join("var").join("omnesagent"))
}
...
mod homebrew_tests {
    ...
    fn homebrew_var_dir_from_exe_detects_cellar_path() {
        let exe = PathBuf::from("/opt/homebrew/Cellar/omnesagent/1.2.3/bin/omnesagent");
        let var_dir = homebrew_var_dir_from_exe(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/opt/homebrew/var/omnesagent")));
    }

    #[test]
    fn homebrew_var_dir_from_exe_detects_intel_cellar_path() {
        let exe = PathBuf::from("/usr/local/Cellar/omnesagent/1.0.0/bin/omnesagent");
        let var_dir = homebrew_var_dir_from_exe(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/usr/local/var/omnesagent")));
    }

    #[test]
    fn homebrew_var_dir_from_exe_ignores_non_homebrew_path() {
        let exe = PathBuf::from("/home/user/.cargo/bin/omnesagent");
        let var_dir = homebrew_var_dir_from_exe(&exe);
    ...
    fn homebrew_var_dir_from_exe_detects_opt_symlink_layout() {
        ...
        let cellar_bin = prefix.join("Cellar/omnesagent/1.2.3/bin");
        std::fs::create_dir_all(&cellar_bin).expect("create Cellar binary dir");
        let cellar_exe = cellar_bin.join("omnesagent");
        ...
        std::os::unix::fs::symlink(
            prefix.join("Cellar/omnesagent/1.2.3"),
            opt_parent.join("omnesagent"),
        )
        ...
        let var_dir = homebrew_var_dir_from_exe(&prefix.join("opt/omnesagent/bin/omnesagent"));
        assert_eq!(var_dir, Some(expected_prefix.join("var/omnesagent")));
    }
}
...
fn install_macos(config: &Config) -> Result<()> {
    ...
    // data so that `brew services start omnesagent` works out of the box.
    ...
    println!("   Start with: omnesagent service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "omnesagent.service"]));
    println!(
...
/// Check if the omnesagent user exists and has expected properties.
...
fn check_omnesagent_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "omnesagent"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser omnesagent && delgroup omnesagent",
            "addgroup -S omnesagent && adduser -S -s /sbin/nologin -H -D -G omnesagent omnesagent",
        )
    } else {
        ("userdel omnesagent", "useradd -r -s /sbin/nologin omnesagent")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'omnesagent' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'omnesagent' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/omnesagent" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: omnesagent user has home directory '{}' (expected /var/lib/omnesagent or /nonexistent)",
                        home
                    );
                }
...
fn ensure_omnesagent_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "omnesagent"]).output();
    if let Ok(output) = output
        && output.status.success()
    {
        return check_omnesagent_user();
    }

    let is_alpine = Path::new("/etc/alpine-release").exists();

    if is_alpine {
        let group_output = Command::new("getent").args(["group", "omnesagent"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "omnesagent"])
                .output()
                .context("Failed to create omnesagent group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create omnesagent group: {}", stderr.trim());
            }
            println!("✅ Created system group: omnesagent");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create omnesagent user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create omnesagent user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "omnesagent"])
            .output()
            .context("Failed to create omnesagent user")?;
    ...
    println!("✅ Created system user: omnesagent");
    Ok(())
}

/// Change ownership of a path to omnesagent:omnesagent
#[cfg(unix)]
fn chown_to_omnesagent(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["omnesagent:omnesagent", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to omnesagent:omnesagent: {}",
...
#[cfg(not(unix))]
fn chown_to_omnesagent(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_omnesagent(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "omnesagent:omnesagent", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to omnesagent:omnesagent: {}",
            path.display().to_string(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_omnesagent(_path: &Path) -> Result<()> {
    Ok(())
}
...
fn resolve_invoking_user_config_dir() -> Option<PathBuf> {
        ...
        && output.status.success()
    {
        ...
        if fields.len() >= 6 {
            return Some(PathBuf::from(fields[5]).join(".omnesagent"));
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'omnesagent' cannot write {} ({details}). \
             Re-run `sudo omnesagent service install` and ensure ownership is omnesagent:omnesagent.",
            path.display().to_string(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/omnesagent",
...
command_user="omnesagent:omnesagent"
...
export HOME="/var/lib/omnesagent"
...
start_pre() {{
    checkpath --directory --owner omnesagent:omnesagent --mode 0750 /var/lib/omnesagent
}}
"#,
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/omnesagent");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo omnesagent service install"
        );
    }
    ...
    if !openrc_executable_path_is_safe(&exe) {
        bail!(
            "OpenRC service executable path contains unsupported shell characters: {}. Install OmnesAgent at /usr/local/bin/omnesagent and retry",
            exe.display()
        );
    }
    ensure_omnesagent_user()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/omnesagent");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/omnesagent");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_omnesagent(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by omnesagent:omnesagent)",
    ...
    chown_recursive_to_omnesagent(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/omnesagent");
    ...
    run_checked(Command::new("rc-update").args(["add", "omnesagent", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/omnesagent");
    println!("   Config path: /etc/omnesagent/config.toml");
    println!("   Start with: sudo omnesagent service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    // as misplaced.)
    let wrapper = base_dir.join("omnesagent-daemon.cmd");
    let stdout_log = logs_dir.join("daemon.stdout.log");
    let stderr_log = logs_dir.join("daemon.stderr.log");
...
// integration dependencies and should run in every omnesagent-runtime test build.
#[cfg(test)]
mod macos_plist_tests {
    ...
    fn macos_plist_renderer_uses_plain_xml_quotes() {
        let plist = render_macos_launch_agent_plist(
            Path::new("/opt/homebrew/bin/omnesagent"),
            Path::new("/opt/homebrew/var/omnesagent"),
            Some(Path::new("/opt/homebrew/var/omnesagent")),
        );
    ...
    fn macos_plist_renderer_escapes_paths_and_omits_homebrew_section_when_absent() {
        let plist = render_macos_launch_agent_plist(
            Path::new("/tmp/Zero<&>\"'Claw/bin/omnesagent"),
            Path::new("/tmp/Zero<&>\"'Claw"),
            None,
        );

        assert!(plist.contains("/tmp/Zero&lt;&amp;&gt;&quot;&apos;Claw/bin/omnesagent"));
    ...
    fn macos_plist_routes_only_launchd_through_bounded_capture() {
        let plist = render_macos_launch_agent_plist(
            Path::new("/usr/local/bin/omnesagent"),
            Path::new("/Users/test/.omnesagent"),
            None,
        );

        assert!(plist.contains("<string>--config-dir</string>"));
        assert!(plist.contains("<string>/Users/test/.omnesagent</string>"));
    ...
    fn macos_plist_renderer_emits_plutil_parseable_xml() {
        let plist = render_macos_launch_agent_plist(
            Path::new("/tmp/Zero<&>\"'Claw/bin/omnesagent"),
            Path::new("/tmp/Zero<&>\"'Claw/var/omnesagent"),
            Some(Path::new("/tmp/Zero<&>\"'Claw/var/omnesagent")),
        );

        let file = std::env::temp_dir().join(format!(
            "omnesagent-launch-agent-plist-{}.plist",
            std::process::id()
        ));
...
mod bounded_service_log_tests {
    ...
    fn openrc_streams_map_to_the_established_log_paths() {
        ...
        assert_eq!(
            ...
            Path::new("/var/log/omnesagent/access.log")
        );
        assert_eq!(
            openrc_log_path(true),
            Path::new("/var/log/omnesagent/error.log")
        );
    }
...
mod linux_service_tests {
    ...
    fn linux_service_base_derives_named_instance_from_config_dir() {
        assert_eq!(
            linux_service_base(&config_at("/home/user/.omnesagent-p100-104/config.toml")),
            "omnesagent-p100-104"
        );
        assert_eq!(
            linux_service_base(&config_at("/home/user/omnesagent-prod/config.toml")),
            "omnesagent-prod"
        );
    }
    ...
    fn linux_service_base_falls_back_for_default_and_unrelated_dirs() {
        assert_eq!(
            linux_service_base(&config_at("/home/user/.omnesagent/config.toml")),
            "omnesagent"
        );
        assert_eq!(
        ...
        assert_eq!(linux_service_base(&config_at("config.toml")), "omnesagent");
    }

    #[test]
    fn linux_service_control_args_use_named_instance() {
        let config = config_at("/home/user/.omnesagent-p100-104/config.toml");

        assert_eq!(
            linux_systemd_action_args(&config, "start"),
            ["--user", "start", "omnesagent-p100-104.service"]
        );
        assert_eq!(
            linux_openrc_action_args(&config, "status"),
            ["omnesagent-p100-104", "status"]
        );
    }
    ...
    fn linux_openrc_log_dir_uses_named_instance() {
        assert_eq!(
            linux_openrc_log_dir(&config_at("/home/user/.omnesagent/config.toml")),
            PathBuf::from("/var/log/omnesagent")
        );
        assert_eq!(
            linux_openrc_log_dir(&config_at("/home/user/.omnesagent-p100-104/config.toml")),
            PathBuf::from("/var/log/omnesagent-p100-104")
        );
    }
    ...
    fn linux_install_scope_rejects_named_instances() {
        assert!(
            ensure_linux_default_install_scope(
                &config_at("/home/user/.omnesagent/config.toml"),
                "install"
            )
            .is_ok()
        );
        ...
        let err = ensure_linux_default_install_scope(
            &config_at("/home/user/.omnesagent-p100-104/config.toml"),
            "install",
        )
        .unwrap_err()
        .to_string();
        assert!(err.contains("only manages the default omnesagent service"));
        assert!(err.contains("omnesagent-p100-104"));
    }
    ...
    fn linux_service_file_stays_default_for_install_path() {
        let file =
            linux_service_file(&config_at("/home/user/.omnesagent-p100-104/config.toml")).unwrap();
        let path = file.to_string_lossy();
        assert!(path.ends_with(".config/systemd/user/omnesagent.service"));
    }
}
...
mod service_helper_tests {
    ...
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "OmnesAgent Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/omnesagent");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/omnesagent"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"omnesagent\""));
        assert!(script.contains("description=\"OmnesAgent daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/omnesagent\""));
        assert!(script.contains("command_args=\"--config-dir /etc/omnesagent daemon\""));
        assert!(!script.contains("env OMNESAGENT_CONFIG_DIR"));
        assert!(!script.contains("env OMNESAGENT_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"omnesagent:omnesagent\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains(
            "output_logger=\"/usr/local/bin/omnesagent service run-openrc-log-writer stdout\""
        ));
        assert!(script.contains(
            "error_logger=\"/usr/local/bin/omnesagent service run-openrc-log-writer stderr\""
        ));
    ...
    fn generate_openrc_script_sets_home_for_browser() {
        ...
        assert!(
            script.contains("export HOME=\"/var/lib/omnesagent\""),
    ...
    fn generate_openrc_script_creates_home_directory() {
        ...
        assert!(
            script.contains("checkpath --directory --owner omnesagent:omnesagent"),
            "start_pre must ensure /var/lib/omnesagent exists with correct ownership"
        );
    }
    ...
    fn openrc_executable_path_rejects_shell_syntax() {
        assert!(openrc_executable_path_is_safe(Path::new(
            "/usr/local/bin/omnesagent"
        )));
        assert!(!openrc_executable_path_is_safe(Path::new(
            "/opt/omnes agent/bin/omnesagent"
        )));
        assert!(!openrc_executable_path_is_safe(Path::new(
            "/opt/$zero/bin/omnesagent"
        )));
    }
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/omnesagent"), true);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/omnesagent'".to_string()
            ]
        );
    }
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/omnesagent/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/omnesagent/workspace'".to_string(),
                "omnesagent".to_string()
            ]
        );
    }
    ...
    fn tail_file_errors_on_missing_file() {
        let missing = Path::new("/tmp/omnesagent-test-nonexistent-log-file.log");
