...
fn load_ftl_with_reader(
    ...
    let path = omnesagent_config::schema::ftl_locale_dir(locale)
        .ok()
...
fn read_config_table() -> Option<toml::Table> {
    if let Ok(custom) = std::env::var("OMNESAGENT_CONFIG_DIR") {
    ...
    if let Some(base) = directories::BaseDirs::new() {
        candidates.push(base.home_dir().join(".omnesagent/config.toml"));
        candidates.push(base.config_dir().join("omnesagent/config.toml"));
    }
...
mod tests {
    ...
    fn zh_cn_wechat_translations_preserve_machine_facing_tokens() {
        ...
        assert!(success.contains("OmnesAgent"));
    }
    ...
    fn daemon_startup_cli_strings_format_in_all_locales() {
        ...
        let path = "/tmp/omnesagent-test/daemon.sock";
        let cases = [
            ("cli-daemon-starting-title", &[][..], &["OmnesAgent"][..]),
            ("cli-daemon-starting-detail", &[][..], &[][..]),
            ("cli-daemon-started-title", &[][..], &["OmnesAgent"][..]),
            (
    ...
    fn channel_runtime_committed_cli_catalogs_format_from_fluent() {
        let cases = [
            ...
            (
                ...
                [
                    "openai.default",
                    "omnesagent models refresh --model-provider openai.default",
                ]
                .as_slice(),
            ),
            (
        ...
        let en_cases: &[FormatCase<'_>] = &[
            ...
            (
                ...
                &[
                    ...
                    "omnesagent skills audit example-skill",
                ],
            ),
        ];
    ...
    fn daemon_gateway_bind_cli_strings_format_from_fluent() {
        ...
        assert!(already_running.contains("OmnesAgent gateway is already running"));
