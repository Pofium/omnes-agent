...
use std::io::Write;
use std::path::Path;
use omnesagent_config::schema::{Config, UNCONFIGURED_CONTEXT_WINDOW_FALLBACK};
...
async fn check_codex_auth_wiring(config: &Config) -> Vec<DiagResult> {
    let auth = omnesagent_providers::auth::AuthService::from_config(config);
...
pub async fn run(config: &Config) -> Result<()> {
    ...
    println!("🩺 OmnesAgent Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `omnesagent doctor` again.");
    }
...
fn create_doctor_model_provider(
    config: &Config,
    provider_name: &str,
) -> anyhow::Result<Box<dyn omnesagent_api::model_provider::ModelProvider>> {
    let api_key = configured_model_provider_api_key(config, provider_name);
    let options = omnesagent_providers::options_for_provider_ref(
        config,
        provider_name,
    ...
    match provider_name.split_once('.') {
        Some((family, alias)) => omnesagent_providers::create_model_provider_for_alias(
            config, family, alias, api_key, &options,
        ),
        None => {
            omnesagent_providers::create_model_provider_with_options(provider_name, api_key, &options)
        }
    }
}
...
/// state), honoring `OMNESAGENT_DATA_DIR` overrides. Both the CLI writer and
...
pub fn persist_model_cache(
    ...
    let mut cache: omnesagent_config::schema::ModelCacheState =
        match std::fs::read_to_string(&cache_path) {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                omnesagent_config::schema::ModelCacheState::default()
            }
            Err(e) => {
    ...
    cache
        .entries
        .push(omnesagent_config::schema::ModelCacheEntry {
...
pub async fn run_models(
    ...
    if targets.is_empty() {
        anyhow::bail!(
            "No configured model_providers to probe — run `omnesagent quickstart` to set one up first"
        );
    }

    println!("🩺 OmnesAgent Doctor — Model Catalog Probe");
...
type FetchContextWindowFn = Box<
    dyn for<'a> Fn(
            &'a str,
            &'a omnesagent_config::schema::ModelProviderConfig,
        ) -> std::pin::Pin<
...
pub async fn update_context_windows(
    config: &mut omnesagent_config::schema::Config,
    provider_override: Option<&str>,
    dry_run: bool,
    ...
    let fetch_fn = fetch_fn.unwrap_or_else(|| {
        Box::new(
            |provider_type: &str,
             provider_config: &omnesagent_config::schema::ModelProviderConfig| {
                Box::pin(omnesagent_providers::fetch_context_window(
                    provider_type,
                    provider_config,
                ))
            },
...
pub async fn fetch_provider_catalog(config: &Config, provider_ref: &str) -> Result<Vec<String>> {
    ...
    omnesagent_providers::ProviderDispatch::from_ref(&*provider)
        .list_models()
        .await
}
...
pub async fn run_configured_models(
    ...
    if entries.is_empty() {
        anyhow::bail!(
            "No configured model_providers — run `omnesagent quickstart` to set one up first"
        );
    }

    if verify {
        println!("🩺 OmnesAgent — Configured Models (--check)");
    } else {
        println!("🩺 OmnesAgent — Configured Models");
    }
    println!();
...
pub fn run_traces(
    ...
    println!();
    println!("Use `omnesagent doctor traces --id <trace-id>` to inspect a full event payload.");
    Ok(())
}
...
/// `migrate_to_current_salvaged` in `omnesagent-config`) so `doctor` names the
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    // its siblings (`crates/omnesagent-channels/src/tts.rs`) bail on a
    ...
    // `crates/omnesagent-channels/src/transcription.rs`. `local_whisper` has
    // no api_key concept (its optional `bearer_token` is a different field)
    // and is never checked here.
    {
        use omnesagent_config::providers::TranscriptionProviderEntry;
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `omnesagent quickstart` to set one up",
        ));
    }
    ...
    for (alias, tg) in &cc.telegram {
        if tg.enabled && omnesagent_config::traits::is_unset_display_value(&tg.bot_token) {
            items.push(DiagItem::warn(
                cat,
                format!(
    ...
    for (alias, dc) in &cc.discord {
        if dc.enabled && omnesagent_config::traits::is_unset_display_value(&dc.bot_token) {
            items.push(DiagItem::warn(
                cat,
                format!(
...
fn localized_validation_warning_message(
    warning: &omnesagent_config::validation_warnings::ValidationWarning,
) -> String {
    match warning.code.as_str() {
        omnesagent_config::validation_warnings::VERIFIABLE_INTENT_TOOL_WITHHELD => {
...
mod tests {
    ...
    fn tts_doctor_no_warning_for_keyless_families() {
        ...
        config.providers.tts.edge.insert(
            "default".to_string(),
            omnesagent_config::schema::EdgeTtsProviderConfig::default(),
        );
        config.providers.tts.piper.insert(
            "default".to_string(),
            omnesagent_config::schema::PiperTtsProviderConfig::default(),
        );
        let mut items = Vec::new();
        check_config_semantics(&config, &mut items);
    ...
    fn transcription_doctor_no_warning_for_local_whisper() {
        ...
        config.providers.transcription.local_whisper.insert(
            "default".to_string(),
            omnesagent_config::schema::LocalWhisperTranscriptionProviderConfig {
    ...
    fn persist_model_cache_concurrent_refreshes_keep_every_provider() {
        ...
        let cache: omnesagent_config::schema::ModelCacheState = serde_json::from_str(&raw).unwrap();
    ...
    fn config_validation_warns_empty_model_route() {
        let config = Config {
            model_routes: vec![omnesagent_config::schema::ModelRouteConfig {
                hint: "fast".into(),
    ...
    fn config_validation_warns_empty_embedding_route_model() {
        let config = Config {
            embedding_routes: vec![omnesagent_config::schema::EmbeddingRouteConfig {
                hint: "semantic".into(),
    ...
    fn config_validation_surfaces_dangling_fallback_ref() {
        use omnesagent_config::schema::{ModelProviderConfig, NvidiaModelProviderConfig};
    ...
    fn config_validation_ignores_disabled_tokenless_bot_channels() {
        ...
        config.channels.telegram.insert(
            "default".to_string(),
            omnesagent_config::schema::TelegramConfig::default(),
        );
        config.channels.discord.insert(
            "default".to_string(),
            omnesagent_config::schema::DiscordConfig::default(),
        );
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(
            first
                ...
                .is_some_and(|name| name.starts_with(".omnesagent_doctor_probe_"))
        );
    }
    ...
    fn verifiable_intent_withheld_warning_uses_fluent() {
        ...
        let warning = omnesagent_config::validation_warnings::ValidationWarning::new(
            omnesagent_config::validation_warnings::VERIFIABLE_INTENT_TOOL_WITHHELD,
            structured_message,
    ...
    fn diagnose_accepts_literal_web_dist_dir() {
        let mut config = Config::default();
        config.gateway.web_dist_dir = Some("/srv/omnesagent/web-dist".to_string());
    ...
    fn openai_codex_slot() -> omnesagent_config::schema::OpenAIModelProviderConfig {
        let mut slot = omnesagent_config::schema::OpenAIModelProviderConfig::default();
    ...
    fn web_dist_dir_expansion_reason_key_detects_tilde_and_env() {
        ...
        assert!(web_dist_dir_expansion_reason_key("/srv/omnesagent/web-dist").is_none());
    ...
    async fn update_context_windows_uses_exact_alias_not_model_uri() {
        ...
        let mock_fetch: FetchContextWindowFn = Box::new(
            |_type: &str, _config: &omnesagent_config::schema::ModelProviderConfig| {
