...
use omnesagent_config::schema::OtelContentPolicy;
...
impl OtelObserver {
    ...
    pub(crate) fn new(
        ...
        let service_name = service_name.unwrap_or("omnesagent");
...
impl Observer for OtelObserver {
    fn record_event(&self, event: &ObserverEvent) {
        let tracer = global::tracer("omnesagent");
        ...
        match event {
                ...
                let span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("gen_ai.agent.invoke")
                        ...
                        .with_attributes(vec![
                            KeyValue::new("gen_ai.provider.name", model_provider.clone()),
                            KeyValue::new("gen_ai.request.model", model.clone()),
                            KeyValue::new("omnesagent.channel", channel.clone().unwrap_or_default()),
                            KeyValue::new(
                                "gen_ai.agent.name",
                                agent_alias.clone().unwrap_or_default(),
                            ),
                            KeyValue::new("omnesagent.turn_id", turn_id.clone().unwrap_or_default()),
                        ]),
                );
            ...
            ObserverEvent::MemoryRecall {
                ...
                let mut span_attrs = vec![
                    // Legacy / OmnesAgent-specific attrs
            ...
            ObserverEvent::RagRetrieve {
                ...
                let mut span_attrs = vec![
                    ...
                    KeyValue::new("duration_s", secs),
                    KeyValue::new("gen_ai.operation.name", "retrieval"),
                    KeyValue::new("gen_ai.system", "omnesagent_rag"),
                    KeyValue::new("omnesagent.channel", channel.clone().unwrap_or_default()),
                    KeyValue::new("gen_ai.agent.name", agent_alias.clone().unwrap_or_default()),
                    KeyValue::new("omnesagent.turn_id", turn_id.clone().unwrap_or_default()),
                ];
...
mod tests {
    ...
    fn content_config_normalizes_zero_max_chars_to_off() {
        use omnesagent_config::schema::{ObservabilityConfig, OtelContentPolicy};
