...
use omnesagent_config::schema::{ObservabilityBackend, ObservabilityConfig};
...
pub struct AgentTurnGuard<'a> {
    ...
    tokens_used: Option<omnesagent_api::observability_traits::TurnTokenUsage>,
    cost_usd: Option<f64>,
    done: bool,
}
...
/// invocation (today: the CLI one-shot, `omnesagent agent -m ...`), whose
