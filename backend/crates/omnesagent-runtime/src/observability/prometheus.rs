...
impl PrometheusObserver {
    pub fn new() -> Self {
        ...
        let agent_starts = IntCounterVec::new(
            prometheus::Opts::new("omnesagent_agent_starts_total", "Total agent invocations"),
        ...
        let tokens_input_total = IntCounterVec::new(
            prometheus::Opts::new("omnesagent_tokens_input_total", "Total input tokens consumed"),
            &["model_provider", "model"],
        )
        .expect("valid metric");
        ...
        let tokens_output_total = IntCounterVec::new(
            prometheus::Opts::new(
                "omnesagent_tokens_output_total",
        ...
        let tool_calls = IntCounterVec::new(
            prometheus::Opts::new("omnesagent_tool_calls_total", "Total tool calls"),
            &["tool", "success"],
        )
        .expect("valid metric");

        let channel_messages = IntCounterVec::new(
            prometheus::Opts::new("omnesagent_channel_messages_total", "Total channel messages"),
            &["channel", "direction"],
        )
        .expect("valid metric");
        ...
        let heartbeat_ticks =
            prometheus::IntCounter::new("omnesagent_heartbeat_ticks_total", "Total heartbeat ticks")
                .expect("valid metric");

        let errors = IntCounterVec::new(
            prometheus::Opts::new("omnesagent_errors_total", "Total errors by component"),
            &["component"],
        )
        .expect("valid metric");

        let cache_hits = IntCounterVec::new(
            prometheus::Opts::new("omnesagent_cache_hits_total", "Total response cache hits"),
            &["cache_type"],
        )
        .expect("valid metric");

        let cache_misses = IntCounterVec::new(
            prometheus::Opts::new("omnesagent_cache_misses_total", "Total response cache misses"),
            &["cache_type"],
        )
        .expect("valid metric");
        ...
        let cache_tokens_saved = IntCounterVec::new(
            prometheus::Opts::new(
                "omnesagent_cache_tokens_saved_total",
        ...
        let agent_duration = HistogramVec::new(
            HistogramOpts::new(
                "omnesagent_agent_duration_seconds",
        ...
        let tool_duration = HistogramVec::new(
            HistogramOpts::new(
                "omnesagent_tool_duration_seconds",
        ...
        let memory_audit_duration = HistogramVec::new(
            HistogramOpts::new(
                "omnesagent_memory_audit_duration_seconds",
        ...
        let request_latency = Histogram::with_opts(
            HistogramOpts::new(
                "omnesagent_request_latency_seconds",
        ...
        let tokens_used = prometheus::IntGauge::new(
            "omnesagent_tokens_used_last",
            "Tokens used in the last request",
        )
        .expect("valid metric");

        let active_sessions = GaugeVec::new(
            prometheus::Opts::new("omnesagent_active_sessions", "Number of active sessions"),
            &[],
        )
        .expect("valid metric");

        let queue_depth = GaugeVec::new(
            prometheus::Opts::new("omnesagent_queue_depth", "Message queue depth"),
            &[],
        )
        .expect("valid metric");

        let deployments_total = IntCounterVec::new(
            prometheus::Opts::new("omnesagent_deployments_total", "Total deployments by status"),
            &["status"],
        )
        .expect("valid metric");
        ...
        let deployment_lead_time = Histogram::with_opts(
            HistogramOpts::new(
                "omnesagent_deployment_lead_time_seconds",
        ...
        let deployment_failure_rate = prometheus::Gauge::new(
            "omnesagent_deployment_failure_rate",
        ...
        let recovery_time = Histogram::with_opts(
            HistogramOpts::new(
                "omnesagent_recovery_time_seconds",
        ...
        let mttr =
            prometheus::Gauge::new("omnesagent_mttr_seconds", "Mean time to recovery in seconds")
                .expect("valid metric");
...
impl Observer for PrometheusObserver {
    fn record_event(&self, event: &ObserverEvent) {
        match event {
            ...
            // future variant added by upstream `omnesagent-api`.
            _ => {}
        }
    }
...
mod tests {
    ...
    fn records_all_events_without_panic() {
        ...
        obs.record_event(&ObserverEvent::AgentEnd {
            ...
            tokens_used: Some(omnesagent_api::observability_traits::TurnTokenUsage {
                input_tokens: 100,
                output_tokens: 0,
            }),
            cost_usd: None,
    ...
    fn encode_produces_prometheus_text_format() {
        ...
        assert!(output.contains("omnesagent_agent_starts_total"));
        assert!(output.contains("omnesagent_tool_calls_total"));
        assert!(output.contains("omnesagent_heartbeat_ticks_total"));
        assert!(output.contains("omnesagent_request_latency_seconds"));
        assert!(output.contains(
            r#"omnesagent_memory_audit_total{action="purge",backend="sqlite",success="true"} 1"#
        ));
        assert!(output.contains("omnesagent_memory_audit_duration_seconds"));
    }
    ...
    fn counters_increment_correctly() {
        ...
        let output = obs.encode();
        assert!(output.contains("omnesagent_heartbeat_ticks_total 3"));
    }
    ...
    fn tool_calls_track_success_and_failure_separately() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"omnesagent_tool_calls_total{success="true",tool="shell"} 2"#));
        assert!(output.contains(r#"omnesagent_tool_calls_total{success="false",tool="shell"} 1"#));
    }
    ...
    fn errors_track_by_component() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"omnesagent_errors_total{component="model_provider"} 2"#));
        assert!(output.contains(r#"omnesagent_errors_total{component="channels"} 1"#));
    }
    ...
    fn gauge_reflects_latest_value() {
        ...
        let output = obs.encode();
        assert!(output.contains("omnesagent_tokens_used_last 200"));
    }
    ...
    fn llm_response_tracks_request_count_and_tokens() {
        ...
        let output = obs.encode();
        assert!(output.contains(
            r#"omnesagent_llm_requests_total{model="claude-sonnet",model_provider="openrouter",success="true"} 2"#
        ));
        assert!(output.contains(
            r#"omnesagent_tokens_input_total{model="claude-sonnet",model_provider="openrouter"} 300"#
        ));
        assert!(output.contains(
            r#"omnesagent_tokens_output_total{model="claude-sonnet",model_provider="openrouter"} 130"#
        ));
    }
    ...
    fn llm_response_without_tokens_increments_request_only() {
        ...
        let output = obs.encode();
        assert!(output.contains(
            r#"omnesagent_llm_requests_total{model="llama3",model_provider="ollama",success="false"} 1"#
        ));
        // Token counters should not appear (no data recorded)
        assert!(!output.contains("omnesagent_tokens_input_total{"));
        assert!(!output.contains("omnesagent_tokens_output_total{"));
    }
    ...
    fn dora_deployment_events_track_counters() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"omnesagent_deployments_total{status="success"} 2"#));
        assert!(output.contains(r#"omnesagent_deployments_total{status="failure"} 1"#));
    }
    ...
    fn dora_failure_rate_gauge_updates() {
        ...
        assert!(output.contains("omnesagent_deployment_failure_rate 0.5"));
    }
    ...
    fn dora_lead_time_and_recovery_metrics() {
        ...
        let output = obs.encode();
        assert!(output.contains("omnesagent_deployment_lead_time_seconds"));
        assert!(output.contains("omnesagent_recovery_time_seconds"));
        assert!(output.contains("omnesagent_mttr_seconds 600"));
    }
    ...
    fn arc_blanket_observer_impl_routes_to_inner() {
        ...
        assert!(
            output.contains(
                r#"omnesagent_channel_messages_total{channel="test-channel",direction="inbound"} 1"#
            ),
