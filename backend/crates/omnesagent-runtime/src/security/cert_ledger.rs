        ...
        if version > SCHEMA_VERSION {
            ...
            bail!(
                "cert ledger is at schema v{version}, newer than the v{SCHEMA_VERSION} this build \
                 understands; upgrade OmnesAgent, or move the ledger aside to start fresh (issued \
...
mod tests {
    ...
    fn revoke_materializes_to_a_configured_crl_path_not_the_default() {
        ...
        let set = omnesagent_tls::load_revoked_fingerprints(&custom).unwrap();
        assert!(
            set.contains("fpa"),
    ...
    fn an_undelivered_active_row_is_revoked_once_the_ttl_has_passed() {
        ...
        let set = omnesagent_tls::load_revoked_fingerprints(&revoked_list_path(dir.path())).unwrap();
        assert!(
    ...
    fn device_revocation_catches_a_renewal_racing_its_snapshot() {
        ...
        let crl = revoked_list_path(dir.path());
        let enforced = omnesagent_tls::load_revoked_fingerprints(&crl).unwrap();
    ...
    fn device_revocation_sweeps_a_certificate_published_before_it_runs() {
        ...
        let enforced =
            omnesagent_tls::load_revoked_fingerprints(&revoked_list_path(dir.path())).unwrap();
