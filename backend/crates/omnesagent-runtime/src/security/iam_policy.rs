...
/// Maps a single Nevis role to OmnesAgent permissions.
...
pub struct RoleMapping {
    ...
    pub omnesagent_permissions: Vec<String>,
...
/// IAM policy engine that maps Nevis roles to OmnesAgent tool permissions.
...
impl IamPolicy {
    ...
    pub fn evaluate_tool_access(
        ...
        for role in &identity.roles {
            ...
            {
                ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"user_id": crate::security::redact(&identity.user_id), "role": key, "tool": normalized_tool})), "IAM policy: tool access ALLOWED");
        ...
        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"user_id": crate::security::redact(&identity.user_id), "tool": normalized_tool})), "IAM policy: tool access DENIED");
    ...
    pub fn evaluate_workspace_access(
        ...
        for role in &identity.roles {
            ...
            {
                ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"user_id": crate::security::redact(&identity.user_id), "role": key, "workspace": normalized_ws})), "IAM policy: workspace access ALLOWED");
                return PolicyDecision::Allow;
            }
        }
        ...
        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"user_id": crate::security::redact(&identity.user_id), "workspace": normalized_ws})), "IAM policy: workspace access DENIED");
        PolicyDecision::Deny(reason)
    }
...
mod tests {
    ...
    fn test_mappings() -> Vec<RoleMapping> {
        vec![
            RoleMapping {
                ...
                omnesagent_permissions: vec!["all".into()],
                workspace_access: vec!["all".into()],
            },
            RoleMapping {
                nevis_role: "operator".into(),
                omnesagent_permissions: vec![
                    "shell".into(),
                    "file_read".into(),
                    "file_write".into(),
                    "memory_search".into(),
            ...
            RoleMapping {
                ...
                omnesagent_permissions: vec!["file_read".into(), "memory_search".into()],
    ...
    fn duplicate_normalized_roles_are_rejected() {
        let mappings = vec![
            ...
            RoleMapping {
                nevis_role: " ADMIN ".into(),
                omnesagent_permissions: vec!["file_read".into()],
