...
const OTP_ISSUER: &str = "OmnesAgent";
...
impl OtpValidator {
    pub fn from_config(
        ...
        let secret_path = secret_file_path(omnesagent_dir);
...
pub fn secret_file_path(omnesagent_dir: &Path) -> PathBuf {
    omnesagent_dir.join(OTP_SECRET_FILE)
}
