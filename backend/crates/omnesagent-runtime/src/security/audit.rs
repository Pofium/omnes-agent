...
use uuid::Uuid;
use omnesagent_config::schema::AuditConfig;
...
impl AuditLogger {
    /// Construct a logger over `<omnesagent_dir>/<config.log_path>`.
    ///
    ...
    pub fn new(config: AuditConfig, omnesagent_dir: PathBuf) -> Result<Self> {
        // Load and validate signing key if sign_events enabled
        let signing_key = if config.sign_events {
            let key_hex = std::env::var("OMNESAGENT_AUDIT_SIGNING_KEY").map_err(|e| {
                ...
                ::omnesagent_log::record!(
                    ...
                    "audit log: sign_events=true but OMNESAGENT_AUDIT_SIGNING_KEY env var is not usable"
                );
                match e {
                    std::env::VarError::NotPresent => anyhow::Error::msg(
                        "sign_events enabled but OMNESAGENT_AUDIT_SIGNING_KEY not set",
                    ),
                    std::env::VarError::NotUnicode(_) => anyhow::Error::msg(
                        "OMNESAGENT_AUDIT_SIGNING_KEY env var is not valid UTF-8",
                    ),
                }
            })?;
            ...
            let key_bytes = hex::decode(&key_hex).map_err(|e| {
                ::omnesagent_log::record!(
                    ...
                    "audit log: OMNESAGENT_AUDIT_SIGNING_KEY env var must be hex-encoded"
                );
                anyhow::Error::msg(format!(
                    "OMNESAGENT_AUDIT_SIGNING_KEY must be hex-encoded: {e}"
                ))
            })?;

            if key_bytes.len() != 32 {
                bail!(
                    "OMNESAGENT_AUDIT_SIGNING_KEY must be 32 bytes (64 hex chars), got {}",
                    key_bytes.len()
                );
            }
        ...
        let log_path = omnesagent_dir.join(&config.log_path);
    ...
    /// Open THE audit logger for `omnesagent_dir` — the single instance every
    ...
    pub fn open_shared(config: AuditConfig, omnesagent_dir: PathBuf) -> Result<std::sync::Arc<Self>> {
        Ok(std::sync::Arc::new(Self::new(config, omnesagent_dir)?))
    }
...
pub fn verify_chain(log_path: &Path) -> Result<u64> {
    ...
    let signing_key = std::env::var("OMNESAGENT_AUDIT_SIGNING_KEY")
        .ok()
...
mod tests {
    ...
    /// Mutex to serialize tests that read/write OMNESAGENT_AUDIT_SIGNING_KEY env var.
    ...
    fn audit_event_with_actor() {
        let event = AuditEvent::new(AuditEventType::CommandExecution).with_actor(
            "telegram".to_string(),
            Some("123".to_string()),
            Some("@omnesagent_user".to_string()),
        );
        ...
        assert_eq!(actor.username, Some("@omnesagent_user".to_string()));
    }
    ...
    fn signature_present_when_sign_events_enabled() -> Result<()> {
        ...
        let old_key = std::env::var("OMNESAGENT_AUDIT_SIGNING_KEY").ok();
        defer! {
            if let Some(key) = old_key {
                // SAFETY: test-only, single-threaded test runner.
                unsafe { std::env::set_var("OMNESAGENT_AUDIT_SIGNING_KEY", key) };
            } else {
                // SAFETY: test-only, single-threaded test runner.
                unsafe { std::env::remove_var("OMNESAGENT_AUDIT_SIGNING_KEY") };
            }
        }
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("OMNESAGENT_AUDIT_SIGNING_KEY", &test_key) };
    ...
    fn constructor_fails_if_sign_events_but_no_key() -> Result<()> {
        ...
        if let Err(e) = result {
            ...
            assert!(
                err_msg.contains("OMNESAGENT_AUDIT_SIGNING_KEY not set"),
                "error: {}",
                err_msg
            );
        }
    ...
    fn constructor_fails_if_signing_key_invalid_hex() -> Result<()> {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("OMNESAGENT_AUDIT_SIGNING_KEY", "not-valid-hex") };
    ...
    fn constructor_fails_if_signing_key_not_unicode_and_does_not_leak_value() -> Result<()> {
        ...
        let _guard = ENV_MUTEX.lock().unwrap();
        let old_key = std::env::var_os("OMNESAGENT_AUDIT_SIGNING_KEY");
        defer! {
            if let Some(key) = old_key {
                // SAFETY: test-only, single-threaded test runner.
                unsafe { std::env::set_var("OMNESAGENT_AUDIT_SIGNING_KEY", key) };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("OMNESAGENT_AUDIT_SIGNING_KEY", bad_value) };
    ...
    fn constructor_fails_if_signing_key_wrong_length() -> Result<()> {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("OMNESAGENT_AUDIT_SIGNING_KEY", &short_key) };
        let tmp = TempDir::new()?;
        let config = AuditConfig {
            enabled: true,
            sign_events: true,
