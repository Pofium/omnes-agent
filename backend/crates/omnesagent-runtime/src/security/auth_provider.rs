...
//! [`omnesagent_api::principal::Principal`] carrying the identity / claim inputs
//! (the resolved OmnesAgent grants are added additively in the later
...
//! unrelated to [`omnesagent_providers::auth`]'s `AuthProvider` enum, which names
...
use omnesagent_api::principal::{AuthMethod, AuthOutcome, DenyReason};
...
mod tests {
    use super::*;
    use omnesagent_api::principal::Principal;
