...
impl SeatbeltSandbox {
    ...
    pub fn with_workspace(workspace: Option<&Path>) -> std::io::Result<Self> {
        ...
        let policy_dir = std::env::temp_dir().join("omnesagent-seatbelt");
...
mod tests {
    ...
    fn generate_policy_uses_provided_workspace_for_access_rules() {
        let workspace = PathBuf::from("/tmp/omnesagent-seatbelt-test-workspace");
        let policy = generate_policy(&workspace);

        assert!(
            policy.contains(
                r#"(allow file-read* (subpath "/tmp/omnesagent-seatbelt-test-workspace"))"#
            )
        );
        assert!(policy.contains(r#"(subpath "/tmp/omnesagent-seatbelt-test-workspace")"#));
    }
