...
use omnesagent_api::model_provider::{ChatMessage, ChatResponse};
use omnesagent_api::tool::ToolResult;
...
impl HookRunner {
    ...
    pub fn from_config(hooks: &omnesagent_config::schema::HooksConfig) -> Self {
        ...
        for h in &self.handlers {
                ...
                .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "before_model_resolve cancelled by hook");
    ...
    pub async fn run_before_prompt_build(&self, mut prompt: String) -> HookResult<String> {
        for h in &self.handlers {
                ...
                .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "before_prompt_build cancelled by hook");
                    return HookResult::Cancel(reason);
                }
                Err(_) => {
                    ::omnesagent_log::record!(
    ...
    pub async fn run_before_llm_call(
        ...
        for h in &self.handlers {
                ...
                .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "before_llm_call cancelled by hook");
                    return HookResult::Cancel(reason);
                }
                Err(_) => {
                    ::omnesagent_log::record!(
    ...
    pub async fn run_before_tool_call(
        ...
        for h in &self.handlers {
                ...
                .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "before_tool_call cancelled by hook");
                    return HookResult::Cancel(reason);
                }
                Err(_) => {
                    ::omnesagent_log::record!(
    ...
    pub async fn run_on_message_received(
        ...
        for h in &self.handlers {
                ...
                .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "on_message_received cancelled by hook");
                    return HookResult::Cancel(reason);
                }
                Err(_) => {
                    ::omnesagent_log::record!(
    ...
    pub async fn run_on_message_sending(
        ...
        for h in &self.handlers {
            ...
            .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "on_message_sending cancelled by hook");
                    return HookResult::Cancel(reason);
                }
                Err(_) => {
                    ::omnesagent_log::record!(
...
mod tests {
    ...
    fn from_config_disabled_builtins_produces_empty_runner() {
        let config = omnesagent_config::schema::HooksConfig {
            enabled: true,
            builtin: omnesagent_config::schema::BuiltinHooksConfig {
                command_logger: false,
                webhook_audit: omnesagent_config::schema::WebhookAuditConfig::default(),
            },
        };
    ...
    fn from_config_skips_invalid_webhook_and_keeps_valid_builtins() {
        let config = omnesagent_config::schema::HooksConfig {
            ...
            builtin: omnesagent_config::schema::BuiltinHooksConfig {
                command_logger: true,
                webhook_audit: omnesagent_config::schema::WebhookAuditConfig {
                    enabled: true,
