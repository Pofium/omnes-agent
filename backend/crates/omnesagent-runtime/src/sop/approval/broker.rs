...
use omnesagent_config::schema::{ApprovalPolicyConfig, SopApprovalConfig};
...
mod tests {
    ...
    use omnesagent_config::schema::{ApprovalGroupConfig, ApprovalPolicyConfig, SopConfig};
    ...
    fn checkpoint_non_member_is_not_authorized_and_stays_paused() {
        let (mut engine, run_id) =
            checkpoint_engine_with_broker(approval_cfg(&["OmnesAgentOperator"], 1));
        let outcome = engine
            .resolve_via_broker(
                &run_id,
                ApprovalDecision::Approve,
                ApprovalPrincipal::cli(Some("OmnesAgentAgent".into())),
            )
            .unwrap();
    ...
    fn checkpoint_quorum_waits_for_distinct_members_and_audits_resolver() {
        let (mut engine, run_id) = checkpoint_engine_with_broker(approval_cfg(
            &["OmnesAgentOperator", "OmnesAgentMaintainer"],
            2,
        ));
        let first = engine
            .resolve_via_broker(
                &run_id,
                ApprovalDecision::Approve,
                ApprovalPrincipal::cli(Some("OmnesAgentOperator".into())),
            )
            .unwrap();
        assert!(matches!(
            first,
        ...
        let second = engine
            .resolve_via_broker(
                &run_id,
                ApprovalDecision::Approve,
                ApprovalPrincipal::cli(Some("OmnesAgentMaintainer".into())),
            )
            .unwrap();
        assert!(matches!(
            second,
        ...
        assert_eq!(resolved.actor.as_deref(), Some("OmnesAgentMaintainer"));
    ...
    fn checkpoint_authorized_deny_audits_actor_and_decision() {
        ...
        let resolved = engine
            ...
            .find(|event| event.kind == "gate_resolved")
            .expect("checkpoint decision audit");
        assert_eq!(resolved.actor.as_deref(), Some("OmnesAgentOperator"));
    ...
    fn non_member_is_not_authorized_and_gate_stays_open() {
        let (mut e, id) = engine_with_broker(approval_cfg(&["OmnesAgentOperator"], 1));
        let out = e
            .resolve_via_broker(
                &id,
                ApprovalDecision::Approve,
    ...
    fn unavailable_live_step_fails_closed_for_approve_and_deny() {
        for decision in [
            ...
            let (mut engine, run_id) = engine_with_broker(approval_cfg(&["OmnesAgentOperator"], 1));
    ...
    fn quorum_of_two_needs_two_distinct_approvers() {
        let (mut e, id) =
            engine_with_broker(approval_cfg(&["OmnesAgentOperator", "OmnesAgentMaintainer"], 2));
    ...
    fn stale_vote_under_old_policy_does_not_count_after_policy_change() {
        ...
        groups.insert(
            "g_old".to_string(),
            ApprovalGroupConfig {
                members: vec!["OmnesAgentOperator".into()],
            },
        );
        groups.insert(
            "g_new".to_string(),
            ApprovalGroupConfig {
                members: vec!["OmnesAgentMaintainer".into()],
            },
        );
        let mut policies = HashMap::new();
        policies.insert(
        ...
        // OmnesAgentOperator (a g_old member) votes under `old`: recorded, quorum not yet met.
        let first = e
            .resolve_via_broker(
                &id,
                ApprovalDecision::Approve,
        ...
        // OmnesAgentMaintainer (a g_new member) votes under `new`. OmnesAgentOperator's stale `old` vote must NOT count.
        let second = e
            .resolve_via_broker(
                &id,
                ApprovalDecision::Approve,
    ...
    fn revoked_member_vote_does_not_count_after_config_reload() {
        // Membership revocation: OmnesAgentOperator votes under `prod`
        // (group `release`), then a live config reload revokes OmnesAgentOperator from `release` while
        ...
        // Live config reload revokes OmnesAgentOperator from the release group (OmnesAgentMaintainer remains).
        e.set_approval_config_for_test(approval_cfg(&["OmnesAgentMaintainer"], 2));

        // OmnesAgentMaintainer votes. OmnesAgentOperator's earlier vote is revalidated against the CURRENT group and
    ...
    fn checkpoint_engine_with_atomic_failure_store() -> (SopEngine, String, Arc<FailingSaveStore>) {
        ...
        let config = SopConfig {
            approval: approval_cfg(&["OmnesAgentOperator"], 1),
            ..SopConfig::default()
        };
    ...
    fn quorum_vote_read_failure_is_surfaced_not_swallowed() {
        ...
        let broker = Arc::new(ApprovalBroker::disabled());
        let sop_config = SopConfig {
            approval: approval_cfg(&["OmnesAgentOperator", "OmnesAgentMaintainer"], 2),
            ..SopConfig::default()
        };
        let mut e = SopEngine::new(sop_config)
            .with_approval_broker(broker)
    ...
    fn quorum_vote_refused_from_a_principal_approval_mode_rejects() {
        ...
        let sop_config = SopConfig {
            approval: approval_cfg(&["bot", "OmnesAgentOperator"], 2),
            approval_mode: omnesagent_config::schema::ApprovalMode::OutOfBandRequired,
            ..SopConfig::default()
        };
        let mut e = SopEngine::new(sop_config).with_approval_broker(broker);
        e.set_sops_for_test(vec![policy_sop("prod")]);
