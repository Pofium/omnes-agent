...
//!
//! Layering: this lives in `omnesagent-runtime` because it needs only
//! [`omnesagent_api::channel::Channel`] (a runtime dependency already), never the
//! `omnesagent-channels` implementations. The DAEMON builds the concrete channel map
//! (via `omnesagent_channels::orchestrator::build_channel_map`) and injects it here, so
...
fn reply_instructions(reference: &str, run_id: &str) -> String {
    format!(
        "Reply `approve {reference}` or `deny {reference}` here, or use \
         `omnesagent sop approve|deny {run_id}`."
    )
}
...
fn build_gate_prompt(
    ...
    notice: &GateNotice<'_>,
) -> omnesagent_api::channel::ChannelGatePrompt {
...
mod tests {
    ...
    fn template_resolves_dotted_paths_and_drops_missing_ones() {
        ...
        assert_eq!(
            ...
            "Issue o/r#7 by omnesagent_user: It broke "
        );
