...
use omnesagent_config::schema::SopApprovalConfig;
...
/// agent alias. The loopback CLI (`omnesagent sop approve`) is currently ANONYMOUS -
...
mod tests {
    ...
    use omnesagent_config::schema::ApprovalGroupConfig;
    ...
    fn bare_member_matches_any_source() {
        let cfg = cfg_with(&[("release", &["OmnesAgentOperator"])]);
    ...
    fn source_qualified_member_scopes_to_one_transport() {
        // `http:OmnesAgentOperator` grants to the HTTP OmnesAgentOperator but NOT the CLI OmnesAgentOperator - so a channel
        // identity does not collide with a same-named identity on another source.
        let cfg = cfg_with(&[("release", &["http:OmnesAgentOperator"])]);
        let r = LocalConfigApprovalIdentityResolver;
        assert!(r.is_member(
            &cfg,
            &ApprovalPrincipal::http(Some("OmnesAgentOperator".into())),
    ...
    fn resolves_multiple_groups() {
        let cfg = cfg_with(&[
            ("release", &["OmnesAgentOperator"]),
            ("sre", &["OmnesAgentOperator"]),
        ]);
        let r = LocalConfigApprovalIdentityResolver;
    ...
    fn unknown_or_identityless_principal_has_no_groups() {
        ...
        let r = LocalConfigApprovalIdentityResolver;
        assert!(
            r.groups_for(&cfg, &ApprovalPrincipal::cli(Some("OmnesAgentAgent".into())))
                .is_empty()
        );
