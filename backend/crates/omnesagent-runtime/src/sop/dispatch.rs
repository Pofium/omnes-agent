...
async fn dispatch_sop_event_filtered(
    ...
    for run in &started_runs {
        let span = omnesagent_log::attribution_span!(run);
        let run_id = run.run_id.clone();
        if let Err(e) = omnesagent_log::scope!(
            session_key: run_id,
            =>
        ...
        match result {
            DispatchResult::Started {
                ...
                SopRunAction::Failed { reason, .. } => {
                    ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"run_id": run_id, "sop_name": sop_name, "reason": reason.to_string()})), &format!("SOP headless dispatch: run {run_id} ('{sop_name}') failed: {reason}"));
                }
            },
            DispatchResult::Skipped { sop_name, reason } => {
                ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"sop_name": sop_name, "reason": reason.to_string()})), &format!("SOP headless dispatch: skipped '{sop_name}': {reason}"));
            }
            DispatchResult::Deferred { sop_name, reason } => {
                ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"sop_name": sop_name, "reason": reason.to_string()})), &format!("SOP headless dispatch: deferred '{sop_name}' (backpressure): {reason}"));
            }
            DispatchResult::Coalesced {
                sop_name,
                existing_run_id,
            } => {
                ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"sop_name": sop_name, "existing_run_id": existing_run_id})), &format!("SOP headless dispatch: coalesced '{sop_name}' into run {existing_run_id}"));
            }
...
mod tests {
    ...
    use omnesagent_memory::traits::{Memory, MemoryCategory, MemoryEntry};
    ...
    impl ::omnesagent_api::attribution::Attributable for TestMemory {
