...
impl SopEngine {
        ...
        match self.store.load_active_runs() {
            Ok(runs) => {
                ...
                for pr in runs {
                    ...
                    {
                        let span = ::omnesagent_log::attribution_span!(&pr.run);
    ...
    pub(crate) fn set_approval_config_for_test(
        &mut self,
        approval: omnesagent_config::schema::SopApprovalConfig,
    ) {
    ...
    pub fn approval_config(&self) -> &omnesagent_config::schema::SopApprovalConfig {
...
pub(crate) fn channel_trigger_topic_matches(
    ...
    let (topic_channel, topic_alias, _event_type) =
        omnesagent_api::channel::ChannelSopTopic::parse(topic);
...
mod tests {
    ...
    /// `omnesagent sop approve` does, replacing the old `approve_step` agent path.
    ...
    fn gate_votes_are_per_step_and_canonical_per_subject() {
        ...
        // Same subject "OmnesAgentOperator" over HTTP then WS: collapses to gateway:OmnesAgentOperator.
        engine
            .record_gate_vote(
                "run-1",
                1,
                "p",
                0,
                &ApprovalPrincipal::http(Some("OmnesAgentOperator".into())),
            )
            .unwrap();
        engine
            .record_gate_vote(
                ...
                "p",
                0,
                &ApprovalPrincipal::ws("c".into(), Some("OmnesAgentOperator".into())),
            )
            .unwrap();
        ...
        engine
            ...
            .unwrap();
        // A CLI actor is a genuinely distinct source (cli:OmnesAgentMaintainer).
        engine
            .record_gate_vote(
                "run-1",
                1,
        ...
        assert_eq!(
            distinct(1),
            2,
            "gateway:OmnesAgentOperator (http+ws collapsed) + cli:OmnesAgentMaintainer = 2 distinct step-1 voters"
        );
        assert_eq!(
            distinct(2),
            1,
    ...
    fn record_gate_vote_is_idempotent_per_voter_and_policy() {
        ...
        let store = std::sync::Arc::new(InMemoryRunStore::new());
        let engine = engine_with_sops(vec![]).with_store(store);
        let omnes_agent_operator = ApprovalPrincipal::cli(Some("OmnesAgentOperator".into()));

        engine
            .record_gate_vote("run-1", 1, "prod", 0, &omnes_agent_operator)
            .unwrap();
        engine
            .record_gate_vote("run-1", 1, "prod", 0, &omnes_agent_operator)
            .unwrap();
        ...
        engine
            .record_gate_vote("run-1", 1, "prod2", 0, &omnes_agent_operator)
            .unwrap();
        assert_eq!(
            engine.gate_votes_for_step("run-1", 1).unwrap().len(),
            2,
    ...
    fn policied_supervised_engine(
        ...
        use omnesagent_config::schema::ApprovalPolicyConfig;
    ...
    fn timeout_escalation_without_distinct_route_resurfaces_request_route() {
        ...
        crate::sop::approval::timeout::apply_timeout_action(
            &mut engine,
            &run_id,
            omnesagent_config::schema::ApprovalTimeoutAction::Escalate,
        );
    ...
    fn timeout_cancel_finishes_run() {
        let mut engine = SopEngine::new(SopConfig {
            approval_timeout_secs: 1,
            approval_timeout_action: omnesagent_config::schema::ApprovalTimeoutAction::Cancel,
            ..SopConfig::default()
        });
        engine.set_sops_for_test(vec![test_sop(
            "s1",
    ...
    fn timeout_auto_approve_legacy_resumes() {
        ...
        let mut engine = SopEngine::new(SopConfig {
            approval_timeout_secs: 1,
            approval_timeout_action: omnesagent_config::schema::ApprovalTimeoutAction::AutoApprove,
            ..SopConfig::default()
        });
        engine.set_sops_for_test(vec![test_sop(
            "s1",
