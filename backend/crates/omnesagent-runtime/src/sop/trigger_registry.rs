...
use omnesagent_api::attribution::ChannelKind;
...
/// decoupled from `omnesagent_config` so the walk is testable without a full
/// `Config`.
...
/// to `build_registry`.
#[must_use]
pub fn registry_from_config(config: &omnesagent_config::schema::Config) -> TriggerSourceRegistry {
...
mod tests {
    ...
    fn retired_wati_kind_is_absent_from_default_trigger_registry() {
        let registry = registry_from_config(&omnesagent_config::schema::Config::default());
        assert!(
