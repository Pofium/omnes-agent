...
impl Tool for SendMessageToPeerTool {
    ...
    fn param_domains(&self) -> Vec<(&'static str, omnesagent_api::tool::OptionDomain)> {
        use omnesagent_api::tool::OptionDomain;
        vec![
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        if target_is_agent {
            ...
            // `omnesagent_spawn::spawn!` task does not inherit the caller's
            ...
            omnesagent_spawn::spawn!(async move {
                ...
                {
                    ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"sender": sender, "recipient": recipient_alias, "error": format!("{}", e)})), "peer-message in-process delivery failed");
                }
            });
...
/// `omnesagent-gateway/src/lib.rs`. Split out from `execute` so the
...
mod tests {
    use super::*;
    use omnesagent_config::multi_agent::{AgentAlias, PeerGroupConfig, PeerUsername};
    ...
    async fn peer_turn_cost_scope_records_recipient_usage_and_enforces_shared_budget() {
        ...
        use omnesagent_config::schema::CostConfig;
        use omnesagent_providers::traits::TokenUsage;
    ...
    async fn peer_turn_cost_scope_through_execute_boundary_attributes_recipient_and_shares_budget()
    {
        ...
        // `~/.omnesagent/data`, and `execute()` resolves cost tracking off the
        ...
        // The recipient's turn runs inside a detached `omnesagent_spawn::spawn!`
