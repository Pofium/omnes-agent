...
impl ScopedToolRegistry {
    ...
    /// notably `omnesagent-channels`, whose `ChannelRuntimeContext` fixtures need
...
fn tool_allowed_in_context(name: &str, exclude_memory: bool, acp_delivery: bool) -> bool {
    (!exclude_memory || !omnesagent_tools::MEMORY_TOOL_NAMES.contains(&name))
...
impl ScopedToolRegistry {
    ...
    pub async fn assemble(spec: ScopedAssembly<'_>) -> ScopedAssembled {
        ...
        let resolution_registry: Vec<Arc<dyn Tool>> = context_filtered_tool_arcs
            ...
            .collect();
        let nat64_prefixes = match omnesagent_infra::net_guard::parse_nat64_prefixes(
...
mod tests {
    ...
    impl omnesagent_api::attribution::Attributable for MockTool {
