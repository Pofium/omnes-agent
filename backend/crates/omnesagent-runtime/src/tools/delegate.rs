    ...
    async fn execute_sync_with_admission(
        ...
        let (result, fallback) = omnesagent_providers::reliable::scope_provider_fallback(async {
            let result = self
                ...
                .await;
            let fallback =
                omnesagent_providers::reliable::take_last_provider_fallback_attribution();
            (result, fallback)
        })
        .await;
...
mod tests {
    ...
    async fn independent_delegate_rejects_target_always_ask() {
        ...
        assert!(
            error
                .contains("OmnesAgent docs, \"Delegation & SubAgents\" > \"What's not supported\""),
    ...
    async fn bounded_agentic_tools_drop_deliver_file_without_acp_transport() {
        ...
        impl omnesagent_api::attribution::Attributable for DeliverFileFixture {
            fn role(&self) -> omnesagent_api::attribution::Role {
                omnesagent_api::attribution::Role::Tool(
                    omnesagent_api::attribution::ToolKind::Plugin,
                )
            }
