...
fn delegate_failure_error(agent_name: &str, error: &anyhow::Error) -> String {
    if error
        .chain()
        .any(|source| source.is::<omnesagent_providers::ReliableProviderTerminalFailure>())
    {
...
pub struct DelegateTool {
    ...
    provider_runtime_options: omnesagent_providers::ModelProviderRuntimeOptions,
...
impl DelegateTool {
    ...
    const INDEPENDENT_ALWAYS_ASK_DOC_REF: &'static str =
        "OmnesAgent docs, \"Delegation & SubAgents\" > \"What's not supported\"";
    ...
    pub fn new(
        ...
        Self::new_with_options(
            ...
            global_credential,
            security,
            omnesagent_providers::ModelProviderRuntimeOptions::default(),
        )
    }
    ...
    pub fn with_multimodal_config(
        mut self,
        config: omnesagent_config::schema::MultimodalConfig,
    ) -> Self {
    ...
    fn build_target_provider(
        ...
        let provider = omnesagent_providers::create_model_provider_with_options(
            provider_type,
            credential,
    ...
    async fn memory_for_target_agent(
        ...
        omnesagent_memory::create_memory_for_agent(config, agent_name, api_key)
            .await
            .map(Some)
    }
...
impl DelegateTool {
    ...
    async fn execute_sync_with_admission(
        ...
        let (result, fallback) = omnesagent_providers::reliable::scope_provider_fallback(async {
            let result = self
                .execute_sync_with_admission_inner(agent_name, prompt, args, admission)
                .await;
            let fallback = omnesagent_providers::reliable::take_last_provider_fallback_attribution();
            (result, fallback)
        })
        .await;
    ...
    fn render_non_agentic_result(
        ...
        match result {
            Ok(response)
                if omnesagent_api::model_provider::strip_think_tags(&response).is_empty() =>
            {
                ToolResult {
                    success: false,
                    output: ToolOutput::default(),
...
impl DelegateTool {
    ...
    async fn execute_parallel(
        ...
        for agent_name in &agent_names {
            ...
            handles.push(omnesagent_spawn::spawn!(
                async move {
                    let inner = DelegateTool {
                        agents,
                        security,
    ...
    async fn execute_agentic_with_admission(
                ...
                let needs_memory_tools = {
                    ...
                    parent_tools.iter().any(|tool| {
                        self.security.is_tool_allowed(tool.name())
                            && omnesagent_tools::MEMORY_TOOL_NAMES.contains(&tool.name())
        ...
        let pacing = omnesagent_config::schema::PacingConfig::default();
...
impl ::omnesagent_api::attribution::Attributable for ToolArcRef {
...
mod tests {
    ...
    use omnesagent_config::scattered_types::{ThinkingConfig, ThinkingLevel};
    use omnesagent_config::schema::{
    ...
    use omnesagent_memory::{AgentScopedMemory, SqliteMemory};
    ...
    omnesagent_api::mock_tool_attribution!(EchoTool, FakeMcpTool);
    ...
    fn security_allowing() -> Arc<SecurityPolicy> {
        Arc::new(SecurityPolicy {
            delegation_policy: omnesagent_config::autonomy::DelegationPolicy {
                mode: omnesagent_config::autonomy::DelegationMode::Allow,
            },
            ..SecurityPolicy::default()
        })
    }
    ...
    impl ::omnesagent_api::attribution::Attributable for OneToolThenFinalModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
    ...
    impl ::omnesagent_api::attribution::Attributable for IndependentRiskPolicyModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for EchoToolResultThenFinalModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for TextFallbackToolModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for InfiniteToolCallModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn delegate_memory_fixture(model_uri: Option<String>) -> DelegateMemoryFixture {
        use omnesagent_config::autonomy::{DelegationMode, DelegationPolicy};
    ...
    impl ::omnesagent_api::attribution::Attributable for MemoryStoreRecallThenFinalModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn start_failing_chat_server_with_error(
        ...
        let task = omnesagent_spawn::spawn!(async move {
    ...
    fn roster_schema_config() -> Arc<omnesagent_config::schema::Config> {
        use omnesagent_config::autonomy::{DelegationMode, DelegationPolicy};
        use omnesagent_config::schema::{AliasedAgentConfig, Config, RiskProfileConfig};
        let root =
            std::env::temp_dir().join(format!("omnesagent-delegate-policy-{}", uuid::Uuid::new_v4()));
    ...
    fn roster_tool(config: Arc<omnesagent_config::schema::Config>) -> DelegateTool {
        let caller_policy =
    ...
    async fn non_agentic_delegate_projects_typed_terminal_failure() {
        let result = DelegateTool::render_non_agentic_result(
            ...
            Err(anyhow::Error::new(
                omnesagent_api::model_provider::SemanticEmptyTerminalCompletion,
            )),
        );
    ...
    impl ::omnesagent_api::attribution::Attributable for McpToolThenFinalModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for FinalOnlyModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for EmptyTerminalModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for ToolThenEmptyTerminalModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for ToolCountModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    struct RecordedThinkingRequest {
        ...
        thinking_display: Option<omnesagent_api::model_provider::ThinkingDisplay>,
    ...
    impl ::omnesagent_api::attribution::Attributable for ThinkingRecordingModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn independent_delegate_uses_target_native_thinking_and_restores_parent_scope() {
        ...
        let parent = Some(omnesagent_config::scattered_types::NativeThinkingParams {
    ...
    async fn independent_delegate_propagates_display_mode_to_target_request() {
        use omnesagent_api::model_provider::ThinkingDisplay;
        use omnesagent_config::scattered_types::ThinkingDisplayMode;
    ...
    async fn deferred_mcp_activation_updates_delegate_parent_tools() {
        ...
        let deferred = crate::tools::DeferredMcpToolSet {
            stubs: vec![{
                let def = omnesagent_tools::mcp_protocol::McpToolDef {
            ...
            registry: Arc::new(
                omnesagent_tools::mcp_client::McpRegistry::connect_all(&[])
                    .await
                    .unwrap(),
            ),
    ...
    fn enriched_prompt_includes_tools_workspace_date() {
        ...
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_enrich_test_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    fn enriched_prompt_includes_shell_policy_when_shell_present() {
        ...
        impl ::omnesagent_api::attribution::Attributable for MockShellTool {
            fn role(&self) -> ::omnesagent_api::attribution::Role {
                ::omnesagent_api::attribution::Role::Tool(
                    ::omnesagent_api::attribution::ToolKind::Shell,
                )
            }
            fn alias(&self) -> &str {
    ...
    fn enriched_prompt_reports_powershell_dialect_for_delegate_with_shell_tool() {
        ...
        impl crate::platform::RuntimeAdapter for PsRuntime {
            ...
            fn shell_profile(&self) -> Option<omnesagent_api::runtime_traits::ShellProfile> {
                Some(omnesagent_api::runtime_traits::ShellProfile {
    ...
    fn config_validation_accepts_minimal_agent() {
        ...
        config.providers.models.ollama.insert(
            "default".into(),
            omnesagent_config::schema::OllamaModelProviderConfig::default(),
        );
        config.risk_profiles.insert(
            "default".into(),
    ...
    fn enriched_prompt_loads_skills_from_scoped_directory() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_skills_test_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    fn enriched_prompt_falls_back_to_default_skills_dir() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_fallback_test_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn background_unknown_agent_rejected() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_bg_unknown_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn check_result_missing_task_id() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_check_noid_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn check_result_nonexistent_task() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_check_miss_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn await_sessions_returns_completed_results() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_await_done_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn await_sessions_reports_failed_results() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_await_failed_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
        let task_id = uuid::Uuid::new_v4().to_string();
    ...
    async fn await_sessions_times_out_with_pending_results() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_await_pending_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn await_sessions_reports_missing_tasks() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_await_missing_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn await_sessions_rejects_duplicate_task_ids() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_await_duplicate_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
        let task_id = uuid::Uuid::new_v4().to_string();
    ...
    async fn await_sessions_rejects_invalid_task_ids() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_await_invalid_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
        let tool = DelegateTool::new(sample_agents(), None, test_security())
    ...
    async fn await_sessions_rejects_too_many_task_ids() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_await_many_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn await_sessions_rejects_invalid_timeout_ms() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_await_bad_timeout_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
        let task_id = uuid::Uuid::new_v4().to_string();
    ...
    async fn await_sessions_rejects_timeout_ms_over_cap() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_await_timeout_cap_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
        let task_id = uuid::Uuid::new_v4().to_string();
    ...
    async fn list_results_empty() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_list_empty_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn cancel_task_nonexistent() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_cancel_miss_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn background_task_result_persisted_to_disk() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_bg_persist_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn check_result_retrieves_persisted_background_result() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_check_retrieve_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn list_results_includes_background_tasks() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_list_tasks_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn check_result_rejects_path_traversal() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_traversal_check_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn cancel_task_rejects_path_traversal() {
        let workspace = std::env::temp_dir().join(format!(
            "omnesagent_delegate_traversal_cancel_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    fn config_with_always_ask_delegate(mode: DelegateExecutionMode) -> Arc<Config> {
        use omnesagent_config::autonomy::{DelegationMode, DelegationPolicy};
        use omnesagent_config::schema::{RiskProfileConfig, RuntimeProfileConfig};

        let root = std::env::temp_dir().join(format!(
            "omnesagent-delegate-always-ask-{}",
            uuid::Uuid::new_v4()
        ));
        let mut config = Config {
            data_dir: root.join("data"),
    ...
    async fn independent_delegate_rejects_target_always_ask() {
        ...
        assert!(
            error.contains("OmnesAgent docs, \"Delegation & SubAgents\" > \"What's not supported\""),
    ...
    async fn delegate_target_inherits_caller_session_workspace_dir() {
        ...
        let session_cwd = PathBuf::from("/tmp/omnesagent-test-delegate-session-cwd-7263");
    ...
    async fn independent_delegate_target_keeps_own_workspace_dir() {
        ...
        let session_cwd = PathBuf::from("/tmp/omnesagent-test-independent-delegate-session-cwd");
        let mut caller_policy =
            SecurityPolicy::for_agent(&config, "caller").expect("caller policy resolves");
        caller_policy.workspace_dir = session_cwd.clone();
        let caller_policy = Arc::new(caller_policy);
    ...
    async fn bounded_agentic_tools_drop_deliver_file_without_acp_transport() {
        ...
        impl omnesagent_api::attribution::Attributable for DeliverFileFixture {
            fn role(&self) -> omnesagent_api::attribution::Role {
...
"#,
        ...
        config.agents.insert(
            ...
            AliasedAgentConfig {
                ...
                runtime_profile: "agentic".into(),
                model_provider: "ollama.target".into(),
                workspace: omnesagent_config::multi_agent::AgentWorkspaceConfig {
...
"#,
    ...
    fn config_with_narrowed_target() -> Arc<omnesagent_config::schema::Config> {
        use omnesagent_config::autonomy::{DelegationMode, DelegationPolicy};
        use omnesagent_config::schema::{AliasedAgentConfig, Config, RiskProfileConfig};
        ...
        config.providers.models.custom.insert(
            ...
            CustomModelProviderConfig {
                base: ModelProviderConfig {
                    ...
                    fallback: vec![omnesagent_config::providers::ModelProviderRef::new(
                        "custom.backup",
                    )],
                    ..ModelProviderConfig::default()
                },
    ...
    async fn delegate_fallback_warning_is_local_to_synchronous_call() {
        ...
        let (result, outer_fallback) =
            omnesagent_providers::reliable::scope_provider_fallback(async {
                ...
                let outer_fallback = omnesagent_providers::reliable::take_last_provider_fallback();
    ...
    impl ::omnesagent_api::attribution::Attributable for FileReadTool {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Tool(::omnesagent_api::attribution::ToolKind::Plugin)
        }
        fn alias(&self) -> &str {
    ...
    impl ::omnesagent_api::attribution::Attributable for FileWriteTool {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Tool(::omnesagent_api::attribution::ToolKind::Plugin)
        }
        fn alias(&self) -> &str {
    ...
    impl ::omnesagent_api::attribution::Attributable for MockShellTool {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Tool(::omnesagent_api::attribution::ToolKind::Shell)
        }
        fn alias(&self) -> &str {
            <Self as Tool>::name(self)
        }
    ...
    impl ::omnesagent_api::attribution::Attributable for ToolListInspector {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
...
mod tool_arc_ref_spec_tests {
    ...
    impl ::omnesagent_api::attribution::Attributable for TriggerOwningTool {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Tool(::omnesagent_api::attribution::ToolKind::Plugin)
        }
        fn alias(&self) -> &str {
