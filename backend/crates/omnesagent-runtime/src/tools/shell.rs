...
use omnesagent_api::platform::is_android;
...
impl Drop for ChildGroupGuard {
    fn drop(&mut self) {
        ...
        ::omnesagent_log::record!(
            WARN,
            ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Kill)
...
/// into shell tools.
pub(crate) const SESSION_ID_ENV_VAR: &str = "OMNESAGENT_SESSION_ID";

fn get_session_id() -> Option<String> {
    omnesagent_api::TOOL_LOOP_SESSION_KEY
...
mod tests {
    ...
    fn is_android_returns_bool_without_panicking() {
        let _ = omnesagent_api::platform::is_android();
    }
    use super::*;
    use crate::platform::{DockerRuntime, NativeRuntime, RuntimeAdapter};
    use crate::security::{AutonomyLevel, SecurityPolicy};
    use omnesagent_config::schema::DockerRuntimeConfig;
    use omnesagent_tools::wrappers::RateLimitedTool;
    ...
    fn medium_risk_write_command() -> &'static str {
        "copy NUL omnesagent_shell_approval_test"
    }

    #[cfg(not(windows))]
    fn medium_risk_write_command() -> &'static str {
        "touch omnesagent_shell_approval_test"
    }
    ...
    async fn shell_executes_windows_nul_redirect_through_cmd_exe() {
        ...
        let result = tool
            .execute(json!({"command": "echo omnesagent_nul_stdout 2>nul"}))
            .await
    ...
    async fn shell_rejects_nul_redirect_through_posix_production_shape() {
        ...
        let tool = wrapped_shell(test_security(AutonomyLevel::Supervised));
        for cmd in ["echo omnesagent x >nul", r"echo omnesagent x >\\.\nul"] {
            let result = tool
    ...
    async fn shell_blocks_symlink_escape_end_to_end() {
        ...
        let root = std::env::temp_dir().join(format!(
            "omnesagent_shell_e2e_symlink_escape_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn shell_does_not_leak_api_key() {
        ...
        let _g2 = EnvGuard::set("OMNESAGENT_API_KEY", "sk-test-secret-67890");
        ...
        assert!(
            ...
            "OMNESAGENT_API_KEY leaked to shell command output"
        );
    }
    ...
    async fn shell_allows_configured_env_passthrough() {
        let _guard = EnvGuard::set("OMNESAGENT_TEST_PASSTHROUGH", "db://unit-test");
        let tool = ShellTool::new(
            test_security_with_env_passthrough(&["OMNESAGENT_TEST_PASSTHROUGH"]),
            test_runtime(),
        );
