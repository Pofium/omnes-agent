...
use serde_json::json;
use std::sync::Arc;
use omnesagent_api::tool::{Tool, ToolOutput, ToolResult, with_ephemeral_workspace_warning};
...
mod tests {
    ...
    use omnesagent_tools::wrappers::{PathGuardedTool, RateLimitedTool};
    ...
    async fn file_read_existing_file() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read");
    ...
    async fn file_read_nonexistent_file() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_missing");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_blocks_path_traversal() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_traversal");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_allows_readonly_mode() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_readonly");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_base64_returns_encoded_bytes() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_base64");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_warns_on_ephemeral_workspace() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_ephemeral");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_base64_not_warned_on_ephemeral_workspace() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_ephemeral_b64");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_failure_not_warned_on_ephemeral_workspace() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_ephemeral_fail");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_no_warning_when_persistent() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_persistent");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_unsupported_encoding_errors() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_bad_encoding");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_empty_file() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_empty");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_nested_path() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_nested");
        let _ = tokio::fs::remove_dir_all(&dir).await;
    ...
    async fn file_read_normalizes_workspace_prefixed_relative_path() {
        let root = std::env::temp_dir().join("omnesagent_test_file_read_workspace_prefixed");
    ...
    async fn file_read_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("omnesagent_test_file_read_symlink_escape");
        let workspace = root.join("workspace");
    ...
    async fn file_read_blocks_outside_workspace_regardless_of_policy() {
        let root = std::env::temp_dir().join("omnesagent_test_file_read_blocks_outside");
        let workspace = root.join("workspace");
        let outside = root.join("outside");
    ...
    async fn file_read_admits_absolute_path_under_read_only_root() {
        let root =
            std::env::temp_dir().join("omnesagent_test_file_read_admits_absolute_path_under_ro_root");
        let workspace = root.join("workspace");
    ...
    async fn file_read_with_offset_and_limit() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_offset");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_offset_beyond_end() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_offset_end");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_rejects_oversized_file() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_large");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_rejects_binary_file() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_reject_binary");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_rejects_pdf_without_rag_pdf() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_reject_pdf");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_rejects_png_image() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_reject_png");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_rejects_jpeg_image() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_reject_jpeg");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_rejects_bmp_image() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_reject_bmp");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_reads_non_utf8_bm_text_lossy() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_bm_text");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_reads_non_utf8_text_lossy() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_legacy_text");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    mod e2e_helpers {
        ...
        use omnesagent_config::schema::MemoryConfig;
        use omnesagent_memory::{self, Memory};
        use omnesagent_providers::{ChatMessage, ChatRequest, ChatResponse, ModelProvider};
        ...
        pub fn make_memory() -> Arc<dyn Memory> {
            ...
            Arc::from(omnesagent_memory::create_memory(&cfg, &std::env::temp_dir(), None).unwrap())
        }
    ...
    async fn e2e_agent_file_read_rejects_binary() {
        ...
        use e2e_helpers::*;
        use omnesagent_providers::{ChatResponse, ModelProvider, ToolCall};

        // ── Set up workspace with binary file ──
        let workspace = std::env::temp_dir().join("omnesagent_test_e2e_file_read_lossy");
    ...
    async fn file_read_blocks_null_byte_in_path() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_null_byte");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_allows_dev_null() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_dev_null");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_allowed_root_with_workspace_only() {
        let root = std::env::temp_dir().join("omnesagent_test_file_read_allowed_root");
        let workspace = root.join("workspace");
    ...
    async fn wrapped_file_read_failure_preserves_rate_limit_budget() {
        let dir = std::env::temp_dir().join("omnesagent_test_file_read_probe");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
