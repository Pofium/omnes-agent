...
/// OmnesAgent CA: the relay binds the node-id to this key and an allowlist keys on
/// its fingerprint.
...
/// The on-demand rotation trigger file. `omnesagent security relay-rotate-node-id`
...
pub async fn run_relay_bridge(cfg: RelayBridgeConfig, cancel: CancellationToken) -> Result<()> {
    ...
    let mut link = {
        ...
        omnesagent_spawn::spawn!(async move { serve_link(c, lc, None).await })
    };
    ...
    loop {
        ...
        let new_link = {
            ...
            omnesagent_spawn::spawn!(async move { serve_link(c, lc, Some(registered_tx)).await })
        };
...
async fn connect_and_register(
    ...
    relay_ws_cfg.max_message_size = Some(omnesagent_relay_proto::MAX_WS_MESSAGE);
    relay_ws_cfg.max_frame_size = Some(omnesagent_relay_proto::MAX_WS_MESSAGE);
    let (mut ws, _resp) =
...
async fn serve_established(
    ...
    let writer = {
        ...
        omnesagent_spawn::spawn!(relay_writer(from_tasks, sink, link_dead))
    };
    ...
    // Keepalive watchdog: ping below NAT timeout, declare dead on silence.
    {
        ...
        let link_dead = link_dead.clone();
        omnesagent_spawn::spawn!(keepalive_watchdog(to_relay, last_seen, link_dead));
    }
...
fn relay_client_config(
    ...
    Option<Arc<omnesagent_tls::RelayPinVerifier>>,
)> {
    ...
    let (verified, verifier) = if insecure {
        ...
        for cert in omnesagent_tls::load_certs(ca)? {
        ...
        let v = Arc::new(omnesagent_tls::RelayPinVerifier::pinned(pin.to_string()));
        (
            builder
                .dangerous()
        ...
        let v = Arc::new(omnesagent_tls::RelayPinVerifier::tofu());
        (
            builder
                .dangerous()
                .with_custom_certificate_verifier(v.clone()),
    ...
    let config = match (outer_cert, outer_key) {
        (Some(cert), Some(key)) => {
            let chain = omnesagent_tls::load_certs(cert)?;
            let key = omnesagent_tls::load_private_key(key)?;
            verified
...
// `omnesagent_spawn::spawn!` attribution rule is for production daemon tasks).
