...
pub async fn migrate_openclaw_memory(
    ...
    if paths_equal(&source_workspace, &config.data_dir) {
        bail!("Source workspace matches current OmnesAgent workspace; refusing self-migration");
    }
    ...
    if reindex {
        ...
        // configured embedder wired in - the same construction `omnesagent
...
fn target_memory_backend(config: &Config) -> Result<Box<dyn Memory>> {
    let backend = omnesagent_memory::backend_kind_from_dotted(&config.memory.backend);
    if omnesagent_memory::classify_memory_backend(&backend)
        == omnesagent_memory::MemoryBackendKind::Qdrant
    {
        bail!(crate::i18n::get_required_cli_string(
            "cli-migrate-openclaw-qdrant-unsupported"
        ));
    }
    omnesagent_memory::create_memory_for_migration(config)
}

/// Memory handle for the post-import `--reindex` pass, with the configured
/// embedder resolved and wired in. Mirrors `omnesagent memory reindex`
...
mod tests {
    ...
    use omnesagent_config::schema::{Config, MemoryConfig};
    use omnesagent_memory::SqliteMemory;
