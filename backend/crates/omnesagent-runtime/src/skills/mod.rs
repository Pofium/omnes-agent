...
const OPEN_SKILLS_SYNC_MARKER: &str = ".omnesagent-open-skills-sync";
const OPEN_SKILLS_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24 * 7;

// ─── Skills registry (omnesagent-skills) ────────────────────────────────────────
const SKILLS_REGISTRY_REPO_URL: &str = "https://github.com/omnesagent-labs/omnesagent-skills";
const SKILLS_REGISTRY_DIR_NAME: &str = "skills-registry";
const SKILLS_REGISTRY_SYNC_MARKER: &str = ".omnesagent-skills-registry-sync";
...
/// Skills live in `~/.omnesagent/workspace/skills/<name>/SKILL.md`
...
pub struct Skill {
    ...
    /// prompt even in [`omnesagent_config::schema::SkillsPromptInjectionMode::Compact`]
...
impl ::omnesagent_api::attribution::Attributable for Skill {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Skill
    }
    fn alias(&self) -> &str {
        &self.name
    }
    ...
    if scripts_blocked && !allow_scripts {
        ...
        eprintln!(
            "warning: skill '{}' was skipped because it contains script files. \
             Set `skills.allow_scripts = true` in your omnesagent config to enable it.",
            path.file_name()
...
pub fn load_skills_for_agent_audited(
    ...
    for bundle_alias in &agent.skill_bundles {
        let bundle = match config.skill_bundles.get(bundle_alias) {
            Some(b) => b,
            None => {
                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": agent_alias, "bundle": bundle_alias, "bundle_alias": bundle_alias})), "skipping skill bundle: [skill_bundles.] is not configured");
                continue;
            }
        };
        let dir = match omnesagent_config::skill_bundles::resolve_directory(
            config,
            &install_root,
            bundle_alias,
        ) {
            Ok(d) => d,
            Err(e) => {
                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": agent_alias, "bundle": bundle_alias, "e": e.to_string()})), "skipping skill bundle: ");
                continue;
            }
        };
    ...
    if let Some(raw) = env_override {
        ...
        if !raw.trim().is_empty() {
            ::omnesagent_log::record!(
                ...
                ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note)
                    .with_outcome(::omnesagent_log::EventOutcome::Unknown),
                "Ignoring invalid OMNESAGENT_OPEN_SKILLS_ENABLED (valid: 1|0|true|false|yes|no|on|off)"
            );
        }
    }
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("OMNESAGENT_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("OMNESAGENT_OPEN_SKILLS_DIR").ok();
...
pub fn skills_prompt_mode_with_loader_fallback(
    mode: omnesagent_config::schema::SkillsPromptInjectionMode,
    read_skill_available: bool,
) -> omnesagent_config::schema::SkillsPromptInjectionMode {
    if matches!(
        mode,
...
) -> Vec<Box<dyn omnesagent_api::tool::Tool>> {
...
fn resolve_elevated_tool(
    ...
    tool: &SkillTool,
    kind_label: &str,
    resolution_registry: &[std::sync::Arc<dyn omnesagent_api::tool::Tool>],
) -> Option<Box<dyn omnesagent_api::tool::Tool>> {
...
pub fn skills_to_tools_with_context(
    skills: &[Skill],
    security: std::sync::Arc<crate::security::SecurityPolicy>,
    unfiltered_registry: &[std::sync::Arc<dyn omnesagent_api::tool::Tool>],
    nat64_prefixes: &[omnesagent_infra::net_guard::Nat64Prefix],
) -> Vec<Box<dyn omnesagent_api::tool::Tool>> {
...
pub(crate) fn skills_to_tools_with_context_and_runtime_optional_nat64(
    ...
    let mut tools: Vec<Box<dyn omnesagent_api::tool::Tool>> = Vec::new();
    for skill in skills {
        for tool in &skill.tools {
            ...
            match tool.kind.as_str() {
                "shell" | "script" => {
                    ...
                    tools.push(Box::new(omnesagent_tools::wrappers::RateLimitedTool::new(
                        inner,
                        security.clone(),
                    )));
                }
    ...
    if !readme.exists() {
        std::fs::write(
            ...
            "# OmnesAgent Skills\n\n\
             ...
             ```bash\n\
             omnesagent skills install <source>\n\
             omnesagent skills list\n\
             ```\n",
        )?;
    }
...
pub fn parse_extra_registry_source(source: &str) -> Option<(String, String)> {
    ...
    if !omnesagent_config::schema::ExternalRegistry::is_valid_name(name) || !is_registry_source(skill)
    {
        return None;
    }
...
pub fn install_extra_registry_skill_source(
    ...
    allow_scripts: bool,
    workspace_dir: &Path,
    extra_registries: &[omnesagent_config::schema::ExternalRegistry],
    suppress_tier_banner: bool,
) -> Result<(PathBuf, usize)> {
    ...
    if registry.kind != omnesagent_config::schema::ExternalRegistryKind::Git {
        anyhow::bail!(
...
pub fn load_plugin_skills_from_config(
    ...
    let host = match omnesagent_plugins::host::PluginHost::from_plugins_dir_with_security(
        &plugins_dir,
        signature_mode,
        trusted_keys,
    ) {
    ...
    for (manifest, skills_dir) in host.skill_plugin_details() {
        ...
        if !plan.admits(
            &manifest.name,
            omnesagent_plugins::PluginCapability::Skill,
            &manifest.name,
        ) {
            continue;
        }
...
mod registry_tests {
    ...
    fn test_is_registry_source_rejects_paths() {
        ...
        assert!(!is_registry_source("~/.omnesagent/skills/foo"));
    }
    ...
    fn extra_registry_rejects_symlinked_root_and_selected_skill() {
        for symlink_root in [true, false] {
            ...
            mark_test_registry_fresh(&registry);
            let configured = [omnesagent_config::schema::ExternalRegistry {
                name: "custom".to_string(),
                url: "unused".to_string(),
                kind: omnesagent_config::schema::ExternalRegistryKind::Git,
                enabled: true,
            }];
    ...
    fn build_install_tier_banner_official_is_single_line() {
        ...
        assert!(banner.contains("Official (omnesagent-labs maintained)"));
    ...
    fn build_install_tier_banner_community_warns() {
        ...
        assert!(banner.contains("not audited by OmnesAgent"));
        assert!(banner.contains("omnesagent skills audit discord-moderator"));
    }
