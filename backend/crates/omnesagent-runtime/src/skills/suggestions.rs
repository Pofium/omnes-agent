...
use std::path::Path;
use omnesagent_config::schema::ExternalRegistryKind;

#[cfg(feature = "plugins-wasm")]
use omnesagent_plugins::registry::{
    ...
    pub fn render_user_message(&self) -> String {
        let (message_key, install_command) = match self.install_kind {
            InstallKind::Skill => (
                ...
                format!("omnesagent skills install {}", self.source),
            ),
            #[cfg(feature = "plugins-wasm")]
...
fn load_cached_installable_skill_capabilities(
    ...
    {
        if !omnesagent_config::schema::ExternalRegistry::is_valid_name(&registry.name) {
            continue;
        }
...
mod tests {
    ...
    fn extra_registry(name: &str, enabled: bool) -> omnesagent_config::schema::ExternalRegistry {
    ...
    fn extra_registry_with_kind(
        ...
        kind: &str,
        enabled: bool,
    ) -> omnesagent_config::schema::ExternalRegistry {
        omnesagent_config::schema::ExternalRegistry {
            name: name.to_string(),
    ...
    fn missing_high_confidence_capability_returns_install_suggestion() {
        ...
        assert!(
            suggestion
                .render_user_message()
                .contains("omnesagent skills install calendar")
        );
    }
...
"#,
        ...
        assert!(suggestion.contains("omnesagent skills install calendar"));
...
"#,
        ...
        assert!(
            suggestion
                .render_user_message()
                .contains("omnesagent skills install registry:acme/team-calendar")
        );
    }
...
"#,
        ...
        assert!(suggestion.contains(
            "omnesagent plugin install team-calendar@0.2.0 --registry https://example.invalid/registry.json"
        ));
    }
...
"#,
        ...
        assert!(suggestion.contains("omnesagent skills install"));
        assert!(!suggestion.contains("omnesagent plugin install"));
    }
