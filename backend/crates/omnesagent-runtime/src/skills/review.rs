...
use omnesagent_config::schema::{MultimodalConfig, PacingConfig, SkillImprovementConfig};
...
pub async fn maybe_run_skill_review(
    ...
    full_config: Option<&omnesagent_config::schema::Config>,
    ...
    let review_default_config = omnesagent_config::schema::Config::default();
    let review_config_ref = full_config.unwrap_or(&review_default_config);
    let review_alias = agent_alias.unwrap_or("default");
    let review_security = Arc::new(omnesagent_config::policy::SecurityPolicy::default());
