...
use omnesagent_api::model_provider::{ChatMessage, ChatRequest, ModelProvider};
use omnesagent_config::schema::SkillCreationConfig;
use omnesagent_memory::embeddings::EmbeddingProvider;
use omnesagent_memory::vector::cosine_similarity;
...
impl SkillCreator {
    ...
    fn normalize_reflected_md(slug: &str, raw: &str) -> Result<String> {
        ...
        doc.frontmatter.author = Some("omnesagent-auto".to_string());
    ...
    fn generate_skill_toml(slug: &str, description: &str, tool_calls: &[ToolCallRecord]) -> String {
        ...
        toml.push_str("author = \"omnesagent-auto\"\n");
    ...
    async fn enforce_lru_limit(&self) -> Result<()> {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            let content = tokio::fs::read_to_string(&manifest).await?;
            // The auto marker appears as `author = "omnesagent-auto"` in TOML and
            // `author: omnesagent-auto` in reflected `SKILL.md`; match the bare
            // token so both formats are evicted.
            if content.contains("omnesagent-auto") || content.contains("auto-generated") {
...
pub fn extract_tool_calls_from_history(
    history: &[omnesagent_providers::ChatMessage],
) -> Vec<ToolCallRecord> {
...
mod tests {
    ...
    use omnesagent_memory::embeddings::{EmbeddingProvider, NoopEmbedding};
...
"#
    ...
    async fn create_from_execution_success() {
        ...
        assert!(toml_content.contains("omnesagent-auto"));
    }
...
"#,
    ...
    async fn reflected_writes_skill_md_on_success() {
        ...
        assert_eq!(doc.frontmatter.author.as_deref(), Some("omnesagent-auto"));
