...
use omnesagent_api::runtime_traits::RuntimeAdapter;
use omnesagent_config::schema::Config;
use omnesagent_config::schema::{CronJobDecl, CronScheduleDecl, CronShellOutputFormat};
...
pub async fn run(
    ...
    match clear_stale_locks(&config) {
        Ok(0) => {}
        Ok(cleared) => ::omnesagent_log::record!(
            INFO,
            ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note)
...
async fn execute_job_now_with_runtime(
    ...
    let span = omnesagent_log::attribution_span!(job);
...
async fn process_due_jobs(
    ...
    let mut in_flight = stream::iter(jobs.into_iter().filter_map(|job| {
        ...
        let security = match SecurityPolicy::for_agent(config, &agent_alias) {
            Ok(s) => Arc::new(s),
            Err(e) => {
                ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"job_id": job.id, "agent": agent_alias, "error": format!("{}", e)})), "Cron job: failed to build SecurityPolicy for owning agent");
                let _ = release_job(config, &job.id);
                return None;
            }
        };
...
async fn run_agent_job(
    ...
    let subagent_span = omnesagent_log::info_span!(
        "subagent",
        category = "cron",
        ...
        SessionTarget::Main | SessionTarget::Isolated => {
            let run_fut = Box::pin(
                crate::agent::run(
                    ...
                    omnesagent_api::ingress::TurnOrigin::Cron,
                    run_overrides,
                )
    ...
    match run_result {
        ...
        Err(e) => {
            if matches!(job.session_target, SessionTarget::Isolated) {
                let mem_session_key = omnesagent_api::session_keys::sanitize_session_key(&format!(
                    "cli:{}",
                    session_path.display()
                ));
                if let Ok(mem) = omnesagent_memory::create_memory_for_agent(
                    config,
                    agent_alias,
                    config
                        .model_provider_for_agent(agent_alias)
...
mod tests {
    ...
    use omnesagent_config::schema::{Config, RuntimeKind};
    ...
    fn cron_agent_run_policy_uses_scheduler_workspace() {
        let workspace = std::path::PathBuf::from("/tmp/omnesagent-cron-agent-workspace");
    ...
    impl RuntimeAdapter for PowerShellProbeRuntime {
        ...
        fn shell_dialect(&self) -> omnesagent_api::runtime_traits::ShellDialect {
            omnesagent_api::runtime_traits::ShellDialect::PowerShell
        }
    ...
    async fn cron_shell_validation_and_execution_share_runtime_adapter() {
        ...
        let security = SecurityPolicy {
            autonomy: omnesagent_config::policy::AutonomyLevel::Full,
    ...
    async fn persist_job_result_delivery_stubbed_succeeds() {
        // Delivery is stubbed (moved to omnesagent-channels orchestrator).
    ...
    // scan_and_redact_output tests moved to omnesagent-channels orchestrator
