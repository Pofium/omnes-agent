...
use omnesagent_config::schema::CronShellOutputFormat;
...
impl ::omnesagent_api::attribution::Attributable for CronJob {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        let kind = match self.schedule {
            Schedule::Cron { .. } => ::omnesagent_api::attribution::CronKind::Cron,
            Schedule::At { .. } => ::omnesagent_api::attribution::CronKind::At,
            Schedule::Every { .. } => ::omnesagent_api::attribution::CronKind::Interval,
        };
        ::omnesagent_api::attribution::Role::Cron(kind)
    }
    fn alias(&self) -> &str {
        &self.id
    }
