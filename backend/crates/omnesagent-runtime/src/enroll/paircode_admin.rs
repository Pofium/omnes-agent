...
fn sas_for_code(ca_cert_pem: &str, code: &str) -> Result<String> {
    let fingerprint = omnesagent_tls::single_cert_pem_sha256_fingerprint(ca_cert_pem)?;
    Ok(omnesagent_tls::enrollment_sas(code, &fingerprint))
}
...
mod tests {
    ...
    async fn running_loop_mints_additional_code_without_restart() {
        ...
        let loop_handle = omnesagent_spawn::spawn!(request_loop(server, loop_data_dir, loop_cancel));
