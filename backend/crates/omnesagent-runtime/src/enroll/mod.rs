...
//!    [`omnesagent_tls::enrollment_sas`], then the client reconnects pinned to that
//!    confirmed CA.
...
use omnesagent_config::pairing::PairingGuard;
...
pub fn relay_profile(
    ...
    relay: &omnesagent_config::schema::RelayConfig,
) -> RelayProfile {
...
impl EnrollServer {
    ...
    async fn process(
        ...
        let issued = omnesagent_tls::sign_csr(
            &self.ca_cert_pem,
            &self.ca_key_pem,
            &device_id,
            &req.csr_pem,
...
mod tests {
    ...
    fn test_server_with_ledger(
        ...
        let (ca_cert, ca_key) = omnesagent_tls::testing::gen_ca();
        // A throwaway server-auth acceptor (not exercised by process()).
        let (srv_cert, srv_key) =
            omnesagent_tls::testing::gen_server_cert(&ca_cert, &ca_key, &["localhost".into()]);
        ...
        let acceptor = omnesagent_tls::build_tls_acceptor(&omnesagent_tls::ServerConfigParams {
    ...
    async fn relay_class_bad_codes_do_not_lock_out_other_relay_clients() {
        ...
        for _ in 0..12 {
            let (csr, _k) = omnesagent_tls::testing::gen_client_csr("x");
        ...
        let (csr, _k) = omnesagent_tls::testing::gen_client_csr("honest");
        let req = EnrollRequest {
            pairing_code: good,
            csr_pem: csr,
        };
    ...
    async fn process_issues_cert_with_valid_code_and_ignores_csr_identity() {
        ...
        let (csr, _key) = omnesagent_tls::testing::gen_client_csr("attacker-wants-this");
        let req = EnrollRequest {
            pairing_code: code,
            csr_pem: csr,
        };
    ...
    async fn process_propagates_certificate_audit_write_failure() {
        ...
        let audit = crate::security::audit::AuditLogger::new(
            omnesagent_config::schema::AuditConfig {
                enabled: true,
        ...
        let (csr, _key) = omnesagent_tls::testing::gen_client_csr("dev");
        let req = EnrollRequest {
            pairing_code: code,
            csr_pem: csr,
        };
    ...
    async fn an_enrollment_response_that_never_reaches_the_client_is_reconciled_away() {
        ...
        let set = omnesagent_tls::load_revoked_fingerprints(&crl).unwrap();
        assert!(
    ...
    async fn the_enrollment_sweep_revokes_into_the_configured_crl_not_the_default() {
        ...
        let set = omnesagent_tls::load_revoked_fingerprints(&custom)
    ...
    async fn over_the_wire_enroll_issues_a_cert_through_tls() {
        ...
        let srv =
            omnesagent_spawn::spawn!(async move { serve_on(listener, server_c, cancel_c).await });
        ...
        let ca_ders = omnesagent_tls::load_certs(&ca_file.path().to_string_lossy()).unwrap();
