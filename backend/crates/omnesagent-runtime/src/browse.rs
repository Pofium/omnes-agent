...
use omnesagent_config::paths::{RootEscapeError, resolve_under};
...
pub enum BrowseError {
    ...
    #[error("file '{0}' exceeds the {1}-byte read cap; download via CLI or omnesagent shell")]
    TooLarge(String, u64),
    #[error(transparent)]
...
/// session write by `omnesagent_infra::session_sqlite`. Deleting it wipes
/// session history.
...
/// Refuses to write to protected files or directories. Creates parent directories if needed.
