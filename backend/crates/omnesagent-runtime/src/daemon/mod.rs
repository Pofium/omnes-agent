...
async fn wait_for_exit_signal(
    ...
    #[cfg(unix)]
    {
        ...
        loop {
            tokio::select! {
                _ = sigint.recv() => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), "Received SIGINT, shutting down...");
                    return Ok(DaemonExit::Shutdown);
                }
                _ = sigterm.recv() => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), "Received SIGTERM, shutting down...");
                    return Ok(DaemonExit::Shutdown);
                }
                _ = sighup.recv() => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), "Received SIGHUP, ignoring (daemon stays running)");
                }
                changed = reload_rx.changed() => {
                    if changed.is_err() {
                        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown), "Reload sender dropped; shutting down");
                        return Ok(DaemonExit::Shutdown);
                    }
                    if *reload_rx.borrow_and_update() {
                        ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), "Reload requested via /admin/reload");
                        return Ok(DaemonExit::Reload);
                    }
                }
                _ = &mut ephemeral_shutdown => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), "Ephemeral daemon: no clients remaining, shutting down");
                    return Ok(DaemonExit::Shutdown);
                }
            }
        }
        ...
        loop {
            tokio::select! {
                res = tokio::signal::ctrl_c() => {
                    res?;
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), "Received Ctrl+C, shutting down...");
                    return Ok(DaemonExit::Shutdown);
                }
                _ = &mut respawn_shutdown => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note), "In-process shutdown requested, shutting down...");
                    return Ok(DaemonExit::Shutdown);
                }
                changed = reload_rx.changed() => {
                    if changed.is_err() {
...
pub enum GatewayBindMode {
    ...
    StartFresh,
    /// A OmnesAgent gateway already holds the address (e.g. a standalone
    /// `omnesagent gateway start`): fail fast rather than start a second gateway
    /// on the same port.
...
async fn omnesagent_gateway_responds(config: &Config, host: &str, port: u16) -> bool {
...
pub async fn detect_gateway_bind_mode(config: &Config, host: &str, port: u16) -> GatewayBindMode {
    ...
    let Ok(addr) = omnesagent_infra::parse_gateway_bind_socket_addr(host, port) else {
        return GatewayBindMode::StartFresh;
    };
...
async fn classify_gateway_bind_outcome(
    ...
    match bind {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::AddrInUse => {
            if omnesagent_gateway_responds(config, host, port).await {
...
pub async fn run(
    ...
    let rpc_ctx = if need_rpc_ctx {
        ...
        let session_backend = omnesagent_infra::make_session_backend(
            &config.data_dir,
        ...
        let acp_session_store: Option<
            std::sync::Arc<omnesagent_infra::acp_session_store::AcpSessionStore>,
        > = match omnesagent_infra::acp_session_store::AcpSessionStore::new(&config.data_dir) {
...
type HeartbeatMcpRegistryTestHook = std::sync::Arc<
    dyn Fn(
            &str,
            &[omnesagent_config::schema::McpServerConfig],
...
fn missing_or_dead_servers(
    granted: Vec<omnesagent_config::schema::McpServerConfig>,
    current: Option<&std::sync::Arc<crate::tools::McpRegistry>>,
) -> Vec<omnesagent_config::schema::McpServerConfig> {
    ...
    loop {
        ...
        let tasks_to_run = if two_phase {
            ...
            let phase1_fut = Box::pin(crate::agent::run(
                ...
                None,
                None,
                omnesagent_api::ingress::TurnOrigin::Daemon,
        ...
        let heartbeat_memory: Option<Box<dyn omnesagent_memory::Memory>> =
            omnesagent_memory::create_memory_from_config(
                &config,
                config
                        ...
                        && let Some(ref mem) = heartbeat_memory
                    {
                        ...
                        let _ = mem
                            .store(
                                ...
                                omnesagent_memory::MemoryCategory::Daily,
                                None,
                            )
                            .await;
                    }
...
fn load_heartbeat_session_context(config: &Config) -> Option<String> {
    use omnesagent_providers::traits::ChatMessage;
...
fn load_jsonl_messages(path: &std::path::Path) -> Vec<omnesagent_providers::traits::ChatMessage> {
    ...
    let mut window: VecDeque<omnesagent_providers::traits::ChatMessage> =
...
// run_mqtt_sop_listener has been moved to omnesagent-channels::orchestrator::mqtt.
...
mod tests {
    ...
    async fn daemon_startup_diagnostics_are_logged_as_structured_event() {
        ...
        let value = recv_log_event(&mut rx, "OmnesAgent daemon started").await;
    ...
    fn daemon_startup_banners_distinguish_starting_from_ready() {
        ...
        assert!(starting.contains("OmnesAgent"));
        ...
        config.gateway.path_prefix = Some("/omnesagent".to_string());
        ...
        assert!(tls_ready.contains("https://127.0.0.1:42617/omnesagent"));
    ...
    fn all_disabled_channels_not_supervised() {
        ...
        config.channels.discord.insert(
            ...
            omnesagent_config::schema::DiscordConfig {
                ...
                mention_only: true,
                stream_mode: omnesagent_config::schema::StreamMode::default(),
                draft_update_interval_ms: 0,
                multi_message_delay_ms: 0,
                stall_timeout_secs: 0,
                slash_commands: false,
                slash_command_scope: omnesagent_config::schema::SlashCommandScope::default(),
                intents_mask: None,
                reaction_notifications: omnesagent_config::schema::DiscordReactionScope::Off,
    ...
    fn detects_dingtalk_as_supervised_channel() {
        ...
        config.channels.dingtalk.insert(
            "default".to_string(),
            omnesagent_config::schema::DingTalkConfig {
                enabled: true,
    ...
    fn detects_mattermost_as_supervised_channel() {
        ...
        config.channels.mattermost.insert(
            "default".to_string(),
            omnesagent_config::schema::MattermostConfig {
                enabled: true,
    ...
    fn detects_qq_as_supervised_channel() {
        ...
        config.channels.qq.insert(
            "default".to_string(),
            omnesagent_config::schema::QQConfig {
                enabled: true,
                app_id: "app-id".into(),
    ...
    async fn sighup_does_not_shut_down_daemon() {
        ...
        let handle = omnesagent_spawn::spawn!(wait_for_exit_signal(reload_rx, false, count));
    ...
    async fn ephemeral_does_not_exit_before_client_connects() {
        ...
        let (_reload_tx, reload_rx) = tokio::sync::watch::channel(false);
        let count = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let handle = omnesagent_spawn::spawn!(wait_for_exit_signal(reload_rx, true, count));
    ...
    async fn ephemeral_exits_after_client_disconnects() {
        ...
        let handle = omnesagent_spawn::spawn!(wait_for_exit_signal(reload_rx, true, count2));
    ...
    async fn ephemeral_grace_period_resets_on_reconnect() {
        ...
        let count = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let count2 = count.clone();
        let mut handle = omnesagent_spawn::spawn!(wait_for_exit_signal(reload_rx, true, count2));
    ...
    /// Raw HTTP/1.1 `/health` body a real OmnesAgent gateway returns (shape
    /// mirrors `handle_health` in `omnesagent-gateway`): `status: ok` plus the
    /// identity fields `require_pairing` and `runtime`.
    fn omnesagent_health_ok_response() -> Vec<u8> {
        http_response(
            "200 OK",
    ...
    #[tokio::test]
    async fn detect_gateway_bind_mode_flags_existing_omnesagent_gateway() {
        // A real OmnesAgent `/health` (status==ok + identity fields) on an
        // occupied port → fail fast with the "gateway already running" message.
        let (port, _release) = spawn_mock_gateway(omnesagent_health_ok_response()).await;
        assert_eq!(
            detect_gateway_bind_mode(&Config::default(), "127.0.0.1", port).await,
            GatewayBindMode::GatewayAlreadyRunning,
            "a OmnesAgent /health on an occupied port is recognised as a gateway"
        );
    }
    ...
    async fn detect_gateway_bind_mode_flags_generic_status_ok_as_occupied() {
        ...
        // OmnesAgent identity fields) must NOT be taken for a gateway — it is a
        // plain occupied port.
        let (port, _release) =
        ...
        assert_eq!(
            detect_gateway_bind_mode(&Config::default(), "127.0.0.1", port).await,
            GatewayBindMode::PortOccupied,
            "a generic status:ok health response is not a OmnesAgent gateway"
        );
    }
    ...
    //    `kill_on_drop(true)` (see `omnesagent-tools::mcp_transport`),
    ...
    fn test_server_config(name: &str) -> omnesagent_config::schema::McpServerConfig {
        omnesagent_config::schema::McpServerConfig {
            name: name.to_string(),
            transport: omnesagent_config::schema::McpTransport::Stdio,
            command: "true".to_string(),
            ..omnesagent_config::schema::McpServerConfig::default()
        }
    }
    ...
    async fn retry_heartbeat_mcp_registry_never_resubmits_healthy_peer_across_ticks() {
        use omnesagent_config::schema::{AliasedAgentConfig, McpBundleConfig};
    ...
    /// `omnesagent-runtime` stays green.
    #[cfg(unix)]
    ...
    async fn heartbeat_worker_preserves_healthy_a_admits_recovered_b_additively() {
        ...
        config
            .mcp
            .servers
            .push(omnesagent_config::schema::McpServerConfig {
