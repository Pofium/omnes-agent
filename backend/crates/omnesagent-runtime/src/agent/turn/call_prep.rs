...
use omnesagent_api::attribution::ToolProvenance;
use omnesagent_tool_call_parser::{ParsedToolCall, canonicalize_json_for_tool_signature};
...
async fn record_duplicate_tool_call(
    ...
    ::omnesagent_log::record!(
        INFO,
        ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Skip)
...
pub(crate) async fn prepare_tool_calls(
    ...
    for (idx, call) in tool_calls.iter().enumerate() {
        ...
        if let Some(hooks) = ctx.hooks {
                ...
                .await
            {
                crate::hooks::HookResult::Cancel(reason) => {
                    ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Cancel).with_category(::omnesagent_log::EventCategory::Tool).with_attrs(::serde_json::json!({"tool": call.name, "reason": reason.to_string()})), "tool call cancelled by hook");
        ...
        maybe_inject_channel_delivery_defaults(
            ...
            ctx.channel_name,
            ctx.channel
                .map(omnesagent_api::attribution::Attributable::alias),
...
mod tests {
    ...
    use std::time::Duration;
    use tokio::sync::mpsc;
    use omnesagent_api::attribution::{Attributable, ToolProvenance};
