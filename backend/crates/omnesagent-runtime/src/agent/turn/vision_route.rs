...
use anyhow::Result;
use omnesagent_config::schema::{Config, MultimodalConfig};
use omnesagent_providers::{ChatMessage, ModelProvider, ProviderCapabilityError, multimodal};
        ...
        && !model_provider.capabilities_for_model(model).vision
    {
        if let Some(ref vp) = multimodal_config.vision_model_provider {
            ...
            let (vp_instance, alias_model) = match config {
                Some(config) => {
                    omnesagent_providers::create_model_provider_from_ref_with_model(config, vp)
                        .map(|resolved| (resolved.provider, resolved.model))
                }
                None => omnesagent_providers::create_model_provider(vp, None)
...
mod tests {
    ...
    fn resolve_vision_provider_honors_configured_alias_vision_override() {
        ...
        impl omnesagent_api::attribution::Attributable for NonVisionPrimary {
...
"#,
    ...
    fn resolve_vision_provider_names_fallback_in_capability_error() {
        ...
        impl omnesagent_api::attribution::Attributable for NonVisionWithNamedFallback {
            fn role(&self) -> omnesagent_api::attribution::Role {
                omnesagent_api::attribution::Role::Provider(
                    omnesagent_api::attribution::ProviderKind::Model(
                        omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    fn resolve_vision_provider_keeps_primary_wording_without_a_named_fallback() {
        ...
        impl omnesagent_api::attribution::Attributable for PlainNonVisionPrimary {
            fn role(&self) -> omnesagent_api::attribution::Role {
                omnesagent_api::attribution::Role::Provider(
                    omnesagent_api::attribution::ProviderKind::Model(
                        omnesagent_api::attribution::ModelProviderKind::Custom,
...
"#
        ...
        omnesagent_providers::ProviderDispatch::from_ref(vision_provider.provider.as_ref())
            .chat(
                omnesagent_providers::ChatRequest {
