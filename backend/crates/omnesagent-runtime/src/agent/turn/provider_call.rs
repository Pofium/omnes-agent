...
use omnesagent_config::schema::StreamReasoningMode;
use omnesagent_providers::dispatch::{AcceptedRoute, AccountedAttempt, with_exact_dispatch_route};
use omnesagent_providers::{ChatMessage, ChatRequest, ChatResponse, ModelProvider, ProviderDispatch};
...
pub(crate) async fn announce_llm_request(
    ...
    {
        let _provider_guard = ::omnesagent_log::attribution_span!(active_model_provider).entered();
        ...
        if let Some((policy, truncate_bytes)) = ::omnesagent_log::llm_request_payload_policy()
            ...
            && let ::serde_json::Value::Object(map) = &mut attrs
        {
            ...
            if let Some(capture) =
                ::omnesagent_log::capture_llm_request(policy, truncate_bytes, &scrubbed)
            {
                map.insert(
...
pub(crate) async fn call_provider(
    ...
    let (chat_result, accounting) = if should_consume_provider_stream {
        ...
        let scope = omnesagent_providers::dispatch::AccountedChatScope::new();
        let (result, live_deltas, protocol_suppressed, visible_text) = scope
            .scope(Box::pin(omnesagent_providers::reliable::scope_provider_fallback(Box::pin(async {
                        ...
                        Err(stream_err) => {
                            ...
                            let recovery = with_exact_dispatch_route(
                                ...
                                ChatRequest {
                                    ...
                                    tools: request_tools,
                                    thinking: omnesagent_api::NATIVE_THINKING_OVERRIDE
...
mod payload_capture_tests {
    ...
    use omnesagent_api::attribution::{Attributable, ModelProviderKind, ProviderKind, Role};
    use omnesagent_config::schema::{PacingConfig, StreamReasoningMode};
    use omnesagent_log::LogConfig;
    ...
    fn install_writer(payload_mode: &str) {
        ...
        omnesagent_log::init_from_config(&cfg, std::path::Path::new("/"));
    }
...
mod streaming_fallback_tests {
    ...
    use omnesagent_api::attribution::{Attributable, ModelProviderKind, ProviderKind, Role};
    use omnesagent_api::model_provider::StreamEvent;
    use omnesagent_config::schema::PacingConfig;
    ...
    use omnesagent_providers::traits::{StreamOptions, StreamResult, TokenUsage};
            ...
            Box::pin(
                futures_util::stream::iter(vec![
                    ...
                    Ok(StreamEvent::TextDelta(
                        omnesagent_api::model_provider::StreamChunk::reasoning("private reasoning"),
                    )),
    ...
    impl ModelProvider for VisibleThenServerStreamFailureProvider {
        ...
        fn stream_chat(
            ...
            Box::pin(futures_util::stream::iter(vec![
                Ok(StreamEvent::TextDelta(
                    omnesagent_api::model_provider::StreamChunk::delta("visible"),
                )),
    ...
    async fn check_empty_stream_recovery(cancel_on_final: bool) {
        ...
        assert!(matches!(
            ...
            omnesagent_api::agent::TurnEvent::Thinking { delta }
        ...
        assert!(matches!(
            ...
            omnesagent_providers::dispatch::AttemptUsageOutcome::OutcomeUnknown {
                observed: Some(usage),
        ...
        assert!(matches!(
            ...
            omnesagent_providers::dispatch::AttemptUsageOutcome::Complete(usage)
    ...
    async fn non_stream_cancellation_keeps_prior_rejected_reliable_attempt() {
        ...
        let _cancel = omnesagent_spawn::spawn!(async move {
