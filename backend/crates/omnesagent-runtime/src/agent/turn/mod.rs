...
use omnesagent_api::ingress::{IngressContext, IngressDecision};
use omnesagent_providers::{ChatMessage, ModelProvider};
...
pub struct ToolLoop<'a> {
    ...
    pub image_cache: Option<&'a mut omnesagent_providers::multimodal::LocalImageCache>,
    ...
    if result.trimmed {
        ...
        *history = trimmed;
        ...
        observer.record_event(
            &omnesagent_api::observability_traits::ObserverEvent::HistoryTrimmed {
                dropped_messages: result.dropped_messages,
                kept_turns: result.kept_turns,
                reason: crate::i18n::get_required_cli_string("history-trim-reason-budget"),
                channel: None,
...
impl<'a> TurnState<'a> {
    ...
    fn append_tool_round(
        ...
        native_tool_calls: &[omnesagent_providers::ToolCall],
...
pub async fn run_tool_call_loop(mut p: ToolLoop<'_>) -> Result<String> {
    ...
    let mut image_cache = Some(match image_cache {
        Some(cache) => cache,
        None => {
            loop_local_image_cache.insert(omnesagent_providers::multimodal::LocalImageCache::new())
        }
    });
    ...
    if let Some(turn_memory) = &memory {
        ...
        if let crate::agent::memory_inject::InjectPolicy::Inject {
            ...
            && !turn_state.history[last_user_idx]
                .content
                .starts_with(omnesagent_memory::MEMORY_CONTEXT_OPEN)
        {
    ...
    for iteration in 0..max_iterations {
        ...
        if iteration == 0 && context_token_budget > 0 {
            ...
            if system_floor >= context_token_budget {
                let __zc_floor_span = ::omnesagent_log::info_span!(
            ...
            if result.trimmed {
                {
                    ...
                    ::omnesagent_log::record!(
                        INFO,
                        ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Delete)
                            .with_category(::omnesagent_log::EventCategory::Agent)
                            .with_attrs(::serde_json::json!({
                                "dropped_messages": result.dropped_messages,
            ...
            && (new_model_provider != provider_name || new_model != model)
        {
            ::omnesagent_log::record!(
                INFO,
                ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Migrate)
        ...
        {
            return Err(omnesagent_providers::ProviderCapabilityError {
        ...
        if ::omnesagent_log::debug_enabled() {
            ::omnesagent_log::record!(
                DEBUG,
                ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note)
                    .with_category(::omnesagent_log::EventCategory::Agent)
            ...
            if malformed_tool_protocol_retries <= MAX_MALFORMED_TOOL_PROTOCOL_RETRIES {
                ...
                let msg = ChatMessage::user(
                    "[Tool call parse error]\n\
                     ...
                     but OmnesAgent could not parse it into a valid tool call. Use the supported \
        ...
        omnesagent_providers::dispatch::commit_accepted_provider_route(accepted_route);
        ...
        let activated = match activated.lock() {
            ...
            Err(poisoned) => {
                ::omnesagent_log::record!(
                    WARN,
                    ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note)
                        .with_category(::omnesagent_log::EventCategory::Tool)
                        .with_outcome(::omnesagent_log::EventOutcome::Unknown),
...
pub struct SopStepReassembly<'a> {
    pub config: &'a omnesagent_config::schema::Config,
}
...
pub(crate) async fn assemble_owned_execution(
    ...
    let memory =
        omnesagent_memory::create_memory_for_agent(config, alias, resolved_key.as_deref()).await?;
...
async fn drive_live_sop_actions(
    ...
    pacing: &omnesagent_config::schema::PacingConfig,
    ...
    mut image_cache: Option<&mut omnesagent_providers::multimodal::LocalImageCache>,
...
mod sop_step_reassembly_tests {
    use super::*;
    use std::sync::atomic::{AtomicUsize, Ordering};
    use omnesagent_providers::{ChatResponse, ToolCall};
    ...
    async fn reassembly_yields_the_named_agents_own_scope() {
        use omnesagent_config::multi_agent::{AgentMemoryConfig, MemoryBackendKind};
        ...
        let root =
            std::env::temp_dir().join(format!("omnesagent-sop-depth2-{}", uuid::Uuid::new_v4()));
    ...
    async fn reassembly_preserves_parent_approval_backchannel() {
        ...
        let root = std::env::temp_dir().join(format!("omnesagent-sop-appr-{}", uuid::Uuid::new_v4()));
        let mut config = Config {
            data_dir: root.join("data"),
            config_path: root.join("config.toml"),
            ..Config::default()
        ...
        let plain_child = plain_parent
            .derive_for_risk_profile(&omnesagent_config::schema::RiskProfileConfig::default());
        assert_eq!(
    ...
    impl ::omnesagent_api::attribution::Attributable for ShellCallingProvider {
    ...
    impl ModelProvider for ShellCallingProvider {
        ...
        async fn chat(
            &self,
            _request: omnesagent_api::model_provider::ChatRequest<'_>,
            _model: &str,
            _temperature: Option<f64>,
        ) -> Result<ChatResponse> {
            Ok(ChatResponse {
    ...
    ::omnesagent_api::tool_attribution!(ShellProbe, ::omnesagent_api::attribution::ToolKind::Plugin);
    ...
    impl ::omnesagent_api::attribution::Attributable for TextProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for CaptureProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ModelProvider for CaptureProvider {
        ...
        async fn chat(
            &self,
            request: omnesagent_api::model_provider::ChatRequest<'_>,
            _model: &str,
            temperature: Option<f64>,
        ) -> Result<ChatResponse> {
    ...
    ::omnesagent_api::tool_attribution!(NamedTool, ::omnesagent_api::attribution::ToolKind::Plugin);
    ...
    fn seeded_owned(
        ...
        mcp_tool_names: std::collections::HashSet<String>,
        tool_filter_groups: Vec<omnesagent_config::schema::ToolFilterGroup>,
        temperature: Option<f64>,
    ) -> OwnedAgentExecution {
        let mut agent = omnesagent_config::schema::AliasedAgentConfig::default();
        ...
        OwnedAgentExecution {
            ...
            agent,
            risk_profile: omnesagent_config::schema::RiskProfileConfig::default(),
            skills: Vec::new(),
            mcp_tool_names,
    ...
    fn start_single_cross_agent_step(
        ...
        use omnesagent_config::schema::SopConfig;
    ...
    impl ::omnesagent_api::attribution::Attributable for ChildSwitchProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    ::omnesagent_api::tool_attribution!(
    ...
    async fn cross_agent_step_model_switch_never_leaks_into_parent_loop() {
        ...
        let mut exec_cache = std::collections::HashMap::new();
        let mut stepper_agent = omnesagent_config::schema::AliasedAgentConfig::default();
        ...
        let stepper_risk_profile = omnesagent_config::schema::RiskProfileConfig {
            level: omnesagent_config::autonomy::AutonomyLevel::Full,
            ..omnesagent_config::schema::RiskProfileConfig::default()
        };
        exec_cache.insert(
            "stepper".to_string(),
