...
use omnesagent_tool_call_parser::{strip_think_tags, strip_trailing_terminal_markers};
...
pub(crate) async fn finish_after_max_iterations(
    ...
    enum SummaryCall {
        Cancelled,
        TimedOut(u64),
        Done(Result<omnesagent_providers::ChatResponse>),
    }
    let summary_call = {
        let summary_request = omnesagent_providers::ChatRequest {
            messages: history,
...
mod graceful_summary_metering_tests {
    ...
    use omnesagent_config::schema::{CostConfig, PacingConfig};
