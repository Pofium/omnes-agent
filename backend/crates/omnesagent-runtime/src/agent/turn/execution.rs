...
use omnesagent_api::model_provider::{ChatRequest, ChatResponse, SemanticEmptyTerminalCompletion};
use omnesagent_config::schema::{MultimodalConfig, PacingConfig};
use omnesagent_providers::dispatch::with_exact_dispatch_route;
use omnesagent_providers::{ModelProvider, ProviderDispatch, multimodal};
...
pub struct ResolvedAgentExecution<'a> {
    ...
    pub config: Option<&'a omnesagent_config::schema::Config>,
...
mod run_model_query_tests {
    ...
    use omnesagent_providers::{ChatMessage, ModelProvider, ReliableRejectedCompletionUsage};
