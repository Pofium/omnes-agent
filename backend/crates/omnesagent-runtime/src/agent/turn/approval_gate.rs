        ...
        && approval_requirement == ApprovalRequirement::Prompt
    {
        ...
        let (decision, decided_by, unanswerable) = if mgr.is_non_interactive() {
            let attributed = if let Some(ch) = ctx.channel {
                let ch_request = omnesagent_api::channel::ChannelApprovalRequest {
            ...
            let decision = match attributed.map(|a| a.response) {
                Some(omnesagent_api::channel::ChannelApprovalResponse::Approve) => {
                    ApprovalResponse::Yes
                }
                Some(omnesagent_api::channel::ChannelApprovalResponse::AlwaysApprove) => {
                    ApprovalResponse::Always
                }
                Some(omnesagent_api::channel::ChannelApprovalResponse::Deny) => ApprovalResponse::No,
                Some(omnesagent_api::channel::ChannelApprovalResponse::DenyWithEdit {
                    replacement,
        ...
        if let ApprovalResponse::ReplaceWith(replacement) = &decision {
            ...
            ::omnesagent_log::record!(
                INFO,
                ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Approve)
                    .with_category(::omnesagent_log::EventCategory::Tool)
