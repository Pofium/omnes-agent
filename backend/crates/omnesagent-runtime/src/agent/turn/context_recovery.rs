...
pub(crate) async fn try_recover_context_overflow(
    ...
    e: &anyhow::Error,
    iteration: usize,
    event_tx: Option<&tokio::sync::mpsc::Sender<omnesagent_api::agent::TurnEvent>>,
        ...
        *history = recovered_history;
        if trimmed {
            ...
            if let Some(tx) = event_tx {
                let _ = tx
                    .send(omnesagent_api::agent::TurnEvent::HistoryTrimmed {
                        dropped_messages,
                        kept_turns,
                        reason: reason.clone(),
                    })
...
mod tests {
    ...
    async fn recovery_emits_history_trimmed_event_on_trim() {
        ...
        match event {
            omnesagent_api::agent::TurnEvent::HistoryTrimmed {
                dropped_messages,
                kept_turns,
                reason,
            } => {
