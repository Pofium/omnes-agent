...
mod tests {
    ...
    fn injected_channel_matches_escalate_tool_schema_parameter() {
        ...
        // lives in omnesagent-tools::escalate tests.
        let tool = omnesagent_tools::escalate::EscalateToHumanTool::new(
            std::sync::Arc::new(omnesagent_config::policy::SecurityPolicy::default()),
            vec![],
            std::sync::Arc::new(parking_lot::RwLock::new(std::collections::HashMap::new())),
        );
        let schema = omnesagent_api::tool::Tool::parameters_schema(&tool);
        assert!(
