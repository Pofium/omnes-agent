...
impl MemoryInjectConfig {
    ...
    pub fn from_memory_config(
        memory: &omnesagent_config::schema::MemoryConfig,
        limit: usize,
    ) -> Self {
...
fn should_skip_entry(key: &str, content: &str) -> bool {
    if omnesagent_memory::is_assistant_autosave_key(key) {
        return true;
    }
    ...
    if omnesagent_memory::is_user_autosave_key(key) {
        return true;
    }
    if omnesagent_memory::should_skip_autosave_content(content) {
        return true;
    }
...
/// Builds an optional `<project_context>` block if the agent operates within a registered project root.
pub async fn render_project_context(
    conn: &std::sync::Arc<parking_lot::Mutex<rusqlite::Connection>>,
    workspace_dir: &std::path::Path,
    query: Option<&str>,
) -> Option<String> {
    let project_service = omnesagent_kag::project::ProjectService::new(conn.clone());
    let ws_str = workspace_dir.to_string_lossy().replace('\\', "/");
    let project_id: Option<String> = {
        let db = conn.lock();
        let mut stmt = db.prepare(
            "SELECT id FROM projects WHERE root_path = ?1 OR ?1 LIKE (root_path || '%') ORDER BY length(root_path) DESC LIMIT 1"
        ).ok()?;
        stmt.query_row(rusqlite::params![ws_str], |r| r.get(0)).ok()
    };

    if let Some(pid) = project_id {
        project_service.build_context(&pid, query).ok()
    } else {
        None
    }
}
...
mod tests {
    ...
    impl ::omnesagent_api::attribution::Attributable for FixtureMemory {
    ...
    async fn recall_routes_through_retrieval_pipeline_decorator() {
        ...
        let pipeline = omnesagent_memory::RetrievalPipeline::new(
            fixture.clone() as std::sync::Arc<dyn Memory>,
            omnesagent_memory::RetrievalConfig::default(),
        );
        let observer = RecordingObserver::default();
    ...
    async fn rerank_bounds_unlimited_recall_sentinel_before_backend_and_scan() {
        ...
        let mem = FixtureMemory::with(pool);
        let mut memory = omnesagent_config::schema::MemoryConfig {
            rerank_enabled: true,
    ...
    fn from_memory_config_threads_rerank_settings() {
        let mut memory = omnesagent_config::schema::MemoryConfig::default();
    ...
    async fn noop_sqlite_bm25_recall_stays_ranked_through_rerank_injection() {
        ...
        let memory = omnesagent_memory::SqliteMemory::new("rerank-bm25", tmp.path()).unwrap();
        assert_eq!(
        ...
        let config = omnesagent_config::schema::MemoryConfig {
            rerank_enabled: true,
