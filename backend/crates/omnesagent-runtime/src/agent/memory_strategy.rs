use std::sync::Arc;
use omnesagent_api::memory_traits::{Memory, MemoryStrategy};
use omnesagent_api::model_provider::ModelProvider;

pub struct DefaultMemoryStrategy {
    memory: Arc<dyn Memory>,
    memory_config: omnesagent_config::schema::MemoryConfig,
...
impl MemoryStrategy for DefaultMemoryStrategy {
    async fn consolidate_turn(
        ...
        omnesagent_memory::consolidation::consolidate_turn(
            provider,
            model,
            temperature,
            self.memory.as_ref(),
    ...
    async fn run_governance(&self) -> anyhow::Result<()> {
        ...
        omnesagent_memory::hygiene::run_if_due(&self.memory_config, &self.workspace_dir)
    }
}
