...
// Re-exported from omnesagent-config.
pub use omnesagent_config::scattered_types::{ThinkingConfig, ThinkingDisplayMode, ThinkingLevel};
...
pub struct ThinkingParams {
    ...
    pub native_thinking: Option<omnesagent_config::scattered_types::NativeThinkingParams>,
}
...
pub fn apply_thinking_level_with_config(
    ...
    use omnesagent_config::scattered_types::{MAX_BUDGET_TOKENS, MIN_BUDGET_TOKENS};
        ...
        && let Some(budget) = config.budget_tokens_for(level)
    {
        ...
        params.native_thinking = Some(omnesagent_config::scattered_types::NativeThinkingParams {
...
mod tests {
    ...
    fn budget_tokens_clamped_to_min_when_below() {
        ...
        use omnesagent_config::scattered_types::MIN_BUDGET_TOKENS;
    ...
    fn budget_tokens_clamped_to_max_when_above() {
        use std::collections::HashMap;
        use omnesagent_config::scattered_types::MAX_BUDGET_TOKENS;
        let mut overrides = HashMap::new();
...
"#;
        ...
        assert_eq!(
            ...
            Some(omnesagent_api::model_provider::ThinkingDisplay::Updates)
        );
    ...
    async fn native_thinking_override_round_trips_through_scope() {
        use omnesagent_config::scattered_types::NativeThinkingParams;
        let installed = Some(NativeThinkingParams {
            budget_tokens: 32_000,
            display: None,
        });
        let read_back = omnesagent_api::NATIVE_THINKING_OVERRIDE
