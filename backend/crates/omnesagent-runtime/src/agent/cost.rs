...
use omnesagent_providers::pricing::ModelRates;
...
fn resolve_rates_opt(pricing: &HashMap<String, f64>, model: &str) -> ModelRates {
    ...
    omnesagent_providers::pricing::model_id_candidates(model)
        .find_map(try_lookup)
        .unwrap_or_default()
}

fn live_pricing_for(model_provider_name: &str, model: &str) -> Option<ModelRates> {
    let snapshot = omnesagent_providers::pricing::current_snapshot();
    omnesagent_providers::pricing::lookup(&snapshot, model_provider_name, model).copied()
}
...
pub fn record_rejected_tool_loop_cost_usage(
    model_provider_name: &str,
    model: &str,
    usage: &omnesagent_providers::traits::TokenUsage,
) -> Option<(u64, f64)> {
...
pub(crate) struct BillableProviderAttempt<'a> {
    pub index: usize,
    pub attempt: &'a omnesagent_providers::dispatch::AccountedAttempt,
    pub usage: &'a omnesagent_providers::traits::TokenUsage,
}
...
pub(crate) fn billable_provider_attempts(
    attempts: &[omnesagent_providers::dispatch::AccountedAttempt],
) -> impl Iterator<Item = BillableProviderAttempt<'_>> {
    use omnesagent_providers::dispatch::AttemptUsageOutcome;
...
fn record_tool_loop_cost_usage_inner(
    ...
    {
        ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_category(::omnesagent_log::EventCategory::Provider).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model_provider": model_provider_name, "model": model, "error": format!("{}", error)})), "Failed to record cost tracking usage: ");
    }
...
mod tests {
    ...
    use omnesagent_api::model_provider::{ChatMessage, ChatRequest, ChatResponse, ModelProvider};
    use omnesagent_config::schema::{Config, DeepseekModelProviderConfig, ModelProviderConfig};
    use omnesagent_providers::ProviderDispatch;
    use omnesagent_providers::dispatch::{AccountedChatScope, with_exact_dispatch_route};
    ...
    async fn settlement_uses_exact_attempt_routes_models_and_record_order() {
        ...
        let records = std::fs::read_to_string(workspace.path().join("state").join("costs.jsonl"))
            ...
            .lines()
            .map(|line| {
                serde_json::from_str::<omnesagent_config::cost::types::CostRecord>(line).unwrap()
            })
            .collect::<Vec<_>>();
    ...
    fn record_tool_loop_cost_usage_applies_cached_input_pricing() {
        ...
        let record: omnesagent_config::cost::types::CostRecord =
    ...
    fn rejected_usage_accumulates_cost_without_replacing_context_window_fill() {
        let turn_usage = Arc::new(Mutex::new(TurnUsage::default()));
        let rejected = omnesagent_providers::traits::TokenUsage {
            input_tokens: Some(80),
            output_tokens: Some(5),
            cached_input_tokens: Some(0),
        };
        let accepted = omnesagent_providers::traits::TokenUsage {
            input_tokens: Some(80),
