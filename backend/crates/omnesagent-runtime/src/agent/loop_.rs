...
pub static CLI_CHANNEL_FN: std::sync::OnceLock<
    Box<dyn Fn() -> Box<dyn omnesagent_api::channel::Channel> + Send + Sync>,
> = std::sync::OnceLock::new();

/// Register the CLI channel factory. Called once at startup by the binary.
pub fn register_cli_channel_fn(
    f: Box<dyn Fn() -> Box<dyn omnesagent_api::channel::Channel> + Send + Sync>,
) {
...
pub type PeripheralToolsFn = Box<
    dyn Fn(
            omnesagent_config::schema::PeripheralsConfig,
        ) -> std::pin::Pin<
...
pub async fn load_peripheral_tools(
    config: omnesagent_config::schema::PeripheralsConfig,
) -> Vec<Box<dyn Tool>> {
...
/// Injected by the binary so `omnesagent-runtime` doesn't depend on
/// `omnesagent-channels`.
type ChannelMapFn = Box<
    dyn Fn()
            -> std::collections::HashMap<String, std::sync::Arc<dyn omnesagent_api::channel::Channel>>
        + Send
        + Sync,
>;
...
use uuid::Uuid;
use omnesagent_api::channel::Channel;
use omnesagent_api::ingress::{IngressContext, TurnOrigin};
use omnesagent_config::schema::Config;
use omnesagent_memory::{self, Memory, MemoryCategory};
#[cfg(test)]
use omnesagent_providers::ChatRequest;
use omnesagent_providers::{self, ChatMessage, ModelProvider, ToolCall};
...
fn should_autosave_user_message(
    auto_save: bool,
    origin: omnesagent_api::ingress::TurnOrigin,
    content: &str,
) -> bool {
    auto_save
        && omnesagent_memory::should_autosave_origin(origin)
        && content.chars().count() >= AUTOSAVE_MIN_MESSAGE_CHARS
        && !omnesagent_memory::should_skip_autosave_content(content)
}
...
pub fn apply_policy_tool_filter(
    ...
    policy: Option<&omnesagent_config::policy::SecurityPolicy>,
...
pub fn mcp_tool_access_policy(
    security: &omnesagent_config::policy::SecurityPolicy,
    caller_allowed: Option<&[String]>,
) -> Option<omnesagent_tools::tool_search::ToolAccessPolicy> {
    omnesagent_tools::tool_search::ToolAccessPolicy::from_security(
...
pub fn eager_mcp_tool_allowed(
    name: &str,
    policy: Option<&omnesagent_tools::tool_search::ToolAccessPolicy>,
) -> bool {
...
pub(crate) fn preactivate_always_filter_groups(
    ...
    groups: &[omnesagent_config::schema::ToolFilterGroup],
    policy: Option<&omnesagent_tools::tool_search::ToolAccessPolicy>,
    delegate_handle: Option<&tools::DelegateParentToolsHandle>,
) -> HashSet<String> {
    use omnesagent_config::schema::ToolFilterGroupMode;
...
// Re-export from omnesagent-types for backwards compatibility.
pub use omnesagent_api::TOOL_LOOP_SESSION_KEY;
pub use omnesagent_api::TOOL_LOOP_THREAD_ID;

// Re-export tool call parsing from the standalone parser crate.
pub use omnesagent_tool_call_parser::{
...
pub(crate) fn scrub_for_export(content: &str) -> String {
    scrub_credentials(&omnesagent_providers::scrub_secret_patterns(
...
) -> Option<omnesagent_api::observability_traits::LlmMessageSnapshot> {
    if !cfg!(feature = "observability-otel") {
        return None;
    }

    use omnesagent_api::observability_traits::{
...
pub(crate) fn build_system_prompt_for_turn(
    ...
    risk_profile: &omnesagent_config::schema::RiskProfileConfig,
    model_provider: &dyn ModelProvider,
    tools_registry: &[Box<dyn Tool>],
    excluded_tools: &[String],
    activated_tools: Option<&Arc<Mutex<crate::tools::ActivatedToolSet>>>,
...
pub use omnesagent_api::TOOL_CHOICE_OVERRIDE;
...
pub async fn agent_turn(
    config: Option<&omnesagent_config::schema::Config>,
    model_provider: &dyn ModelProvider,
    ...
    let result = run_tool_call_loop(ToolLoop {
        ...
        exec: ResolvedAgentExecution::resolve(
            ...
            ResolvedRuntimeKnobs {
                ...
                excluded_tools,
                dedup_exempt_tools,
                pacing: &omnesagent_config::schema::PacingConfig::default(),
                strict_tool_parsing,
                parallel_tools,
                max_tool_result_chars,
                context_token_budget,
    ...
    let tokens_used = TOOL_LOOP_COST_TRACKING_CONTEXT
        ...
        .and_then(|ctx| {
            ...
            (usage.input_tokens > 0 || usage.output_tokens > 0).then_some(
                omnesagent_api::observability_traits::TurnTokenUsage {
...
fn resolved_agent_for_turn(
    config: &omnesagent_config::schema::Config,
    agent_alias: &str,
) -> Result<omnesagent_config::schema::AliasedAgentConfig> {
    let agent = config
...
fn api_key_and_uri_for_provider(
    config: &omnesagent_config::schema::Config,
    provider_name: &str,
    fallback: Option<&omnesagent_config::schema::ModelProviderConfig>,
...
pub async fn run(
    ...
    let memory_composite = {
        use omnesagent_config::multi_agent::MemoryBackendKind;
    ...
    let __zc_attribution_span =
        ::omnesagent_log::attribution_span!(&crate::agent::AgentAttribution(__zc_alias.as_str()));
    let __zc_scope_span = ::omnesagent_log::info_span!(
    ...
    let __zc_body = async move {
        ...
        if interactive
            && matches!(
                ...
                omnesagent_config::schema::ObservabilityBackend::Prometheus
            )
        {
            ::omnesagent_log::record!(
                WARN,
        ...
        let mem: Arc<dyn Memory> = if memory_free {
            ...
            match overrides.memory {
                Some(m) => m,
                None => {
                    omnesagent_memory::create_memory_for_agent(
                        &config,
                        agent_alias,
        ...
        ::omnesagent_log::record!(
            INFO,
            ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Load)
                .with_category(::omnesagent_log::EventCategory::Memory)
        ...
        let (sop_engine, sop_audit) = if config.sop.runtime_enabled() {
            let sop_mem: Arc<dyn omnesagent_memory::Memory> =
        ...
        if count > 0 {
            ::omnesagent_log::record!(
                INFO,
                ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Register)
        ...
        {
            let span = omnesagent_log::Span::current();
        ...
        let agent_runtime_options = match agent_provider_resolved.as_ref() {
            Some((ty, alias, _)) => {
                omnesagent_providers::provider_runtime_options_for_alias(&config, ty, alias)
            }
            None => omnesagent_providers::provider_runtime_options_for_agent(&config, agent_alias),
        };
        ...
        let provider_runtime_options = omnesagent_providers::options_for_provider_ref(
            &config,
            &provider_name,
        ...
        if matches!(
            ...
            omnesagent_config::schema::SkillsPromptInjectionMode::Compact
        ) {
            tool_descs.push((
                "read_skill",
        ...
        if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
            ...
            tool_descs.push((
            "arduino_upload",
            "Upload agent-generated Arduino sketch. Use when: user asks for 'make a heart', 'blink pattern', or custom LED behavior on Arduino. You write the full .ino code; OmnesAgent compiles and uploads it. Pin 13 = built-in LED on Uno.",
        ));
            tool_descs.push((
            "hardware_memory_map",
        ...
        let memory_session_id = session_state_file.as_deref().and_then(|path| {
            ...
            if raw.is_empty() {
                ...
                Some(omnesagent_api::session_keys::sanitize_session_key(&format!(
                    "cli:{raw}"
                )))
            }
        });
        ...
        if let Some(msg) = message {
            ...
            loop {
                ...
                match omnesagent_api::NATIVE_THINKING_OVERRIDE
                    .scope(
                    ...
                    Err(e) => {
                        if let Some((new_model_provider, new_model)) = is_model_switch_requested(&e)
                        {
                            ::omnesagent_log::record!(
                                ...
                                ::omnesagent_log::Event::new(
                                    module_path!(),
                                    ::omnesagent_log::Action::Migrate
                                )
                                .with_category(::omnesagent_log::EventCategory::Provider),
                                &format!(
                            ...
                            model_provider =
                                omnesagent_providers::create_routed_model_provider_with_options(
                                    ...
                                    &config.model_routes,
                                    &new_model,
                                    &omnesagent_providers::options_for_provider_ref(
                                        &config,
                                        &new_model_provider,
                                        &omnesagent_providers::provider_runtime_options_for_agent(
                                            &config,
                                            agent_alias,
                                        ),
                                    ),
                    ...
                    match creation_result {
                        Ok(Some(slug)) => {
                            ::omnesagent_log::record!(
                                ...
                                ::omnesagent_log::Event::new(
                                    module_path!(),
                                    ::omnesagent_log::Action::Register
                                )
                                .with_category(::omnesagent_log::EventCategory::Agent)
            ...
            loop {
                ...
                if let Some(suggestion) = crate::skills::render_missing_skill_install_suggestion(
                    ...
                    final_output = suggestion;
                    if let Err(e) = omnesagent_api::channel::Channel::send(
                        &*cli,
                        &omnesagent_api::channel::SendMessage::new(
                            format!("\n{final_output}\n"),
                            "user",
                        ),
                    )
                ...
                let consumer_handle = omnesagent_spawn::spawn!(async move {
                    use std::io::Write;
                ...
                let ctrlc_handle = omnesagent_spawn::spawn!(async move {
                ...
                let response = loop {
                        ...
                        .await
                    {
                        ...
                        Err(e) => {
                            ...
                            if omnesagent_providers::reliable::is_context_window_exceeded(&e) {
                                ::omnesagent_log::record!(
                                    WARN,
                                    ::omnesagent_log::Event::new(
                                        module_path!(),
                                        ::omnesagent_log::Action::Retry
                                    )
                                    .with_category(::omnesagent_log::EventCategory::Agent),
                                ...
                                if result.trimmed {
                                    ...
                                    history = trimmed;
                                    {
                                        let __zc_trim_span = ::omnesagent_log::info_span!(
                                            target: "omnesagent_log_internal_scope",
                                            "omnesagent_scope",
                                            model = %model_name,
                ...
                if content_was_streamed.load(std::sync::atomic::Ordering::Relaxed) {
                    println!();
                } else if let Err(e) = omnesagent_api::channel::Channel::send(
                    &*cli,
                    &omnesagent_api::channel::SendMessage::new(format!("\n{final_output}\n"), "user"),
                )
                .await
                {
                    eprintln!("\nError sending CLI response: {e}\n");
...
pub async fn process_message(
    ...
    let __zc_body = async move {
        ...
        let mem: Arc<dyn Memory> = omnesagent_memory::create_memory_for_agent(
            &config,
            agent_alias,
            agent_model_provider
                .as_ref()
        ...
        if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
            ...
            tool_descs.push((
            "arduino_upload",
            "Upload Arduino sketch. Use for 'make a heart', custom patterns. You write full .ino code; OmnesAgent uploads it.",
        ));
            tool_descs.push((
            "hardware_memory_map",
        ...
        // omnesagent.turn_id attribute; nesting it is a tracked follow-up.
        let turn_id = uuid::Uuid::new_v4().to_string();
        let rag_limit = if eff_compact_context { 2 } else { 5 };
        let hw_context = hardware_rag
            .as_ref()
        ...
        let routed_approval_channel_ref = routed_approval_channel
            .as_ref()
            .map(|c| c as &dyn omnesagent_api::channel::Channel);
...
mod tests {
    ...
    fn autosave_decision_keys_on_origin_before_content() {
        use omnesagent_api::ingress::TurnOrigin;
    ...
    use omnesagent_providers::{ChatMessage, ToolCall};
    use omnesagent_tool_call_parser::parse_tool_calls;
    ...
    impl ::omnesagent_api::attribution::Attributable for SeedMockChannel {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Channel(
                ::omnesagent_api::attribution::ChannelKind::Matrix,
            )
        }
    ...
    use omnesagent_memory::{Memory, MemoryCategory, SqliteMemory};
    use omnesagent_providers::ChatResponse;
    use omnesagent_providers::router::{Route, RouterModelProvider};

    macro_rules! impl_test_model_provider_attribution {
        ($ty:ty) => {
            impl ::omnesagent_api::attribution::Attributable for $ty {
                fn role(&self) -> ::omnesagent_api::attribution::Role {
    ...
    fn direct_cli_terminal_completion_projection_localizes_delivery() {
        let error =
            anyhow::Error::new(omnesagent_api::model_provider::SemanticEmptyTerminalCompletion);
        let diagnostic = error.to_string();
    ...
    impl ::omnesagent_api::attribution::Attributable for NonVisionModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ModelProvider for VisionModelProvider {
        ...
        async fn chat(
            ...
            self.calls.fetch_add(1, Ordering::SeqCst);
            let marker_count =
                omnesagent_providers::multimodal::count_image_markers(request.messages);
            if marker_count == 0 {
    ...
    impl ::omnesagent_api::attribution::Attributable for VisionModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for ScriptedModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for RecordingModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ModelProvider for StreamingScriptedModelProvider {
        ...
        fn stream_chat_with_history(
            ...
            'static,
            omnesagent_providers::traits::StreamResult<StreamChunk>,
        > {
    ...
    impl ::omnesagent_api::attribution::Attributable for StreamingScriptedModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for VisibleThenServerStreamFailureModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ModelProvider for StreamingNativeToolEventModelProvider {
        ...
        fn stream_chat(
            ...
            'static,
            omnesagent_providers::traits::StreamResult<StreamEvent>,
        > {
            self.stream_calls.fetch_add(1, Ordering::SeqCst);
    ...
    impl ::omnesagent_api::attribution::Attributable for StreamingNativeToolEventModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for RouteAwareStreamingModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for ApprovingChannel {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Channel(
                ::omnesagent_api::attribution::ChannelKind::AcpChannel,
            )
        }
    ...
    impl ::omnesagent_api::attribution::Attributable for SourcedDenyChannel {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Channel(
                ::omnesagent_api::attribution::ChannelKind::AcpChannel,
            )
...
{"name":"file_write","arguments":{"path":"a.txt","content":"x"}}
    ...
    async fn operator_sourced_deny_is_still_reported_as_a_user_denial() {
        let content =
            tool_results_for_denying_channel(::omnesagent_api::channel::ApprovalSource::Operator)
                .await;
        assert!(
    ...
    async fn run_tool_call_loop_skips_oversized_image_payload() {
        ...
        let tools_registry =
            crate::tools::scoped::ScopedToolRegistry::from_raw_for_test(Vec::new());
        let observer = NoopObserver;
        let multimodal = omnesagent_config::schema::MultimodalConfig {
            max_images: 4,
            max_image_size_mb: 1,
...
{"name":"probe","arguments":{"value":"ok"}}
    ...
    impl ::omnesagent_api::attribution::Attributable for MutatingImageProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn run_tool_call_loop_ingress_default_loop_is_behavior_identical() {
        ...
        let external = run_with(IngressContext {
            ...
            source_class: omnesagent_api::ingress::SourceClass::External,
            sender: Some("attacker".to_string()),
            transport: omnesagent_api::ingress::Transport::Channel {
                kind: "github".to_string(),
                alias: "gh".to_string(),
            },
            trust: omnesagent_api::ingress::TrustClass::Untrusted,
            origin: omnesagent_api::ingress::TurnOrigin::Channel,
        })
        .await;
    ...
    #[async_trait]
    impl omnesagent_memory::Memory for StaticRecallMemory {
        fn name(&self) -> &str {
            "static-recall"
        }
        async fn store(
        ...
        async fn get(&self, _key: &str) -> anyhow::Result<Option<omnesagent_memory::MemoryEntry>> {
            Ok(None)
        }
        async fn list(
            &self,
    ...
    impl ::omnesagent_api::attribution::Attributable for StaticRecallMemory {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
    ...
    async fn run_tool_call_loop_memory_injection_keys_on_origin() {
        async fn run_case(
            mem: &dyn omnesagent_memory::Memory,
            origin: omnesagent_api::ingress::TurnOrigin,
        ...
        .await;
        assert!(msg.starts_with(omnesagent_memory::MEMORY_CONTEXT_OPEN));
        ...
        let (msg, recalls) = run_case(
            &StaticRecallMemory,
            omnesagent_api::ingress::TurnOrigin::SubTurn,
        )
        .await;
    ...
    fn should_execute_tools_in_parallel_returns_false_when_approval_is_required() {
        ...
        let approval_cfg = omnesagent_config::schema::RiskProfileConfig::default();
    ...
    fn should_execute_tools_in_parallel_returns_true_when_cli_has_no_interactive_approvals() {
        ...
        let approval_cfg = omnesagent_config::schema::RiskProfileConfig {
...
{"name":"sop_execute","arguments":{"name":"live-sop"}}
        ...
        let mut engine = crate::sop::SopEngine::new(omnesagent_config::schema::SopConfig::default());
    ...
    async fn run_tool_call_loop_enforces_sop_step_tool_scope() {
        ...
        let mut engine = crate::sop::SopEngine::new(omnesagent_config::schema::SopConfig {
            step_scope_enforce: true,
            ..omnesagent_config::schema::SopConfig::default()
        });
        engine.replace_sops_for_test(vec![sop]);
        let engine = Arc::new(Mutex::new(engine));
    ...
    async fn run_tool_call_loop_parallel_cancel_no_double_terminal_for_completed_call() {
        ...
        let (event_tx, mut event_rx) =
            tokio::sync::mpsc::channel::<omnesagent_api::agent::TurnEvent>(64);
        ...
        while let Ok(event) = event_rx.try_recv() {
            if let omnesagent_api::agent::TurnEvent::ToolResult { id, output, .. } = event {
...
{"name":"other_tool","arguments":{"value":"B"}}
    ...
    async fn run_tool_call_loop_preserves_visible_stream_failure_partial_in_both_histories() {
        ...
        match &events[0] {
            omnesagent_api::agent::TurnEvent::Chunk { delta } => assert_eq!(delta, "visible"),
...
{"name":"count_tool","arguments":{"value":"A"}}
    ...
    async fn parsed_tool_call_iteration_text_is_suppressed() {
        ...
        let result = run_tool_call_loop(ToolLoop {
            ...
            image_cache: None,
            memory: None,
            ingress: omnesagent_api::ingress::IngressContext::sub_turn(),
            agent_alias: None,
            turn_id: &turn_id,
        })
        .await
    ...
    async fn consume_provider_streaming_response_buffers_split_tool_protocol_markers() {
        ...
        let outcome = consume_provider_streaming_response(
            ...
            None, // event_tx
            false,
            omnesagent_config::schema::StreamReasoningMode::Status,
        )
        .await
...
{"name":"verbose_checker","arguments":{"value":"check"}}
    ...
    impl crate::observability::Observer for RecallCountingObserver {
        ...
        fn record_metric(&self, _metric: &omnesagent_api::observability_traits::ObserverMetric) {}
    ...
    #[async_trait]
    impl omnesagent_memory::Memory for FailingRecallMemory {
        fn name(&self) -> &str {
            "failing-recall"
        }
        async fn store(
    ...
    impl ::omnesagent_api::attribution::Attributable for FailingRecallMemory {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Memory(
                ::omnesagent_api::attribution::MemoryKind::InMemory,
            )
...
Let me check the result."#;
    ...
    fn native_tools_system_prompt_contains_zero_xml() {
        ...
        let system_prompt = build_system_prompt_with_mode(
            ...
            omnesagent_config::schema::SkillsPromptInjectionMode::Full,
    ...
    fn native_tool_specs_present_signal_uses_effective_specs_not_provider_capability() {
        let provider =
            ScriptedModelProvider::from_text_responses(vec!["ok"]).with_native_tool_support();
        assert!(omnesagent_providers::ModelProvider::supports_native_tools(
            &provider
        ));
    ...
    async fn turn_prompt_skill_callable_names_follow_assembled_registry() {
        use omnesagent_config::schema::{RiskProfileConfig, SkillsPromptInjectionMode};
    ...
    async fn consume_provider_streaming_response_accumulates_split_reasoning_chunks() {
        ...
        impl ::omnesagent_api::attribution::Attributable for MultiChunkModelProvider {
            fn role(&self) -> ::omnesagent_api::attribution::Role {
                ::omnesagent_api::attribution::Role::Provider(
                    ::omnesagent_api::attribution::ProviderKind::Model(
                        ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    fn filter_tool_specs_always_group_includes_matching_mcp_tool() {
        use omnesagent_config::schema::{ToolFilterGroup, ToolFilterGroupMode};
    ...
    async fn make_deferred_set(names: &[&str]) -> crate::tools::DeferredMcpToolSet {
        ...
        let stubs = names
            .iter()
            .map(|name| {
                omnesagent_tools::mcp_deferred::DeferredMcpToolStub::new(
                    (*name).to_string(),
                    omnesagent_tools::mcp_protocol::McpToolDef {
                        name: (*name).to_string(),
        ...
        crate::tools::DeferredMcpToolSet {
            stubs,
            registry,
            security: Arc::new(omnesagent_config::policy::SecurityPolicy::default()),
        }
    }

    fn always_group(patterns: &[&str]) -> Vec<omnesagent_config::schema::ToolFilterGroup> {
        use omnesagent_config::schema::{ToolFilterGroup, ToolFilterGroupMode};
        vec![ToolFilterGroup {
            mode: ToolFilterGroupMode::Always,
    ...
    async fn preactivate_respects_mcp_access_policy() {
        ...
        let policy = omnesagent_tools::tool_search::ToolAccessPolicy::from_security(
            None,
            Some(&excluded),
            None,
        )
...
{"name":"failing_shell","arguments":{"command":"rm -rf /"}}
    ...
    async fn cost_tracking_records_usage_when_scoped() {
        ...
        let cost_config = omnesagent_config::schema::CostConfig {
            enabled: true,
    ...
    async fn recovered_rejected_usage_is_billed_without_replacing_accepted_context_usage() {
        ...
        use omnesagent_api::agent::TurnEvent;
        use omnesagent_providers::reliable::ReliableModelProvider;
    ...
    async fn budget_rejection_emits_no_waiting_on_model_lifecycle() {
        ...
        use crate::observability::noop::NoopObserver;
        use std::collections::HashMap;
        use omnesagent_api::channel::ProgressEvent;
    ...
    async fn trim_record_carries_model_attribution() {
        ...
        .instrument(omnesagent_log::attribution_span!(
        ...
        assert_eq!(
            value["omnesagent"]["model"], "claude-opus-4-8",
            "trim record must carry model attribution, got: {value}"
        );
        assert_eq!(
            value["omnesagent"]["model_provider"], "anthropic.personal",
            "trim record must carry model_provider attribution, got: {value}"
        );
        assert_eq!(
            value["omnesagent"]["agent_alias"], "trimtest",
    ...
    async fn skill_review_fork_records_cost_usage_under_parent_scope() {
        ...
        let review_config = omnesagent_config::schema::SkillImprovementConfig {
            enabled: true,
            cooldown_secs: 0,
    ...
    async fn reflected_skill_creation_records_cost_usage_under_parent_scope() {
        ...
        let creator = SkillCreator::new(
            ...
            omnesagent_config::schema::SkillCreationConfig {
                enabled: true,
                reflection_enabled: true,
                ..omnesagent_config::schema::SkillCreationConfig::default()
            },
        );
        let tool_calls = vec![
            ToolCallRecord {
    ...
    use omnesagent_api::tool::Tool as TestTool;
    use omnesagent_config::policy::SecurityPolicy as TestPolicy;
    ...
    fn agent_provider_composite_returns_dotted_ref_not_bare_family() {
        use omnesagent_config::providers::ModelProviderRef;
        use omnesagent_config::schema::{
    ...
    fn direct_turn_agent_config_resolves_runtime_profile_tunables() {
        use omnesagent_config::providers::RuntimeProfileRef;
        use omnesagent_config::schema::{AliasedAgentConfig, RuntimeProfileConfig};
    ...
    fn single_shot_run_routes_narration_by_interactive_mode() {
        fn run_helper(interactive: bool) -> std::process::Output {
            ...
            std::process::Command::new(current_exe)
                ...
                .env(
                    "OMNESAGENT_TEST_SINGLE_SHOT_INTERACTIVE",
    ...
    async fn single_shot_narration_boundary_helper_8760() {
        ...
        let interactive = std::env::var("OMNESAGENT_TEST_SINGLE_SHOT_INTERACTIVE")
    ...
    async fn process_message_seam_narrows_safe_defaults_outside_allowed_tools() {
        ...
        let risk = omnesagent_config::schema::RiskProfileConfig::default();
        let mem: Arc<dyn omnesagent_memory::Memory> =
            Arc::new(omnesagent_memory::NoneMemory::new("test"));
...
{"name":"count_tool","arguments":{"value":"X"}}
    ...
    /// real `$HOME/.omnesagent`. `Config::default()` points `data_dir` (the
    ...
    async fn run_brackets_successful_turn_with_agent_start_and_agent_end() {
        ...
        let server = omnesagent_spawn::spawn!(async move {
            axum::serve(listener, app)
                .await
