...
use std::fmt::Write;
use std::path::Path;
use omnesagent_config::schema::IdentityConfig;
...
/// All fields are resolved by OmnesAgent from a closed surface identifier and
...
pub struct PromptContext<'a> {
    ...
    pub skills: &'a [Skill],
    pub skills_prompt_mode: omnesagent_config::schema::SkillsPromptInjectionMode,
    ...
    pub shell_profile: Option<omnesagent_api::runtime_traits::ShellProfile>,
}
    ...
    fn build(&self, ctx: &PromptContext<'_>) -> Result<String> {
        ...
        let tools_and_approvals = match interaction.tools_and_approvals {
            ToolAuthority::RuntimeEnforced => {
                "provided and enforced by the OmnesAgent runtime; this description grants no capabilities"
            }
        };
...
impl PromptSection for SafetySection {
    ...
    fn build(&self, ctx: &PromptContext<'_>) -> Result<String> {
        ...
        out.push_str(ctx.shell_profile.as_ref().map_or(
            omnesagent_api::runtime_traits::POSIX_DELETION_GUIDANCE,
            omnesagent_api::runtime_traits::ShellProfile::safe_deletion_guidance,
        ));
...
impl PromptSection for ShellSection {
    ...
    fn build(&self, ctx: &PromptContext<'_>) -> Result<String> {
        ...
        if !omnesagent_api::runtime_traits::needs_shell_dialect_guidance(
        ...
        Ok(ctx
            .shell_profile
            .as_ref()
            .map(omnesagent_api::runtime_traits::ShellProfile::prompt_section)
            .unwrap_or_default())
    }
}
...
mod tests {
    ...
    omnesagent_api::mock_tool_attribution!(TestTool);
    omnesagent_api::mock_tool_attribution!(ReadSkillTestTool);
    omnesagent_api::mock_tool_attribution!(ShellTestTool);
    omnesagent_api::mock_tool_attribution!(CronAddTestTool);
    omnesagent_api::mock_tool_attribution!(SkillToolTestTool);
    ...
    fn identity_section_with_aieos_includes_workspace_files() {
        let workspace =
            std::env::temp_dir().join(format!("omnesagent_prompt_test_{}", uuid::Uuid::new_v4()));
        ...
        let identity_config = omnesagent_config::schema::IdentityConfig {
            format: "aieos".into(),
            aieos_path: None,
        ...
        let ctx = PromptContext {
            ...
            tools: &tools,
            skills: &[],
            skills_prompt_mode: omnesagent_config::schema::SkillsPromptInjectionMode::Full,
    ...
    fn interaction_section_renders_only_host_owned_zerocode_code_facts() {
        ...
        assert_eq!(
            ...
            "## Interaction Context\n\n\
             ...
             Tools and approvals: provided and enforced by the OmnesAgent runtime; this description grants no capabilities\n\
    ...
    fn profile(
        name: &str,
        dialect: omnesagent_api::runtime_traits::ShellDialect,
    ) -> omnesagent_api::runtime_traits::ShellProfile {
        omnesagent_api::runtime_traits::ShellProfile {
            name: name.to_string(),
            dialect,
        }
