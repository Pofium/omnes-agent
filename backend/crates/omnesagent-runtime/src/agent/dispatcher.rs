...
use std::fmt::Write;
use omnesagent_providers::{ChatMessage, ChatResponse, ConversationMessage, ToolResultMessage};
...
impl XmlToolDispatcher {
    fn parse_xml_tool_calls(response: &str) -> (String, Vec<ParsedToolCall>) {
        ...
        let cleaned = omnesagent_tool_call_parser::strip_think_tags(response);
        ...
        let calls = response
            ...
            .map(|tc| ParsedToolCall {
                ...
                arguments: serde_json::from_str(&tc.arguments).unwrap_or_else(|e| {
                    ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_category(::omnesagent_log::EventCategory::Tool).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"tool": tc.name, "error": format!("{}", e)})), "Failed to parse native tool call arguments as JSON; defaulting to empty object");
