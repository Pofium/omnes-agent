...
use omnesagent_config::schema::{AliasedAgentConfig, MemoryConfig};
use omnesagent_memory::{self, Memory};

omnesagent_api::mock_tool_attribution!(CountingTool, EchoTool, FailingTool, PanickingTool);
...
impl ::omnesagent_api::attribution::Attributable for ToolSpecCaptureModelProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::Custom,
...
fn make_sqlite_memory() -> (Arc<dyn Memory>, tempfile::TempDir) {
    ...
    let mem = Arc::from(omnesagent_memory::create_memory(&cfg, tmp.path(), None).unwrap());
    (mem, tmp)
}
