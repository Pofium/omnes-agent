...
use tokio::sync::mpsc;
use omnesagent_api::ingress::IngressContext;
use omnesagent_api::model_provider::TokenUsage;
...
fn mem_none(workspace: &Path) -> Arc<dyn Memory> {
    let cfg = omnesagent_config::schema::MemoryConfig {
        backend: "none".into(),
        ..omnesagent_config::schema::MemoryConfig::default()
    };
    Arc::from(
        omnesagent_memory::create_memory(&cfg, workspace, None)
...
impl ::omnesagent_api::attribution::Attributable for ScriptedProvider {
...
omnesagent_api::tool_attribution!(CountingTool, ::omnesagent_api::attribution::ToolKind::Plugin);
...
async fn safety_net_agent_turn_errors_at_iteration_cap() {
    ...
    let mut agent = build_agent_with_runtime(
        ...
        omnesagent_config::schema::ResolvedRuntime {
            max_tool_iterations: 3,
            ..omnesagent_config::schema::ResolvedRuntime::default()
        },
    );
    let err = agent
        .turn("loop forever")
...
async fn safety_net_thinking_never_leaks_into_draft_or_chunks() {
    ...
    impl ModelProvider for ThinkStreamProvider {
        ...
        fn stream_chat(
            ...
            _: &str,
            _: Option<f64>,
            _: omnesagent_providers::traits::StreamOptions,
        ) -> futures_util::stream::BoxStream<
            'static,
            omnesagent_providers::traits::StreamResult<omnesagent_providers::traits::StreamEvent>,
        > {
            use futures_util::StreamExt as _;
            use omnesagent_providers::traits::{StreamChunk, StreamEvent};
    ...
    impl ::omnesagent_api::attribution::Attributable for ThinkStreamProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
...
async fn safety_net_streaming_approval_deny_with_edit_round_trip() {
    ...
    impl ::omnesagent_api::attribution::Attributable for EditChannel {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
    ...
    #[async_trait]
    impl omnesagent_api::channel::Channel for EditChannel {
        fn name(&self) -> &str {
            "edit-channel"
        }
        ...
        async fn listen(
            &self,
            _tx: mpsc::Sender<omnesagent_api::channel::ChannelMessage>,
        ) -> anyhow::Result<()> {
            Ok(())
        }
        async fn request_approval(
            ...
            *self.seen_summary.lock() = Some(request.arguments_summary.clone());
            Ok(Some(
                omnesagent_api::channel::ChannelApprovalResponse::DenyWithEdit {
...
async fn safety_net_task_locals_probe_per_entry_path() {
    ...
    omnesagent_api::tool_attribution!(ProbeTool, ::omnesagent_api::attribution::ToolKind::Plugin);
    #[async_trait]
    impl Tool for ProbeTool {
        ...
        async fn execute(&self, _args: serde_json::Value) -> Result<crate::tools::ToolResult> {
            let thread = omnesagent_api::TOOL_LOOP_THREAD_ID
                .try_with(|v| v.is_some())
                .unwrap_or(false);
            let session = omnesagent_api::TOOL_LOOP_SESSION_KEY
                .try_with(|v| v.is_some())
                .unwrap_or(false);
            let choice = omnesagent_api::TOOL_CHOICE_OVERRIDE
                .try_with(|v| v.is_some())
                .unwrap_or(false);
...
async fn safety_net_turn_survives_in_loop_history_pruning() {
    ...
    let runtime = omnesagent_config::schema::ResolvedRuntime {
...
impl ::omnesagent_api::attribution::Attributable for ErrAfterScriptProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::Custom,
...
struct StreamThenHangProvider {
    events: parking_lot::Mutex<
        Vec<omnesagent_api::model_provider::StreamResult<omnesagent_api::model_provider::StreamEvent>>,
    >,
}
...
impl ModelProvider for StreamThenHangProvider {
    ...
    fn stream_chat(
        ...
        'static,
        omnesagent_api::model_provider::StreamResult<omnesagent_api::model_provider::StreamEvent>,
    > {
        use futures_util::StreamExt as _;
...
impl ::omnesagent_api::attribution::Attributable for StreamThenHangProvider {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Provider(
            ::omnesagent_api::attribution::ProviderKind::Model(
                ::omnesagent_api::attribution::ModelProviderKind::Custom,
...
fn text_delta(
    ...
    Ok(omnesagent_api::model_provider::StreamEvent::TextDelta(
        omnesagent_api::model_provider::StreamChunk {
...
async fn safety_net_stream_error_persists_only_forwarded_text() {
    let provider = StreamThenHangProvider {
        events: parking_lot::Mutex::new(vec![
            ...
            Err(omnesagent_api::model_provider::StreamError::Http(
...
impl ::omnesagent_api::attribution::Attributable for RecordingApprovalChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(
            ::omnesagent_api::attribution::ChannelKind::AcpChannel,
        )
...
#[async_trait]
impl omnesagent_api::channel::Channel for RecordingApprovalChannel {
    fn name(&self) -> &str {
        "acp"
    }
    async fn send(&self, _message: &omnesagent_api::channel::SendMessage) -> anyhow::Result<()> {
    ...
    async fn request_approval(
        &self,
        _recipient: &str,
        _request: &omnesagent_api::channel::ChannelApprovalRequest,
    ) -> anyhow::Result<Option<omnesagent_api::channel::ChannelApprovalResponse>> {
        self.requests.fetch_add(1, Ordering::SeqCst);
...
fn approval_agent(
    ...
    channel: Option<Arc<dyn omnesagent_api::channel::Channel>>,
) -> TestAgent {
    let workspace = test_workspace();
...
async fn safety_net_loop_approval_requested_then_executed_on_approve() {
    ...
    let mut agent = approval_agent(
        ...
        Some(Arc::new(RecordingApprovalChannel {
            response: omnesagent_api::channel::ChannelApprovalResponse::Approve,
            requests: Arc::clone(&requests),
        })),
    );
    let (tx, _rx) = mpsc::channel(256);
...
async fn safety_net_loop_shell_keeps_runtime_approval_from_always_allowlist() {
    ...
    let mut agent = approval_agent(
        ...
        Some(Arc::new(RecordingApprovalChannel {
            response: omnesagent_api::channel::ChannelApprovalResponse::AlwaysApprove,
            requests: Arc::clone(&requests),
        })),
    );
    let (tx, _rx) = mpsc::channel(256);
