...
use omnesagent_memory::{self, Memory, MemoryCategory};
#[cfg(test)]
use omnesagent_providers::ChatRequest;
use omnesagent_providers::{
    self, ChatMessage, ConversationMessage, ModelProvider, ToolResultMessage,
};

// Re-export TurnEvent from omnesagent-types for backwards compatibility.
pub use omnesagent_api::agent::TurnEvent;
...
pub fn build_session_model_provider(
    ...
    let model_provider_runtime_options = omnesagent_providers::provider_runtime_options_for_alias(
        config,
        &model_provider_name,
        &model_provider_alias,
    );

    let model_provider = omnesagent_providers::create_routed_model_provider_with_options(
        config,
        model_provider_ref,
...
pub fn tool_dispatcher_for_provider(
    agent_cfg: &omnesagent_config::schema::AliasedAgentConfig,
...
pub(crate) enum RoutedApproval {
    ...
    Decided {
        response: omnesagent_api::channel::ChannelApprovalResponse,
        decider: Option<String>,
        source: omnesagent_api::channel::ApprovalSource,
    },
...
pub(crate) async fn resolve_routed_approval(
    ...
    route: &omnesagent_config::autonomy::ApprovalRoute,
    recipient: &str,
    request: &omnesagent_api::channel::ChannelApprovalRequest,
) -> RoutedApproval {
    let approver: Option<(String, Arc<dyn omnesagent_api::channel::Channel>)> = handles
        .read()
        .iter()
    ...
    // a missing decider.
    let (reason, source): (&str, omnesagent_api::channel::ApprovalSource) =
        if let Some((channel_name, channel)) = approver {
                ...
                .await
            {
                ...
                Ok(Ok(None)) => (
                    ...
                    omnesagent_api::channel::ApprovalSource::Unreachable,
                ),
                Ok(Err(_)) => (
                ...
                Err(_) => (
                    "approver timed out",
                    omnesagent_api::channel::ApprovalSource::TimedOut,
                ),
            }
        } else {
            (
                "approver channel not registered",
                omnesagent_api::channel::ApprovalSource::Unavailable,
            )
        };

    match route.on_no_approver {
        omnesagent_config::autonomy::OnNoApprover::Deny => {
            ...
            RoutedApproval::Decided {
                response: omnesagent_api::channel::ChannelApprovalResponse::Deny,
                decider: None,
        ...
        omnesagent_config::autonomy::OnNoApprover::InheritOriginator => {
            ::omnesagent_log::record!(
                WARN,
...
impl ::omnesagent_api::attribution::Attributable for RoutedApprovalChannel {
    fn role(&self) -> ::omnesagent_api::attribution::Role {
        ::omnesagent_api::attribution::Role::Channel(::omnesagent_api::attribution::ChannelKind::Cli)
    }
    fn alias(&self) -> &str {
        "approval-route"
    }
...
impl omnesagent_api::channel::Channel for RoutedApprovalChannel {
    fn name(&self) -> &str {
        "approval-route"
    }

    async fn send(&self, _message: &omnesagent_api::channel::SendMessage) -> anyhow::Result<()> {
        Ok(())
    }

    async fn listen(
        &self,
        _tx: tokio::sync::mpsc::Sender<omnesagent_api::channel::ChannelMessage>,
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    async fn request_approval(
        ...
        recipient: &str,
        request: &omnesagent_api::channel::ChannelApprovalRequest,
    ) -> anyhow::Result<Option<omnesagent_api::channel::ChannelApprovalResponse>> {
        Ok(self
    ...
    async fn request_approval_attributed(
        ...
        recipient: &str,
        request: &omnesagent_api::channel::ChannelApprovalRequest,
    ) -> anyhow::Result<Option<omnesagent_api::channel::AttributedApprovalResponse>> {
        match resolve_routed_approval(&self.handles, &self.route, recipient, request).await {
            ...
            RoutedApproval::Decided {
                ...
                omnesagent_api::channel::AttributedApprovalResponse::from_runtime(response, source)
...
pub struct Agent {
    ...
    config: omnesagent_config::schema::AliasedAgentConfig,
    ...
    multimodal_config: omnesagent_config::schema::MultimodalConfig,
    model_name: String,
    ...
    identity_config: omnesagent_config::schema::IdentityConfig,
    interaction_context: Option<InteractionContext>,
    skills: Vec<crate::skills::Skill>,
    skills_prompt_mode: omnesagent_config::schema::SkillsPromptInjectionMode,
    auto_save: bool,
    ...
    classification_config: omnesagent_config::schema::QueryClassificationConfig,
    available_hints: Vec<String>,
    route_model_by_hint: HashMap<String, String>,
    response_cache: Option<Arc<omnesagent_memory::response_cache::ResponseCache>>,
    ...
    shell_profile: Option<omnesagent_api::runtime_traits::ShellProfile>,
    ...
    approval_route: Option<omnesagent_config::autonomy::ApprovalRoute>,
    ...
    image_cache: omnesagent_providers::multimodal::LocalImageCache,
...
pub struct ProviderSwitchConfig {
    pub config: Option<std::sync::Arc<omnesagent_config::schema::Config>>,
}
...
impl AgentChannelHandles {
    ...
    pub fn register_channel(
        ...
        channel: Arc<dyn omnesagent_api::channel::Channel>,
    ) {
        let name = name.into();
    ...
    pub fn get_channel(&self, name: &str) -> Option<Arc<dyn omnesagent_api::channel::Channel>> {
        for handle in self.populated_handles().into_iter().flatten() {
...
pub struct AgentBuilder {
    ...
    config: Option<omnesagent_config::schema::AliasedAgentConfig>,
    structured_history_cap_resolver: Option<Arc<dyn Fn() -> usize + Send + Sync>>,
    multimodal_config: Option<omnesagent_config::schema::MultimodalConfig>,
    model_name: Option<String>,
    model_provider_name: Option<String>,
    temperature: Option<f64>,
    workspace_dir: Option<std::path::PathBuf>,
    agent_workspace_dir: Option<std::path::PathBuf>,
    identity_config: Option<omnesagent_config::schema::IdentityConfig>,
    interaction_context: Option<InteractionContext>,
    skills: Option<Vec<crate::skills::Skill>>,
    skills_prompt_mode: Option<omnesagent_config::schema::SkillsPromptInjectionMode>,
    auto_save: Option<bool>,
    memory_session_id: Option<String>,
    classification_config: Option<omnesagent_config::schema::QueryClassificationConfig>,
...
impl AgentBuilder {
    ...
    pub fn config(mut self, config: omnesagent_config::schema::AliasedAgentConfig) -> Self {
    ...
    pub fn response_cache(
        mut self,
        cache: Option<Arc<omnesagent_memory::response_cache::ResponseCache>>,
    ) -> Self {
    ...
    pub fn shell_profile(
        mut self,
        profile: Option<omnesagent_api::runtime_traits::ShellProfile>,
    ) -> Self {
    ...
    pub fn approval_route(
        mut self,
        route: Option<omnesagent_config::autonomy::ApprovalRoute>,
    ) -> Self {
    ...
    pub fn build(self) -> Result<Agent> {
        ...
        if exclude_memory {
            tools.retain(|t| !omnesagent_tools::MEMORY_TOOL_NAMES.contains(&t.name()));
        }

        let memory: Arc<dyn Memory> = if exclude_memory {
            Arc::new(omnesagent_memory::NoneMemory::new("none"))
        } else {
        ...
        Ok(Agent {
            ...
            memory_inject_cfg: self.memory_inject_cfg.unwrap_or_else(|| {
                crate::agent::memory_inject::MemoryInjectConfig::from_memory_config(
                    &omnesagent_config::schema::MemoryConfig::default(),
            ...
            image_cache: omnesagent_providers::multimodal::LocalImageCache::new(),
...
impl Agent {
    ...
    fn full_config(&self) -> Option<&omnesagent_config::schema::Config> {
    ...
    pub fn populate_channels(
        &self,
        channel_map: &std::collections::HashMap<String, Arc<dyn omnesagent_api::channel::Channel>>,
    ) -> Vec<String> {
    ...
    fn memory_injection_active(&self) -> bool {
        ...
        matches!(
            crate::agent::memory_inject::resolve_inject_policy(
                omnesagent_api::ingress::TurnOrigin::AgentDirect,
    ...
    fn response_cache_key_for_messages(
        ...
        if self.temperature != Some(0.0)
            ...
            || omnesagent_api::NATIVE_THINKING_OVERRIDE
        ...
        Some(omnesagent_memory::response_cache::ResponseCache::cache_key(
    ...
    ) -> Option<anyhow::Result<omnesagent_api::tool::ToolResult>> {
    ...
    async fn from_config_with_session_cwd_and_mcp_approval_mode(
        ...
        if let Err(e) = tokio::fs::create_dir_all(&agent_workspace).await {
            ::omnesagent_log::record!(WARN, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_outcome(::omnesagent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": agent_alias, "workspace": agent_workspace.display().to_string(), "e": e.to_string()})), "Failed to create per-agent workspace dir (continuing): ");
        }
        ...
        let memory: Arc<dyn Memory> = omnesagent_memory::create_memory_for_agent(
            config,
            agent_alias,
        ...
        let (sop_engine, sop_audit) = match (sop_engine, sop_audit) {
            ...
            (None, None) if config.sop.runtime_enabled() => {
                let mem: Arc<dyn omnesagent_memory::Memory> =
                    omnesagent_memory::create_memory_for_agent(config, agent_alias, None).await?;
        ...
        let provider_runtime_options = omnesagent_providers::provider_runtime_options_for_alias(
            config,
            provider_name,
            provider_alias,
        );
        ...
        let model_provider: Box<dyn ModelProvider> =
            omnesagent_providers::create_routed_model_provider_with_options(
                config,
                &provider_ref,
                agent_model_provider.and_then(|e| e.api_key.as_deref()),
        ...
        let response_cache = if config.memory.response_cache_enabled {
            omnesagent_memory::response_cache::ResponseCache::with_hot_cache(
                &config.data_dir,
    ...
    fn trim_history(&mut self, turn_id: Option<&str>) -> Option<HistoryTrimNotice> {
        ...
        {
            let scope_span = ::omnesagent_log::info_span!(
                target: "omnesagent_log_internal_scope",
                "omnesagent_scope",
                agent_alias = ::omnesagent_log::field::Empty,
                channel = %channel,
                trace_id = ::omnesagent_log::field::Empty,
            );
    ...
    /// is consumed from the `omnesagent_providers::reliable` task-local (single
    ...
    async fn append_model_fallback_notice(
        response: String,
        fallback: Option<&omnesagent_providers::reliable::ProviderFallbackInfo>,
        event_tx: &tokio::sync::mpsc::Sender<TurnEvent>,
    ) -> String {
            ...
            .and_then(|cfg| cfg.config.as_ref())
        {
            Some(full_config) => {
                ...
                let runtime_options = new_model_provider
                    .split_once('.')
                    .map(|(family, alias)| {
                        omnesagent_providers::provider_runtime_options_for_alias(
                            full_config.as_ref(),
                            family,
                            alias,
                        )
    ...
    fn classify_model(&self, user_message: &str) -> String {
            ...
            && self.available_hints.contains(&decision.hint)
        {
            let resolved_model = self
                ...
                .map(String::as_str)
                .unwrap_or("unknown");
            ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"hint": decision.hint.as_str(), "model": resolved_model, "rule_priority": decision.priority, "message_length": user_message.len()})), "Classified message route");
        ...
        if let Some(ref ac) = self.config.resolved.auto_classify {
            ...
            {
                ::omnesagent_log::record!(INFO, ::omnesagent_log::Event::new(module_path!(), ::omnesagent_log::Action::Note).with_attrs(::serde_json::json!({"hint": hint, "complexity": format!("{:?}", tier), "message_length": user_message.len()})), "Auto-classified by complexity");
        ...
        for msg in loop_messages {
                ...
                })
            {
                let tool_calls = calls
                    .iter()
                    .map(|c| omnesagent_providers::ToolCall {
                        id: c
                            .get("id")
                            .and_then(|v| v.as_str())
                            .unwrap_or_default()
    ...
    pub async fn turn(&mut self, user_message: &str) -> Result<String> {
        ...
        let knobs = crate::agent::loop_::LoopKnobs {
            ...
            draft_reasoning: omnesagent_config::schema::StreamReasoningMode::Status,
        };
        ...
        let pacing = omnesagent_config::schema::PacingConfig {
            loop_detection_enabled: false,
            ..omnesagent_config::schema::PacingConfig::default()
        };
        ...
        let turn_loop = crate::agent::loop_::TOOL_LOOP_COST_TRACKING_CONTEXT.scope(
            ...
            crate::agent::tool_receipts::scope_receipts(
                ...
                crate::agent::loop_::run_tool_call_loop(crate::agent::loop_::ToolLoop {
                    ...
                    ingress: omnesagent_api::ingress::IngressContext::agent_direct(),
        ...
        let (loop_result, turn_provider_recovery, turn_provider_context_truncated) =
            omnesagent_providers::reliable::scope_provider_fallback(async {
                ...
                (
                    result,
                    omnesagent_providers::reliable::take_last_provider_fallback(),
                    omnesagent_providers::reliable::take_last_provider_context_truncation(),
                )
            })
            .await;
        ...
        if usage.input_tokens > 0 || usage.output_tokens > 0 {
            guard.set_usage(
                Some(omnesagent_api::observability_traits::TurnTokenUsage {
    ...
    pub async fn turn_streamed_with_steering_state(
        ...
        // task-local record inside `omnesagent_providers::reliable`, consumed
        // once per round below; this is a per-turn transient resolved at
        // use-time, never stored on the agent.
        let mut turn_provider_recovery: Option<omnesagent_providers::reliable::ProviderFallbackInfo>;
        ...
        let approval_bridge: Option<Box<dyn omnesagent_api::channel::Channel>> =
            self.channel_handles.ask_user.as_ref().map(|handles| {
                ...
                )) as Box<dyn omnesagent_api::channel::Channel>
            });
    ...
    pub async fn run_interactive(&mut self) -> Result<()> {
        println!("🦀 OmnesAgent Interactive Mode");
        ...
        let listen_handle = omnesagent_spawn::spawn!(async move {
            let _ = omnesagent_api::channel::Channel::listen(&*cli, tx).await;
        });
...
mod tests {
    ...
    use omnesagent_api::observability_traits::ObserverMetric;
    ...
    fn build_session_model_provider_returns_full_canonical_ref() {
        use omnesagent_config::schema::{ModelProviderConfig, OpenAIModelProviderConfig};
    ...
    omnesagent_api::mock_tool_attribution!(
        CountingTool,
        NamedMockTool,
        MockTool,
        SlowTool,
    ...
    struct MockModelProvider {
        responses: Mutex<Vec<omnesagent_providers::ChatResponse>>,
    }
    ...
    impl ModelProvider for MockModelProvider {
        ...
        async fn chat(
            ...
            _model: &str,
            _temperature: Option<f64>,
        ) -> Result<omnesagent_providers::ChatResponse> {
            let mut guard = self.responses.lock();
            if guard.is_empty() {
                return Ok(omnesagent_providers::ChatResponse {
                    text: Some("done".into()),
                    tool_calls: vec![],
                    usage: None,
    ...
    impl ::omnesagent_api::attribution::Attributable for MockModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
                ),
            )
        }
        fn alias(&self) -> &str {
    ...
    fn blank_input_agent(model_provider: Box<dyn ModelProvider>) -> Agent {
        let memory_cfg = omnesagent_config::schema::MemoryConfig {
            backend: "none".into(),
            ..omnesagent_config::schema::MemoryConfig::default()
        };
        let mem: Arc<dyn Memory> = Arc::from(
            omnesagent_memory::create_memory(&memory_cfg, std::path::Path::new("/tmp"), None)
    ...
    fn fallback_info(
        ...
        actual_provider: &str,
        actual_model: &str,
    ) -> omnesagent_providers::reliable::ProviderFallbackInfo {
        omnesagent_providers::reliable::ProviderFallbackInfo {
    ...
    impl ModelProvider for RuntimeStreamingProbeProvider {
        ...
        async fn chat(
            ...
            Ok(omnesagent_providers::ChatResponse {
        ...
        fn stream_chat(
            ...
            _model: &str,
            _temperature: Option<f64>,
            _options: omnesagent_providers::traits::StreamOptions,
        ) -> futures_util::stream::BoxStream<
            'static,
            omnesagent_providers::traits::StreamResult<omnesagent_providers::traits::StreamEvent>,
        > {
            ...
            match self.stream {
                ...
                RuntimeStreamPlan::Text(text) => futures_util::stream::iter(vec![
                    Ok(omnesagent_providers::traits::StreamEvent::TextDelta(
                        omnesagent_providers::traits::StreamChunk::delta(text),
                    )),
                    Ok(omnesagent_providers::traits::StreamEvent::Final),
                ])
                .boxed(),
                RuntimeStreamPlan::EmptyWithUsage => futures_util::stream::iter(vec![
                    Ok(omnesagent_providers::traits::StreamEvent::Usage(
                        omnesagent_providers::traits::TokenUsage {
                ...
                RuntimeStreamPlan::Error => futures_util::stream::iter(vec![Err(
                    omnesagent_providers::traits::StreamError::ModelProvider(
    ...
    impl ::omnesagent_api::attribution::Attributable for RuntimeStreamingProbeProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    ) -> omnesagent_providers::reliable::ReliableModelProvider {
        omnesagent_providers::reliable::ReliableModelProvider::new(
            "test",
            vec![
                (
    ...
    async fn streamed_turn_surfaces_provider_fallback_notice() {
        ...
        impl ::omnesagent_api::attribution::Attributable for FailingModelProvider {
            fn role(&self) -> ::omnesagent_api::attribution::Role {
                ::omnesagent_api::attribution::Role::Provider(
                    ::omnesagent_api::attribution::ProviderKind::Model(
                        ::omnesagent_api::attribution::ModelProviderKind::Custom,
        ...
        let reliable = omnesagent_providers::reliable::ReliableModelProvider::new(
            "test",
            vec![
                (
                    "provider-requested".to_string(),
        ...
        let reliable = omnesagent_providers::reliable::ReliableModelProvider::new(
            ...
            vec![
                ...
                (
                    "fallback".to_string(),
                    Box::new(MockModelProvider {
                        responses: Mutex::new(vec![omnesagent_providers::ChatResponse {
    ...
    async fn fallback_tool_call_then_primary_final_clears_the_stale_notice() {
        ...
        macro_rules! attributable {
            ($type:ty, $alias:literal) => {
                impl ::omnesagent_api::attribution::Attributable for $type {
                    fn role(&self) -> ::omnesagent_api::attribution::Role {
                        ::omnesagent_api::attribution::Role::Provider(
                            ::omnesagent_api::attribution::ProviderKind::Model(
                                ::omnesagent_api::attribution::ModelProviderKind::Custom,
        ...
        impl ModelProvider for FallbackToolCall {
            ...
            async fn chat(
                ...
                Ok(omnesagent_providers::ChatResponse {
                    ...
                    tool_calls: vec![omnesagent_providers::ToolCall {
        ...
        let mut agent = Agent::builder()
            ...
            .memory(Arc::from(
                omnesagent_memory::create_memory(
                    &omnesagent_config::schema::MemoryConfig {
                        backend: "none".into(),
                        ..Default::default()
                    },
    ...
    impl ::omnesagent_api::attribution::Attributable for ModelCaptureModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for TranscriptCaptureModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for AlwaysFailModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for ContextWindowModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for CountingAnswerModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::omnesagent_api::attribution::Attributable for HookProtocolCaptureProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
                ...
                async move {
                    match step {
                        0 if should_fail => Some((
                            Err(omnesagent_providers::traits::StreamError::ModelProvider(
                        ...
                        0 => Some((
                            Ok(omnesagent_providers::traits::StreamEvent::TextDelta(
                                omnesagent_providers::traits::StreamChunk {
                                    delta,
                                    is_final: false,
                                    reasoning: None,
                                    token_count: 0,
                        ...
                        1 => {
                            ...
                            Some((Ok(omnesagent_providers::traits::StreamEvent::Final), 2))
                        }
                        _ => None,
                    }
                }
    ...
    impl ::omnesagent_api::attribution::Attributable for StreamingSteeringModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ModelProvider for MultimodalCaptureProvider {
        ...
        > {
            ...
            if self.streamed {
                let chunk = omnesagent_providers::traits::StreamEvent::TextDelta(
                    omnesagent_providers::traits::StreamChunk {
                ...
                stream::iter(vec![Ok(omnesagent_providers::traits::StreamEvent::Final)]).boxed()
            }
        }
    ...
    impl ::omnesagent_api::attribution::Attributable for MultimodalCaptureProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn direct_agent_turn_stdout_boundary_helper_4721() {
        ...
        let model_provider = Box::new(MockModelProvider {
            responses: Mutex::new(vec![
                omnesagent_providers::ChatResponse {
    ...
    impl ::omnesagent_api::attribution::Attributable for ToolThenFailingModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn direct_agent_strict_tool_parsing_ignores_xml_dispatcher_calls() {
        ...
        let agent_config = omnesagent_config::schema::AliasedAgentConfig {
            resolved: omnesagent_config::schema::ResolvedRuntime {
                strict_tool_parsing: true,
                ..Default::default()
            },
            ..omnesagent_config::schema::AliasedAgentConfig::default()
        };
        let mut agent = Agent::builder()
    ...
    fn native_agent_prompt_omits_duplicate_tools_section() {
        ...
        let mem: Arc<dyn Memory> = Arc::from(
            omnesagent_memory::create_memory(&memory_cfg, workspace.path(), None)
                .expect("memory creation should succeed with valid config"),
        );
        let observer: Arc<dyn Observer> = Arc::from(crate::observability::NoopObserver {});
    ...
    mod surface2_tests {
        ...
        impl ::omnesagent_api::attribution::Attributable for CapturingModelProvider {
            fn role(&self) -> ::omnesagent_api::attribution::Role {
                ::omnesagent_api::attribution::Role::Provider(
                    ::omnesagent_api::attribution::ProviderKind::Model(
                        ::omnesagent_api::attribution::ModelProviderKind::Custom,
        ...
        fn dispatcher_selection_uses_the_selected_routes_tool_capability() {
            ...
            let router = omnesagent_providers::router::RouterModelProvider::new(
                "test",
                vec![
                ...
                vec![(
                    "text".to_string(),
                    omnesagent_providers::router::Route {
            ...
            let config = omnesagent_config::schema::AliasedAgentConfig::default();
        ...
        fn streamed_provider_switch_refreshes_active_loop_skills_prompt() {
            ...
            let observer: Arc<dyn Observer> = Arc::from(crate::observability::NoopObserver {});
            let (provider, _) = capturing_provider(false);
            let config = omnesagent_config::schema::AliasedAgentConfig {
                resolved: omnesagent_config::schema::ResolvedRuntime {
                    strict_tool_parsing: true,
                    ..Default::default()
                },
            ...
            let mut agent = Agent::builder()
                ...
                .config(config)
                .skills(skills)
                .skills_prompt_mode(omnesagent_config::schema::SkillsPromptInjectionMode::Compact)
                .workspace_dir(workspace.path().to_path_buf())
                .build()
                .expect("agent builder should succeed");
            agent.history = vec![ConversationMessage::Chat(ChatMessage::system(
        ...
        async fn turn_rebuilds_prompt_for_vision_routed_xml_provider() {
            ...
            let mm_config = omnesagent_config::schema::MultimodalConfig {
    ...
    async fn turn_routes_with_hint_when_query_classification_matches() {
        ...
        let mut agent = Agent::builder()
            ...
            .classification_config(omnesagent_config::schema::QueryClassificationConfig {
                enabled: true,
                rules: vec![omnesagent_config::schema::ClassificationRule {
                    hint: "fast".to_string(),
    ...
    async fn from_config_passes_extra_headers_to_custom_provider() {
        ...
        let server_handle = omnesagent_spawn::spawn!(async move {
        ...
        let mut config = omnesagent_config::schema::Config {
        ...
        };
        {
            ...
            entry.extra_headers.insert(
                ...
                "omnesagent-web-test/1.0".to_string(),
            );
            entry
                .extra_headers
                .insert("X-Title".to_string(), "omnesagent-web".to_string());
        }
        ...
        config.risk_profiles.insert(
            ...
            omnesagent_config::schema::RiskProfileConfig::default(),
        );
        let agent_cfg = omnesagent_config::schema::AliasedAgentConfig {
    ...
    async fn from_config_preserves_alias_for_cost_recording() {
        ...
        let usage = omnesagent_providers::traits::TokenUsage {
    ...
    fn session_cwd_keeps_workspace_in_allowed_roots() {
        let workspace = std::env::temp_dir().join("omnesagent_test_session_cwd_workspace");
        let session = std::env::temp_dir().join("omnesagent_test_session_cwd_session");
        ...
        let risk_profile = omnesagent_config::schema::RiskProfileConfig::default();
    ...
    fn set_tool_dispatcher_refreshes_existing_system_prompt() {
        use omnesagent_api::model_provider::{ChatMessage, ConversationMessage};
    ...
    fn seed_conversation_history_preserves_tool_call_variants() {
        use omnesagent_api::model_provider::{
    ...
    fn seed_conversation_history_trims_over_cap_restore_without_splitting_tools() {
        use omnesagent_providers::{ToolCall, ToolResultMessage};
    ...
    impl ModelProvider for StreamToolCaptureModelProvider {
        ...
        > {
            ...
            *count += 1;
            if *count == 1 {
                let tc = omnesagent_providers::traits::StreamEvent::ToolCall(
                    omnesagent_providers::ToolCall {
                        id: "00000000-0000-0000-0000-000000000001".into(),
                        name: "echo".into(),
                        arguments: "{}".into(),
                        extra_content: None,
    ...
    impl ::omnesagent_api::attribution::Attributable for StreamToolCaptureModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    fn tool_receipts_enabled_config(enabled: bool) -> omnesagent_config::schema::AliasedAgentConfig {
        omnesagent_config::schema::AliasedAgentConfig {
            resolved: omnesagent_config::schema::ResolvedRuntime {
                tool_receipts: omnesagent_config::schema::ToolReceiptsConfig {
                    enabled,
                    ..Default::default()
                },
                ..Default::default()
    ...
    fn show_in_response_config(show: bool) -> omnesagent_config::schema::AliasedAgentConfig {
        omnesagent_config::schema::AliasedAgentConfig {
            resolved: omnesagent_config::schema::ResolvedRuntime {
                tool_receipts: omnesagent_config::schema::ToolReceiptsConfig {
                    enabled: true,
    ...
    fn streamed_agent_with_config(config: omnesagent_config::schema::AliasedAgentConfig) -> Agent {
        let model_provider = Box::new(StreamToolCaptureModelProvider {
            tools_received: Arc::new(Mutex::new(Vec::new())),
            call_count: Arc::new(Mutex::new(0)),
        });
    ...
    impl ModelProvider for TwoToolCallStreamModelProvider {
        ...
        > {
            ...
            *count += 1;
            if *count == 1 {
                stream::iter(vec![
                    Ok(omnesagent_providers::traits::StreamEvent::ToolCall(
                        omnesagent_providers::ToolCall {
                            id: "00000000-0000-0000-0000-000000000001".into(),
                            name: "echo".into(),
                            arguments: "{}".into(),
    ...
    impl ::omnesagent_api::attribution::Attributable for TwoToolCallStreamModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ModelProvider for PreExecutedToolModelProvider {
        ...
        > {
            ...
            stream::iter(vec![
                Ok(
                    omnesagent_providers::traits::StreamEvent::PreExecutedToolCall {
                        name: "file_read".into(),
                ...
                Ok(
                    omnesagent_providers::traits::StreamEvent::PreExecutedToolResult {
                        name: "file_read".into(),
                        output: "a".into(),
                    },
                ),
                ...
                Ok(omnesagent_providers::traits::StreamEvent::TextDelta(
                    omnesagent_providers::traits::StreamChunk::delta("done"),
                )),
                Ok(omnesagent_providers::traits::StreamEvent::Final),
            ])
            .boxed()
    ...
    impl ::omnesagent_api::attribution::Attributable for PreExecutedToolModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn turn_normalizes_user_image_markers_before_provider_call() {
        ...
        let mut agent = Agent::builder()
            ...
            .tool_dispatcher(Box::new(NativeToolDispatcher))
            .workspace_dir(std::path::PathBuf::from("/tmp"))
            .multimodal_config(omnesagent_config::schema::MultimodalConfig::default())
            .build()
            .expect("agent builder should succeed with valid config");
    ...
    fn trim_history_test_agent(max_history_messages: usize, observer: Arc<dyn Observer>) -> Agent {
        ...
        let agent_config = omnesagent_config::schema::AliasedAgentConfig {
            resolved: omnesagent_config::schema::ResolvedRuntime::default(),
            ..omnesagent_config::schema::AliasedAgentConfig::default()
        };
    ...
    async fn trim_history_runs_after_direct_vision_resolution_error() {
        ...
        let capability_error = error
            .downcast_ref::<omnesagent_providers::ProviderCapabilityError>()
    ...
    fn trim_history_log_uses_canonical_attribution() {
        ...
        let mut log_rx = omnesagent_log::subscribe_or_install();
        ...
        let event: omnesagent_log::LogEvent =
            serde_json::from_value(value).expect("captured trim event should deserialize");

        assert_eq!(event.omnesagent.get("agent_alias"), Some("trim-test-agent"));
        assert_eq!(
            event.omnesagent.get("channel_type"),
            Some("trim-test-channel")
        );
        assert_eq!(event.omnesagent.get("channel"), None);
        ...
        omnesagent_log::clear_broadcast_hook();
    }
    ...
    impl ::omnesagent_api::attribution::Attributable for NarrationStreamModelProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn response_cache_key_uses_full_provider_visible_transcript() {
        ...
        let cache = Arc::new(
            omnesagent_memory::response_cache::ResponseCache::new(tmp.path(), 60, 100)
    ...
    async fn before_llm_hook_mutates_ephemeral_request_and_attributes_selected_model() {
        ...
        impl ModelProvider for RequestCaptureProvider {
            ...
            async fn chat(
                ...
                Ok(omnesagent_providers::ChatResponse {
                    ...
                    tool_calls: vec![],
                    usage: Some(omnesagent_providers::traits::TokenUsage {
        ...
        impl ::omnesagent_api::attribution::Attributable for RequestCaptureProvider {
            fn role(&self) -> ::omnesagent_api::attribution::Role {
                ::omnesagent_api::attribution::Role::Provider(
                    ::omnesagent_api::attribution::ProviderKind::Model(
                        ::omnesagent_api::attribution::ModelProviderKind::Custom,
        ...
        let memory: Arc<dyn Memory> = Arc::from(
            omnesagent_memory::create_memory(&memory_cfg, tmp.path(), None)
                .expect("memory creation should succeed"),
        );
        let mut hooks = crate::hooks::HookRunner::new();
    ...
    async fn before_llm_hook_rejects_strict_mixed_selected_route_before_dispatch() {
        ...
        let text_requests: CapturedToolProtocolRequests = Arc::new(Mutex::new(Vec::new()));
        let mixed = omnesagent_providers::reliable::ReliableModelProvider::new(
            "mixed",
            vec![
                (
                    "native".into(),
    ...
    async fn response_cache_separates_configured_provider_identity() {
        ...
        let build =
            |provider_alias: &str,
             ...
             cache: Arc<omnesagent_memory::response_cache::ResponseCache>| {
                Agent::builder()
                    .model_provider(Box::new(TranscriptCaptureModelProvider {
    ...
    async fn response_cache_distinguishes_model_pin_identity() {
        ...
        let build = |answer: &str, pinned_model: &str, seen: Arc<Mutex<Vec<Vec<ChatMessage>>>>| {
            let pinned = omnesagent_providers::model_pin::ModelPinnedProvider::builder("shared-pin")
    ...
    async fn response_cache_bypasses_native_thinking_overrides() {
        ...
        let answer_a = omnesagent_api::NATIVE_THINKING_OVERRIDE
            .scope(
                Some(omnesagent_api::model_provider::NativeThinkingParams {
                    budget_tokens: 1_024,
                    display: None,
                }),
            ...
            .await
            .unwrap();
        let answer_b = omnesagent_api::NATIVE_THINKING_OVERRIDE
            .scope(
                Some(omnesagent_api::model_provider::NativeThinkingParams {
                    budget_tokens: 2_048,
                    display: None,
    ...
    async fn response_cache_does_not_cross_serve_memory_conditioned_answers() {
        ...
        impl Memory for FixtureRecallMemory {
            ...
            async fn recall(
                ...
                _: Option<&str>,
                _: Option<&str>,
            ) -> anyhow::Result<Vec<omnesagent_memory::MemoryEntry>> {
                Ok(vec![omnesagent_memory::MemoryEntry {
                    id: "deploy".into(),
                    key: "deploy".into(),
        ...
        impl ::omnesagent_api::attribution::Attributable for FixtureRecallMemory {
            fn role(&self) -> ::omnesagent_api::attribution::Role {
        ...
        let harm_cache = Arc::new(
            omnesagent_memory::response_cache::ResponseCache::new(harm_dir.path(), 60, 100)
        ...
        let ctrl_cache = Arc::new(
            omnesagent_memory::response_cache::ResponseCache::new(ctrl_dir.path(), 60, 100)
                .expect("response cache"),
        );
        let none_cfg = omnesagent_config::schema::MemoryConfig {
            backend: "none".into(),
            ..omnesagent_config::schema::MemoryConfig::default()
        };
        let none_mem = || -> Arc<dyn Memory> {
            Arc::from(
                omnesagent_memory::create_memory(&none_cfg, std::path::Path::new("/tmp"), None)
                    .expect("none memory"),
            )
        };
        let seen_c = Arc::new(Mutex::new(Vec::new()));
    ...
    async fn turn_streamed_cancel_during_stream_emits_llm_response_failure() {
        ...
        let canceller = omnesagent_spawn::spawn!(async move {
    ...
    fn agent_end_tokens(
        event: &ObserverEvent,
    ) -> Option<omnesagent_api::observability_traits::TurnTokenUsage> {
        match event {
    ...
    async fn turn_reuses_outer_cost_tracking_context() {
        ...
        let tracker = Arc::new(
            CostTracker::new(
                omnesagent_config::schema::CostConfig {
                    enabled: true,
                    track_per_agent: true,
                    ..omnesagent_config::schema::CostConfig::default()
                },
                workspace.path(),
            )
    ...
    fn try_apply_model_switch_succeeds_with_switch_config() {
        let switch_cfg = ProviderSwitchConfig {
            config: Some(std::sync::Arc::new(
                omnesagent_config::schema::Config::default(),
            )),
        };
    ...
    fn try_apply_model_switch_prefers_route_api_key() {
        let route = omnesagent_config::schema::ModelRouteConfig {
        ...
        let route_config = omnesagent_config::schema::Config {
    ...
    impl ::omnesagent_api::attribution::Attributable for StreamSwitchTriggerProvider {
        fn role(&self) -> ::omnesagent_api::attribution::Role {
            ::omnesagent_api::attribution::Role::Provider(
                ::omnesagent_api::attribution::ProviderKind::Model(
                    ::omnesagent_api::attribution::ModelProviderKind::Custom,
    ...
    fn turn_streamed_applies_pending_model_switch_for_next_call() {
        ...
        let switch_cfg = ProviderSwitchConfig {
            config: Some(std::sync::Arc::new(omnesagent_config::schema::Config {
                reliability: omnesagent_config::schema::ReliabilityConfig {
                    provider_retries: 0,
                    provider_backoff_ms: 0,
                    ..omnesagent_config::schema::ReliabilityConfig::default()
                },
                ..omnesagent_config::schema::Config::default()
            })),
        };
...
mod approval_route_tests {
    ...
    use std::collections::HashMap;
    use omnesagent_api::channel::{ChannelApprovalRequest, ChannelApprovalResponse};
    use omnesagent_config::autonomy::{ApprovalRoute, OnNoApprover};
    ...
    impl omnesagent_api::attribution::Attributable for StubChannel {
        fn role(&self) -> omnesagent_api::attribution::Role {
            omnesagent_api::attribution::Role::Channel(omnesagent_api::attribution::ChannelKind::Cli)
        }
        fn alias(&self) -> &str {
    ...
    #[async_trait::async_trait]
    impl omnesagent_api::channel::Channel for StubChannel {
        fn name(&self) -> &str {
            &self.name
        }
        async fn send(&self, _m: &omnesagent_api::channel::SendMessage) -> anyhow::Result<()> {
            Ok(())
        }
        async fn listen(
            &self,
    ...
    fn registry(channels: Vec<StubChannel>) -> tools::PerToolChannelHandle {
        let mut map: HashMap<String, Arc<dyn omnesagent_api::channel::Channel>> = HashMap::new();
        for c in channels {
    ...
    async fn approver_answer_is_used_and_attributed() {
        ...
        match resolve_routed_approval(&h, &route("ops", OnNoApprover::Deny), "r", &req()).await {
            RoutedApproval::Decided {
                ...
                assert_eq!(
                    source,
                    omnesagent_api::channel::ApprovalSource::Operator,
        ...
        assert!(
            matches!(
                ...
                RoutedApproval::Decided {
                    ...
                    source: omnesagent_api::channel::ApprovalSource::Unreachable,
                    ..
                }
            ),
    ...
    async fn slow_approver_times_out_and_fails_closed() {
        ...
        assert!(
            matches!(
                ...
                RoutedApproval::Decided {
                    response: ChannelApprovalResponse::Deny,
                    source: omnesagent_api::channel::ApprovalSource::TimedOut,
                    ..
                }
            ),
    ...
    use omnesagent_api::channel::Channel as _;
