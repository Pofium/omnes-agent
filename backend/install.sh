...
# ── OmnesAgent installer ───────────────────────────────────────────
# Builds and installs OmnesAgent from source.
...
REPO_URL="https://github.com/omnesagent-labs/omnesagent.git"
...
QUICKSTART_COMMAND="omnesagent quickstart"
...
# (the TUI) is the default app. Tauri-based apps (e.g. omnesagent-desktop)
...
install_prebuilt() {
  ...
  version=$(curl -fsSL "https://api.github.com/repos/omnesagent-labs/omnesagent/releases/latest" |
  ...
  asset_name="omnesagent-${triple}.tar.gz"
  asset_url="https://github.com/omnesagent-labs/omnesagent/releases/download/${version}/${asset_name}"
  sha256_url="https://github.com/omnesagent-labs/omnesagent/releases/download/${version}/SHA256SUMS"

  echo
  printf "%s\n" "$(bold "Installing OmnesAgent ${version} (pre-built)")"
  ...
  info "Run 'omnesagent channel list' to inspect this binary. For other channels such as Slack, build from source with --preset full."
  echo
  ...
  if [ "$DRY_RUN" = true ]; then
    ...
    info "[dry-run] Would install to $CARGO_HOME/bin/omnesagent"
  ...
  install -m 755 "$tmp_dir/omnesagent" "$CARGO_HOME/bin/omnesagent"
...
Options:
  ...
  --no-modify-path     Don't add OmnesAgent to PATH in your shell profile; just
                       print the line to add manually
  --skip-quickstart       Skip the post-install quickstart prompt
  --uninstall          Remove OmnesAgent binary and optionally config/data
...
Examples:
  ...
  $0 --uninstall                               # remove OmnesAgent

Environment:
  OMNESAGENT_INSTALL_DIR   Source checkout override (default: PREFIX/.omnesagent/src)
  OMNESAGENT_CARGO_FEATURES  Extra cargo features (legacy; prefer --features)
EOF
}
...
do_uninstall() {
  echo
  printf "%s\n" "$(bold "Uninstalling OmnesAgent")"
  echo

  local bin="$CARGO_HOME/bin/omnesagent"
  ...
  local config_dir="$PREFIX/.omnesagent"
  ...
  if [ -f "$profile" ] && grep -q "# >>> omnesagent >>>" "$profile" 2>/dev/null; then
    local tmp_profile
    tmp_profile=$(mktemp)
    if sed '/# >>> omnesagent >>>/,/# <<< omnesagent <<</d' "$profile" >"$tmp_profile" 2>/dev/null &&
      cat "$tmp_profile" >"$profile" 2>/dev/null; then
      info "Removed PATH entry from $profile"
    else
      warn "Could not edit $profile — remove the omnesagent PATH block manually"
    fi
    rm -f "$tmp_profile"
  fi

  # Check if another omnesagent still lurks in PATH
  local other_bin
  other_bin=$(PATH="$ORIGINAL_PATH" command -v omnesagent 2>/dev/null || true)
  if [ -n "$other_bin" ]; then
    local other_version
    other_version=$("$other_bin" --version 2>/dev/null | awk '{print $NF}' || echo "unknown")
    echo
    warn "Another omnesagent found at $other_bin (v$other_version)"
    warn "Remove it manually if you want a full uninstall"
  fi

  echo
  info "OmnesAgent uninstalled"
  exit 0
}

# ── Quickstart-needed status check ───────────────────────────────
#
# Detect whether the operator already has a configured OmnesAgent so the
...
quickstart_needed() {
  cfg="$PREFIX/.omnesagent/config.toml"
...
resolve_web_data_dir() {
  case "$(uname -s)" in
  Darwin)
    printf '%s' "${HOME}/Library/Application Support/omnesagent/web/dist"
    ;;
  MINGW* | CYGWIN* | MSYS*)
    printf '%s' "${LOCALAPPDATA}/omnesagent/web/dist"
    ;;
  *)
    printf '%s' "${XDG_DATA_HOME:-${PREFIX}/.local/share}/omnesagent/web/dist"
    ;;
  esac
}
...
if [ -n "${OMNESAGENT_CARGO_FEATURES:-}" ]; then
  USER_FEATURES="${USER_FEATURES:+$USER_FEATURES,}$OMNESAGENT_CARGO_FEATURES"
fi
...
while [ $# -gt 0 ]; do
  ...
  -V | --version)
    if [ -f "Cargo.toml" ]; then
      ...
      echo "install.sh for OmnesAgent v$VERSION"
    else
...
INSTALL_DIR="${OMNESAGENT_INSTALL_DIR:-$PREFIX/.omnesagent/src}"
ORIGINAL_PATH="$PATH"
...
if [ "$INSTALL_MODE" = "" ]; then
  ...
  if [ -n "$triple" ]; then
    if [ "$GUIDED_INSTALL_MODE" = "choice" ] && [ -t 0 ]; then
      echo
      printf "  %s\n" "$(bold "How would you like to install OmnesAgent?")"
...
if [ "${PREBUILT_OK:-false}" != true ]; then
  ...
  echo
  printf "%s\n" "$(bold "OmnesAgent — source install")"
  if [ "$PREFIX" != "$HOME" ]; then
    printf "  prefix: %s\n" "$(bold "$PREFIX")"
  fi
  echo

  if [ -f "Cargo.toml" ] && grep -q "omnesagent" "Cargo.toml" 2>/dev/null; then
    INSTALL_DIR="$(pwd)"
  ...
  if [ "$DRY_RUN" != true ]; then
    ...
    if ! version_gte "$RUST_VERSION" "$MSRV"; then
      die "Rust $RUST_VERSION is too old. OmnesAgent requires $MSRV+ (edition $EDITION). Run: rustup update stable"
    fi
...
See all available features:
  ...
  PATH_BIN=$(PATH="$ORIGINAL_PATH" command -v omnesagent 2>/dev/null || true)
  if [ -n "$PATH_BIN" ]; then
    PATH_VERSION=$("$PATH_BIN" --version 2>/dev/null | awk '{print $NF}' || echo "unknown")
    TARGET_BIN="$CARGO_HOME/bin/omnesagent"
    if [ "$PATH_BIN" != "$TARGET_BIN" ]; then
      warn "omnesagent found at $PATH_BIN (v$PATH_VERSION)"
  ...
  echo
  printf "%s\n" "$(bold "Building OmnesAgent v$VERSION")"
  ...
  # exist before cargo install reaches omnesagent-gateway's build script.
  ...
  *--no-default-features*)
  ...
  # Apps connect to omnesagent-runtime's RPC server, so they need the
  ...
  *--no-default-features*)
  ...
  if [ "$DRY_RUN" != true ]; then
    ...
    if [ -f "$BIN" ]; then
      ...
      ACTIVE_BIN=$(PATH="$ORIGINAL_PATH" command -v omnesagent 2>/dev/null || true)
      if [ -n "$ACTIVE_BIN" ] && [ "$ACTIVE_BIN" != "$BIN" ]; then
        ACTIVE_VERSION=$("$ACTIVE_BIN" --version 2>/dev/null | awk '{print $NF}' || echo "unknown")
        echo
        warn "$(bold "WARNING:") omnesagent in your PATH is $ACTIVE_BIN (v$ACTIVE_VERSION)"
...
if [ "$SKIP_QUICKSTART" = false ] && [ "$DRY_RUN" != true ] && [ -f "$BIN" ]; then
  ...
  # OmnesAgent — re-installs should not re-prompt.
  if ! quickstart_needed; then
    info "Existing OmnesAgent config detected at $PREFIX/.omnesagent/config.toml — skipping setup prompt."
    ...
    echo
    printf "%s\n" "$(bold "OmnesAgent installed. How would you like to complete setup?")"
    printf "  [1] CLI quickstart  ($QUICKSTART_COMMAND)\n"
    printf "  [2] Open gateway in browser (omnesagent daemon + dashboard)\n"
    ...
    2)
      ...
      info "Stop the daemon with Ctrl+C when done; then run 'omnesagent service install' for always-on."
      "$BIN" daemon || warn "Daemon exited with an error — run 'omnesagent daemon' manually"
      ;;
    3)
      info "Skipped setup. Run '$QUICKSTART_COMMAND' (CLI) or 'omnesagent daemon' (browser) when ready."
      ;;
    *)
...
elif [ -f "$CARGO_HOME/bin/omnesagent" ] && "$CARGO_HOME/bin/omnesagent" --help 2>/dev/null | grep -q '\bdaemon\b'; then
  info "Done. Run $(bold "omnesagent daemon") for the always-on daemon + web dashboard,"
  info "or $(bold "omnesagent agent") for a one-off CLI chat."
else
  info "Done. Run $(bold "omnesagent agent") to start chatting."
fi
echo
