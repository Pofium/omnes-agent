...
pub fn fluent_catalog_roots_named(root: &Path) -> Vec<(&'static str, PathBuf)> {
    vec![
        ("runtime", root.join("crates/omnesagent-runtime/locales")),
        ("zerocode", root.join("apps/omnescode/locales")),
    ]
}
...
pub fn fluent_locales_dir(root: &Path) -> PathBuf {
    root.join("crates/omnesagent-runtime/locales")
}
...
pub fn build_model_provider(
...
) -> anyhow::Result<(Box<dyn omnesagent_api::model_provider::ModelProvider>, String)> {
    let home =
    ...
    let dir_candidates: Vec<std::path::PathBuf> = match config_dir {
        ...
        None => vec![
            std::path::PathBuf::from(format!("{home}/.omnesagent")),
            std::path::PathBuf::from(format!("{home}/.config/omnesagent")),
        ],
    };
    let dir = dir_candidates
        ...
        .ok_or_else(|| {
            anyhow::Error::msg(
                "config.toml not found (looked under --config-dir / ~/.omnesagent / ~/.config/omnesagent)",
            )
        })?;

    let raw = std::fs::read_to_string(dir.join("config.toml"))?;
    let mut config: omnesagent_config::schema::Config = toml::from_str(&raw)?;

    // Decrypt secrets through the canonical store (same path the daemon uses).
    let store = omnesagent_config::secrets::SecretStore::new(&dir, config.secrets.encrypt);
    ...
    let options = omnesagent_providers::provider_runtime_options_for_alias(&config, kind, &alias);
    let provider = omnesagent_providers::create_resilient_model_provider_from_ref(
        &config,
        &dotted,
        api_key.as_deref(),
        None,
