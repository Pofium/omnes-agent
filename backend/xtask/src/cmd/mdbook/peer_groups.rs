        ...
        .min_by_key(|(i, _)| *i)
    {
        ...
        let rendered = match marker {
            ...
            "{{#channel-streaming-matrix" => {
                ...
                omnesagent_config::schema_markdown::channel_streaming_matrix(&schema.to_value())
            }
...
fn render_config_fields(arg: &str) -> anyhow::Result<String> {
    let path = arg.trim();
    let schema = schemars::schema_for!(omnesagent_config::schema::Config);
    omnesagent_config::schema_markdown::field_table_for_path(&schema.to_value(), path, false, None)
        .map_err(anyhow::Error::msg)
}

fn sop_trigger_variants() -> anyhow::Result<(serde_json::Value, Vec<(String, serde_json::Value)>)> {
    let schema = schemars::schema_for!(omnesagent_runtime::sop::types::SopTrigger);
...
fn render_sop_trigger(arg: &str) -> anyhow::Result<String> {
    ...
    let fields = omnesagent_config::schema_markdown::field_table(&node, true, None, None);
...
fn config_section_label(path: &str) -> anyhow::Result<String> {
    ...
    if let Some(section) = omnesagent_config::sections::Section::from_key(path) {
    ...
    Ok(omnesagent_config::sections::humanize_section_key(path))
}
...
/// encrypt on write: the gateway dashboard, zerocode, and `omnesagent config set`
...
```sh
omnesagent config set {path}    # prompts for masked input, stores encrypted
```
...
/// `omnesagent config set`) minus the masked-secret framing. The arg is the full
...
```sh
omnesagent config set {path} <value>
```
...
```sh
omnesagent config set {p} true     # thread replies on
omnesagent config set {p} false    # replies at the channel root
```
...
conversation. OmnesAgent derives a distinct session key per thread, so every
...
the gateway dashboard, zerocode, or `omnesagent config set`.",
        key = p.key,
        agents = agents,
...
fn validate_keys(params: &[PeerParams]) -> anyhow::Result<()> {
    let inventory = omnesagent_config::schema::ChannelsConfig::default();
...
/// each row's `value`. One widget, both tabs, no literal `OMNESAGENT_...` name.
...
/// Render a single bare `OMNESAGENT_...` env-var name for inline prose or a
...
fn render_model_provider_endpoint(name: &str, format_fixed: impl FnOnce(&str) -> String) -> String {
    use omnesagent_providers::factory::ProviderEndpoint;

    let endpoint = omnesagent_providers::factory::endpoint_for_family(name);
...
fn render_model_provider_catalog_table() -> String {
    use omnesagent_providers::ModelProviderCategory as C;
    ...
    let providers = omnesagent_providers::list_model_providers();
...
fn render_model_provider_fields() -> String {
    ...
    let provider_defaults =
        serde_json::to_value(omnesagent_config::schema::ModelProviderConfig::default()).ok();

    let base: std::collections::BTreeSet<String> = providers
        .iter()
        .map(|p| {
            omnesagent_config::schema_markdown::section_field_names(
                &schema,
    ...
    if let Some(first) = providers.first() {
        let base_table = omnesagent_config::schema_markdown::field_table_for_path(
            &schema,
        ...
        let extras_of_first = omnesagent_config::schema_markdown::section_field_names(
            &schema,
            &format!("providers.models.{}", first.name),
        );
        let non_base: std::collections::BTreeSet<String> =
            extras_of_first.difference(&base).cloned().collect();
        let base_only = omnesagent_config::schema_markdown::field_table_for_path_excluding(
            &schema,
            &format!("providers.models.{}", first.name),
            false,
            provider_defaults.as_ref(),
    ...
    for category in C::all() {
        ...
        for p in rows {
            ...
            let extras = omnesagent_config::schema_markdown::field_table_for_path_excluding(
                &schema,
                &path,
                false,
                provider_defaults.as_ref(),
...
fn env_form(path: &str) -> String {
    format!("OMNESAGENT_{}", path.replace('.', "__"))
}
...
/// `OMNESAGENT_...=value` line. No env-var name is literal in the page — every
...
mod generated_prose_gate {
    ...
    fn secret_config_escapes_alias_placeholder_in_rendered_markdown() {
        ...
        assert!(rendered.contains("omnesagent config set channels.discord.<alias>.bot_token"));
    }
    ...
    fn config_explainers_keep_alias_placeholder_raw_in_markdown() {
        ...
        assert!(
            thread.contains("omnesagent config set channels.matrix.<alias>.reply_in_thread true")
        );
        assert!(streaming.contains("`channels.slack.<alias>.stream_drafts`"));
        assert!(
            streaming.contains("omnesagent config set channels.slack.<alias>.stream_drafts <value>")
        );
    }
    ...
    fn provider_docs_render_canonical_endpoint_classifications() {
        ...
        for provider in omnesagent_providers::list_model_providers() {
            use omnesagent_providers::factory::ProviderEndpoint;

            let endpoint = omnesagent_providers::factory::endpoint_for_family(provider.name)
