...
mod tests {
    ...
    fn flags_translated_command_literal_for_6407() {
        let issue = entry(
            "[`omnesagent daemon`↴](#omnesagent-daemon)",
            "[`omnesagent 守护进程`↴](#omnesagent-daemon)",
        );
    ...
    fn flags_translated_product_name_for_6407() {
        let issue = entry("The OmnesAgent Maturity Framework", "零爪成熟度框架");
        assert_eq!(
            protected_literal_reason(&issue),
    ...
    fn allows_translated_prose_around_preserved_literals() {
        let clean = entry(
            "Run `omnesagent daemon` after setting `[observability]`.",
            "设置 `[observability]` 后运行 `omnesagent daemon`。",
        );
        assert_eq!(protected_literal_reason(&clean), None);
    }
    ...
    fn allows_translated_cli_placeholders() {
        let clean = entry(
            "**Usage:** `omnesagent [OPTIONS] <COMMAND>`",
            "**Uso:** `omnesagent [OPCIONES] <COMANDO>`",
        );
        assert_eq!(protected_literal_reason(&clean), None);
    }
    ...
    fn flags_translation_introduced_posix_local_path() {
        let issue = entry(
            ...
            "Le journal est dans /Users/alice/omnesagent/docs/book/po/fr.failures.log.",
        );
        let entries = vec![issue];
        let leaks = audit_local_path_leaks(&entries);
        assert_eq!(
            leaks[0].1.as_str(),
            "/Users/alice/omnesagent/docs/book/po/fr.failures.log"
        );
    }
    ...
    fn flags_translation_introduced_private_tmp_and_volume_paths() {
        let private_tmp = entry(
            "The failure log is next to the catalog.",
            "日志位于 /private/tmp/omnesagent/zh-CN.failures.log。",
        );
        assert_eq!(
            introduced_local_absolute_path(&private_tmp.msgid, &private_tmp.msgstr).as_deref(),
            Some("/private/tmp/omnesagent/zh-CN.failures.log")
        );

        let volume = entry(
            "The failure log is next to the catalog.",
            "ログは /Volumes/Example Disk/omnesagent/ja.failures.log にあります。",
        );
        assert_eq!(
            introduced_local_absolute_path(&volume.msgid, &volume.msgstr).as_deref(),
            Some("/Volumes/Example Disk/omnesagent/ja.failures.log")
        );
    }
    ...
    fn flags_translation_introduced_windows_local_path() {
        let issue = entry(
            "The failure log is next to the catalog.",
            r#"El registro está en C:\Users\Alice\omnesagent\docs\book\po\es.failures.log."#,
        );
        assert_eq!(
            introduced_local_absolute_path(&issue.msgid, &issue.msgstr).as_deref(),
            Some(r#"C:\Users\Alice\omnesagent\docs\book\po\es.failures.log"#)
        );
    }
    ...
    fn allows_source_preserved_absolute_path() {
        let clean = entry(
            "Write `/home/alice/omnesagent/web/dist` instead of `~/omnesagent/web/dist`.",
            "Escriba `/home/alice/omnesagent/web/dist` en lugar de `~/omnesagent/web/dist`.",
        );
        assert_eq!(protected_literal_reason(&clean), None);
        assert_eq!(
