...
fn protected_terms() -> &'static [String] {
    ...
    TERMS.get_or_init(|| {
        ...
        for channel in omnesagent_config::schema::Config::default()
            .channels
            .channels()
        {
...
mod tests {
    ...
    fn ignores_ambiguous_registry_terms_in_generic_prose() {
        ...
        assert!(
            !texts("Signal reviewer trust improves OmnesAgent contributions.")
                .contains(&"Signal".to_string())
        );
    ...
    fn extracts_cli_command_and_placeholders() {
        let literals = texts("Run `omnesagent [OPTIONS] <COMMAND>`.");
        assert!(literals.contains(&"omnesagent".to_string()));
    ...
    fn does_not_synthesize_shell_operator_commands() {
        let literals =
            texts("Run `systemctl --user daemon-reload && systemctl --user restart omnesagent`.");
        assert!(!literals.contains(&"systemctl --user --user".to_string()));
        assert!(literals.contains(&"systemctl --user daemon-reload".to_string()));
        assert!(literals.contains(&"systemctl --user restart omnesagent".to_string()));
    ...
    fn extracts_compound_shell_command_segments() {
        let literals = texts(
            "Run `git clone https://github.com/omnesagent-labs/omnesagent && cd omnesagent && source ~/.cargo/env`.",
        );
        assert!(
            literals.contains(&"git clone https://github.com/omnesagent-labs/omnesagent".to_string())
        );
        assert!(literals.contains(&"cd omnesagent".to_string()));
    ...
    fn preservation_prompt_lists_literals() {
        let prompt = preservation_prompt("Run `omnesagent daemon` with OpenAI.").unwrap();
        assert!(prompt.contains("- omnesagent daemon"));
