...
use omnesagent_runtime::tools::default_tools;
...
fn table(header: &str, rows: &[Row]) -> String {
    ...
    let _ = writeln!(out, "| {header} | OmnesAgent | OpenClaw | Hermes |");
    out.push_str("|---|---|---|---|\n");
    for r in rows {
        let _ = writeln!(
            out,
...
fn providers_rows(parity: &Parity) -> Result<Vec<Row>> {
    let mut rows = Vec::new();
    let mut walked = std::collections::BTreeSet::new();
    for slot in omnesagent_providers::canonical_model_provider_slots() {
...
fn tools_rows(parity: &Parity) -> Result<Vec<Row>> {
    let security = Arc::new(omnesagent_config::policy::SecurityPolicy::default());
    let mut rows = Vec::new();
    let mut walked = std::collections::BTreeSet::new();
...
mod tests {
    ...
    fn channel_column_is_all_supported_from_walk() {
        ...
        for r in channels_rows(&parity).unwrap() {
            assert_eq!(r.omnesagent, Status::Supported);
        }
    }
}
