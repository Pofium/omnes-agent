...
mod tests {
    ...
    fn relay_dockerfile_build_is_lockfile_enforced() {
        let dockerfile = std::fs::read_to_string(root().join("apps/omnesrelay/Dockerfile")).unwrap();
        assert!(
            dockerfile.contains("cargo build --release --locked -p omnesrelay"),
            "the relay image build must use --locked for a reproducible signed artifact"
        );
        assert!(
            !dockerfile.contains("cargo build --release -p omnesrelay"),
