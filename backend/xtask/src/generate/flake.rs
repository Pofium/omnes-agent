...
//! fixed set. We generate a sentinel-delimited zone defining the omnesagent +
...
/// omnesagent, zerocode, and default. Indented to sit inside the per-system `in {`
/// block of the flake.
pub fn render_zone(root: &Path) -> anyhow::Result<String> {
    ...
    let lines = [
        ...
        "        # Override with `packages.omnesagent.override { features = [ ... ]; }`.".to_string(),
        format!("        omnesagentDefaultFeatures = [ {feature_list} ];"),
        "        buildOmnesagent = { pname, cargoPkg, features ? omnesagentDefaultFeatures }:"
            .to_string(),
...
mod tests {
    ...
    fn zone_exposes_overridable_features() {
        ...
        assert!(
            z.contains("omnesagentDefaultFeatures"),
