...
pub fn render_features(
    ...
    Ok(format!("{indent}OMNESAGENT_FEATURES=\"{}\"", list.join(",")))
}
...
pub fn render_features_arg(manifest_dir: &Path, selection: &Selection) -> anyhow::Result<String> {
    ...
    Ok(format!("ARG OMNESAGENT_CARGO_FLAGS=\"{flags}\""))
}
...
mod tests {
    ...
    fn full_renders_explicit_default_leaves() {
        ...
        // Full emits the explicit resolved default leaves as a OMNESAGENT_FEATURES
        // assignment (drift-checkable), not a bare comment.
        assert!(b.contains("OMNESAGENT_FEATURES="));
