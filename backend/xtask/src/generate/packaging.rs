...
mod tests {
    ...
    fn srcinfo_is_a_complete_projection_of_pkgbuild() {
        ...
        assert!(rendered.contains(&format!(
            "\tsource = omnesagentlabs-{version}.tar.gz::https://github.com/omnesagent-labs/omnesagent/archive/refs/tags/v{version}.tar.gz\n"
        )));
  ...
  "architecture": {
    "64bit": {
      "url": "https://example.test/releases/download/v0.5.9/omnesagent.zip"
    }
  },
  "autoupdate": {
    "architecture": {
      "64bit": {
        "url": "https://example.test/releases/download/v$version/omnesagent.zip"
      }
    }
  },
  "x": 1
        ...
        assert!(out.contains("releases/download/v0.8.0/omnesagent.zip"));
        assert!(out.contains("releases/download/v$version/omnesagent.zip"));
    ...
    fn scoop_errors_without_version_key() {
        let current = r#"{
  "architecture": {"64bit": {"url": "https://example.test/v0.5.9/omnesagent.zip"}},
  "autoupdate": {"architecture": {"64bit": {"url": "https://example.test/v$version/omnesagent.zip"}}}
}"#;
  ...
  "version": "0.5.9",
  "architecture": {"64bit": {"url": "https://example.test/v0.5.9/omnesagent.zip"}},
  "autoupdate": {"architecture": {"64bit": {"url": "https://example.test/latest/omnesagent.zip"}}}
}"#;
        assert!(rewrite_scoop_release_fields(current, "1.0").is_err());
    }
