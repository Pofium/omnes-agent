//! Mirror the canonical English tool catalogue into `omnesagent-tools`.
//!
//! `crates/omnesagent-runtime/locales/en/tools.ftl` stays the source of truth, and
//! `cargo fluent fill` keeps translations beside it. `omnesagent-tools` embeds the
//! English strings itself rather than depending on `omnesagent-runtime`, because
//! the dependency runs runtime -> tools and importing back would invert it.
//!
//! That embed used to be `include_str!("../../omnesagent-runtime/locales/en/tools.ftl")`,
//! which reaches outside the crate directory. `cargo package` copies only the
//! package directory, so the read made `omnesagent-tools` unpublishable. Mirroring
...
pub const SOURCE: &str = "crates/omnesagent-runtime/locales/en/tools.ftl";

const HEADER: &str = "\
# GENERATED from crates/omnesagent-runtime/locales/en/tools.ftl by
