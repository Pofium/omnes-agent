...
const ZONE_LAYOUTS: [ZoneLayout; 12] = [
    ...
    ZoneLayout {
        ...
        range_start: "  # ── Summary ───────────────────────────────────────────────────────",
        range_end: "BIN=\"$CARGO_HOME/bin/omnesagent\"\n\n# ── PATH setup",
    },
    ZoneLayout {
...
fn render_route_contract(routes: &[InstallRoute]) -> anyhow::Result<String> {
    ...
    anyhow::ensure!(
        quickstart_parts.next() == Some("omnesagent"),
        "guided Quickstart command must invoke omnesagent"
    );
...
mod tests {
    ...
    fn route_contract_comes_from_canonical_routes() {
        ...
        assert!(contract.contains("QUICKSTART_COMMAND=\"omnesagent quickstart\""));
    ...
    fn route_contract_rejects_source_path_and_quickstart_drift() {
        ...
        mutate_unix_variant(&mut wrong_quickstart, RouteId::UnixGuided, |variant| {
            variant.quickstart_command = "omnesagent configure";
        });
    ...
    fn real_guided_flow_uses_generated_quickstart_command() {
        ...
        assert!(!guided.contains("omnesagent quickstart"));
