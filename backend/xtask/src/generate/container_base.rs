...
"#,
    ...
    fn check_flags_orphan_reference() {
        let content = "FROM ${OMNESAGENT_BASE_RUST_SLIM} AS x\n";
    ...
    fn splice_zones_rejects_non_plain_node_discover_tag() {
        ...
        let content = format!(
            "{}\nARG OMNESAGENT_BASE_NODE=node:24.1-bookworm-slim@sha256:{}\n{}\n",
            begin("base-arg-node"),
            "a".repeat(64),
            end("base-arg-node")
        );
