    ...
    pub fn command(self) -> Option<&'static str> {
        match self {
            Self::PipedScript => Some(
                "curl -fsSL https://raw.githubusercontent.com/omnesagent-labs/omnesagent/master/install.sh | sh",
            ),
...
pub const QUICKSTART_COMMAND: &str = "omnesagent quickstart";
...
pub enum Value {
    ...
    WebDataDir,
    /// Install bin dir (cargo bin on Unix, %USERPROFILE%\.omnesagent\bin on Win).
    BinDir,
...
pub fn dist_target_exclusions(
    ...
    parse_target_exclusions(pkg.metadata.get("omnesagent"))
}
...
/// `[package.metadata.omnesagent] container_excluded_features`; never shadowed.
...
fn required_registry_list(pkg: &cargo_metadata::Package, key: &str) -> anyhow::Result<Vec<String>> {
    parse_required_registry_list(pkg.metadata.get("omnesagent"), key)
}
...
fn parse_required_registry_list(
    ...
    let value = registry
        .and_then(|omnesagent| omnesagent.get(key))
        .ok_or_else(|| anyhow::Error::msg(format!("missing [package.metadata.omnesagent] {key}")))?;
    parse_nonempty_string_list(value, &format!("[package.metadata.omnesagent] {key}"))
}
...
fn parse_target_exclusions(
    ...
    let value = registry
        .and_then(|omnesagent| omnesagent.get("dist_target_exclusions"))
        .ok_or_else(|| {
            anyhow::Error::msg("missing [package.metadata.omnesagent.dist_target_exclusions]")
        })?;
    let entries = value.as_object().ok_or_else(|| {
        anyhow::Error::msg("[package.metadata.omnesagent.dist_target_exclusions] must be a table")
    })?;
    anyhow::ensure!(
        !entries.is_empty(),
...
pub fn web_data_dir_expr(platform: Platform) -> &'static str {
    match platform {
        ...
        Platform::Unix => "${XDG_DATA_HOME:-${PREFIX}/.local/share}/omnesagent/web/dist",
        Platform::Windows => "%LOCALAPPDATA%\\omnesagent\\web\\dist",
    }
}
...
mod tests {
    ...
    fn release_workflows_delegate_target_policy_to_generator() {
        ...
        let omnesagent_upload = manual
            .find("name: omnesagent-manual-${{ inputs.distribution }}-${{ matrix.target }}")
            .unwrap();
        let companion = manual.find("- name: Build ZeroCode companion").unwrap();
        assert!(
            target_env < release_step
                && release_step < omnesagent_upload
                && omnesagent_upload < companion
        );
        assert!(
    ...
    fn web_data_dir_expr_matches_data_local_dir_semantics() {
        ...
        assert!(win.contains("LOCALAPPDATA") && win.ends_with("omnesagent\\web\\dist"));
        let unix = web_data_dir_expr(Platform::Unix);
        assert!(unix.contains("XDG_DATA_HOME") && unix.ends_with("omnesagent/web/dist"));
    }
}
