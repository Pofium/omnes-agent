...
fn app_sentence(policy: AppPolicy) -> String {
    match policy {
        ...
        AppPolicy::CoreOnly => {
            "This command installs the core `omnesagent` binary; it does not install optional apps."
                .to_owned()
        }
    }
}
...
fn render_readme_unix_fast_block(routes: &[InstallRoute]) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "```sh\n{command}\n\"${{CARGO_HOME:-$HOME/.cargo}}/bin/omnesagent\" {quickstart_subcommand}\n```"
    ))
}
...
fn render_windows_prebuilt_block(routes: &[InstallRoute]) -> anyhow::Result<String> {
    ...
    Ok(r#"```powershell
# Installation and PATH setup are idempotent. If omnesagent is already at the
# latest release and on the user PATH, those steps are skipped; Quickstart
# still runs at the end.
$ver = (Invoke-RestMethod 'https://api.github.com/repos/omnesagent-labs/omnesagent/releases/latest').tag_name.TrimStart('v')
$dst = "$env:USERPROFILE\.omnesagent\bin"
$exe = "$dst\omnesagent.exe"
...
if ($current -ne $ver) {
    $url = "https://github.com/omnesagent-labs/omnesagent/releases/download/v$ver/omnesagent-x86_64-pc-windows-msvc.zip"
    New-Item -ItemType Directory -Force -Path $dst | Out-Null
    Invoke-WebRequest -Uri $url -OutFile "$env:TEMP\omnesagent.zip" -UseBasicParsing
    Expand-Archive -Force -Path "$env:TEMP\omnesagent.zip" -DestinationPath $dst
}
...
mod tests {
    ...
    fn markdown_projects_route_behavior() {
        ...
        assert!(markdown.contains("prints `omnesagent quickstart`"));
        assert!(markdown.contains("offers `omnesagent quickstart` or browser-based Quickstart"));
        ...
        assert!(markdown.contains("running `omnesagent quickstart` automatically"));
    ...
    fn windows_prebuilt_route_rejects_handwritten_guide_drift() {
        ...
        let mutations = [
            ...
            (
                ...
                "& $exe quickstart",
                "Write-Host 'Run omnesagent quickstart later'",
            ),
        ];
    ...
    fn unix_fast_command_zones_come_from_the_route_contract() {
        ...
        assert!(readme.contains("/bin/omnesagent\" quickstart"));
    }
