...
:: ============================================================================
:: OmnesAgent Windows Setup Script
:: Simplifies building and installing OmnesAgent on Windows.
...
set "REPO=https://github.com/omnesagent-labs/omnesagent"
...
echo %BOLD%%BLUE%  OmnesAgent Windows Setup  v%VERSION%%RESET%
echo %BOLD%%BLUE%=========================================%RESET%
echo.
...
if "%DRY_RUN%"=="true" (
    ...
    echo   [dry-run] Would install to %USERPROFILE%\.omnesagent\bin
    echo   [dry-run] Would add %USERPROFILE%\.omnesagent\bin to PATH
    goto :dry_run_done
)
...
if not defined DOWNLOAD_URL (
    ...
    set "DOWNLOAD_URL=https://github.com/omnesagent-labs/omnesagent/releases/latest/download/omnesagent-%TARGET%.zip"
)

echo   Downloading from release...
curl -sSfL -o "%TEMP%\omnesagent-windows.zip" "!DOWNLOAD_URL!"
if %ERRORLEVEL% NEQ 0 (
...
:: Extract
echo   Extracting...
mkdir "%USERPROFILE%\.omnesagent\bin" 2>nul
tar -xf "%TEMP%\omnesagent-windows.zip" -C "%USERPROFILE%\.omnesagent\bin"
if %ERRORLEVEL% NEQ 0 (
    powershell -Command "Expand-Archive -Force '%TEMP%\omnesagent-windows.zip' '%USERPROFILE%\.omnesagent\bin'"
)

:: Add to PATH if not already there
echo %PATH% | findstr /I /C:".omnesagent\bin" >nul 2>&1
if %ERRORLEVEL% NEQ 0 (
    setx PATH "%PATH%;%USERPROFILE%\.omnesagent\bin" >nul 2>&1
    set "PATH=%PATH%;%USERPROFILE%\.omnesagent\bin"
    echo   %GREEN%OK%RESET% Added to PATH
)

echo   %GREEN%OK%RESET% Binary installed to %USERPROFILE%\.omnesagent\bin\omnesagent.exe
if exist "%USERPROFILE%\.omnesagent\bin\zerocode.exe" (
    echo   %GREEN%OK%RESET% TUI installed to %USERPROFILE%\.omnesagent\bin\zerocode.exe
)
goto verify
...
echo.
echo %BOLD%[3/5] Building OmnesAgent (%BUILD_DESC%)...%RESET%
echo   Target: %TARGET%
...
if "%DRY_RUN%"=="true" (
    ...
    echo   [dry-run] Would install to %USERPROFILE%\.omnesagent\bin
    echo   [dry-run] Would build web dashboard ^(cargo web build^) and install to %LOCALAPPDATA%\omnesagent\web\dist
    echo   [dry-run] Would add %USERPROFILE%\.omnesagent\bin to PATH
    goto :dry_run_done
)
...
if not exist "Cargo.toml" (
    echo   %RED%ERROR: Cargo.toml not found. Run this script from the omnesagent repository root.%RESET%
    echo   Example:
    echo     git clone %REPO%
    echo     cd omnesagent
    echo     setup.bat
...
mkdir "%USERPROFILE%\.omnesagent\bin" 2>nul
copy /Y "target\%TARGET%\release\omnesagent.exe" "%USERPROFILE%\.omnesagent\bin\omnesagent.exe" >nul
if exist "target\%TARGET%\release\zerocode.exe" (
    copy /Y "target\%TARGET%\release\zerocode.exe" "%USERPROFILE%\.omnesagent\bin\zerocode.exe" >nul
    echo   %GREEN%OK%RESET% TUI installed to %USERPROFILE%\.omnesagent\bin\zerocode.exe
)
set "BIN_PATH=%USERPROFILE%\.omnesagent\bin\omnesagent.exe"
for /f %%S in ('powershell -NoProfile -Command "[math]::Round(((Get-Item -LiteralPath ''%BIN_PATH%'').Length / 1MB), 2)"') do (
    set "BINARY_MB=%%S"
)
if defined BINARY_MB (
    echo   %GREEN%OK%RESET% Installed to %USERPROFILE%\.omnesagent\bin\omnesagent.exe ^(%BINARY_MB% MB^)
) else (
    echo   %GREEN%OK%RESET% Installed to %USERPROFILE%\.omnesagent\bin\omnesagent.exe ^(size unavailable^)
)
...
:: (%LOCALAPPDATA%\omnesagent\web\dist) so a service-launched daemon finds
...
if %ERRORLEVEL% EQU 0 (
    ...
    if %ERRORLEVEL% EQU 0 (
        if exist "web\dist\index.html" (
            mkdir "%LOCALAPPDATA%\omnesagent\web\dist" 2>nul
            robocopy "web\dist" "%LOCALAPPDATA%\omnesagent\web\dist" /MIR /NFL /NDL /NJH /NJS >nul
            echo   %GREEN%OK%RESET% Web dashboard installed to %LOCALAPPDATA%\omnesagent\web\dist
        )
    ) else (
...
"%USERPROFILE%\.omnesagent\bin\omnesagent.exe" --version >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    for /f "tokens=*" %%v in ('"%USERPROFILE%\.omnesagent\bin\omnesagent.exe" --version 2^>nul') do (
        echo   %GREEN%OK%RESET% %%v
    )
) else (
    omnesagent --version >nul 2>&1
    if %ERRORLEVEL% EQU 0 (
        for /f "tokens=*" %%v in ('omnesagent --version 2^>nul') do (
            echo   %GREEN%OK%RESET% %%v
        )
    ) else (
...
echo %BOLD%%GREEN%  OmnesAgent setup complete!%RESET%
echo %BOLD%%GREEN%=========================================%RESET%
echo.
...
echo     2. Run: omnesagent quickstart
echo     3. Configure a model provider during Quickstart
echo     4. Launch the TUI when installed: zerocode
) else (
echo     1. PATH is ready in this terminal and future terminals
if /I "%MODE%"=="minimal" (
echo     2. Minimal build excludes quickstart ^(omnesagent quickstart is unavailable^)
echo     3. Configure model providers with the supported config surface
echo     4. Use reduced CLI path: omnesagent agent --message "Hello"
) else (
echo     2. Run: omnesagent quickstart
echo     3. Configure a model provider during Quickstart
echo     4. Launch the TUI when installed: zerocode
...
echo     scoop bucket add omnesagent https://github.com/omnesagent-labs/scoop-omnesagent
echo     scoop install omnesagent
echo.
echo   Documentation: https://github.com/omnesagent-labs/omnesagent
echo.
goto :end
...
:show_help
echo.
echo OmnesAgent Windows Setup Script
echo.
