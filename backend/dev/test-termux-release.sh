...
TAG="v${VERSION}"
ASSET_NAME="omnesagent-${TARGET}.tar.gz"
ASSET_URL="https://github.com/omnesagent-labs/omnesagent/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t omnesagent-termux-test-XXXXXX)"
...
# --- Test 5: Contains omnesagent binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^omnesagent$"; then
  pass "Archive contains 'omnesagent' binary"
else
  fail "Archive does not contain 'omnesagent' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/omnesagent-labs/omnesagent/releases/download/${TAG}/SHA256SUMS"
