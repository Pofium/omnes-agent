...
# Installs every skill from omnesagent-labs/omnesagent-skills by bare name,
...
# ── Resolve omnesagent binary ─────────────────────────────────────
OMNESAGENT=""
for candidate in \
  "$REPO_ROOT/target/debug/omnesagent" \
  "$REPO_ROOT/target/release/omnesagent" \
  "$(command -v omnesagent 2>/dev/null || true)"; do
  if [ -n "$candidate" ] && [ -x "$candidate" ]; then
    OMNESAGENT="$candidate"
    break
  fi
done

if [ -z "$OMNESAGENT" ]; then
  printf "${RED}Error: No omnesagent binary found. Run 'cargo build' first.${RESET}\n"
  exit 1
fi
...
SKILLS_DIR="$HOME/.omnesagent/workspace/skills"
REGISTRY_DIR="$HOME/.omnesagent/workspace/skills-registry"
...
if [ ! -d "$REGISTRY_DIR/skills" ]; then
  ...
  "$OMNESAGENT" skills install __bootstrap_probe__ 2>&1 || true
fi
...
for skill in $REGISTRY_SKILLS; do
  OUTPUT=$("$OMNESAGENT" skills install "$skill" 2>&1) || true
...
LIST_OUTPUT=$("$OMNESAGENT" skills list 2>&1)
...
ERR_OUTPUT=$("$OMNESAGENT" skills install nonexistent-skill-xyz 2>&1 || true)
...
DUP_OUTPUT=$("$OMNESAGENT" skills install auto-coder 2>&1 || true)
...
if [ -f "$REGISTRY_DIR/.omnesagent-skills-registry-sync" ]; then
...
for skill in $INSTALLED_SKILLS; do
  REMOVE_OUTPUT=$("$OMNESAGENT" skills remove "$skill" 2>&1) || true
