#!/usr/bin/env bash
# Launch the OmnesAgent Tauri desktop app in dev mode.
#
# Assumes a OmnesAgent daemon is reachable on 127.0.0.1:42617. Run
# `omnesagent daemon` (NOT `omnesagent gateway start`) — only the daemon attaches
...
pkill -f 'target/debug/omnesagent-desktop' 2>/dev/null || true
sleep 0.5
...
if cargo tauri --version >/dev/null 2>&1; then
  ...
  exec cargo run -p omnesagent-desktop
fi
