# OmnesAgent Development Environment

A fully containerized development sandbox for OmnesAgent agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`omnesagent-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `omnesagent` CLI commands manually, debug the binary, or check logs internally.

- **Path**: `/omnesagent-data`
- **User**: `nobody` (65534)
...
- **Agent**: `/omnesagent-data/workspace`
...
The agent configuration lives in `target/.omnesagent` (mounted to `/omnesagent-data/.omnesagent`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.omnesagent` (config/DB) but leaves the `playground/` directory intact. To fully wipe workspace data, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `omnesagent-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=omnesagent-target`) to speed repeat local image builds.
