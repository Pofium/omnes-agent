...
build_smoke_image() {
  if docker buildx version >/dev/null 2>&1; then
    ...
    local build_args=(
      ...
      -t omnesagent-local-smoke:latest
      .
    )
    ...
    DOCKER_BUILDKIT=1 docker build --target dev -t omnesagent-local-smoke:latest .
  fi
}

print_help() {
  cat <<'EOF'
OmnesAgent Local CI in Docker
...
case "$1" in
  ...
  test)
    ...
    # CI uses `cargo nextest run --locked --workspace --exclude omnesagent-desktop`
    ...
    run_in_ci "cargo test --locked --workspace --exclude omnesagent-desktop --verbose"
  ...
  docker-smoke)
    build_smoke_image
    docker run --rm omnesagent-local-smoke:latest --version
    ;;
