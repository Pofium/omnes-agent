...
case "$PROFILE" in
  release) cargo build --release --bin omnesagent ;;
  dev|"")  cargo build --bin omnesagent ;;
  *) echo "Usage: $0 [dev|release]" >&2; exit 2 ;;
esac

if [ "$PROFILE" = release ]; then
  BIN="$REPO_ROOT/target/release/omnesagent"
else
  BIN="$REPO_ROOT/target/debug/omnesagent"
fi
...
echo "  [ ] Quickstart prints the next-step instructions (omnesagent agent / omnesagent daemon)"
