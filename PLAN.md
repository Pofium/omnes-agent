...
## 9. План фронтенда: интеграция и управление локальным слоем System One (Laya)

### 9.1. Концепция и поведение автозагрузки
Слой System One на базе модели Laya (ModernBERT-large + Decision Head, 421M параметров, ~800 МБ весов) по умолчанию отключен (`provider: off`), чтобы гарантировать нулевой сетевой трафик и отсутствие самовольных скачиваний без ведома пользователя.

**Поведение при включении:**
1. **Первичная активация:** Когда пользователь переключает тумблер System One в интерфейсе (или выставляет `provider: candle`), клиент опрашивает шлюз о наличии локальных весов.
2. **Проверка кэша:** Шлюз через `hf-hub` проверяет стандартную локальную директорию Hugging Face (`~/.cache/huggingface/hub/models--convaiinnovations--laya`). Если файлы уже есть на диске, статус мгновенно переходит в `Ready (Local)`.
3. **Автоматическая загрузка:** Если веса отсутствуют и путь `model_path` не задан вручную, инициируется фоновое скачивание из репозитория `convaiinnovations/laya` (`config.json`, `tokenizer.json`, `model.safetensors`). В UI отображается плашка загрузки с прогрессом.
4. **Оффлайн-режим и ручной путь:** Пользователь может задать локальную папку с весами (`OMNESAGENT_S1_PATH`) в настройках — в этом случае сетевые обращения к Hugging Face полностью блокируются.
5. **Отказоустойчивость:** При отсутствии соединения или ошибке скачивания агент не падает, а выводит ненавязчивое предупреждение и автоматически откатывается на стандартный эвристический режим.

### 9.2. Архитектура UI-компонентов фронтенда

#### А. Раздел настроек `System One (Laya)` в `desktop_settings_dialog.dart`
- **Главный переключатель:** `Включить System One (Laya)` (выбор провайдера: `Выключен` / `Candle (Локальный)` / `Fake (Тестовый)`).
- **Карточка статуса модели:**
  - `Не установлена` (серая плашка, кнопка «Загрузить модель (~800 МБ)»).
  - `Скачивание...` (синий индикатор, прогресс-бар, кнопка «Отмена»).
  - `Готова к работе` (зеленый бейдж, объем в памяти ~800 МБ, задержка отклика ~25 мс, устройство CPU/CUDA).
  - `Ошибка загрузки` (красный бейдж, пояснение причины, кнопка «Повторить»).
- **Кастомный путь к весам:** Текстовое поле с валидацией пути к локальному каталогу весов и кнопкой обзора файлов.
- **Гранулярные тогглы модулей:**
  - `Быстрый фильтр каналов` (отсечение спама и нерелевантных реплик за 30 мс).
  - `Компактификация истории` (семантическая оценка ценности реплик 1..5 перед отправкой в облачную LLM).
  - `Ранжирование памяти` (отсечение нерелевантных фактов через Noul с порогом вероятности 0.40).
  - `Шлюз инструментов` (быстрое определение потребности в терминале/файлах/вебе).
  - `Семантический роутер моделей` (автовыбор между fast, code и reasoning моделями).

#### Б. Индикация в рабочем пространстве `task_workspace_view.dart`
- **S1 Fast-Path Badge:** Микро-бейдж `⚡ S1: 28ms` в строке статуса сообщения при срабатывании локального гейта или фильтра каналов.
- **Индикатор компрессии контекста:** Отображение количества сэкономленных токенов и витков диалога после семантического тримминга S1.

### 9.3. Чек-лист реализации на фронтенде
- [ ] Добавить модель состояния `SystemOneState` в `task_workspace_controller.dart` (`provider`, `status`, `progress`, `localPath`, `moduleToggles`).
- [ ] Добавить REST-эндпоинты в клиент `GatewayHttpClient`: `GET /api/v1/s1/status`, `POST /api/v1/s1/download`, `POST /api/v1/s1/config`.
- [ ] Сверстать секцию настроек `System One` в `desktop_settings_dialog.dart` по гайдлайнам Cyber/reui + shadcn/ui.
- [ ] Добавить WebSocket-слушатель событий прогресса загрузки модели (`s1_download_progress`).
- [ ] Добавить визуальный бейдж `⚡ S1` в карточки сообщений чата при использовании локального инференса.
